/*
Set server-specific delivery details CJM 20211221
*/
CREATE PROCEDURE [Processing].[Email_ProgressReport]
(
	@FileType VARCHAR(30)
...
BEGIN
	...
	----------------------------------------------------------------------
	-- Get last task before this one (since this task is the running of this sproc)
	...
	----------------------------------------------------------------------
	DECLARE @ResetTime INT = Processing.getTimeDiff()
	DECLARE @Today DATE = Processing.getCurrentDate()
	...
	BEGIN
		...
		DECLARE @recipients VARCHAR(MAX), 
			@Subject NVARCHAR(255) = '[' + @@SERVERNAME + '] notification: Reward Affinity - Progress Log';
			   
		IF @@SERVERNAME = 'DIMAIN2'
			SELECT @recipients = 'Neon_Ops@affinitysolutions.com'
				, @Subject = 'Reward Affinity - Progress Log'

		IF @@SERVERNAME <> 'DIMAIN2'
			SET  @recipients = 'Christopher.Morris@RewardInsight.com';
