...
******************************************************************************/
CREATE PROCEDURE [Processing].[Email_Failure]
AS
BEGIN
	SET NOCOUNT ON;
	...
	SELECT
	  ...
	  , vpl.SourceType
	  , CAST(ISNULL(vpl.ErrorDetails, '')	AS VARCHAR(MAX))  AS ErrorDetails
	...
	FROM Processing.vw_PackageLog_Latest vpl

	-- Default subject if NULL
	SELECT @Subject = @@SERVERNAME + ' ' + COALESCE(@Subject, SourceName + ' Failed')
