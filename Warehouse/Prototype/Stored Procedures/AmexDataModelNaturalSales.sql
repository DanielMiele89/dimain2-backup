...
-- =============================================
CREATE PROCEDURE [Prototype].[AmexDataModelNaturalSales]
...
--Select * from Warehouse.Relational.Brand where BrandName like '%waitrose%'
	...
	IF @Brands IS NOT NULL	
		Begin
							...
							FROM	
									...
									Where CHARINDEX(',' + CAST(BrandID AS VARCHAR) + ',', ',' + @Brands + ',') > 0) A
		END
	IF @Brands IS NULL 
		Begin
...
/* Code Graveyard: Where good code goes to die

IF OBJECT_ID('Sandbox.Satty.AmexModelNatualSales') IS NOT NULL DROP TABLE Sandbox.Satty.AmexModelNatualSales
Create Table Sandbox.Satty.AmexModelNatualSales
	(			BrandID INT
				,CycleID INT
				,CycleStart DATE
				,CycleEnd DATE
