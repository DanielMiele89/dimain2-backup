...
***************************************************************************/
CREATE PROCEDURE [Prototype].[ROCP2_Code_02E_SpenderAdjustmentsv4_AMEX]
	(
	@IndividualBrand BIT
	)
AS
...
DELETE FROM Prototype.ROCP2_SpendersAdj_AMEX
...
TRUNCATE TABLE Prototype.ROCP2_SpendersAdj_AMEX
...
Select	a.*
		,case when 0 < BASEATF then 
			ATF /CAST( BASEATF AS real) 
		 else 
			NULL 
		 end as ATFRatio
		,case when 0 < BASESpenders then 
			case when (Spenders/CAST( BASESpenders AS real)) < 1 then
				1
			else
				Spenders /CAST( BASESpenders AS real)
			end
		 else 
			NULL 
...
INSERT INTO Prototype.ROCP2_SpendersAdj_AMEX
