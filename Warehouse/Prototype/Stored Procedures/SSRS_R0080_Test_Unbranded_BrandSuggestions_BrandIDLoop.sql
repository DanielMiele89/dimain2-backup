...
-- *****************************************************************************************************
CREATE PROCEDURE [Prototype].[SSRS_R0080_Test_Unbranded_BrandSuggestions_BrandIDLoop](
			@BrandIDs INT)
...
TRUNCATE TABLE [Prototype].[SSRS_R0080_Test_Unbranded_BrandSuggestions]

WHILE @RowNo <= @MaxRowNo
BEGIN
	SET	@BrandID = (Select BrandID from #B where RowNo = @RowNo)
	EXEC	[Prototype].[SSRS_R0080_Test_Unbranded_BrandSuggestions_SingleBrand] @BrandID
	SET	@RowNo = @RowNo+1
	--SELECT @RowNo
