...
Version 4:
	Removed ConsumerTrans from Spend C table and created seperatly before Insert into SpendC
	helped reduce query time from 3 hours to 1 hour for Waitrose and Competitors

=================================================================================================*/
--Define Date (Transactional Universe to be considered)
--Approximate Time: 40 Minutes W/ Fixed Base; 20 Minutes W/O Fixed Base

CREATE PROCEDURE [Prototype].[Drivers_to_Change_v4]		
	(@EDate DATE				-- End Date
...
--	DECLARE @EDate DATE				= '2017-09-30'	-- End Date
--	DECLARE @BrandList VARCHAR(500) = '485,292,254,5,379,21,92,425'	-- Brand List (ALWAYS Main Brand 1st)
...
--	DECLARE @OnlineVariable BIT		= NULL			-- NULL = All Transactions, 0 = Offline Only, 1 = Online Only
	...
	-- Below remains to be Alan's code and part Satty's
	----------------------------------------------------------------------------------------
	----------  Extracting Competitor Spend
	----------------------------------------------------------------------------------------
	/*				Version 4 Changes - Created seperate Transaction table 				  */
	--Create Competitor Spend and Transactions
	IF object_id('tempdb..#Transaction') is not null drop table #Transaction					
	Select		CT.CINID, CT.TranDate, B.BrandID
	INTO		#Transaction
	from		#cc_competitor b
				inner join	Warehouse.Relational.ConsumerTransaction (nolock) ct on b.ConsumerCombinationID=ct.ConsumerCombinationID	
				and (ct.TranDate between @StartDate and @Edate)
	Where		ISRefund = 0 
				and	(isonline = @onlineVariable or @onlineVariable IS NULL)
	Option(Recompile)

	CREATE CLUSTERED INDEX T_CINID ON #Transaction(CINID)

	--Shorten list through Customer table
	...
	While @RowNoC <= @MaxRowNoC
			Begin
				...
				select	ct.cinid
						,1 as Flag
						,DATEADD(m, DATEDIFF(m, 0, ct.TranDate), 0)
				from		#CustomerList c with (nolock)
				inner join	#Transaction (nolock) ct on c.cinid=ct.cinid 
				where		c.RowNo Between @RowNoC and @RowNoC+(@ChunkSizeC-1) 
				group by	ct.CINID
							,Ct.BrandID
							,DATEADD(m, DATEDIFF(m, 0, ct.TranDate), 0)
				Option(Recompile)
...
DECLARE @User nvarchar(30) = (SELECT USER_NAME())
	...
	EXEC('
			SELECT	* 
			FROM	Sandbox.' + @User + '.DoC_TotalSpend
			ORDER BY 1 ASC
		')
	...
	EXEC('
			SELECT	*
			FROM	Sandbox.' + @User + '.DoC_ExistingSpend
			ORDER BY 1 ASC
		')
	...
	EXEC('
			SELECT	*
			FROM	Sandbox.' + @User + '.DoC_AcquireCSSpend
			ORDER BY 1 ASC
		')
	...
	EXEC('
			SELECT	*
			FROM	Sandbox.' + @User + '.DoC_AcquireNewSpend
			ORDER BY 1 ASC
		')
	...
	EXEC('
			SELECT	*
			FROM	Sandbox.' + @User + '.DoC_AggSpend
			ORDER BY 1 ASC
		')
	...
	EXEC('
			SELECT	*
			FROM	Sandbox.' + @User + '.DoC_TotalSpendChange
			ORDER BY 1 ASC
		')
	...
	EXEC('
			SELECT	*
			FROM	Sandbox.' + @User + '.DoC_ExistingSpendChange
			ORDER BY 1 ASC
		')
	...
	EXEC('
			SELECT	*
			FROM	Sandbox.' + @User + '.DoC_AcquireCSSpendChange
			ORDER BY 1 ASC
		')
	...
	EXEC('
			SELECT	*
			FROM	Sandbox.' + @User + '.DoC_AcquireNewSpendChange
			ORDER BY 1 ASC
		')
