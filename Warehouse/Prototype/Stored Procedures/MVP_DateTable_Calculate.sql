...
-- =============================================
CREATE PROCEDURE [Prototype].[MVP_DateTable_Calculate]
AS
BEGIN
	...
	IF OBJECT_ID('Warehouse.Prototype.MVP_DateTable') IS NOT NULL DROP TABLE Warehouse.Prototype.MVP_DateTable
	CREATE TABLE Warehouse.Prototype.MVP_DateTable
		(
	...
	INSERT INTO Warehouse.Prototype.MVP_DateTable
		SELECT	* 
		FROM	CTE
	...
	JOIN	(
				SELECT	TOP 28 *
				FROM	Warehouse.Prototype.MVP_DateTable
				WHERE	CycleEnd < DATEADD(DAY,-7,GETDATE())
				ORDER BY ID DESC
			) b
		ON	dt.ID = b.ID

	-- SELECT * FROM Warehouse.Prototype.MVP_DateTable
END
