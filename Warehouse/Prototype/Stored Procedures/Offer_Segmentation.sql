...
CREATE Procedure [Prototype].[Offer_Segmentation] 
	(@PartnerNo int
	,@DateToBeSeg date
	,@IronOfferID int)
...
******************************************************************/
	...
	INSERT INTO #Customers
		EXEC('
				...
				Join		SLC_Report.dbo.Fan f on iom.CompositeID = f.CompositeID
...
******************************************************************/
	...
	INNER JOIN [Warehouse].[Prototype].[AirBnB_ALS_Splits] as c
		on	s.FanID = c.FanID and
			c.PartnerID = @PartnerID and
			s.Segment <> c.ShopperSegmentTypeID and
			c.EndDate IS NULL

	Insert into [Warehouse].[Prototype].[AirBnB_ALS_Splits]
	Select s.FanID, @PartnerID, s.Segment, @StartDate, NULL
	From #AllCustomers as s
	left Outer join [Warehouse].[Prototype].[AirBnB_ALS_Splits] as c
		on	s.FanID = c.FanID and
			c.PartnerID = @PartnerID and
			s.Segment = c.ShopperSegmentTypeID and
			c.EndDate IS NULL
