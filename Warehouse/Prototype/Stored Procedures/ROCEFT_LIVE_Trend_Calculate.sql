...
-- =============================================
CREATE PROCEDURE [Prototype].[ROCEFT_LIVE_Trend_Calculate]
	...
	IF @Bespoke = 1
		BEGIN
			IF @TableName IS NULL 
			  BEGIN
				RAISERROR ('Error #1 --- You need to supply a table name in a Bespoke Trend', 0, 1) WITH NOWAIT
				RETURN
			  END
			ELSE
				BEGIN
					IF 1 < @BrandCount
					  BEGIN
						RAISERROR ('Error #2 --- Bespoke Trend requires a single brand.', 0, 1) WITH NOWAIT
						RETURN
					  END
				END
		END
