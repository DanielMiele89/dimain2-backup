...
***************************************************************************/
CREATE PROCEDURE [Prototype].[ROCP2_Code_01_CustsDatesBrands]
	(
...
SELECT	a.*,
	COALESCE(blk.acquireL,AcquireL0) as AcquireL,
	COALESCE(blk.LapserL,LapserL0) as LapserL
...
FROM	(
	SELECT	b.BrandID,
		...
		WHERE	b.SectorID IN
			...
			WHERE SectorGroupID in (1,2) AND SectorName NOT IN ('Gambling','Utilities','Sports And Clubs')
			)
		)a
