...
CREATE PROCEDURE [Prototype].[StoredProcedureSearch_UpdateDefinition]	@DefinitionUpdateStart VARCHAR(MAX)
																	,	@DefinitionUpdateEnd VARCHAR(MAX)
																	,	@SearchString VARCHAR(MAX)
WITH EXECUTE AS OWNER
AS
BEGIN

	--DECLARE @DefinitionUpdateStart VARCHAR(MAX) = 'REPLACE(REPLACE(REPLACE(REPLACE('
	--	,	@DefinitionUpdateEnd VARCHAR(MAX) = ', ''['', ''''), '']'', ''''), ''dbo'', ''''), ''SLC_Report.'', ''SLC_REPL.'')'
	--	,	@SearchString VARCHAR(MAX) = '%SLC_REPL..Club%Status%'
...
     			 , o.type_desc AS ObjectType
			...
			WHERE ' + @DefinitionUpdateStart + 'm.definition' + @DefinitionUpdateEnd + ' LIKE ''' + @SearchString + ''''
