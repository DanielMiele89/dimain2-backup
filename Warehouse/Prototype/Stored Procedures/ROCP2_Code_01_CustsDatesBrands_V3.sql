...
***************************************************************************/
CREATE PROCEDURE [Prototype].[ROCP2_Code_01_CustsDatesBrands_V3]
	(
...
IF @IndividualBrand = 0
	BEGIN
		-- Make a correction for Warehouse.Relational.CustomerAttribute.FirstTranDate being fucked
		IF OBJECT_ID('tempdb..#CINs') IS NOT NULL DROP TABLE #CINs
		SELECT	DISTINCT cl.CIN as SourceUID
				,cl.CINID
		INTO	#CINs
		FROM	Relational.CINList cl WITH (NOLOCK) 
		JOIN	Relational.CustomerAttribute ca WITH (NOLOCK)
		...
		INSERT INTO Prototype.ROCP2_SegFore_FixedBase
			...
			FROM #cins_excCBP
			WHERE rand1 <= 2000000 -- Restricting customer base to 1m.
...
FROM	(
		SELECT	b.BrandID,
				b.BrandName,
				b.SectorID,
				---- Overwrite big 4 : could consider other overrides -- and using MTR for current brands
				CASE
					WHEN Brandname IN ('Tesco', 'Asda','Sainsburys','Morrisons') THEN 3
					ELSE lk.acquireL
				END AS AcquireL0,
		...
		LEFT JOIN Prototype.ROCP2_SegFore_SectorTimeFrame_LK lk
			ON lk.sectorid = b.sectorID
		WHERE	b.SectorID IN	(	-- Sector Exclusions Below
									SELECT	DISTINCT 
										SectorID 
									FROM Relational.BrandSector 
									WHERE SectorGroupID in (1,2) AND SectorName NOT IN ('Gambling')
