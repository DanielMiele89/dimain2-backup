...
-- *****************************************************************************************************
CREATE PROCEDURE [Prototype].[SSRS_R0080_Test_UnbrandedCCsToBeExcluded](
			@CC varchar(MAX))
...
INSERT INTO [Prototype].[SSRS_R0080_Test_Unbranded_CCsToBeExcluded] (ConsumerCombinationID)
SELECT cc.ConsumerCombinationID
FROM #CC cc
LEFT OUTER JOIN [Prototype].[SSRS_R0080_Test_Unbranded_CCsToBeExcluded] ce
	ON cc.ConsumerCombinationID = ce.ConsumerCombinationID
WHERE ce.ConsumerCombinationID IS NULL

DELETE FROM [Warehouse].[Prototype].[SSRS_R0080_Test_Unbranded_BrandSuggestions]
WHERE ConsumerCombinationID IN (SELECT ConsumerCombinationID
								FROM #CC)

UPDATE [Warehouse].[Prototype].[SSRS_R0080_Test_Unbranded_CCsToBeExcluded]
SET MID = cc.MID
FROM [Warehouse].[Relational].[ConsumerCombination] cc
INNER JOIN [Warehouse].[Prototype].[SSRS_R0080_Test_Unbranded_CCsToBeExcluded] cce
	ON cc.ConsumerCombinationID = cce.consumercombinationid
WHERE cce.MID IS NULL