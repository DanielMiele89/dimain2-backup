...
***************************************************************************/
CREATE PROCEDURE [Prototype].[ROCP2_Code_02F_FindNaturalSales_FiAlgorithm]  -- THIS SIMPLE FI ALGORITHM
	(
	@IndividualBrand BIT
	)
AS
...
FROM	(
	SELECT	b.CINID
		...
		--	ELSE 'ERROR'
		--END as Segment_Ag1
     ,case when s.SpderA is NULL and coalesce(Response_Index,100)>=80 and marketablebyemail=1 then 'Acquire' 
           when s.SpderA is NULL OR marketablebyemail=0  then 'LowInterest'  -- Need to expliciting CODE the NON-Marketable: SB is taking them out to start with and then doing cuts
		                                                                    -- might be better but don't have time to recode.  Not urgent
		   
           when s.SpderLastx is NULL and s.SpderA=1 and sales>=coalesce(cl.Topsalescut,0) then 'WinbackPrime'
		   when s.SpderLastx is NULL and s.SpderA=1 then 'Winback'
		   when  s.SpderLastx is not null   and (s.sales>=coalesce(c.Topsalescut,0)) then 'Retain'
           when s.SpderLastx is not null   then 'Grow'
