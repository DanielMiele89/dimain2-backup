...
CREATE PROCEDURE [MIDI].[ManualModule_ExistingCombinations]
...
*******************************************************************************************************************************************/
	...
	SELECT	MID
		...
		,	MCCID
		,	OriginatorID
		,	MIN(CONVERT(INT, IsCreditOrigin)) AS IsCreditOrigin
		,	COUNT(*) AS MatchCount
	FROM [Relational].[ConsumerCombination] cc
	WHERE cc.BrandID = 944
	AND cc.MID != ''
	AND EXISTS (SELECT	ct.ConsumerCombinationID
				FROM [Relational].[ConsumerTransaction_MyRewards] ct
				WHERE ct.ConsumerCombinationID = cc.ConsumerCombinationID
				AND ct.TranDate > '2020-11-27'
				GROUP BY ct.ConsumerCombinationID
				HAVING SUM(ct.Amount) > 10000)
	GROUP BY	MID
			,	Narrative
			,	LocationCountry
			,	MCCID
