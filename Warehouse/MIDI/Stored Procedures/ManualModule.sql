...
*******************************************************************************************************************************************/
		...
		IF OBJECT_ID('tempdb..#Combinations_Temp') IS NOT NULL DROP TABLE #Combinations_Temp
		SELECT	DISTINCT
				BrandID
			,	MID
			,	Narrative
		...
		UPDATE c
		SET Narrative = LEFT(Narrative, CHARINDEX(' ', Narrative)) + '_____' + SUBSTRING(Narrative, CHARINDEX(' ', Narrative) + 6, 9999)
		,	Narrative_Cleaned = LEFT(Narrative_Cleaned, CHARINDEX(' ', Narrative_Cleaned)) + '_____' + SUBSTRING(Narrative_Cleaned, CHARINDEX(' ', Narrative_Cleaned) + 6, 9999)
		FROM #Combinations_Temp c
		WHERE Narrative LIKE 'NOW _____ %'
		OR Narrative LIKE 'NOW _____'
		OR Narrative LIKE 'CRV*NOW _____'
		OR Narrative LIKE 'CRV*NOW _____ %'
		
		UPDATE mnc
		SET	Narrative_Cleaned = ISNULL(x.Narrative_Cleaned, mnc.Narrative)
		FROM #Combinations_Temp mnc -- 90,605
