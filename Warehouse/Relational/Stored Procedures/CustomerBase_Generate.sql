...
-- =============================================
CREATE PROCEDURE [Relational].[CustomerBase_Generate] 
	(
...
BEGIN
	...
	BEGIN TRY
		...
		SELECT CINID from Relational.SandyAccount_IncCINIDs --JEA 07/4/2014 - exclude those deactivated due to Project Sandy
		
		--Add a primary key to assist the gathering of trans count data
		ALTER TABLE #CustBaseTest ADD PRIMARY KEY(CINID)
		
		
		--Obtain the tran count for the customers - WARNING: this is the most intensive part of the process because of the size of ConsumerTransaction
		UPDATE #CustBaseTest SET TranCount = t.TranCount
		FROM #CustBaseTest c
		INNER JOIN (SELECT ct.CINID, COUNT(1) AS TranCount
					FROM Relational.ConsumerTransaction C WITH (NOLOCK)
					INNER JOIN Relational.ConsumerSector cs ON C.ConsumerCombinationID = cs.ConsumerCombinationID  --Only retail transactions are to be counted
					INNER JOIN #CustBaseTest ct ON c.CINID = ct.CINID
					WHERE c.TranDate BETWEEN @StartDate AND @EndDate
					GROUP BY ct.CINID) t ON c.CINID = t.CINID

		UPDATE #CustBaseTest SET TranCount = c.TranCount + t.TranCount
		FROM #CustBaseTest c
		INNER JOIN (SELECT ct.CINID, COUNT(1) AS TranCount
					FROM Relational.ConsumerTransactionHolding C WITH (NOLOCK)
					INNER JOIN Relational.ConsumerSector cs ON C.ConsumerCombinationID = cs.ConsumerCombinationID  --Only retail transactions are to be counted
					INNER JOIN #CustBaseTest ct ON c.CINID = ct.CINID
					WHERE c.TranDate BETWEEN @StartDate AND @EndDate
					GROUP BY ct.CINID) t ON c.CINID = t.CINID
...
GO
GRANT EXECUTE
    ON OBJECT::[Relational].[CustomerBase_Generate] TO [InsightTeam]
    AS [dbo];
