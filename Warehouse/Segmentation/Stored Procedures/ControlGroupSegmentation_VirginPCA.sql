...
CREATE PROCEDURE [Segmentation].[ControlGroupSegmentation_VirginPCA] (	@PartnerID INT
																,	@EnDate DATE
																,	@TName VARCHAR(200))
...
BEGIN
	...
	/*
	Execution	TableName
EXEC Warehouse.Segmentation.ControlGroupSegmentation_VirginPCA 4117, '2021-01-28','Sandbox.Rory.VirginPCAControl411720210128'	Sandbox.Rory.VirginPCAControl4117202101288
*/
	...
	--DECLARE @TName varchar(200) = 'Sandbox.Rory.VirginPCAControl342120210225'	
	...
	BEGIN
		...
		BEGIN -- Load source control group members for retailers requiring bespoke setup
			...
			From (
					...
					Inner join [WH_VirginPCA].[Derived].[Customer] c -- Make sure in programme
						on cg.FanID = c.FanID
					Inner join [WH_VirginPCA].[Derived].[CINList] cl
	...
	***********************************************************************************************/
	...
	Set	
		EndDate = @EndTime
	Where
		StoredProcedureName = 'ControlGroupSegmentation_VirginPCA' 
