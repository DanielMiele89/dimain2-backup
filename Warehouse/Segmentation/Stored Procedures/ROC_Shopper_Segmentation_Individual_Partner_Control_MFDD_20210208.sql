...
CREATE PROCEDURE [Segmentation].[ROC_Shopper_Segmentation_Individual_Partner_Control_MFDD_20210208] (
		@PartnerNo int,
		@EnDate Date,
		@TName varchar(200)
		)
...
BEGIN
	...
	***********************************************************************************************/
	...
	INNER JOIN Warehouse.Relational.DirectDebitOriginator o
		ON oin.OIN = o.OIN
	WHERE
		oin.PartnerID IN (SELECT PartnerID FROM #PartnerIDs);

	CREATE UNIQUE CLUSTERED INDEX CIX_DDSuppliers ON #DDSuppliers (DirectDebitOriginatorID, PartnerID, StartDate, EndDate);
	...
	***********************************************************************************************/
	...
	Select
		...
		, c.CINID			
		, Max([Date]) as LatestTran
		, Sum(Amount) as Spend
		, Case
			When Max([Date]) < @LDate Then 1
			Else
				Case when Max([Date]) < Dateadd(month,-(@Shopper),Dateadd(day,DATEDIFF(dd, 0,@EndDate)-2,0)) Then 0 Else null End
		End as Lapsed
		, 0 as Prime
		, 0 as Segment
