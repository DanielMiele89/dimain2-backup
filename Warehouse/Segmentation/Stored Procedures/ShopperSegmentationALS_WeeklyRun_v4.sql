...
*/
CREATE PROCEDURE [Segmentation].[ShopperSegmentationALS_WeeklyRun_v4] (@EmailDate DATE)
AS
BEGIN
	...
	*******************************************************************************************************************************************/
		...
		--DECLARE @EmailDate DATE = DATEADD(day, 0, '2020-11-19')
	...
	*******************************************************************************************************************************************/
		...
		SELECT	als.EmailDate
			...
			,	0 AS IsDD
			,	CASE
					WHEN EXISTS (SELECT 1 FROM [Selections].[ROCShopperSegment_PreSelection_ALS] al WHERE als.ClientServicesRef = al.ClientServicesRef AND al.EmailDate < @EDate) THEN 0
					ELSE 1
				END AS FirstEmail
			,	CASE
					WHEN RandomThrottle = 0 AND Throttling != '0,0,0,0,0,0' THEN 1
					ELSE NULL
				END AS ToBeRanked
		INTO #UpcomingCampaigns
		FROM [Selections].[ROCShopperSegment_PreSelection_ALS] als
		WHERE als.EmailDate = @EDate
		UNION ALL
		SELECT	cs_c.EmailDate
			,	cs_c.PartnerID
			,	cs_c.ClientServicesRef
			,	@FullCycle AS IsFullCycle
			,	NULL AS SegmentationOverride
			,	1 AS IsPOS
			,	0 AS IsDD
			,	CASE
					WHEN EXISTS (SELECT 1 FROM [Selections].[CampaignSetup_DD] cs_p WHERE cs_c.ClientServicesRef = cs_p.ClientServicesRef AND cs_p.EmailDate < @EDate) THEN 0
					ELSE 1
				END AS FirstEmail
			,	CASE
					WHEN RandomThrottle = 0 AND Throttling != '0,0,0,0,0,0' THEN 1
					ELSE NULL
				END AS ToBeRanked
		FROM [Selections].[CampaignSetup_DD] cs_c
		WHERE cs_c.EmailDate = @EDate
	...
	*******************************************************************************************************************************************/
		...
		UpcomingCampaigns AS (	SELECT	PartnerID
									,	MAX(COALESCE(ToBeRanked, 0)) AS ToBeRanked
									,	MAX(IsPOS) AS IsPOS
									,	MAX(IsDD) AS IsDD
									,	MAX(COALESCE(SegmentationOverride, ToBeRanked, IsFullCycle, FirstEmail)) AS SegmetationToRun
	...
	*******************************************************************************************************************************************/
		...
		BEGIN
			...
			IF @IsPOS = 1 EXEC [Segmentation].[Segmentation_IndividualPartner_POS] @PartnerID, @ToBeRanked, 1	--	PartnerID, ToBeRanked, WeekyRun
			IF @IsDD = 1 EXEC [Segmentation].[Segmentation_IndividualPartner_DD] @PartnerID, @ToBeRanked, 1, 56, 1	--	PartnerID, ToBeRanked, ExlcudeNewJoiners, NewJoinerLength_Days, WeekyRun

			SET @RowNo = @RowNo+1

		END

		ALTER INDEX [ix_PartnerID_FanID] ON [Segmentation].[Roc_Shopper_Segment_CustomerRanking] REBUILD WITH (DATA_COMPRESSION = ROW, FILLFACTOR = 80, SORT_IN_TEMPDB = ON)
		ALTER INDEX [IX_DDCustRank_PartnerFan] ON [Segmentation].[CustomerRanking_DD] REBUILD WITH (DATA_COMPRESSION = ROW, FILLFACTOR = 80, SORT_IN_TEMPDB = ON)
