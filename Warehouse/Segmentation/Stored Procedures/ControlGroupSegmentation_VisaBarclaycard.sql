...
CREATE PROCEDURE [Segmentation].[ControlGroupSegmentation_VisaBarclaycard] (	@PartnerID INT
																,	@EnDate DATE
																,	@TName VARCHAR(200))
...
BEGIN
	...
	/*
	Execution	TableName
EXEC Warehouse.Segmentation.ControlGroupSegmentation_VisaBarclaycard 4117, '2021-01-28','Sandbox.Rory.VisaBarclaycardControl411720210128'	Sandbox.Rory.VisaBarclaycardControl4117202101288
*/
	...
	--DECLARE @TName varchar(200) = 'Sandbox.Rory.VisaBarclaycardControl342120210225'	
	...
	BEGIN
		...
		BEGIN -- Load source control group members for retailers requiring bespoke setup
			...
			From (
					...
					Inner join [WH_Visa].[Derived].[Customer] c -- Make sure in programme
						on cg.FanID = c.FanID
					Inner join [WH_Visa].[Derived].[CINList] cl
	...
	***********************************************************************************************/
	...
	Set	
		EndDate = @EndTime
	Where
		StoredProcedureName = 'ControlGroupSegmentation_VisaBarclaycard' 
