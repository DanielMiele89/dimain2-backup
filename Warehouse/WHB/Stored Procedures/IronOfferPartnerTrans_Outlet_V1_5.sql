...
- 09/09/2014 - Removed Index DROP as now table has OutletID as it's PRIMARY KEY
- 31/01/2020 Jason Shipp: Added PartnerOutletReference to load
- 17/02/2020 Jason Shipp: Added nFI OutletIDs to load, by adding nFI.Relational.Partner to the partner INNER JOIN condition
			
*/

CREATE PROCEDURE [WHB].[IronOfferPartnerTrans_Outlet_V1_5]
AS
BEGIN
...
BEGIN TRY
	...
	select	ro.ID as OutletID,
			...
			ro.PartnerOutletReference				as PartnerOutletReference
	from	SLC_Report.dbo.RetailOutlet ro with (nolock)
			left join SLC_Report.dbo.Fan f with (nolock) on ro.FanID = f.ID
			inner join (select PartnerID from Warehouse.Relational.[Partner] Union Select PartnerID from nFI.Relational.[Partner]) p on ro.PartnerID = p.PartnerID
