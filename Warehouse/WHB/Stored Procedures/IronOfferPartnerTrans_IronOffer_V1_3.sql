...
Purpose:	Full load of IronOffer table
...
*/
CREATE PROCEDURE [WHB].[IronOfferPartnerTrans_IronOffer_V1_3]
...
BEGIN TRY
	...
	/*--------------------------------------------------------------------------------------------------
	-----------------------------Grabbing Commision Values--------------------------------------------
	----------------------------------------------------------------------------------------------------*/
	IF OBJECT_ID('tempdb.dbo.#temp', 'U') IS NOT NULL
	DROP TABLE #temp;


	SELECT	RequiredIronOfferID AS IronOfferID
		,	MAX(CASE 
					WHEN MaximumUsesPerFan IS NULL THEN 'Unlimited'
					ELSE CAST(MaximumUsesPerFan AS varchar)
				END) AS SingleUseOffer
		,	MAX(CASE 
					WHEN CommissionAmount IS NOT NULL THEN 'Cashback'
					WHEN CommissionRate IS NOT NULL THEN 'Rate'
				END) AS 'CommissionType'
		,   MAX(CommissionLimit) AS CommissionLimit
	INTO #temp
	FROM [SLC_REPL].[dbo].[PartnerCommissionRule] pcr
	WHERE	(MaximumUsesPerFan IS NOT NULL
			OR (pcr.CommissionRate IS NOT NULL OR pcr.CommissionAmount IS NOT NULL)
			OR CommissionLimit IS NOT NULL)
	AND TypeID = 1
	AND Status = 1
	AND RequiredIronOfferID IS NOT NULL
	...
	SELECT Cast(i.[ID] as Int)							as IronOfferID
		  ...
		  ,Cast(I2.[AutoAddToNewRegistrants] as bit)		as AutoAddToNewRegistrants
		  ,Cast(I.[IsDefaultCollateral] as bit)			as IsDefaultCollateral
		  ,Cast(I.[IsSignedOff] as bit)					as IsSignedOff
		  ,Cast(I2.[AreEligibleMembersCommitted] as bit)	as AreEligibleMembersCommitted
		  ,Cast(I2.[AreControlMembersCommitted] as bit)	as AreControlMembersCommitted
		  ...
		  ,Case
		   ...
		   Case
			...
			End as CampaignType,
			t.SingleUseOffer as EarningCount,
			t.CommissionType as EarningType,
			t.CommissionLimit as EarningLimit
	FROM [SLC_REPL].[dbo].[IronOffer] as I with (nolock)
	Left Outer Join [SLC_Report].[dbo].[IronOffer] as I2 with (nolock)
		ON i.ID = i2.ID
	...
	From Relational.IronOffer as i
	left outer join SLC_REPL..IronOfferClub as ioc
