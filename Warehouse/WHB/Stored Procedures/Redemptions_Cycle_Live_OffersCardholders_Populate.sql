...
CREATE PROCEDURE [WHB].[Redemptions_Cycle_Live_OffersCardholders_Populate]
...
BEGIN TRY
	...
	--------------------------------------------------------------------------------------------------*/
		...
		SELECT StoredProcedureName = OBJECT_NAME(@@PROCID)
			 , TableSchemaName = 'MI'
			 , TableName = 'Cycle_Live_OffersCardholders'
			 , StartDate = GETDATE()
			 , EndDate = NULL
			 , TableRowCount  = NULL
			 , AppendReload = 'A'
	...
	***************************************************************************/
	...
	FROM
		...
		LEFT JOIN -- Get cashback rates
			...
			FROM relational.IronOffer_PartnerCommissionRule pcr
			WHERE 
				pcr.TypeID = 1
				AND pcr.Status = 1
			GROUP BY  
		...
		LEFT JOIN Relational.IronOffer_Campaign_HTM camp
			ON io.IronOfferID = camp.IronOfferID
		LEFT JOIN 
			(SELECT DISTINCT OfferID FROM Relational.PartnerOffers_Base) base
	...
	***************************************************************************/
	...
	FROM
		...
		WHERE
			CAST(c.ActivatedDate AS date) <= @CycleStart
			AND(
	...
	***************************************************************************/
	...
	IF OBJECT_ID('tempdb..#Cardholders') IS NOT NULL 
	...
	***************************************************************************/
	...
	IF OBJECT_ID('tempdb..#NonBaseOfferCardholders') IS NOT NULL 
		DROP TABLE #NonBaseOfferCardholders;
	...
	***************************************************************************/
	...
	IF OBJECT_ID('tempdb..#BaseOfferCardholders') IS NOT NULL 
		DROP TABLE #BaseOfferCardholders;
	...
	***************************************************************************/
	...
	TRUNCATE TABLE MI.Cycle_Live_OffersCardholders;
 
	INSERT INTO MI.Cycle_Live_OffersCardholders
		(ReportDate
		, CycleStart
		, CycleEnd
		, ClubID
	...
	/**************************************************************************
	-- Create table for holding data to feed into report --

	CREATE TABLE Warehouse.MI.Cycle_Live_OffersCardholders
