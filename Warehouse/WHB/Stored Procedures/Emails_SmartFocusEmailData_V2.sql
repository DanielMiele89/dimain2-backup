...
Update:		This version is being amended for use as a stored procedure and to be ultimately automated.
			...
			06/08/2021 CJM reverted back to ordinary indexes
*/
CREATE PROCEDURE [WHB].[Emails_SmartFocusEmailData_V2]
AS
BEGIN
...
IF EXISTS (SELECT 1 FROM sys.indexes WHERE object_id = OBJECT_ID('Relational.EmailEvent') AND [name] = 'CSX_All')
	DROP INDEX [CSX_All] ON [Relational].[EmailEvent]

INSERT INTO [Relational].[EmailEvent] (EventID, EventDateTime, EventDate, FanID, CompositeID, CampaignKey, EmailEventCodeID)
SELECT ee.EventID
	, ee.EventDateTime
...
--ALTER INDEX IDX_FanID ON Relational.EmailEvent REBUILD WITH (SORT_IN_TEMPDB = ON, FILLFACTOR = 70) -- 00:36:43
--ALTER INDEX IDX_Camp ON Relational.EmailEvent REBUILD WITH (SORT_IN_TEMPDB = ON, FILLFACTOR = 70) -- 00:39:06
