/*
Migration changes 19/11/2021 CJM
Commented out index builds and rebuilds
*/
CREATE PROCEDURE [WHB].[Emails_LionSendTracking_V2]
as
Begin
...
*******************************************************************************************************************************************/
	...
	IF NOT EXISTS (	SELECT 1
		...
		BEGIN
			...
			-- Migration changes CJM 19/11/2021
			IF EXISTS (SELECT 1 FROM sys.indexes WHERE object_id = OBJECT_ID('Lion.LionSend_Offers') AND [name] = 'CSX_LionSendOffers_All')				  
				DROP INDEX [CSX_LionSendOffers_All] ON [Lion].[LionSend_Offers]

			IF EXISTS (SELECT 1 FROM sys.indexes WHERE object_id = OBJECT_ID('Lion.LionSend_Customers') AND [name] = 'CSX_LionSendCustomers_All')				  
