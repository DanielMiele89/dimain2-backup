...
-- =============================================
Create PROCEDURE [MI].[RetailerMonthlyReportSPLITTotalsSinceLaunch_Fetch]
...
FROM #MonthTotals RM
  ...
  -- INNER JOIN [Warehouse].[MI].retailerreportmonthlylabels ML on RM.[LabelID] = ML.[LabelID]
   INNER JOIN [Warehouse].[MI].[RetailerReportSplitMonthly] RMC ON RM.partnerID = RMC.partnerID AND RMC.monthid = RM.Monthid And RM.SplitID = RMC.SplitID And RM.StatusTypeID = RMC.StatusTypeID
WHERE  RM.[monthid] = @MonthID 
