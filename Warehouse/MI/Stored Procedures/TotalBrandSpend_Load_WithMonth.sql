...
-- =============================================
CREATE PROCEDURE [MI].[TotalBrandSpend_Load_WithMonth]
...
BEGIN
	...
	SET @ThisMonthStart = DATEADD(MONTH, -1, @CurrentMonthStart)
	SET @ThisMonthStartYearAgo = DATEADD(YEAR, -1, @ThisMonthStart)
	...
	SELECT b.BrandID, SUM(ct.Amount) AS SpendThisMonth
		, COUNT(1) AS TranCountThisMonth
		, COUNT(DISTINCT ct.CINID) AS CustomerCountThisMonth
	INTO #TotalSpendThisMonth
	FROM Relational.ConsumerTransaction ct WITH (NOLOCK)
	INNER JOIN #Combos b ON ct.ConsumerCombinationID = b.ConsumerCombinationID
	WHERE ct.TranDate BETWEEN @ThisMonthStart AND @ThisYearEnd
	GROUP BY b.BrandID

	INSERT INTO MI.TotalBrandSpendLoadAudit(AuditAction, AuditDate)
	VALUES('Total Spend This Month', GETDATE())
	...
	--Online Spend This Month
	SELECT b.BrandID, SUM(ct.Amount) AS SpendThisMonth
		, COUNT(1) AS TranCountThisMonth
		, COUNT(DISTINCT ct.CINID) AS CustomerCountThisMonth
	INTO #OnlineSpendThisMonth
	FROM Relational.ConsumerTransaction ct WITH (NOLOCK)
	INNER JOIN #Combos b ON ct.ConsumerCombinationID = b.ConsumerCombinationID
	WHERE ct.TranDate BETWEEN @ThisMonthStart AND @ThisYearEnd
	AND ct.IsOnline = 1
	...
	INSERT INTO MI.TotalBrandSpendLoadAudit(AuditAction, AuditDate)
	VALUES('Online Spend This Month', GETDATE())
	...
	--Total Spend Last Month - year ago
	SELECT b.BrandID, SUM(ct.Amount) AS SpendThisMonthYearAgo
		, COUNT(1) AS TranCountThisMonthYearAgo
		, COUNT(DISTINCT ct.CINID) AS CustomerCountThisMonthYearAgo
	INTO #TotalSpendThisMonthYearAgo
	FROM Relational.ConsumerTransaction ct WITH (NOLOCK)
	INNER JOIN #Combos b ON ct.ConsumerCombinationID = b.ConsumerCombinationID
	WHERE ct.TranDate BETWEEN @ThisMonthStartYearAgo AND @LastYearEnd
	GROUP BY b.BrandID

	INSERT INTO MI.TotalBrandSpendLoadAudit(AuditAction, AuditDate)
	VALUES('Total Spend This Month - year ago', GETDATE())
	...
	--online spend last month - year ago
	SELECT b.BrandID, SUM(ct.Amount) AS SpendThisMonthYearAgo
	, COUNT(1) AS TranCountThisMonthYearAgo
	, COUNT(DISTINCT ct.CINID) AS CustomerCountThisMonthYearAgo
	INTO #OnlineSpendThisMonthYearAgo
	FROM Relational.ConsumerTransaction ct WITH (NOLOCK)
	INNER JOIN #Combos b ON ct.ConsumerCombinationID = b.ConsumerCombinationID
	WHERE ct.TranDate BETWEEN @ThisMonthStartYearAgo AND @LastYearEnd
	AND ct.IsOnline = 1
	...
	INSERT INTO MI.TotalBrandSpendLoadAudit(AuditAction, AuditDate)
	VALUES('Online Spend This Month - year ago', GETDATE())
	...
	--Total Spend This Month - FIXED BASE
	SELECT b.BrandID, SUM(ct.Amount) AS SpendThisMonth
		, COUNT(1) AS TranCountThisMonth
		, COUNT(DISTINCT ct.CINID) AS CustomerCountThisMonth
	INTO #TotalSpendThisMonthFixedBase
	FROM Relational.ConsumerTransaction ct WITH (NOLOCK)
	INNER JOIN #Combos b ON ct.ConsumerCombinationID = b.ConsumerCombinationID
	INNER JOIN InsightArchive.WETSFixedBase w ON ct.CINID = w.CINID
	WHERE ct.TranDate BETWEEN @ThisMonthStart AND @ThisYearEnd
	...
	INSERT INTO MI.TotalBrandSpendLoadAudit(AuditAction, AuditDate)
	VALUES('Total Spend This Month - Fixed Base', GETDATE())
	...
	--Online Spend This Month - FIXED BASE
	SELECT b.BrandID, SUM(ct.Amount) AS SpendThisYear
		, COUNT(1) AS TranCountThisYear
		, COUNT(DISTINCT ct.CINID) AS CustomerCountThisYear
	INTO #OnlineSpendThisMonthFixedBase
	FROM Relational.ConsumerTransaction ct WITH (NOLOCK)
	INNER JOIN #Combos b ON ct.ConsumerCombinationID = b.ConsumerCombinationID
	INNER JOIN InsightArchive.WETSFixedBase w ON ct.CINID = w.CINID
	WHERE ct.TranDate BETWEEN @ThisMonthStart AND @ThisYearEnd
	...
	INSERT INTO MI.TotalBrandSpendLoadAudit(AuditAction, AuditDate)
	VALUES('Online Spend This Month - Fixed Base', GETDATE())
	...
	VALUES('Total Spend Last Year - Fixed Base', GETDATE())
		...
		--Total Spend this month last year - FIXED BASE
	SELECT b.BrandID, SUM(ct.Amount) AS SpendThisMonthLastYear
		, COUNT(1) AS TranCountThisMonthLastYear
		, COUNT(DISTINCT ct.CINID) AS CustomerCountThisMonthLastYear
	INTO #TotalSpendThisMonthLastYearFixedBase
	FROM Relational.ConsumerTransaction ct WITH (NOLOCK)
	INNER JOIN #Combos b ON ct.ConsumerCombinationID = b.ConsumerCombinationID
	INNER JOIN InsightArchive.WETSFixedBase w ON ct.CINID = w.CINID
	WHERE ct.TranDate BETWEEN @ThisMonthStartYearAgo AND @LastYearEnd
	...
	INSERT INTO MI.TotalBrandSpendLoadAudit(AuditAction, AuditDate)
	VALUES('Total Spend This Month Last Year - Fixed Base', GETDATE())
	...
	SELECT b.BrandID, SUM(ct.Amount) AS SpendThisMonthLastYear
		, COUNT(1) AS TranCountThisMonthLastYear
		, COUNT(DISTINCT ct.CINID) AS CustomerCountThisMonthLastYear
	INTO #OnlineSpendThisMonthLastYearFixedBase
	FROM Relational.ConsumerTransaction ct WITH (NOLOCK)
	INNER JOIN #Combos b ON ct.ConsumerCombinationID = b.ConsumerCombinationID
	INNER JOIN InsightArchive.WETSFixedBase w ON ct.CINID = w.CINID
	WHERE ct.TranDate BETWEEN @LastYearStart AND @LastYearEnd
	...
	INSERT INTO MI.TotalBrandSpendLoadAudit(AuditAction, AuditDate)
	VALUES('Retailer Prospects Refreshed', GETDATE())
