...
CREATE PROCEDURE MI.CampaignResults_InternalFinalCSR_Store AS -- unhide this row to modify SP
...
BEGIN 
    ...
    LEFT JOIN Warehouse.MI.CampaignInternalResultsFinalWave rf 
    ...
    DELETE FROM csr
    FROM Warehouse.MI.CampaignInternalResultsFinalCSR  csr
    ...
    INSERT INTO Warehouse.MI.CampaignInternalResultsFinalCSR 
    (ClientServicesRef,
    ...
    INNER JOIN Warehouse.MI.CampaignInternalResultsFinalWave rf  ON rf.ClientServicesRef=wm.ClientServicesRef
    WHERE NOT EXISTS (SELECT 1 FROM Warehouse.MI.CampaignInternalResultsFinalCSR old 
    WHERE old.ClientServicesRef=wm.ClientServicesRef)
    GROUP BY wm.ClientServicesRef, 
    wm.ControlGroup,wm.CustomerUniverse,wm.AggregationLevel

    			 -- Uplif calculations
			 UPDATE Warehouse.MI.CampaignInternalResultsFinalCSR
			 ...
			 UPDATE Warehouse.MI.CampaignInternalResultsFinalCSR SET
			...
			-- Significance calculations
			 ...
			 SignificantUpliftSPC=CASE WHEN TScoreSPC>=Probability_05 THEN 'High'
						  ...
						  ELSE 'No' END
			 FROM Warehouse.MI.CampaignInternalResultsFinalCSR w
    ...
    LEFT JOIN Warehouse.MI.CampaignInternalResultsFinalWave_BespokeCell rf 
    ...
    -- Insert new results on CSR level as a sum of Wave results
    INSERT INTO Warehouse.MI.CampaignInternalResultsFinalCSR_BespokeCell 
    ...
    INNER JOIN Warehouse.MI.CampaignInternalResultsFinalWave_BespokeCell rf  ON rf.ClientServicesRef=wm.ClientServicesRef
    WHERE NOT EXISTS (SELECT 1 FROM Warehouse.MI.CampaignInternalResultsFinalCSR_BespokeCell old 
    WHERE old.ClientServicesRef=wm.ClientServicesRef AND old.Cell=rf.Cell)
    GROUP BY wm.ClientServicesRef, rf.Cell,
    wm.ControlGroup,wm.CustomerUniverse,wm.Level

    			 -- Uplif calculations
			 UPDATE Warehouse.MI.CampaignInternalResultsFinalCSR_BespokeCell
			 SET SalesUplift=CASE WHEN IncrementalSales<Sales THEN 1.0*IncrementalSales/(Sales-IncrementalSales) ELSE IncrementalSales END
			   ,RRUplift=CASE WHEN IncrementalSpenders<Spenders THEN 1.0*IncrementalSpenders/(Sales-IncrementalSpenders) ELSE IncrementalSpenders END
			   ,ATVUplift=CASE WHEN IncrementalTransactions<Transactions AND IncrementalSales<Sales AND Transactions>0 THEN (1.0*Sales/Transactions)/(1.0*(Sales-IncrementalSales)/(Transactions-IncrementalTransactions))-1.0 ELSE 0 END
			   ,ATFUplift=CASE WHEN IncrementalSpenders<Spenders AND IncrementalTransactions<Transactions AND Spenders>0 THEN (1.0*Transactions/Spenders)/(1.0*(Transactions-IncrementalTransactions)/(Spenders-IncrementalSpenders))-1.0 ELSE 0 END
			 ...
			 UPDATE Warehouse.MI.CampaignInternalResultsFinalCSR_BespokeCell SET
			 TScoreSPC=CASE WHEN PooledStdDevSPC>0 THEN ABS(1.0*IncrementalSales/Cardholders)/PooledStdDevSPC ELSE 0 END,
			 TScoreRR=CASE WHEN PooledStdDevRR>0 THEN ABS(1.0*IncrementalSpenders/Cardholders)/PooledStdDevRR ELSE 0 END,
			 TScoreSPS=CASE WHEN PooledStdDevSPS>0 THEN ABS(1.0*SPS_Diff)/PooledStdDevSPS ELSE 0 END
			 WHERE TScoreSPC IS NULL
			...
			-- Significance calculations
			 ...
			 SignificantUpliftSPC=CASE WHEN TScoreSPC>=Probability_05 THEN 'High'
						  WHEN  TScoreSPC>=Probability_20 THEN 'Moderate'
						  ELSE 'No' END
			 FROM Warehouse.MI.CampaignInternalResultsFinalCSR_BespokeCell w
			 LEFT JOIN Warehouse.Stratification.TTestValues t on w.DegreesOfFreedomSPC
			 BETWEEN t.MinDegreesOfFreedom AND COALESCE(t.MaxDegreesOfFreedom,w.DegreesOfFreedomSPC) and t.Tailes=2
			 WHERE PValueSPC IS NULL
    ...
    LEFT JOIN Warehouse.MI.CampaignInternalResultsFinalWave_Segment rf 
    ...
    -- Insert new results on CSR level as a sum of Wave results
    INSERT INTO Warehouse.MI.CampaignInternalResultsFinalCSR_Segment 
    ...
    INNER JOIN Warehouse.MI.CampaignInternalResultsFinalWave_Segment rf  ON rf.ClientServicesRef=wm.ClientServicesRef
    WHERE NOT EXISTS (SELECT 1 FROM Warehouse.MI.CampaignInternalResultsFinalCSR_Segment old 
    ...
    wm.ControlGroup,wm.CustomerUniverse,wm.Level
    			 ...
    			 -- Uplif calculations
			 UPDATE Warehouse.MI.CampaignInternalResultsFinalCSR_Segment
			 SET SalesUplift=CASE WHEN IncrementalSales<Sales THEN 1.0*IncrementalSales/(Sales-IncrementalSales) ELSE IncrementalSales END
			   ,RRUplift=CASE WHEN IncrementalSpenders<Spenders THEN 1.0*IncrementalSpenders/(Sales-IncrementalSpenders) ELSE IncrementalSpenders END
			   ,ATVUplift=CASE WHEN IncrementalTransactions<Transactions AND IncrementalSales<Sales AND Transactions>0 THEN (1.0*Sales/Transactions)/(1.0*(Sales-IncrementalSales)/(Transactions-IncrementalTransactions))-1.0 ELSE 0 END
			   ,ATFUplift=CASE WHEN IncrementalSpenders<Spenders AND IncrementalTransactions<Transactions AND Spenders>0 THEN (1.0*Transactions/Spenders)/(1.0*(Transactions-IncrementalTransactions)/(Spenders-IncrementalSpenders))-1.0 ELSE 0 END
			 ...
			 UPDATE Warehouse.MI.CampaignInternalResultsFinalCSR_Segment SET
			 TScoreSPC=CASE WHEN PooledStdDevSPC>0 THEN ABS(1.0*IncrementalSales/Cardholders)/PooledStdDevSPC ELSE 0 END,
			 TScoreRR=CASE WHEN PooledStdDevRR>0 THEN ABS(1.0*IncrementalSpenders/Cardholders)/PooledStdDevRR ELSE 0 END,
			 TScoreSPS=CASE WHEN PooledStdDevSPS>0 THEN ABS(1.0*SPS_Diff)/PooledStdDevSPS ELSE 0 END
			 WHERE TScoreSPC IS NULL
			...
			-- Significance calculations
			 ...
			 SignificantUpliftSPC=CASE WHEN TScoreSPC>=Probability_05 THEN 'High'
						  WHEN  TScoreSPC>=Probability_20 THEN 'Moderate'
						  ELSE 'No' END
			 FROM Warehouse.MI.CampaignInternalResultsFinalCSR_Segment w
			 LEFT JOIN Warehouse.Stratification.TTestValues t on w.DegreesOfFreedomSPC
			 BETWEEN t.MinDegreesOfFreedom AND COALESCE(t.MaxDegreesOfFreedom,w.DegreesOfFreedomSPC) and t.Tailes=2
			 WHERE PValueSPC IS NULL
    ...
    LEFT JOIN Warehouse.MI.CampaignInternalResultsFinalWave_SuperSegment rf 
    ON rf.ClientServicesRef=d.ClientServicesRef AND rf.StartDate=d.StartDate AND rf.SegmentID=d.SegmentID
    GROUP BY d.ClientServicesRef
    HAVING COUNT(DISTINCT d.StartDate)=COUNT(DISTINCT rf.StartDate)
 ...
 -- Insert new results on CSR level as a sum of Wave results
    INSERT INTO Warehouse.MI.CampaignInternalResultsFinalCSR_SuperSegment 
    (ClientServicesRef,SegmentID,
    ControlGroup,CustomerUniverse,Level,
    ControlGroupSize,Cardholders,Sales,Transactions,Spenders,
    IncrementalSales,IncrementalMargin,IncrementalTransactions,IncrementalSpenders,
    ...
    INNER JOIN Warehouse.MI.CampaignInternalResultsFinalWave_SuperSegment rf  ON rf.ClientServicesRef=wm.ClientServicesRef
    WHERE NOT EXISTS (SELECT 1 FROM Warehouse.MI.CampaignInternalResultsFinalCSR_SuperSegment old 
    WHERE old.ClientServicesRef=wm.ClientServicesRef AND old.SegmentID=rf.SegmentID)
    GROUP BY wm.ClientServicesRef, rf.SegmentID,
    wm.ControlGroup,wm.CustomerUniverse,wm.Level

    			 -- Uplif calculations
			 UPDATE Warehouse.MI.CampaignInternalResultsFinalCSR_SuperSegment
			 SET SalesUplift=CASE WHEN IncrementalSales<Sales THEN 1.0*IncrementalSales/(Sales-IncrementalSales) ELSE IncrementalSales END
			   ,RRUplift=CASE WHEN IncrementalSpenders<Spenders THEN 1.0*IncrementalSpenders/(Sales-IncrementalSpenders) ELSE IncrementalSpenders END
			   ,ATVUplift=CASE WHEN IncrementalTransactions<Transactions AND IncrementalSales<Sales AND Transactions>0 THEN (1.0*Sales/Transactions)/(1.0*(Sales-IncrementalSales)/(Transactions-IncrementalTransactions))-1.0 ELSE 0 END
			   ,ATFUplift=CASE WHEN IncrementalSpenders<Spenders AND IncrementalTransactions<Transactions AND Spenders>0 THEN (1.0*Transactions/Spenders)/(1.0*(Transactions-IncrementalTransactions)/(Spenders-IncrementalSpenders))-1.0 ELSE 0 END
			 ...
			 UPDATE Warehouse.MI.CampaignInternalResultsFinalCSR_SuperSegment SET
			 TScoreSPC=CASE WHEN PooledStdDevSPC>0 THEN ABS(1.0*IncrementalSales/Cardholders)/PooledStdDevSPC ELSE 0 END,
			 TScoreRR=CASE WHEN PooledStdDevRR>0 THEN ABS(1.0*IncrementalSpenders/Cardholders)/PooledStdDevRR ELSE 0 END,
			 TScoreSPS=CASE WHEN PooledStdDevSPS>0 THEN ABS(1.0*SPS_Diff)/PooledStdDevSPS ELSE 0 END
			 WHERE TScoreSPC IS NULL
			...
			-- Significance calculations
			 ...
			 SignificantUpliftSPC=CASE WHEN TScoreSPC>=Probability_05 THEN 'High'
						  WHEN  TScoreSPC>=Probability_20 THEN 'Moderate'
						  ELSE 'No' END
			 FROM Warehouse.MI.CampaignInternalResultsFinalCSR_SuperSegment w
			 LEFT JOIN Warehouse.Stratification.TTestValues t on w.DegreesOfFreedomSPC
			 BETWEEN t.MinDegreesOfFreedom AND COALESCE(t.MaxDegreesOfFreedom,w.DegreesOfFreedomSPC) and t.Tailes=2
			 WHERE PValueSPC IS NULL
