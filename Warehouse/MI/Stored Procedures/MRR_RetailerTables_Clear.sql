...
-- =============================================
CREATE PROCEDURE [MI].[MRR_RetailerTables_Clear] 
	(
		@DateID INT
	)
...
BEGIN
	...
	DELETE FROM MI.ControlSalesWorking WHERE DateID = @DateID
	DELETE FROM MI.MemberssalesWorking WHERE DateID = @DateID
	...
	WHERE m.DateID = @DateID

	DELETE FROM MI.Uplift_Results_Table WHERE DateID = @DateID

	DELETE FROM mi.RetailerReportMetric WHERE DateID = @DateID
