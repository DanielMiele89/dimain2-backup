...
CREATE PROCEDURE MI.CampaignResultsLTE_ExternalFinalCSR_Store AS -- unhide this row to modify SP
...
BEGIN 
    ...
    SELECT 'CampaignResultsLTE_ExternalFinalCSR_Store', 
    SYSTEM_USER, GETDATE()
    ...
    -- List of all Campaigns (ClientServicesRef) that can be stored in CampaignExternalResultsLTEFinalCSR Tables (all the Waves were measured)
    ...
    LEFT JOIN Warehouse.MI.CampaignExternalResultsLTEFinalWave rf 
    ...
    -- Check if ResultsLTE for uncompleted camapign are stored in CampaignExternalResultsLTEFinalCSR, if yes delete them
    DELETE FROM csr
    FROM Warehouse.MI.CampaignExternalResultsLTEFinalCSR  csr
    ...
    INSERT INTO Warehouse.MI.CampaignExternalResultsLTEFinalCSR 
    ...
    INNER JOIN Warehouse.MI.CampaignExternalResultsLTEFinalWave rf  ON rf.ClientServicesRef=wm.ClientServicesRef
    WHERE NOT EXISTS (SELECT 1 FROM Warehouse.MI.CampaignExternalResultsLTEFinalCSR old 
    WHERE old.ClientServicesRef=wm.ClientServicesRef AND old.Effect=rf.Effect)
    GROUP BY rf.Effect, wm.ClientServicesRef,
    wm.ControlGroup,wm.CustomerUniverse,wm.AggregationLevel

    			 -- Uplif calculations
			 UPDATE Warehouse.MI.CampaignExternalResultsLTEFinalCSR
			 ...
			 UPDATE Warehouse.MI.CampaignExternalResultsLTEFinalCSR SET
			...
			-- Significance calculations
			 ...
			 SignificantUpliftSPC=CASE WHEN TScoreSPC>=Probability_05 THEN 'High'
						  ...
						  ELSE 'No' END
			 FROM Warehouse.MI.CampaignExternalResultsLTEFinalCSR w
    ...
    -- List of all Campaigns (ClientServicesRef) that can be stored in CampaignExternalResultsLTEFinalCSR_BespokeCell Tables (all the Waves were measured)
    ...
    LEFT JOIN Warehouse.MI.CampaignExternalResultsLTEFinalWave_BespokeCell rf 
    ...
    -- Insert new ResultsLTE on CSR level as a sum of Wave ResultsLTE
    INSERT INTO Warehouse.MI.CampaignExternalResultsLTEFinalCSR_BespokeCell 
    ...
    INNER JOIN Warehouse.MI.CampaignExternalResultsLTEFinalWave_BespokeCell rf  ON rf.ClientServicesRef=wm.ClientServicesRef
    WHERE NOT EXISTS (SELECT 1 FROM Warehouse.MI.CampaignExternalResultsLTEFinalCSR_BespokeCell old 
    ...
    wm.ControlGroup,wm.CustomerUniverse,wm.Level
			 ...
			 UPDATE Warehouse.MI.CampaignExternalResultsLTEFinalCSR_BespokeCell SET
			 TScoreSPC=CASE WHEN PooledStdDevSPC>0 THEN ABS(1.0*IncrementalSales/Cardholders)/PooledStdDevSPC ELSE 0 END,
			 TScoreRR=CASE WHEN PooledStdDevRR>0 THEN ABS(1.0*IncrementalSpenders/Cardholders)/PooledStdDevRR ELSE 0 END,
			 TScoreSPS=CASE WHEN PooledStdDevSPS>0 THEN ABS(1.0*SPS_Diff)/PooledStdDevSPS ELSE 0 END
			 WHERE TScoreSPC IS NULL
			...
			-- Significance calculations
			 ...
			 SignificantUpliftSPC=CASE WHEN TScoreSPC>=Probability_05 THEN 'High'
						  WHEN  TScoreSPC>=Probability_20 THEN 'Moderate'
						  ELSE 'No' END
			 FROM Warehouse.MI.CampaignExternalResultsLTEFinalCSR_BespokeCell w
			 LEFT JOIN Warehouse.Stratification.TTestValues t on w.DegreesOfFreedomSPC
			 BETWEEN t.MinDegreesOfFreedom AND COALESCE(t.MaxDegreesOfFreedom,w.DegreesOfFreedomSPC) and t.Tailes=2
			 WHERE PValueSPC IS NULL
    ...
    -- List of all Campaigns (ClientServicesRef) that can be stored in CampaignExternalResultsLTEFinalCSR_Segment Tables (all the Waves were measured)
    ...
    LEFT JOIN Warehouse.MI.CampaignExternalResultsLTEFinalWave_Segment rf 
    ...
    -- Insert new ResultsLTE on CSR level as a sum of Wave ResultsLTE
    INSERT INTO Warehouse.MI.CampaignExternalResultsLTEFinalCSR_Segment 
    ...
    INNER JOIN Warehouse.MI.CampaignExternalResultsLTEFinalWave_Segment rf  ON rf.ClientServicesRef=wm.ClientServicesRef
    WHERE NOT EXISTS (SELECT 1 FROM Warehouse.MI.CampaignExternalResultsLTEFinalCSR_Segment old 
    ...
    wm.ControlGroup,wm.CustomerUniverse,wm.Level
    			 ...
    			 -- Uplif calculations
			 UPDATE Warehouse.MI.CampaignExternalResultsLTEFinalCSR_Segment
			 SET SalesUplift=CASE WHEN IncrementalSales<Sales THEN 1.0*IncrementalSales/(Sales-IncrementalSales) ELSE IncrementalSales END
			   ,RRUplift=CASE WHEN IncrementalSpenders<Spenders THEN 1.0*IncrementalSpenders/(Sales-IncrementalSpenders) ELSE IncrementalSpenders END
			   ,ATVUplift=CASE WHEN IncrementalTransactions<Transactions AND IncrementalSales<Sales AND Transactions>0 THEN (1.0*Sales/Transactions)/(1.0*(Sales-IncrementalSales)/(Transactions-IncrementalTransactions))-1.0 ELSE 0 END
			   ,ATFUplift=CASE WHEN IncrementalSpenders<Spenders AND IncrementalTransactions<Transactions AND Spenders>0 THEN (1.0*Transactions/Spenders)/(1.0*(Transactions-IncrementalTransactions)/(Spenders-IncrementalSpenders))-1.0 ELSE 0 END
			 ...
			 UPDATE Warehouse.MI.CampaignExternalResultsLTEFinalCSR_Segment SET
			 TScoreSPC=CASE WHEN PooledStdDevSPC>0 THEN ABS(1.0*IncrementalSales/Cardholders)/PooledStdDevSPC ELSE 0 END,
			 TScoreRR=CASE WHEN PooledStdDevRR>0 THEN ABS(1.0*IncrementalSpenders/Cardholders)/PooledStdDevRR ELSE 0 END,
			 TScoreSPS=CASE WHEN PooledStdDevSPS>0 THEN ABS(1.0*SPS_Diff)/PooledStdDevSPS ELSE 0 END
			 WHERE TScoreSPC IS NULL
			...
			-- Significance calculations
			 ...
			 SignificantUpliftSPC=CASE WHEN TScoreSPC>=Probability_05 THEN 'High'
						  WHEN  TScoreSPC>=Probability_20 THEN 'Moderate'
						  ELSE 'No' END
			 FROM Warehouse.MI.CampaignExternalResultsLTEFinalCSR_Segment w
			 LEFT JOIN Warehouse.Stratification.TTestValues t on w.DegreesOfFreedomSPC
			 BETWEEN t.MinDegreesOfFreedom AND COALESCE(t.MaxDegreesOfFreedom,w.DegreesOfFreedomSPC) and t.Tailes=2
			 WHERE PValueSPC IS NULL
    ...
    -- List of all Campaigns (ClientServicesRef) that can be stored in CampaignExternalResultsLTEFinalCSR_SuperSegment Tables (all the Waves were measured)
    ...
    LEFT JOIN Warehouse.MI.CampaignExternalResultsLTEFinalWave_SuperSegment rf 
    ON rf.ClientServicesRef=d.ClientServicesRef AND rf.StartDate=d.StartDate AND rf.SegmentID=d.SegmentID
    GROUP BY d.ClientServicesRef
    HAVING COUNT(DISTINCT d.StartDate)=COUNT(DISTINCT rf.StartDate)
 ...
 -- Insert new ResultsLTE on CSR level as a sum of Wave ResultsLTE
    INSERT INTO Warehouse.MI.CampaignExternalResultsLTEFinalCSR_SuperSegment 
     (Effect, ClientServicesRef,SegmentID,
    ControlGroup,CustomerUniverse,Level,
    ControlGroupSize,Cardholders,Sales,Transactions,Spenders,
    IncrementalSales,IncrementalMargin,IncrementalTransactions,IncrementalSpenders,
    ...
    INNER JOIN Warehouse.MI.CampaignExternalResultsLTEFinalWave_SuperSegment rf  ON rf.ClientServicesRef=wm.ClientServicesRef
    WHERE NOT EXISTS (SELECT 1 FROM Warehouse.MI.CampaignExternalResultsLTEFinalCSR_SuperSegment old 
    WHERE old.ClientServicesRef=wm.ClientServicesRef AND old.Effect=rf.Effect AND old.SegmentID=rf.SegmentID)
    GROUP BY rf.Effect, wm.ClientServicesRef,rf.SegmentID,
    wm.ControlGroup,wm.CustomerUniverse,wm.Level

    			 -- Uplif calculations
			 UPDATE Warehouse.MI.CampaignExternalResultsLTEFinalCSR_SuperSegment
			 SET SalesUplift=CASE WHEN IncrementalSales<Sales THEN 1.0*IncrementalSales/(Sales-IncrementalSales) ELSE IncrementalSales END
			   ,RRUplift=CASE WHEN IncrementalSpenders<Spenders THEN 1.0*IncrementalSpenders/(Sales-IncrementalSpenders) ELSE IncrementalSpenders END
			   ,ATVUplift=CASE WHEN IncrementalTransactions<Transactions AND IncrementalSales<Sales AND Transactions>0 THEN (1.0*Sales/Transactions)/(1.0*(Sales-IncrementalSales)/(Transactions-IncrementalTransactions))-1.0 ELSE 0 END
			   ,ATFUplift=CASE WHEN IncrementalSpenders<Spenders AND IncrementalTransactions<Transactions AND Spenders>0 THEN (1.0*Transactions/Spenders)/(1.0*(Transactions-IncrementalTransactions)/(Spenders-IncrementalSpenders))-1.0 ELSE 0 END
			 ...
			 UPDATE Warehouse.MI.CampaignExternalResultsLTEFinalCSR_SuperSegment SET
			 TScoreSPC=CASE WHEN PooledStdDevSPC>0 THEN ABS(1.0*IncrementalSales/Cardholders)/PooledStdDevSPC ELSE 0 END,
			 TScoreRR=CASE WHEN PooledStdDevRR>0 THEN ABS(1.0*IncrementalSpenders/Cardholders)/PooledStdDevRR ELSE 0 END,
			 TScoreSPS=CASE WHEN PooledStdDevSPS>0 THEN ABS(1.0*SPS_Diff)/PooledStdDevSPS ELSE 0 END
			 WHERE TScoreSPC IS NULL
			...
			-- Significance calculations
			 ...
			 SignificantUpliftSPC=CASE WHEN TScoreSPC>=Probability_05 THEN 'High'
						  WHEN  TScoreSPC>=Probability_20 THEN 'Moderate'
						  ELSE 'No' END
			 FROM Warehouse.MI.CampaignExternalResultsLTEFinalCSR_SuperSegment w
			 LEFT JOIN Warehouse.Stratification.TTestValues t on w.DegreesOfFreedomSPC
			 BETWEEN t.MinDegreesOfFreedom AND COALESCE(t.MaxDegreesOfFreedom,w.DegreesOfFreedomSPC) and t.Tailes=2
			 WHERE PValueSPC IS NULL
