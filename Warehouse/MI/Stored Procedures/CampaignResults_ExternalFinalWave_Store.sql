...
CREATE PROCEDURE [MI].[CampaignResults_ExternalFinalWave_Store] (@ClientServicesRef VARCHAR(25), @StartDate DATE, -- unhide this row to modify SP
...
SELECT ' CampaignResults_ExternalFinalWave_Store', 
...
IF (SELECT COUNT(*) FROM Warehouse.MI.CampaignExternalResults_Workings WHERE ClientServicesRef=@ClientServicesRef AND StartDate=@StartDate)>0
...
BEGIN 
    ...
    BEGIN
	   --IF 'Out of Programme' control exists use this one, otherwise, use 'In programme plus CBP Halo'
	   SET @ControlGroup=(SELECT MAX(w.ControlGroup) FROM Warehouse.MI.CampaignExternalResults_Workings w 
    ...
    BEGIN
	   ...
	   BEGIN	  
		  SET @AggregationLevel=(SELECT MIN(w.Level) FROM Warehouse.MI.CampaignExternalResults_Workings w 
		  WHERE ClientServicesRef=@ClientServicesRef AND StartDate=@StartDate 
    ...
    IF (SELECT COUNT(*) FROM Warehouse.MI.CampaignExternalResults_Workings WHERE ClientServicesRef=@ClientServicesRef AND StartDate=@StartDate
	   AND ControlGroup=@ControlGroup AND CustomerUniverse=@CustomerUniverse AND Level=@AggregationLevel)>0
    OR (SELECT COUNT(*) FROM Warehouse.MI. CampaignInternalResults_Workings WHERE ClientServicesRef=@ClientServicesRef AND StartDate=@StartDate
	   AND ControlGroup=@ControlGroup AND CustomerUniverse=@CustomerUniverse AND Level=@AggregationLevel)>0
    
    -- If everything ok, populates ResultsFinal Table
    BEGIN 

		 -- Populate CampaignExternalResultsFinalWave (delete old entries first)
		DELETE FROM Warehouse.MI.CampaignExternalResultsFinalWave
		WHERE ClientServicesRef=@ClientServicesRef AND StartDate=@StartDate

		INSERT INTO Warehouse.MI.CampaignExternalResultsFinalWave
		...
		s.Commission,CASE WHEN m.UpliftCommission>0 THEN 1.0*s.Commission/m.UpliftCommission ELSE 1.0 END*CampaignCost,
		s.RewardOverride,CASE WHEN m.UpliftRewardOverride>0 THEN 1.0*s.RewardOverride/m.UpliftRewardOverride ELSE 1.0 END*m.IncrementalOverride	
		FROM (SELECT ClientServicesRef,StartDate, SUM(Cardholders) Cardholders, SUM(Sales) Sales, SUM(Transactions) Transactions, SUM(Spenders) Spenders,
			 ...
			 FROM (SELECT * FROM Warehouse.MI. CampaignInternalResults_PureSales UNION SELECT * FROM Warehouse.MI.CampaignExternalResults_PureSales) a
		...
		ON p.ClientServicesRef=SUBSTRING(s.ClientServicesRef, 0, ISNULL(NULLIF(charindex('Bespoke', s.ClientServicesRef, 0), 0), 99))  AND p.StartDate=s.StartDate      
		LEFT JOIN (SELECT ClientServicesRef,StartDate, SUM(Cardholders_C) ControlGroupSize, SUM(IncrementalSales) IncrementalSales, 
				...
				SUM(Commission_M) UpliftCommission,SUM(RewardOverride_M) UpliftRewardOverride				
				FROM (SELECT * FROM Warehouse.MI. CampaignInternalResults_Workings UNION SELECT * FROM Warehouse.MI.CampaignExternalResults_Workings) a
				WHERE ControlGroup=@ControlGroup AND CustomerUniverse=@CustomerUniverse AND Level=@AggregationLevel
				AND ClientServicesRef=@ClientServicesRef AND StartDate=@StartDate AND SalesType='Main Results (Qualifying MIDs or Channels Only)'
		...
		WHERE s.Cardholders>0
			 ...
			 UPDATE Warehouse.MI.CampaignExternalResultsFinalWave SET
 		...
 		-- Populate CampaignExternalResultsFinalWave_BespokeCell (delete old entries first)   
    		DELETE FROM Warehouse.MI.CampaignExternalResultsFinalWave_BespokeCell
		WHERE ClientServicesRef=@ClientServicesRef AND StartDate=@StartDate

		INSERT INTO Warehouse.MI.CampaignExternalResultsFinalWave_BespokeCell
		...
		WHERE s.Cardholders>0
			 ...
			 UPDATE Warehouse.MI.CampaignExternalResultsFinalWave_BespokeCell SET
			 TScoreSPC=CASE WHEN PooledStdDevSPC>0 THEN ABS(1.0*IncrementalSales/Cardholders)/PooledStdDevSPC ELSE 0 END,
			 TScoreRR=CASE WHEN PooledStdDevRR>0 THEN ABS(1.0*IncrementalSpenders/Cardholders)/PooledStdDevRR ELSE 0 END,
			 TScoreSPS=CASE WHEN PooledStdDevSPS>0 THEN ABS(1.0*SPS_Diff)/PooledStdDevSPS ELSE 0 END
			 WHERE ClientServicesRef=@ClientServicesRef AND StartDate=@StartDate
 		...
 		-- Populate CampaignExternalResultsFinalWave_Segment (delete old entries first)   
    		DELETE FROM Warehouse.MI.CampaignExternalResultsFinalWave_Segment
		WHERE ClientServicesRef=@ClientServicesRef AND StartDate=@StartDate

		INSERT INTO Warehouse.MI.CampaignExternalResultsFinalWave_Segment
		...
		WHERE s.Cardholders>0
			 ...
			 -- Uplif calculations
			 UPDATE Warehouse.MI.CampaignExternalResultsFinalWave_Segment
			 SET SalesUplift=CASE WHEN IncrementalSales<Sales THEN 1.0*IncrementalSales/(Sales-IncrementalSales) ELSE IncrementalSales END
			   ,RRUplift=CASE WHEN IncrementalSpenders<Spenders THEN 1.0*IncrementalSpenders/(Sales-IncrementalSpenders) ELSE IncrementalSpenders END
			   ,ATVUplift=CASE WHEN IncrementalTransactions<Transactions AND IncrementalSales<Sales AND Transactions>0 THEN (1.0*Sales/Transactions)/(1.0*(Sales-IncrementalSales)/(Transactions-IncrementalTransactions))-1.0 ELSE 0 END
			   ,ATFUplift=CASE WHEN IncrementalSpenders<Spenders AND IncrementalTransactions<Transactions AND Spenders>0 THEN (1.0*Transactions/Spenders)/(1.0*(Transactions-IncrementalTransactions)/(Spenders-IncrementalSpenders))-1.0 ELSE 0 END
			 ...
			 UPDATE Warehouse.MI.CampaignExternalResultsFinalWave_Segment SET
			 TScoreSPC=CASE WHEN PooledStdDevSPC>0 THEN ABS(1.0*IncrementalSales/Cardholders)/PooledStdDevSPC ELSE 0 END,
			 TScoreRR=CASE WHEN PooledStdDevRR>0 THEN ABS(1.0*IncrementalSpenders/Cardholders)/PooledStdDevRR ELSE 0 END,
			 TScoreSPS=CASE WHEN PooledStdDevSPS>0 THEN ABS(1.0*SPS_Diff)/PooledStdDevSPS ELSE 0 END
			 WHERE ClientServicesRef=@ClientServicesRef AND StartDate=@StartDate
 		...
 		-- Populate CampaignExternalResultsFinalWave_SuperSegment (delete old entries first)   
    		DELETE FROM Warehouse.MI.CampaignExternalResultsFinalWave_SuperSegment
		WHERE ClientServicesRef=@ClientServicesRef AND StartDate=@StartDate

		INSERT INTO Warehouse.MI.CampaignExternalResultsFinalWave_SuperSegment
		(ClientServicesRef,StartDate, SegmentID, ControlGroup,
		CustomerUniverse,Level,
		ControlGroupSize,Cardholders,Sales,Transactions,Spenders,
		IncrementalSales,IncrementalMargin,
		...
		WHERE s.Cardholders>0
			 ...
			 -- Uplif calculations
			 UPDATE Warehouse.MI.CampaignExternalResultsFinalWave_SuperSegment
			 SET SalesUplift=CASE WHEN IncrementalSales<Sales THEN 1.0*IncrementalSales/(Sales-IncrementalSales) ELSE IncrementalSales END
			   ,RRUplift=CASE WHEN IncrementalSpenders<Spenders THEN 1.0*IncrementalSpenders/(Sales-IncrementalSpenders) ELSE IncrementalSpenders END
			   ,ATVUplift=CASE WHEN IncrementalTransactions<Transactions AND IncrementalSales<Sales AND Transactions>0 THEN (1.0*Sales/Transactions)/(1.0*(Sales-IncrementalSales)/(Transactions-IncrementalTransactions))-1.0 ELSE 0 END
			   ,ATFUplift=CASE WHEN IncrementalSpenders<Spenders AND IncrementalTransactions<Transactions AND Spenders>0 THEN (1.0*Transactions/Spenders)/(1.0*(Transactions-IncrementalTransactions)/(Spenders-IncrementalSpenders))-1.0 ELSE 0 END
			 ...
			 UPDATE Warehouse.MI.CampaignExternalResultsFinalWave_SuperSegment SET
			 TScoreSPC=CASE WHEN PooledStdDevSPC>0 THEN ABS(1.0*IncrementalSales/Cardholders)/PooledStdDevSPC ELSE 0 END,
			 TScoreRR=CASE WHEN PooledStdDevRR>0 THEN ABS(1.0*IncrementalSpenders/Cardholders)/PooledStdDevRR ELSE 0 END,
			 TScoreSPS=CASE WHEN PooledStdDevSPS>0 THEN ABS(1.0*SPS_Diff)/PooledStdDevSPS ELSE 0 END
			 WHERE ClientServicesRef=@ClientServicesRef AND StartDate=@StartDate
