...
CREATE PROCEDURE [MI].[TotalBrandSpend_RBSG_UpdateSingleBrand] (@BrandID INT)
AS
BEGIN
	...
	*******************************************************************************************************************************************/
		...
		WHERE tbs.SectorName IN (SELECT bs.SectorName
	...
	*******************************************************************************************************************************************/
		...
		FROM [MI].[TotalBrandSpend_RBSG] tbs
		...
		SELECT ROW_NUMBER() OVER (ORDER BY bs.FilterID, bs.BrandName, NEWID()) + (SELECT MAX(ID) FROM [MI].[TotalBrandSpend_RBSG]) AS ID
			 , *
		FROM (	SELECT DISTINCT
				...
				WHERE bs.CurrentYear = 1
				AND bs.BrandID = @BrandID) bs
...
GO
GRANT EXECUTE
    ON OBJECT::[MI].[TotalBrandSpend_RBSG_UpdateSingleBrand] TO [New_Branding]
    AS [dbo];
