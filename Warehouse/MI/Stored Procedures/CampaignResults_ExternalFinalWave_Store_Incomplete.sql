...
CREATE PROCEDURE [MI].[CampaignResults_ExternalFinalWave_Store_Incomplete] (@ClientServicesRef VARCHAR(25), @StartDate DATE, -- unhide this row to modify SP
...
SELECT ' CampaignResults_ExternalFinalWave_Store_Incomplete', 
...
IF (SELECT COUNT(*) FROM Warehouse.MI.CampaignExternalResults_Workings_Incomplete WHERE ClientServicesRef=@ClientServicesRef AND StartDate=@StartDate)>0
	 OR (SELECT COUNT(*) FROM Warehouse.MI.CampaignInternalResults_Workings_Incomplete WHERE ClientServicesRef=@ClientServicesRef AND StartDate=@StartDate)>0
...
BEGIN 
    ...
    BEGIN
	   ...
	   SET @ControlGroup=(SELECT MAX(w.ControlGroup) FROM Warehouse.MI.CampaignExternalResults_Workings_Incomplete w 
    ...
    BEGIN
	   ...
	   BEGIN	  
		  SET @AggregationLevel=(SELECT MIN(w.Level) FROM Warehouse.MI.CampaignExternalResults_Workings_Incomplete w 
		  WHERE ClientServicesRef=@ClientServicesRef AND StartDate=@StartDate 
    ...
    BEGIN 
		...
		DELETE FROM Warehouse.MI.CampaignExternalResultsFinalWave_Incomplete
		WHERE ClientServicesRef=@ClientServicesRef AND StartDate=@StartDate

		INSERT INTO Warehouse.MI.CampaignExternalResultsFinalWave_Incomplete
		...
		WHERE s.Cardholders>0
			 ...
			 -- Uplif calculations
			 UPDATE Warehouse.MI.CampaignExternalResultsFinalWave_Incomplete
			 ...
			 UPDATE Warehouse.MI.CampaignExternalResultsFinalWave_Incomplete SET
 		...
 		-- Populate CampaignExternalResultsFinalWave_BespokeCell (delete old entries first)   
    		DELETE FROM Warehouse.MI.CampaignExternalResultsFinalWave_BespokeCell_Incomplete
		WHERE ClientServicesRef=@ClientServicesRef AND StartDate=@StartDate

		INSERT INTO Warehouse.MI.CampaignExternalResultsFinalWave_BespokeCell_Incomplete
		...
		WHERE s.Cardholders>0
			 ...
			 -- Uplif calculations
			 UPDATE Warehouse.MI.CampaignExternalResultsFinalWave_BespokeCell_Incomplete
			 SET SalesUplift=CASE WHEN IncrementalSales<Sales THEN 1.0*IncrementalSales/(Sales-IncrementalSales) ELSE IncrementalSales END
			   ,RRUplift=CASE WHEN IncrementalSpenders<Spenders THEN 1.0*IncrementalSpenders/(Sales-IncrementalSpenders) ELSE IncrementalSpenders END
			   ,ATVUplift=CASE WHEN IncrementalTransactions<Transactions AND IncrementalSales<Sales AND Transactions>0 THEN (1.0*Sales/Transactions)/(1.0*(Sales-IncrementalSales)/(Transactions-IncrementalTransactions))-1.0 ELSE 0 END
			   ,ATFUplift=CASE WHEN IncrementalSpenders<Spenders AND IncrementalTransactions<Transactions AND Spenders>0 THEN (1.0*Transactions/Spenders)/(1.0*(Transactions-IncrementalTransactions)/(Spenders-IncrementalSpenders))-1.0 ELSE 0 END
			 ...
			 UPDATE Warehouse.MI.CampaignExternalResultsFinalWave_BespokeCell_Incomplete SET
			 TScoreSPC=CASE WHEN PooledStdDevSPC>0 THEN ABS(1.0*IncrementalSales/Cardholders)/PooledStdDevSPC ELSE 0 END,
			 TScoreRR=CASE WHEN PooledStdDevRR>0 THEN ABS(1.0*IncrementalSpenders/Cardholders)/PooledStdDevRR ELSE 0 END,
			 TScoreSPS=CASE WHEN PooledStdDevSPS>0 THEN ABS(1.0*SPS_Diff)/PooledStdDevSPS ELSE 0 END
			 WHERE ClientServicesRef=@ClientServicesRef AND StartDate=@StartDate
 		...
 		-- Populate CampaignExternalResultsFinalWave_Segment (delete old entries first)   
    		DELETE FROM Warehouse.MI.CampaignExternalResultsFinalWave_Segment_Incomplete
		WHERE ClientServicesRef=@ClientServicesRef AND StartDate=@StartDate

		INSERT INTO Warehouse.MI.CampaignExternalResultsFinalWave_Segment_Incomplete
		...
		WHERE s.Cardholders>0
			 ...
			 -- Uplif calculations
			 UPDATE Warehouse.MI.CampaignExternalResultsFinalWave_Segment_Incomplete
			 SET SalesUplift=CASE WHEN IncrementalSales<Sales THEN 1.0*IncrementalSales/(Sales-IncrementalSales) ELSE IncrementalSales END
			   ,RRUplift=CASE WHEN IncrementalSpenders<Spenders THEN 1.0*IncrementalSpenders/(Sales-IncrementalSpenders) ELSE IncrementalSpenders END
			   ,ATVUplift=CASE WHEN IncrementalTransactions<Transactions AND IncrementalSales<Sales AND Transactions>0 THEN (1.0*Sales/Transactions)/(1.0*(Sales-IncrementalSales)/(Transactions-IncrementalTransactions))-1.0 ELSE 0 END
			   ,ATFUplift=CASE WHEN IncrementalSpenders<Spenders AND IncrementalTransactions<Transactions AND Spenders>0 THEN (1.0*Transactions/Spenders)/(1.0*(Transactions-IncrementalTransactions)/(Spenders-IncrementalSpenders))-1.0 ELSE 0 END
			 ...
			 UPDATE Warehouse.MI.CampaignExternalResultsFinalWave_Segment_Incomplete SET
			 TScoreSPC=CASE WHEN PooledStdDevSPC>0 THEN ABS(1.0*IncrementalSales/Cardholders)/PooledStdDevSPC ELSE 0 END,
			 TScoreRR=CASE WHEN PooledStdDevRR>0 THEN ABS(1.0*IncrementalSpenders/Cardholders)/PooledStdDevRR ELSE 0 END,
			 TScoreSPS=CASE WHEN PooledStdDevSPS>0 THEN ABS(1.0*SPS_Diff)/PooledStdDevSPS ELSE 0 END
			 WHERE ClientServicesRef=@ClientServicesRef AND StartDate=@StartDate
 		...
 		-- Populate CampaignExternalResultsFinalWave_SuperSegment (delete old entries first)   
    		DELETE FROM Warehouse.MI.CampaignExternalResultsFinalWave_SuperSegment_Incomplete
		WHERE ClientServicesRef=@ClientServicesRef AND StartDate=@StartDate

		INSERT INTO Warehouse.MI.CampaignExternalResultsFinalWave_SuperSegment_Incomplete
		(ClientServicesRef,StartDate, SegmentID, ControlGroup,
		CustomerUniverse,Level,
		ControlGroupSize,Cardholders,Sales,Transactions,Spenders,
		IncrementalSales,IncrementalMargin,
		...
		WHERE s.Cardholders>0
			 ...
			 -- Uplif calculations
			 UPDATE Warehouse.MI.CampaignExternalResultsFinalWave_SuperSegment_Incomplete
			 SET SalesUplift=CASE WHEN IncrementalSales<Sales THEN 1.0*IncrementalSales/(Sales-IncrementalSales) ELSE IncrementalSales END
			   ,RRUplift=CASE WHEN IncrementalSpenders<Spenders THEN 1.0*IncrementalSpenders/(Sales-IncrementalSpenders) ELSE IncrementalSpenders END
			   ,ATVUplift=CASE WHEN IncrementalTransactions<Transactions AND IncrementalSales<Sales AND Transactions>0 THEN (1.0*Sales/Transactions)/(1.0*(Sales-IncrementalSales)/(Transactions-IncrementalTransactions))-1.0 ELSE 0 END
			   ,ATFUplift=CASE WHEN IncrementalSpenders<Spenders AND IncrementalTransactions<Transactions AND Spenders>0 THEN (1.0*Transactions/Spenders)/(1.0*(Transactions-IncrementalTransactions)/(Spenders-IncrementalSpenders))-1.0 ELSE 0 END
			 ...
			 UPDATE Warehouse.MI.CampaignExternalResultsFinalWave_SuperSegment_Incomplete SET
			 TScoreSPC=CASE WHEN PooledStdDevSPC>0 THEN ABS(1.0*IncrementalSales/Cardholders)/PooledStdDevSPC ELSE 0 END,
			 TScoreRR=CASE WHEN PooledStdDevRR>0 THEN ABS(1.0*IncrementalSpenders/Cardholders)/PooledStdDevRR ELSE 0 END,
			 TScoreSPS=CASE WHEN PooledStdDevSPS>0 THEN ABS(1.0*SPS_Diff)/PooledStdDevSPS ELSE 0 END
			 WHERE ClientServicesRef=@ClientServicesRef AND StartDate=@StartDate
...
Run Store Procedure MI. CampaignInternalResults_Calculate_Incomplete first.' 
