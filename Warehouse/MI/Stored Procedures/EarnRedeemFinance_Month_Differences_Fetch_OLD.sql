...
-- =============================================
CREATE PROCEDURE [MI].[EarnRedeemFinance_Month_Differences_Fetch_OLD] 
...
BEGIN
	...
	*/
	...
	DECLARE @MonthDate DATE, @ArchiveDate DATE, @ArchiveMonthDate DATE

	--set @ArchiveDate to PREVIOUS archive date insert value
	SELECT @ArchiveDate = MAX(ArchiveDate)
	...
	SELECT @ArchiveMonthDate = MAX(MonthDate)
	FROM MI.EarnRedeemFinance_EarnRedeemByMonth_Archive
	WHERE ArchiveDate = @ArchiveDate
...
--Create a list of brand names iwth an identity column for row colours
	...
	(
		SELECT B.BrandListID 
			...
			, SUM(RedemptionValue) AS RedemptionValue
		FROM MI.EarnRedeemFinance_EarnRedeemByMonth e
		INNER JOIN #Brands b ON e.BrandID = b.BrandID AND b.ChargeTypeID = e.ChargeTypeID
		WHERE MonthDate <= @MonthDate AND MonthDate <= @ArchiveMonthDate
		GROUP BY b.BrandName, b.BrandListID, e.PaymentMethodID, e.IsRBS
		...
		SELECT B.BrandListID 
			...
			, -SUM(Earnings) AS Earnings
			, -SUM(RedemptionValue) AS RedemptionValue
		FROM MI.EarnRedeemFinance_EarnRedeemByMonth_Archive e
	...
	(
		...
		--archived values are added as negatives
		SELECT b.brandListID
			, B.BrandName AS Brand
			, e.PaymentMethodID
			, e.IsRBS
			...
			, -RedemptionValue AS RedemptionValue
		FROM MI.EarnRedeemFinance_EarnRedeemByMonth_Archive E
		INNER JOIN #Brands b ON e.BrandID = b.BrandID AND e.ChargeTypeID = b.ChargeTypeID
		WHERE MonthDate > @MonthDate
		AND ArchiveDate = @ArchiveDate
