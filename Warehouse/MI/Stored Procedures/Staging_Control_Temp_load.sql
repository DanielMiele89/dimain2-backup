...
-- =============================================
CREATE PROCEDURE [MI].[Staging_Control_Temp_load] (@DateID INT, @PartnerID INT, @ClientServiceRef nvarchar(30))
...
IF (SELECT COUNT(*) FROM Relational.Control_Stratified_Compressed CON 
WHERE MinMonthID<=@DateID and MaxMonthID>=@DateID 
AND CON.Partnerid = @Partnerid 
AND COALESCE(CON.ClientServicesRef,'0') = @ClientServiceRef)>0 

BEGIN
    insert   Into #cus_stage1
    select @DateID as DateID,  c.FanID,   @PartnerID as PartnerID , @ClientServiceRef as ClientServicesRef
    From  Relational.Control_Stratified_Compressed C 
    WHERE MinMonthID<=@DateID and MaxMonthID>=@DateID 
    AND C.Partnerid = @Partnerid 
    AND COALESCE(C.ClientServicesRef,'0') = @ClientServiceRef
END
...
BEGIN
    ...
    WHERE MinMonthID<=@DateID and MaxMonthID>=@DateID 
    AND C.Partnerid = '0'
    AND COALESCE(C.ClientServicesRef,'0') = '0'
END
...
SELECT c.*, mt.CumulativeTypeID,
CustomerAttributeID_0=NULL/*CASE WHEN ecr.CustType='E' THEN 3 WHEN ecr.CustType='L' THEN 2 WHEN mt.CumulativeTypeID=0 THEN 1 END*/,
CustomerAttributeID_0BP=NULL/*CASE WHEN c.PartnerID='3960' /*BP*/ AND  ecr.CustType IN ('E' ,'L') THEN 4 END*/,
CustomerAttributeID_1=CASE WHEN ecy.CustType='E' THEN 1003 WHEN ecy.CustType='L' THEN 1002 WHEN mt.CumulativeTypeID IN (0,1) THEN 1001 END,
CustomerAttributeID_1BP=CASE WHEN c.PartnerID='3960' /*BP*/ AND  ecy.CustType IN ('E' ,'L') THEN 1004 END,
CustomerAttributeID_2=CASE WHEN ec.CustType='E' THEN 2003 WHEN ec.CustType='L' THEN 2002 WHEN mt.CumulativeTypeID IN (0,2) THEN 2001 END,
CustomerAttributeID_2BP=CASE WHEN c.PartnerID='3960' /*BP*/ AND  ec.CustType IN ('E' ,'L') THEN 2004 END,
CustomerAttributeID_3=NULL/*CASE WHEN c.CumulativeTypeID IN (0) THeN coh.FirstMonth+3000 END*/
into #cus
FROM #cus_stage1 c
...
--AND  mt.CumulativeTypeID IN (0)


insert into MI.Staging_Control_Temp (FanID,
ProgramID,
PartnerID,
ClientServicesRef,
CumulativeTypeID,
