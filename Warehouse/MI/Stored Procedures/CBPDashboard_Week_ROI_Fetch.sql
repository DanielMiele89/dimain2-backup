...
-- =============================================
CREATE PROCEDURE [MI].[CBPDashboard_Week_ROI_Fetch] 
...
BEGIN
	...
	SELECT m.PartnerID, m.IncrementalSales, m.Commission, m.margin, mt.Tier, b.ChargeOnRedeem
	INTO #Figures
	FROM MI.RetailerReportMetric m
	INNER JOIN Relational.Master_Retailer_Table mt ON m.PartnerID = mt.PartnerID
	INNER JOIN Relational.[Partner] p ON m.PartnerID = p.PartnerID
	INNER JOIN Relational.Brand b on p.BrandID = b.BrandID
	WHERE m.DateID = @MonthID
	AND m.ClientServiceRef = '0'
	AND m.PaymentTypeID = 0
	AND m.ChannelID = 0
	AND m.CustomerAttributeID = 0
	AND m.Mid_SplitID = 0
	AND m.CumulativeTypeID = 0
	AND m.PeriodTypeID = 0
