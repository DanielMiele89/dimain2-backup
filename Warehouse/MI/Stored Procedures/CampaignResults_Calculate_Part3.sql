...
CREATE PROCEDURE [MI].[CampaignResults_Calculate_Part3] (@DatabaseName NVARCHAR(400)='Sandbox') 
...
Output:
-- Warehouse.MI.CampaignInternalResults_AdjFactor
-- Warehouse.MI.CampaignInternalResults_PureSales
-- Warehouse.MI.CampaignInternalResults_Workings

-- Warehouse.MI.CampaignExternalResults_AdjFactor
-- Warehouse.MI.CampaignExternalResults_PureSales
-- Warehouse.MI.CampaignExternalResults_Workings
*/
...
CREATE CLUSTERED INDEX IND ON #ReportBase (FanID, ClientServicesRef, StartDate, HTMID, SuperSegmentID, Cell)
...
CREATE CLUSTERED INDEX IND ON #ReportBaseSS (FanID, ClientServicesRef, StartDate, HTMID, SuperSegmentID, Cell)
...
CREATE CLUSTERED INDEX IND ON #ReportBaseOutOfProgramme (FanID, ClientServicesRef, StartDate, HTMID, SuperSegmentID, Cell)
...
CREATE CLUSTERED INDEX IND ON #ReportBaseOutOfProgrammeSS (FanID, ClientServicesRef, StartDate, HTMID, SuperSegmentID, Cell)
...
INSERT INTO Warehouse.MI.CampaignInternalResults_AdjFactor
...
AND NOT EXISTS (SELECT 1 FROM Warehouse.MI.CampaignInternalResults_AdjFactor old 
...
INSERT INTO Warehouse.MI.CampaignInternalResults_PureSales
SELECT aa.ControlGroup, aa.SalesType, aa.CustomerUniverse, 
aa.ClientServicesRef, aa.StartDate, 
aa.Level, aa.SoW SegmentID, aa.Cell,
...
AND NOT EXISTS (SELECT 1 FROM Warehouse.MI.CampaignInternalResults_PureSales old 
WHERE aa.ControlGroup=old.ControlGroup AND aa.SalesType=old.SalesType AND aa.CustomerUniverse=old.CustomerUniverse
AND aa.ClientServicesRef=old.ClientServicesRef AND aa.StartDate=old.StartDate  
AND aa.Level=old.Level AND aa.SoW=old.SegmentID AND aa.Cell=old.Cell)

INSERT INTO Warehouse.MI.CampaignExternalResults_AdjFactor
SELECT aa.ControlGroup, aa.SalesType, aa.CustomerUniverse, 
aa.ClientServicesRef, aa.StartDate, 
aa.Level, aa.SoW SegmentID, aa.Cell,
CASE WHEN aa.ControlGroup='In programme plus CBP Halo' 
...
AND NOT EXISTS (SELECT 1 FROM Warehouse.MI.CampaignExternalResults_AdjFactor old 
WHERE aa.ControlGroup=old.ControlGroup AND aa.SalesType=old.SalesType AND aa.CustomerUniverse=old.CustomerUniverse
AND aa.ClientServicesRef=old.ClientServicesRef AND aa.StartDate=old.StartDate  
AND aa.Level=old.Level AND aa.SoW=old.SegmentID AND aa.Cell=old.Cell)

-- Cap Adjustoments Value to the value between 0.5 and 2
UPDATE Warehouse.MI.CampaignInternalResults_AdjFactor
SET IsCapped=1, Adj_FactorRR=2
FROM Warehouse.MI.CampaignInternalResults_AdjFactor a
...
UPDATE Warehouse.MI.CampaignExternalResults_AdjFactor
SET IsCapped=1, Adj_FactorRR=2
FROM Warehouse.MI.CampaignExternalResults_AdjFactor a
INNER JOIN #ControlGroups aa 
	   	  ON aa.SalesType=a.SalesType AND
		  aa.CustomerUniverse=a.CustomerUniverse AND
		  aa.ClientServicesRef=a.ClientServicesRef AND 
...
ALTER INDEX ALL ON Warehouse.MI.CampaignInternalResults_AdjFactor REBUILD 
ALTER INDEX ALL ON Warehouse.MI.CampaignExternalResults_AdjFactor REBUILD  
...
INSERT INTO Warehouse.MI.CampaignExternalResults_PureSales
SELECT aa.ControlGroup, aa.SalesType, aa.CustomerUniverse, 
aa.ClientServicesRef, aa.StartDate, 
aa.Level, aa.SoW SegmentID, aa.Cell,
COALESCE(ps.Cardholders_M, s.Cardholders_M) Cardholders, COALESCE(ps.Spenders_M, s.Spenders_M) Spenders, 
...
AND COALESCE(ps.Cardholders_M, s.Cardholders_M)>0
AND NOT EXISTS (SELECT 1 FROM Warehouse.MI.CampaignExternalResults_PureSales old 
WHERE aa.ControlGroup=old.ControlGroup AND aa.SalesType=old.SalesType AND aa.CustomerUniverse=old.CustomerUniverse
AND aa.ClientServicesRef=old.ClientServicesRef AND aa.StartDate=old.StartDate  
AND aa.Level=old.Level AND aa.SoW=old.SegmentID AND aa.Cell=old.Cell)
...
ALTER INDEX ALL ON Warehouse.MI.CampaignInternalResults_PureSales REBUILD 
ALTER INDEX ALL ON Warehouse.MI.CampaignExternalResults_PureSales REBUILD  

INSERT INTO Warehouse.MI.CampaignInternalResults_Workings
...
AND NOT EXISTS (SELECT 1 FROM Warehouse.MI.CampaignInternalResults_Workings old 
WHERE aa.ControlGroup=old.ControlGroup AND aa.SalesType=old.SalesType AND aa.CustomerUniverse=old.CustomerUniverse
AND aa.ClientServicesRef=old.ClientServicesRef AND aa.StartDate=old.StartDate  
AND aa.Level=old.Level AND aa.SoW=old.SegmentID AND aa.Cell=old.Cell)
...
INSERT INTO Warehouse.MI.CampaignExternalResults_Workings
(ControlGroup, SalesType, CustomerUniverse, 
ClientServicesRef, StartDate, 
Level, SegmentID, Cell,
Cardholders_M, Spenders_M, Sales_M, Transactions_M, 
...
AND NOT EXISTS (SELECT 1 FROM Warehouse.MI.CampaignExternalResults_Workings old 
WHERE aa.ControlGroup=old.ControlGroup AND aa.SalesType=old.SalesType AND aa.CustomerUniverse=old.CustomerUniverse
AND aa.ClientServicesRef=old.ClientServicesRef AND aa.StartDate=old.StartDate  
AND aa.Level=old.Level AND aa.SoW=old.SegmentID AND aa.Cell=old.Cell)
...
ALTER INDEX ALL ON Warehouse.MI.CampaignInternalResults_Workings REBUILD 
ALTER INDEX ALL ON Warehouse.MI.CampaignExternalResults_Workings REBUILD  

------------------------------------------------------------------------------------------------------------------------
--- 7. Incrementality Calculations -------------------------------------------------------------------------------------
------------------------------------------------------------------------------------------------------------------------
UPDATE Warehouse.MI.CampaignInternalResults_Workings SET 
...
UPDATE Warehouse.MI.CampaignExternalResults_Workings SET 
SPC_Mail=CASE WHEN Cardholders_M>0 THEN 1.0*Sales_M/Cardholders_M ELSE 0 END,
SPC_Control=CASE WHEN Cardholders_C>0 THEN 1.0*Sales_C/Cardholders_C ELSE 0 END*Adj_FactorSPC,
MailedCommissionRate=CASE WHEN Sales_M>0 THEN 1.0*Commission_M/Sales_M ELSE 0 END,	
ControlCommissionRate=CASE WHEN Sales_C>0 THEN 1.0*Commission_C/Sales_C ELSE 0 END,
...
SPS_Mail=CASE WHEN Spenders_M>0 THEN 1.0*Sales_M/Spenders_M ELSE 0 END,	
SPS_Control=CASE WHEN Spenders_C>0 THEN 1.0*Sales_C/Spenders_C ELSE 0 END*1.0*Adj_FactorSPC/Adj_FactorRR	
FROM Warehouse.MI.CampaignExternalResults_Workings w
WHERE EXISTS (SELECT 1 FROM #ControlGroups aa
WHERE aa.ClientServicesRef=w.ClientServicesRef AND aa.StartDate=w.StartDate)

UPDATE Warehouse.MI.CampaignExternalResults_Workings
SET ControlCommissionRate=MailedCommissionRate,
ControlOfferRate=MailedOfferRate
FROM Warehouse.MI.CampaignExternalResults_Workings w
WHERE (ControlCommissionRate>MailedCommissionRate
...
DELETE FROM Warehouse.MI.CampaignExternalResults_Workings WHERE CustomerUniverse='EMAIL_OPEN'
DELETE FROM Warehouse.MI.CampaignExternalResults_AdjFactor WHERE CustomerUniverse='EMAIL_OPEN'
DELETE FROM Warehouse.MI.CampaignExternalResults_PureSales WHERE CustomerUniverse='EMAIL_OPEN'
