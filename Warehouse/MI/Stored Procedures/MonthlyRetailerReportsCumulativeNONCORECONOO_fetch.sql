...
-- Edited by AJS on 29/05/2014
-- =============================================


CREATE PROCEDURE [MI].[MonthlyRetailerReportsCumulativeNONCORECONOO_fetch] 
...
--declare @MonthID as int, @PartnerID as int, @CONPartnerID as int
--set @MonthID =29
--set @PartnerID =3996
--set @CONPartnerID =0
declare @startID int 
...
Where	CON.MonthID = @MonthID and p.PartnerID = @PartnerID and CON.PartnerID = @CONPartnerID and
		SUT.Amount > 0
group by case when SUT.isonline = 1 then 105 else 106 end
