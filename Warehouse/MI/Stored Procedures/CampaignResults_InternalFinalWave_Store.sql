...
CREATE PROCEDURE [MI].[CampaignResults_InternalFinalWave_Store] (@ClientServicesRef VARCHAR(25), @StartDate DATE, -- unhide this row to modify SP
...
SELECT 'CampaignResults_InternalFinalWave_Store', 
...
IF (SELECT COUNT(*) FROM Warehouse.MI.CampaignInternalResults_Workings WHERE ClientServicesRef=@ClientServicesRef AND StartDate=@StartDate)>0
...
BEGIN 
    ...
    BEGIN
	   ...
	   SET @ControlGroup=(SELECT MIN(w.ControlGroup) FROM Warehouse.MI.CampaignInternalResults_Workings w 
    ...
    BEGIN
	   ...
	   BEGIN	  
		  SET @AggregationLevel=(SELECT MIN(w.Level) FROM Warehouse.MI.CampaignInternalResults_Workings w 
		  WHERE ClientServicesRef=@ClientServicesRef AND StartDate=@StartDate
    ...
    BEGIN 
		...
		DELETE FROM Warehouse.MI.CampaignInternalResultsFinalWave
		WHERE ClientServicesRef=@ClientServicesRef AND StartDate=@StartDate

		INSERT INTO Warehouse.MI.CampaignInternalResultsFinalWave
		...
		WHERE s.Cardholders>0
			 ...
			 -- Uplif calculations
			 UPDATE Warehouse.MI.CampaignInternalResultsFinalWave
			 ...
			 UPDATE Warehouse.MI.CampaignInternalResultsFinalWave SET
			...
			-- Significance calculations
			 ...
			 SignificantUpliftSPC=CASE WHEN TScoreSPC>=Probability_05 THEN 'High'
						  ...
						  ELSE 'No' END
			 FROM Warehouse.MI.CampaignInternalResultsFinalWave w
 		...
 		-- Populate CampaignInternalResultsFinalWave_BespokeCell (delete old entries first)   
    		DELETE FROM Warehouse.MI.CampaignInternalResultsFinalWave_BespokeCell
		WHERE ClientServicesRef=@ClientServicesRef AND StartDate=@StartDate

		INSERT INTO Warehouse.MI.CampaignInternalResultsFinalWave_BespokeCell
		...
		WHERE s.Cardholders>0
			 ...
			 -- Uplif calculations
			 UPDATE Warehouse.MI.CampaignInternalResultsFinalWave_BespokeCell
			 SET SalesUplift=CASE WHEN IncrementalSales<Sales THEN 1.0*IncrementalSales/(Sales-IncrementalSales) ELSE IncrementalSales END
			   ,RRUplift=CASE WHEN IncrementalSpenders<Spenders THEN 1.0*IncrementalSpenders/(Sales-IncrementalSpenders) ELSE IncrementalSpenders END
			   ,ATVUplift=CASE WHEN IncrementalTransactions<Transactions AND IncrementalSales<Sales AND Transactions>0 THEN (1.0*Sales/Transactions)/(1.0*(Sales-IncrementalSales)/(Transactions-IncrementalTransactions))-1.0 ELSE 0 END
			   ,ATFUplift=CASE WHEN IncrementalSpenders<Spenders AND IncrementalTransactions<Transactions AND Spenders>0 THEN (1.0*Transactions/Spenders)/(1.0*(Transactions-IncrementalTransactions)/(Spenders-IncrementalSpenders))-1.0 ELSE 0 END
			 ...
			 UPDATE Warehouse.MI.CampaignInternalResultsFinalWave_BespokeCell SET
			 TScoreSPC=CASE WHEN PooledStdDevSPC>0 THEN ABS(1.0*IncrementalSales/Cardholders)/PooledStdDevSPC ELSE 0 END,
			 TScoreRR=CASE WHEN PooledStdDevRR>0 THEN ABS(1.0*IncrementalSpenders/Cardholders)/PooledStdDevRR ELSE 0 END,
			 TScoreSPS=CASE WHEN PooledStdDevSPS>0 THEN ABS(1.0*SPS_Diff)/PooledStdDevSPS ELSE 0 END
			 WHERE ClientServicesRef=@ClientServicesRef AND StartDate=@StartDate
			...
			-- Significance calculations
			 ...
			 SignificantUpliftSPC=CASE WHEN TScoreSPC>=Probability_05 THEN 'High'
						  WHEN  TScoreSPC>=Probability_20 THEN 'Moderate'
						  ELSE 'No' END
			 FROM Warehouse.MI.CampaignInternalResultsFinalWave_BespokeCell w
			 LEFT JOIN Warehouse.Stratification.TTestValues t on w.DegreesOfFreedomSPC
			 BETWEEN t.MinDegreesOfFreedom AND COALESCE(t.MaxDegreesOfFreedom,w.DegreesOfFreedomSPC) and t.Tailes=2
			 WHERE w.ClientServicesRef=@ClientServicesRef AND w.StartDate=@StartDate
 		...
 		-- Populate CampaignInternalResultsFinalWave_Segment (delete old entries first)   
    		DELETE FROM Warehouse.MI.CampaignInternalResultsFinalWave_Segment
		WHERE ClientServicesRef=@ClientServicesRef AND StartDate=@StartDate

		INSERT INTO Warehouse.MI.CampaignInternalResultsFinalWave_Segment
		...
		WHERE s.Cardholders>0
			 ...
			 -- Uplif calculations
			 UPDATE Warehouse.MI.CampaignInternalResultsFinalWave_Segment
			 SET SalesUplift=CASE WHEN IncrementalSales<Sales THEN 1.0*IncrementalSales/(Sales-IncrementalSales) ELSE IncrementalSales END
			   ,RRUplift=CASE WHEN IncrementalSpenders<Spenders THEN 1.0*IncrementalSpenders/(Sales-IncrementalSpenders) ELSE IncrementalSpenders END
			   ,ATVUplift=CASE WHEN IncrementalTransactions<Transactions AND IncrementalSales<Sales AND Transactions>0 THEN (1.0*Sales/Transactions)/(1.0*(Sales-IncrementalSales)/(Transactions-IncrementalTransactions))-1.0 ELSE 0 END
			   ,ATFUplift=CASE WHEN IncrementalSpenders<Spenders AND IncrementalTransactions<Transactions AND Spenders>0 THEN (1.0*Transactions/Spenders)/(1.0*(Transactions-IncrementalTransactions)/(Spenders-IncrementalSpenders))-1.0 ELSE 0 END
			 ...
			 UPDATE Warehouse.MI.CampaignInternalResultsFinalWave_Segment SET
			 TScoreSPC=CASE WHEN PooledStdDevSPC>0 THEN ABS(1.0*IncrementalSales/Cardholders)/PooledStdDevSPC ELSE 0 END,
			 TScoreRR=CASE WHEN PooledStdDevRR>0 THEN ABS(1.0*IncrementalSpenders/Cardholders)/PooledStdDevRR ELSE 0 END,
			 TScoreSPS=CASE WHEN PooledStdDevSPS>0 THEN ABS(1.0*SPS_Diff)/PooledStdDevSPS ELSE 0 END
			 WHERE ClientServicesRef=@ClientServicesRef AND StartDate=@StartDate
			...
			-- Significance calculations
			 ...
			 SignificantUpliftSPC=CASE WHEN TScoreSPC>=Probability_05 THEN 'High'
						  WHEN  TScoreSPC>=Probability_20 THEN 'Moderate'
						  ELSE 'No' END
			 FROM Warehouse.MI.CampaignInternalResultsFinalWave_Segment w
			 LEFT JOIN Warehouse.Stratification.TTestValues t on w.DegreesOfFreedomSPC
			 BETWEEN t.MinDegreesOfFreedom AND COALESCE(t.MaxDegreesOfFreedom,w.DegreesOfFreedomSPC) and t.Tailes=2
			 WHERE w.ClientServicesRef=@ClientServicesRef AND w.StartDate=@StartDate
 		...
 		-- Populate CampaignInternalResultsFinalWave_SuperSegment (delete old entries first)   
    		DELETE FROM Warehouse.MI.CampaignInternalResultsFinalWave_SuperSegment
		WHERE ClientServicesRef=@ClientServicesRef AND StartDate=@StartDate

		INSERT INTO Warehouse.MI.CampaignInternalResultsFinalWave_SuperSegment
		(ClientServicesRef,StartDate, SegmentID, ControlGroup,
		CustomerUniverse,Level,
		ControlGroupSize,Cardholders,Sales,Transactions,Spenders,
		IncrementalSales,IncrementalMargin,
		...
		WHERE s.Cardholders>0
			 ...
			 UPDATE Warehouse.MI.CampaignInternalResultsFinalWave_SuperSegment SET
			 TScoreSPC=CASE WHEN PooledStdDevSPC>0 THEN ABS(1.0*IncrementalSales/Cardholders)/PooledStdDevSPC ELSE 0 END,
			 TScoreRR=CASE WHEN PooledStdDevRR>0 THEN ABS(1.0*IncrementalSpenders/Cardholders)/PooledStdDevRR ELSE 0 END,
			 TScoreSPS=CASE WHEN PooledStdDevSPS>0 THEN ABS(1.0*SPS_Diff)/PooledStdDevSPS ELSE 0 END
			 WHERE ClientServicesRef=@ClientServicesRef AND StartDate=@StartDate
			...
			-- Significance calculations
			 ...
			 SignificantUpliftSPC=CASE WHEN TScoreSPC>=Probability_05 THEN 'High'
						  WHEN  TScoreSPC>=Probability_20 THEN 'Moderate'
						  ELSE 'No' END
			 FROM Warehouse.MI.CampaignInternalResultsFinalWave_SuperSegment w
			 LEFT JOIN Warehouse.Stratification.TTestValues t on w.DegreesOfFreedomSPC
			 BETWEEN t.MinDegreesOfFreedom AND COALESCE(t.MaxDegreesOfFreedom,w.DegreesOfFreedomSPC) and t.Tailes=2
			 WHERE w.ClientServicesRef=@ClientServicesRef AND w.StartDate=@StartDate
