...
-- =============================================
CREATE PROCEDURE [MI].[EarnRedeemFinance_NegativeValues_RESOLVE] 
...
BEGIN
	...
	SELECT @NegativeCount = COUNT(1) FROM MI.EarnRedeemFinance_Earnings WHERE EarnRedeemable < 0
	...
	BEGIN
		...
		INNER JOIN (SELECT n.ID, MAX(e.ID) AS PreviousEarningID
					FROM #NegativeEarnings n
					INNER JOIN MI.EarnRedeemFinance_Earnings e
								ON n.FanID = e.FanID 
		...
		UPDATE MI.EarnRedeemFinance_Earnings SET EarnRedeemable = 0
		...
		UPDATE MI.EarnRedeemFinance_Earnings SET EarnRedeemable = EarnRedeemable + n.NegativeAmount --NB: adding a negative value, not subtracting
		FROM MI.EarnRedeemFinance_Earnings e
		INNER JOIN
		(
