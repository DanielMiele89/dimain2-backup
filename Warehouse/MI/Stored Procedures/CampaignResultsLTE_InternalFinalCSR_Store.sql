...
CREATE PROCEDURE MI.CampaignResultsLTE_InternalFinalCSR_Store AS -- unhide this row to modify SP
...
BEGIN 
    ...
    SELECT 'CampaignResultsLTE_InternalFinalCSR_Store', 
    SYSTEM_USER, GETDATE()
    ...
    -- List of all Campaigns (ClientServicesRef) that can be stored in CampaignInternalResultsLTEFinalCSR Tables (all the Waves were measured)
    ...
    LEFT JOIN Warehouse.MI.CampaignInternalResultsLTEFinalWave rf 
    ...
    -- Check if ResultsLTE for uncompleted camapign are stored in CampaignInternalResultsLTEFinalCSR, if yes delete them
    DELETE FROM csr
    FROM Warehouse.MI.CampaignInternalResultsLTEFinalCSR  csr
    WHERE csr.ClientServicesRef NOT IN 
	   (SELECT ClientServicesRef FROM #Waves_Measured)

    -- Check if ResultsLTE are already stored in CSR tables and if Wave ResultsLTE were modified afterwards, if yes delete them
    DELETE FROM csr
    FROM Warehouse.MI.CampaignInternalResultsLTEFinalCSR  csr
    INNER JOIN #Waves_Measured wm ON wm.ClientServicesRef=csr.ClientServicesRef
    AND wm.Inserted>csr.Inserted -- Wave ResultsLTE modified, so CSR table needs to be updated accordingly

    -- Insert new ResultsLTE on CSR level as a sum of Wave ResultsLTE
    INSERT INTO Warehouse.MI.CampaignInternalResultsLTEFinalCSR 
     (Effect, ClientServicesRef,
    ...
    SELECT DISTINCT rf.Effect, wm.ClientServicesRef, 
    ...
    INNER JOIN Warehouse.MI.CampaignInternalResultsLTEFinalWave rf  ON rf.ClientServicesRef=wm.ClientServicesRef
    WHERE NOT EXISTS (SELECT 1 FROM Warehouse.MI.CampaignInternalResultsLTEFinalCSR old 
    WHERE old.ClientServicesRef=wm.ClientServicesRef AND old.Effect=rf.Effect)
    GROUP BY rf.Effect, wm.ClientServicesRef,
    wm.ControlGroup,wm.CustomerUniverse,wm.AggregationLevel
			 ...
			 UPDATE Warehouse.MI.CampaignInternalResultsLTEFinalCSR SET
			...
			-- Significance calculations
			 ...
			 SignificantUpliftSPC=CASE WHEN TScoreSPC>=Probability_05 THEN 'High'
						  ...
						  ELSE 'No' END
			 FROM Warehouse.MI.CampaignInternalResultsLTEFinalCSR w
    ...
    -- List of all Campaigns (ClientServicesRef) that can be stored in CampaignInternalResultsLTEFinalCSR_BespokeCell Tables (all the Waves were measured)
    ...
    LEFT JOIN Warehouse.MI.CampaignInternalResultsLTEFinalWave_BespokeCell rf 
    ...
    -- Insert new ResultsLTE on CSR level as a sum of Wave ResultsLTE
    INSERT INTO Warehouse.MI.CampaignInternalResultsLTEFinalCSR_BespokeCell 
     (Effect, ClientServicesRef,Cell,
    ...
    QualyfingSales,Cashback,QualyfingCashback,
    Commission,CampaignCost,RewardOverride,IncrementalOverride)
    SELECT DISTINCT rf.Effect, wm.ClientServicesRef, rf.Cell,
    ...
    INNER JOIN Warehouse.MI.CampaignInternalResultsLTEFinalWave_BespokeCell rf  ON rf.ClientServicesRef=wm.ClientServicesRef
    WHERE NOT EXISTS (SELECT 1 FROM Warehouse.MI.CampaignInternalResultsLTEFinalCSR_BespokeCell old 
    WHERE old.ClientServicesRef=wm.ClientServicesRef AND old.Effect=rf.Effect AND old.Cell=rf.Cell)
    GROUP BY rf.Effect, wm.ClientServicesRef,rf.Cell,
    wm.ControlGroup,wm.CustomerUniverse,wm.Level

    			 -- Uplif calculations
			 UPDATE Warehouse.MI.CampaignInternalResultsLTEFinalCSR_BespokeCell
			 SET SalesUplift=CASE WHEN IncrementalSales<Sales THEN 1.0*IncrementalSales/(Sales-IncrementalSales) ELSE IncrementalSales END
			   ,RRUplift=CASE WHEN IncrementalSpenders<Spenders THEN 1.0*IncrementalSpenders/(Sales-IncrementalSpenders) ELSE IncrementalSpenders END
			   ,ATVUplift=CASE WHEN IncrementalTransactions<Transactions AND IncrementalSales<Sales AND Transactions>0 THEN (1.0*Sales/Transactions)/(1.0*(Sales-IncrementalSales)/(Transactions-IncrementalTransactions))-1.0 ELSE 0 END
			   ,ATFUplift=CASE WHEN IncrementalSpenders<Spenders AND IncrementalTransactions<Transactions AND Spenders>0 THEN (1.0*Transactions/Spenders)/(1.0*(Transactions-IncrementalTransactions)/(Spenders-IncrementalSpenders))-1.0 ELSE 0 END
			 ...
			 UPDATE Warehouse.MI.CampaignInternalResultsLTEFinalCSR_BespokeCell SET
			 TScoreSPC=CASE WHEN PooledStdDevSPC>0 THEN ABS(1.0*IncrementalSales/Cardholders)/PooledStdDevSPC ELSE 0 END,
			 TScoreRR=CASE WHEN PooledStdDevRR>0 THEN ABS(1.0*IncrementalSpenders/Cardholders)/PooledStdDevRR ELSE 0 END,
			 TScoreSPS=CASE WHEN PooledStdDevSPS>0 THEN ABS(1.0*SPS_Diff)/PooledStdDevSPS ELSE 0 END
			 WHERE TScoreSPC IS NULL
			...
			-- Significance calculations
			 ...
			 SignificantUpliftSPC=CASE WHEN TScoreSPC>=Probability_05 THEN 'High'
						  WHEN  TScoreSPC>=Probability_20 THEN 'Moderate'
						  ELSE 'No' END
			 FROM Warehouse.MI.CampaignInternalResultsLTEFinalCSR_BespokeCell w
			 LEFT JOIN Warehouse.Stratification.TTestValues t on w.DegreesOfFreedomSPC
			 BETWEEN t.MinDegreesOfFreedom AND COALESCE(t.MaxDegreesOfFreedom,w.DegreesOfFreedomSPC) and t.Tailes=2
			 WHERE PValueSPC IS NULL
    ...
    -- List of all Campaigns (ClientServicesRef) that can be stored in CampaignInternalResultsLTEFinalCSR_Segment Tables (all the Waves were measured)
    ...
    LEFT JOIN Warehouse.MI.CampaignInternalResultsLTEFinalWave_Segment rf 
    ...
    -- Insert new ResultsLTE on CSR level as a sum of Wave ResultsLTE
    INSERT INTO Warehouse.MI.CampaignInternalResultsLTEFinalCSR_Segment 
     (Effect, ClientServicesRef,SegmentID,
    ControlGroup,CustomerUniverse,Level,
    ControlGroupSize,Cardholders,Sales,Transactions,Spenders,
    IncrementalSales,IncrementalMargin,IncrementalTransactions,IncrementalSpenders,
    PooledStdDevSPC, DegreesOfFreedomSPC,
    ...
    QualyfingSales,Cashback,QualyfingCashback,
    Commission,CampaignCost,RewardOverride,IncrementalOverride)
    SELECT DISTINCT rf.Effect, wm.ClientServicesRef, rf.SegmentID,
    wm.ControlGroup,wm.CustomerUniverse,wm.Level,
    SUM(rf.ControlGroupSize), SUM(rf.Cardholders),SUM(rf.Sales),SUM(rf.Transactions),SUM(rf.Spenders),
    SUM(rf.IncrementalSales),SUM(rf.IncrementalMargin),SUM(rf.IncrementalTransactions),SUM(rf.IncrementalSpenders),
    -- Assuming that Variance for SUM of Sales/Spenders/SPS is Average weigthed by number of Cardholders (or Spenders for SPS), and DF are just sum of DF
    ...
    INNER JOIN Warehouse.MI.CampaignInternalResultsLTEFinalWave_Segment rf  ON rf.ClientServicesRef=wm.ClientServicesRef
    WHERE NOT EXISTS (SELECT 1 FROM Warehouse.MI.CampaignInternalResultsLTEFinalCSR_Segment old 
    WHERE old.ClientServicesRef=wm.ClientServicesRef AND old.Effect=rf.Effect AND old.SegmentID=rf.SegmentID)
    GROUP BY rf.Effect, wm.ClientServicesRef,rf.SegmentID,
    wm.ControlGroup,wm.CustomerUniverse,wm.Level

    			 -- Uplif calculations
			 UPDATE Warehouse.MI.CampaignInternalResultsLTEFinalCSR_Segment
			 SET SalesUplift=CASE WHEN IncrementalSales<Sales THEN 1.0*IncrementalSales/(Sales-IncrementalSales) ELSE IncrementalSales END
			   ,RRUplift=CASE WHEN IncrementalSpenders<Spenders THEN 1.0*IncrementalSpenders/(Sales-IncrementalSpenders) ELSE IncrementalSpenders END
			   ,ATVUplift=CASE WHEN IncrementalTransactions<Transactions AND IncrementalSales<Sales AND Transactions>0 THEN (1.0*Sales/Transactions)/(1.0*(Sales-IncrementalSales)/(Transactions-IncrementalTransactions))-1.0 ELSE 0 END
			   ,ATFUplift=CASE WHEN IncrementalSpenders<Spenders AND IncrementalTransactions<Transactions AND Spenders>0 THEN (1.0*Transactions/Spenders)/(1.0*(Transactions-IncrementalTransactions)/(Spenders-IncrementalSpenders))-1.0 ELSE 0 END
			 ...
			 UPDATE Warehouse.MI.CampaignInternalResultsLTEFinalCSR_Segment SET
			 TScoreSPC=CASE WHEN PooledStdDevSPC>0 THEN ABS(1.0*IncrementalSales/Cardholders)/PooledStdDevSPC ELSE 0 END,
			 TScoreRR=CASE WHEN PooledStdDevRR>0 THEN ABS(1.0*IncrementalSpenders/Cardholders)/PooledStdDevRR ELSE 0 END,
			 TScoreSPS=CASE WHEN PooledStdDevSPS>0 THEN ABS(1.0*SPS_Diff)/PooledStdDevSPS ELSE 0 END
			 WHERE TScoreSPC IS NULL
			...
			-- Significance calculations
			 ...
			 SignificantUpliftSPC=CASE WHEN TScoreSPC>=Probability_05 THEN 'High'
						  WHEN  TScoreSPC>=Probability_20 THEN 'Moderate'
						  ELSE 'No' END
			 FROM Warehouse.MI.CampaignInternalResultsLTEFinalCSR_Segment w
			 LEFT JOIN Warehouse.Stratification.TTestValues t on w.DegreesOfFreedomSPC
			 BETWEEN t.MinDegreesOfFreedom AND COALESCE(t.MaxDegreesOfFreedom,w.DegreesOfFreedomSPC) and t.Tailes=2
			 WHERE PValueSPC IS NULL
    ...
    -- List of all Campaigns (ClientServicesRef) that can be stored in CampaignInternalResultsLTEFinalCSR_SuperSegment Tables (all the Waves were measured)
    ...
    LEFT JOIN Warehouse.MI.CampaignInternalResultsLTEFinalWave_SuperSegment rf 
    ON rf.ClientServicesRef=d.ClientServicesRef AND rf.StartDate=d.StartDate AND rf.SegmentID=d.SegmentID
    GROUP BY d.ClientServicesRef
    HAVING COUNT(DISTINCT d.StartDate)=COUNT(DISTINCT rf.StartDate)
 ...
 -- Insert new ResultsLTE on CSR level as a sum of Wave ResultsLTE
    INSERT INTO Warehouse.MI.CampaignInternalResultsLTEFinalCSR_SuperSegment 
     (Effect, ClientServicesRef,SegmentID,
    ControlGroup,CustomerUniverse,Level,
    ControlGroupSize,Cardholders,Sales,Transactions,Spenders,
    IncrementalSales,IncrementalMargin,IncrementalTransactions,IncrementalSpenders,
    ...
    INNER JOIN Warehouse.MI.CampaignInternalResultsLTEFinalWave_SuperSegment rf  ON rf.ClientServicesRef=wm.ClientServicesRef
    WHERE NOT EXISTS (SELECT 1 FROM Warehouse.MI.CampaignInternalResultsLTEFinalCSR_SuperSegment old 
    WHERE old.ClientServicesRef=wm.ClientServicesRef AND old.Effect=rf.Effect AND old.SegmentID=rf.SegmentID)
    GROUP BY rf.Effect, wm.ClientServicesRef,rf.SegmentID,
    wm.ControlGroup,wm.CustomerUniverse,wm.Level

    			 -- Uplif calculations
			 UPDATE Warehouse.MI.CampaignInternalResultsLTEFinalCSR_SuperSegment
			 SET SalesUplift=CASE WHEN IncrementalSales<Sales THEN 1.0*IncrementalSales/(Sales-IncrementalSales) ELSE IncrementalSales END
			   ,RRUplift=CASE WHEN IncrementalSpenders<Spenders THEN 1.0*IncrementalSpenders/(Sales-IncrementalSpenders) ELSE IncrementalSpenders END
			   ,ATVUplift=CASE WHEN IncrementalTransactions<Transactions AND IncrementalSales<Sales AND Transactions>0 THEN (1.0*Sales/Transactions)/(1.0*(Sales-IncrementalSales)/(Transactions-IncrementalTransactions))-1.0 ELSE 0 END
			   ,ATFUplift=CASE WHEN IncrementalSpenders<Spenders AND IncrementalTransactions<Transactions AND Spenders>0 THEN (1.0*Transactions/Spenders)/(1.0*(Transactions-IncrementalTransactions)/(Spenders-IncrementalSpenders))-1.0 ELSE 0 END
			 ...
			 UPDATE Warehouse.MI.CampaignInternalResultsLTEFinalCSR_SuperSegment SET
			 TScoreSPC=CASE WHEN PooledStdDevSPC>0 THEN ABS(1.0*IncrementalSales/Cardholders)/PooledStdDevSPC ELSE 0 END,
			 TScoreRR=CASE WHEN PooledStdDevRR>0 THEN ABS(1.0*IncrementalSpenders/Cardholders)/PooledStdDevRR ELSE 0 END,
			 TScoreSPS=CASE WHEN PooledStdDevSPS>0 THEN ABS(1.0*SPS_Diff)/PooledStdDevSPS ELSE 0 END
			 WHERE TScoreSPC IS NULL
			...
			-- Significance calculations
			 ...
			 SignificantUpliftSPC=CASE WHEN TScoreSPC>=Probability_05 THEN 'High'
						  WHEN  TScoreSPC>=Probability_20 THEN 'Moderate'
						  ELSE 'No' END
			 FROM Warehouse.MI.CampaignInternalResultsLTEFinalCSR_SuperSegment w
			 LEFT JOIN Warehouse.Stratification.TTestValues t on w.DegreesOfFreedomSPC
			 BETWEEN t.MinDegreesOfFreedom AND COALESCE(t.MaxDegreesOfFreedom,w.DegreesOfFreedomSPC) and t.Tailes=2
			 WHERE PValueSPC IS NULL
