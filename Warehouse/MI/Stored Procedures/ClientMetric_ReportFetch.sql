...
***************************************************************************/
CREATE PROCEDURE [MI].[ClientMetric_ReportFetch]
AS
BEGIN
    ...
    (
	   ...
	   FROM (SELECT DISTINCT Retailer from MI.ClientMetrics) x
	   CROSS JOIN (Select Distinct forecast from MI.ClientMetrics) y
    )
    , Dates
    AS 
    (
	   SELECT DISTINCT [Date]
	   FROM MI.ClientMetrics
    )
    , Clubs
    AS
    (
    ...
    LEFT JOIN MI.ClientMetrics c on m.Date = c.Date 
