...
CREATE PROCEDURE [MI].[CampaignResults_Calculate_Part2] 
...
IF @Error=0 
    ...
    --- OPTIONAL: 
	    ...
	    --FROM Warehouse.Relational.SchemeUpliftTrans_Legacy
	    --WHERE PartnerID IN (SELECT DISTINCT PartnerID FROM ' + @DatabaseName + '.' + @SchemaName + '.' + 'CampM_Partner_LK)
    ...
    -- Otherwise use Warehouse.Relational.SchemeUpliftTrans_Legacy
    ...
    BEGIN
	   ...
	   SELECT DISTINCT w.Period, a.CinID, a.FanID, a.CompositeID, a.ClientServicesRef, a.StartDate, 
	    ...
	    INNER JOIN Warehouse.Relational.PartnerTrans  t ON t.TransactionAmount>0 AND a.FanID=t.FanID -- and t.IronOfferID = a.IronOfferID
    ...
    END
    ELSE
    -- Transactions during Campaign using SchemeUpliftTrans_Legacy (Updated every Monday morning)
    BEGIN
	    INSERT INTO #TransactionsRaw
	    SELECT DISTINCT w.Period, a.CinID, a.FanID, a.CompositeID, a.ClientServicesRef, a.StartDate, 
	    a.Grp, a.ControlType, a.HTMID, a.HTM_Description, a.SuperSegmentID, a.SuperSegment_Description, a.Cell,
	    ...
	    FROM ' + @DatabaseName + '.' + @SchemaName + '.' + 'CampM_CustSelected  a
	    INNER JOIN Warehouse.Relational.SchemeUpliftTrans_Legacy  t ON t.Amount>0 AND a.FanID=t.FanID AND t.IsRetailReport=1
    ...
    -- For Out of Programme Control always use Warehouse.Relational.SchemeUpliftTrans_Legacy
    ...
    EXEC MI.CampaignHistoryIndexes_Rebuild 
    ...
    -- we cannot use use Warehouse.Relational.PartnerTrans and have to use Warehouse.Relational.SchemeUpliftTrans_Legacy for Transactions Below Spend Treshhold
    ...
    BEGIN 
	   ...
	   -- Use SchemeUpliftTrans_Legacy for other Transactions
	   INSERT INTO #CommissionRaw
	   SELECT DISTINCT w.Period, a.CinID, a.FanID, a.CompositeID, a.ClientServicesRef, a.StartDate, 
	    a.Grp, a.ControlType, a.HTMID, a.HTM_Description, a.SuperSegmentID, a.SuperSegment_Description, a.Cell,
	    CONCAT(t.FileID,t.RowNum) MatchID, Amount Sales, DENSE_RANK() OVER (PARTITION BY CONCAT(t.FileID,t.RowNum), a.CINID, a.FANID, a.CompositeID, a.ClientServicesRef, 
