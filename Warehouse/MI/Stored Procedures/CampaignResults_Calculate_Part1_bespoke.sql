...
CREATE PROCEDURE [MI].[CampaignResults_Calculate_Part1_bespoke] (@ClientServicesRef varchar(25)=NULL, @StartDate DATE=NULL, @DatabaseName NVARCHAR(400)='Sandbox', @Bespoke int = 0) AS -- -- unhide this row to modify SP
...
DECLARE @CSString nvarchar(15) = CASE @Bespoke WHEN 0 THEN '' ELSE ' Bespoke ' + CAST (@Bespoke as nvarchar) END
...
IF @Error=0 
    ...
    SELECT DISTINCT ClientServicesRef + @CSString
    ...
    SELECT DISTINCT ch.ClientServicesRef  + @CSString as ClientServicesRef,ch.IronOfferID, ch.CashbackRate, 
    ...
    WHERE ch.ClientServicesRef  + @CSString IN (SELECT OfferRequestID from #OfferRequestID)  
    ...
    --AND sc.ClientServicesRef NOT LIKE 'WA%'
  ...
  --  AND NOT EXISTS
  --  (
	 --  SELECT 1 FROM Relational.Campaign_History c2
	 --  JOIN Relational.IronOffer_Campaign_HTM h on h.IronOfferID = c2.IronOfferID
	 --  WHERE c2.FanID = b.FanID
		--  		  AND c2.IronOfferID = 9074
		--  and c2.grp = 'Mail'
		 
  --  )  -- im.CompositeID check is getting rid of opt outs not assigned to offer
	 --  AND sc.ClientServicesRef like 'WA%'
  ...
  --  LEFT JOIN Warehouse.MI.SuperSegmentGroups s ON s.SuperSegmentID=l.SuperSegmentID
  --  LEFT JOIN warehouse.Relational.HeadroomTargetingModel_Groups htm_G ON COALESCE(sc.HTMSegment, CASE WHEN b.HTMID=0 THEN NULL ELSE b.HTMID END, sow.HTMID,9999)=htm_G.HTMID 
  --  WHERE (b.Grp='Mail' AND im.CompositeID IS NOT NULL)
  --  AND (@Bespoke = 1 AND NOT EXISTS
  --  (
	 --  SELECT 1 FROM Relational.Campaign_History c2
	 --  JOIN Relational.IronOffer_Campaign_HTM h on h.IronOfferID = c2.IronOfferID
	 --  WHERE c2.FanID = b.FanID
  ...
  --  ))  -- im.CompositeID check is getting rid of opt outs not assigned to offer
    ...
    --SELECT DISTINCT b.FanID
	   ...
	   --, sc.Base_CommissionRate
	   --, sc.SuperSegmentID
	   --, sc.SuperSegment_Description
    --FROM Sandbox.Phil.MandB_fakeControls b
    --JOIN Relational.IronOffer_Campaign_HTM h on h.ClientServicesRef = b.ClientServicesRef
