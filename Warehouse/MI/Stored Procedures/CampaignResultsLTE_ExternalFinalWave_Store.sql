...
CREATE PROCEDURE [MI].[CampaignResultsLTE_ExternalFinalWave_Store] (@ClientServicesRef VARCHAR(25), @StartDate DATE, -- unhide this row to modify SP
...
SELECT 'CampaignResultsLTE_ExternalFinalWave_Store', 
...
BEGIN 
   ...
   END
    ...
    BEGIN 
	 	...
	 	-- Populate CampaignExternalResultsLTEFinalWave (delete old entries first)
		DELETE FROM Warehouse.MI.CampaignExternalResultsLTEFinalWave
		WHERE ClientServicesRef=@ClientServicesRef AND StartDate=@StartDate

		INSERT INTO Warehouse.MI.CampaignExternalResultsLTEFinalWave
		...
		FROM (SELECT Effect,ClientServicesRef,StartDate, SUM(Cardholders) Cardholders, SUM(Sales) Sales, SUM(Transactions) Transactions, SUM(Spenders) Spenders,
			 ...
			 FROM (SELECT * FROM Warehouse.MI.CampaignInternalResultsLTE_PureSales UNION SELECT * FROM Warehouse.MI.CampaignExternalResultsLTE_PureSales) a
		...
		LEFT JOIN (SELECT Effect,ClientServicesRef,StartDate, SUM(Cardholders_M) UpliftCardholders, SUM(Cardholders_C) ControlGroupSize, SUM(IncrementalSales) IncrementalSales, 
				...
				FROM (SELECT * FROM Warehouse.MI.CampaignInternalResultsLTE_Workings UNION SELECT * FROM Warehouse.MI.CampaignExternalResultsLTE_Workings) a
				WHERE ControlGroup=@ControlGroup AND CustomerUniverse=@CustomerUniverse AND Level=@AggregationLevel
				AND ClientServicesRef=@ClientServicesRef AND StartDate=@StartDate AND SalesType='Main Results (Qualifying MIDs or Channels Only)'
		...
		WHERE s.Cardholders>0
			 ...
			 UPDATE Warehouse.MI.CampaignExternalResultsLTEFinalWave SET
			...
			-- Significance calculations
			 ...
			 SignificantUpliftSPC=CASE WHEN TScoreSPC>=Probability_05 THEN 'High'
						  ...
						  ELSE 'No' END
			 FROM Warehouse.MI.CampaignExternalResultsLTEFinalWave w
 		...
 		-- Populate CampaignExternalResultsLTEFinalWave_BespokeCell (delete old entries first)   
    		DELETE FROM Warehouse.MI.CampaignExternalResultsLTEFinalWave_BespokeCell
		WHERE ClientServicesRef=@ClientServicesRef AND StartDate=@StartDate

		INSERT INTO Warehouse.MI.CampaignExternalResultsLTEFinalWave_BespokeCell
		...
		WHERE s.Cardholders>0
			 ...
			 -- Uplif calculations
			 UPDATE Warehouse.MI.CampaignExternalResultsLTEFinalWave_BespokeCell
			 SET SalesUplift=CASE WHEN IncrementalSales<Sales THEN 1.0*IncrementalSales/(Sales-IncrementalSales) ELSE IncrementalSales END
			   ,RRUplift=CASE WHEN IncrementalSpenders<Spenders THEN 1.0*IncrementalSpenders/(Sales-IncrementalSpenders) ELSE IncrementalSpenders END
			   ,ATVUplift=CASE WHEN IncrementalTransactions<Transactions AND IncrementalSales<Sales AND Transactions>0 THEN (1.0*Sales/Transactions)/(1.0*(Sales-IncrementalSales)/(Transactions-IncrementalTransactions))-1.0 ELSE 0 END
			   ,ATFUplift=CASE WHEN IncrementalSpenders<Spenders AND IncrementalTransactions<Transactions AND Spenders>0 THEN (1.0*Transactions/Spenders)/(1.0*(Transactions-IncrementalTransactions)/(Spenders-IncrementalSpenders))-1.0 ELSE 0 END
			 ...
			 UPDATE Warehouse.MI.CampaignExternalResultsLTEFinalWave_BespokeCell SET
			 TScoreSPC=CASE WHEN PooledStdDevSPC>0 THEN ABS(1.0*IncrementalSales/Cardholders)/PooledStdDevSPC ELSE 0 END,
			 TScoreRR=CASE WHEN PooledStdDevRR>0 THEN ABS(1.0*IncrementalSpenders/Cardholders)/PooledStdDevRR ELSE 0 END,
			 TScoreSPS=CASE WHEN PooledStdDevSPS>0 THEN ABS(1.0*SPS_Diff)/PooledStdDevSPS ELSE 0 END
			 WHERE ClientServicesRef=@ClientServicesRef AND StartDate=@StartDate
			...
			-- Significance calculations
			 ...
			 SignificantUpliftSPC=CASE WHEN TScoreSPC>=Probability_05 THEN 'High'
						  WHEN  TScoreSPC>=Probability_20 THEN 'Moderate'
						  ELSE 'No' END
			 FROM Warehouse.MI.CampaignExternalResultsLTEFinalWave_BespokeCell w
			 LEFT JOIN Warehouse.Stratification.TTestValues t on w.DegreesOfFreedomSPC
			 BETWEEN t.MinDegreesOfFreedom AND COALESCE(t.MaxDegreesOfFreedom,w.DegreesOfFreedomSPC) and t.Tailes=2
			 WHERE w.ClientServicesRef=@ClientServicesRef AND w.StartDate=@StartDate
 		...
 		-- Populate CampaignExternalResultsLTEFinalWave_Segment (delete old entries first)   
    		DELETE FROM Warehouse.MI.CampaignExternalResultsLTEFinalWave_Segment
		WHERE ClientServicesRef=@ClientServicesRef AND StartDate=@StartDate

		INSERT INTO Warehouse.MI.CampaignExternalResultsLTEFinalWave_Segment
		...
		WHERE s.Cardholders>0
			 ...
			 -- Uplif calculations
			 UPDATE Warehouse.MI.CampaignExternalResultsLTEFinalWave_Segment
			 SET SalesUplift=CASE WHEN IncrementalSales<Sales THEN 1.0*IncrementalSales/(Sales-IncrementalSales) ELSE IncrementalSales END
			   ,RRUplift=CASE WHEN IncrementalSpenders<Spenders THEN 1.0*IncrementalSpenders/(Sales-IncrementalSpenders) ELSE IncrementalSpenders END
			   ,ATVUplift=CASE WHEN IncrementalTransactions<Transactions AND IncrementalSales<Sales AND Transactions>0 THEN (1.0*Sales/Transactions)/(1.0*(Sales-IncrementalSales)/(Transactions-IncrementalTransactions))-1.0 ELSE 0 END
			   ,ATFUplift=CASE WHEN IncrementalSpenders<Spenders AND IncrementalTransactions<Transactions AND Spenders>0 THEN (1.0*Transactions/Spenders)/(1.0*(Transactions-IncrementalTransactions)/(Spenders-IncrementalSpenders))-1.0 ELSE 0 END
			 ...
			 UPDATE Warehouse.MI.CampaignExternalResultsLTEFinalWave_Segment SET
			 TScoreSPC=CASE WHEN PooledStdDevSPC>0 THEN ABS(1.0*IncrementalSales/Cardholders)/PooledStdDevSPC ELSE 0 END,
			 TScoreRR=CASE WHEN PooledStdDevRR>0 THEN ABS(1.0*IncrementalSpenders/Cardholders)/PooledStdDevRR ELSE 0 END,
			 TScoreSPS=CASE WHEN PooledStdDevSPS>0 THEN ABS(1.0*SPS_Diff)/PooledStdDevSPS ELSE 0 END
			 WHERE ClientServicesRef=@ClientServicesRef AND StartDate=@StartDate
			...
			-- Significance calculations
			 ...
			 SignificantUpliftSPC=CASE WHEN TScoreSPC>=Probability_05 THEN 'High'
						  WHEN  TScoreSPC>=Probability_20 THEN 'Moderate'
						  ELSE 'No' END
			 FROM Warehouse.MI.CampaignExternalResultsLTEFinalWave_Segment w
			 LEFT JOIN Warehouse.Stratification.TTestValues t on w.DegreesOfFreedomSPC
			 BETWEEN t.MinDegreesOfFreedom AND COALESCE(t.MaxDegreesOfFreedom,w.DegreesOfFreedomSPC) and t.Tailes=2
			 WHERE w.ClientServicesRef=@ClientServicesRef AND w.StartDate=@StartDate
 		...
 		-- Populate CampaignExternalResultsLTEFinalWave_SuperSegment (delete old entries first)   
    		DELETE FROM Warehouse.MI.CampaignExternalResultsLTEFinalWave_SuperSegment
		WHERE ClientServicesRef=@ClientServicesRef AND StartDate=@StartDate

		INSERT INTO Warehouse.MI.CampaignExternalResultsLTEFinalWave_SuperSegment
		(Effect,ClientServicesRef,StartDate, SegmentID, ControlGroup,
		CustomerUniverse,Level,
		ControlGroupSize,Cardholders,Sales,Transactions,Spenders,
		IncrementalSales,IncrementalMargin,
		...
		WHERE s.Cardholders>0
			 ...
			 -- Uplif calculations
			 UPDATE Warehouse.MI.CampaignExternalResultsLTEFinalWave_SuperSegment
			 SET SalesUplift=CASE WHEN IncrementalSales<Sales THEN 1.0*IncrementalSales/(Sales-IncrementalSales) ELSE IncrementalSales END
			   ,RRUplift=CASE WHEN IncrementalSpenders<Spenders THEN 1.0*IncrementalSpenders/(Sales-IncrementalSpenders) ELSE IncrementalSpenders END
			   ,ATVUplift=CASE WHEN IncrementalTransactions<Transactions AND IncrementalSales<Sales AND Transactions>0 THEN (1.0*Sales/Transactions)/(1.0*(Sales-IncrementalSales)/(Transactions-IncrementalTransactions))-1.0 ELSE 0 END
			   ,ATFUplift=CASE WHEN IncrementalSpenders<Spenders AND IncrementalTransactions<Transactions AND Spenders>0 THEN (1.0*Transactions/Spenders)/(1.0*(Transactions-IncrementalTransactions)/(Spenders-IncrementalSpenders))-1.0 ELSE 0 END
			 ...
			 UPDATE Warehouse.MI.CampaignExternalResultsLTEFinalWave_SuperSegment SET
			 TScoreSPC=CASE WHEN PooledStdDevSPC>0 THEN ABS(1.0*IncrementalSales/Cardholders)/PooledStdDevSPC ELSE 0 END,
			 TScoreRR=CASE WHEN PooledStdDevRR>0 THEN ABS(1.0*IncrementalSpenders/Cardholders)/PooledStdDevRR ELSE 0 END,
			 TScoreSPS=CASE WHEN PooledStdDevSPS>0 THEN ABS(1.0*SPS_Diff)/PooledStdDevSPS ELSE 0 END
			 WHERE ClientServicesRef=@ClientServicesRef AND StartDate=@StartDate
			...
			-- Significance calculations
			 ...
			 SignificantUpliftSPC=CASE WHEN TScoreSPC>=Probability_05 THEN 'High'
						  WHEN  TScoreSPC>=Probability_20 THEN 'Moderate'
						  ELSE 'No' END
			 FROM Warehouse.MI.CampaignExternalResultsLTEFinalWave_SuperSegment w
			 LEFT JOIN Warehouse.Stratification.TTestValues t on w.DegreesOfFreedomSPC
			 BETWEEN t.MinDegreesOfFreedom AND COALESCE(t.MaxDegreesOfFreedom,w.DegreesOfFreedomSPC) and t.Tailes=2
			 WHERE w.ClientServicesRef=@ClientServicesRef AND w.StartDate=@StartDate
