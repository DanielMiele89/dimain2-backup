...
CREATE PROCEDURE [MI].[CampaignResults_Calculate_Part1_SSControl] (@ClientServicesRef varchar(25)=NULL, @StartDate DATE=NULL, @DatabaseName NVARCHAR(400)='Sandbox') 
--WITH EXECUTE AS OWNER
AS -- -- unhide this row to modify SP
--DECLARE @ClientServicesRef varchar(25); SET @ClientServicesRef='AA000';DECLARE  @StartDate DATE; SET @StartDate='1999-01-01'; DECLARE @DatabaseName NVARCHAR(400); SET @DatabaseName=@DatabaseName  -- unhide this row to run code once
...
IF @Error=0 
    ...
    SELECT DISTINCT ch.ClientServicesRef,ch.IronOfferID, ch.CashbackRate, 
    CASE WHEN ch.PartnerID='3960' THEN CashbackRate-1 ELSE ch.CommissionRate END CommissionRate , 
    ch.HTMSegment, ch.PartnerID,  ch.Base_CashbackRate Base_CashbackRate,  -- populated only for core retailers
    CASE WHEN ch.PartnerID='3960' THEN COALESCE(ch.Base_CashbackRate-1,0)  
    ELSE ch.Base_CommissionRate END Base_CommissionRate, -- populated only for core retailers
    /* Bespoke SU018 30/3/2016 Lloyd/Nick io.Startdate */-- CASE WHEN ch.ClientServicesRef = 'SU018' THEN '2016-01-25' ELSE io.Startdate END AS StartDate, io.Enddate
    ...
    --AND io.EndDate<GETDATE() -- measure only campaigns that finished
    ...
    DECLARE @CTable nvarchar(40) = CASE WHEN (SELECT COUNT(1) FROM Sandbox.Beyers.Roc_NewControls_1807 a JOIN #SelectIronCodes sc on sc.PartnerID = a.PartnerID) > 0 THEN 'Sandbox.Beyers.Roc_NewControls_1807' ELSE 'Sandbox.Beyers.ROC_NewControls_0108' END
    DECLARE @CPartner nvarchar(30) = CASE WHEN @CTable = 'Sandbox.Beyers.Roc_NewControls_1807' THEN 'PartnerID' ELSE 'ShopperSegmentPartner' END
    ...
    SELECT DISTINCT b.FanID
		...
		,sc.IronOfferID
		 ,''Control'' as Grp
		 ,''Random'' as ControlType
		 ,sc.partnerID
		 ,COALESCE(sc.HTMSegment, NULL, sow.HTMID,9999) HTMID -- select SoW stored in Warehouse.Relational.IronOffer_Campaign_HTM, if null take value stored in Warehouse.Relational.Campaign_History, if null take one calculated in #SoW_FanID
		 ,COALESCE(HTM_Description,'''') HTM_Description   --- HTM group at time of selection (see 1 line above how it is defined)
		 ,sc.StartDate
		 ,sc.Enddate
		 ,sc.CashbackRate
		 ...
		 ,COALESCE(SuperSegmentDescription,'''') SuperSegment_Description
    FROM '+@CTable +' b
    INNER JOIN #SelectIronCodes sc ON sc.PartnerID = b.'+@CPartner+' and sc.IronOfferID in (10035,10060,10079,10083,10102,10132,10135,10184,10191,10200,10203,10206)
    LEFT JOIN Warehouse.Relational.IronOfferMember im ON im.CompositeID=b.Compositeid AND im.IronOfferID=sc.IronOfferID
    LEFT JOIN #SoW_FanID sow ON sow.FanID=b.FanID AND sc.ClientServicesRef=sow.ClientServicesRef and sc.PartnerID=sow.PartnerID
    LEFT JOIN Warehouse.MI.SuperSegmentHTMLink l ON l.HTMID=COALESCE(sc.HTMSegment,  NULL, sow.HTMID,9999)
    LEFT JOIN Warehouse.MI.SuperSegmentGroups s ON s.SuperSegmentID=l.SuperSegmentID
    LEFT JOIN warehouse.Relational.HeadroomTargetingModel_Groups htm_G ON COALESCE(sc.HTMSegment, NULL, sow.HTMID,9999)=htm_G.HTMID 
   ')


   DECLARE @Cnt int
   SET @Cnt = (SELECT COUNT(1) FROM #CustSelected0 WHERE Grp = 'Mail')/1.3
    ...
    DECLARE @ColName nvarchar(30), @DBName nvarchar(50)

    SET @ColName = (
	   SELECT MAX(ColumnName) FROM Sandbox.Hayden.SSControlGroups s
	   JOIN #OfferRequestID o on o.OfferRequestID = s.ClientServicesRef
    )

    SET @DBName = (
	   SELECT MAX(TableName) FROM Sandbox.Hayden.SSControlGroups s
	   JOIN #OfferRequestID o on o.OfferRequestID = s.ClientServicesRef
	   WHERE inProg = 0
    )
    ...
    EXEC ('
    INSERT INTO #CustSelected0
	 --  SELECT ID, ClientServicesRef, IronOfferID, Grp, ControlType, PartnerID
	--	  , HTMID, HTM_Description, StartDate, EndDate, CashbackRate, Base_CashbackRate, CommissionRate, Base_CommissionRate, SuperSegmentID, SuperSegment_Description
	--   FROM (
		--  SELECT *, ROW_NUMBER() OVER (Partition By IronOfferID ORDER BY NewID()) as Rank FROM (
			 SELECT DISTINCT b.ID
				  ...
				  ,sc.IronOfferID
				   ...
				   ,''Out of Programme'' ControlType
				   ,sc.partnerID
				   ,COALESCE(sc.HTMSegment, NULL, sow.HTMID,9999) HTMID -- select SoW stored in Warehouse.Relational.IronOffer_Campaign_HTM, if null take value stored in Warehouse.Relational.Campaign_History, if null take one calculated in #SoW_FanID_Control 
				   ,COALESCE(HTM_Description,'''') HTM_Description    --- HTM group at time of selection (see 1 line above how it is defined)
				   ,sc.StartDate
				   ,sc.Enddate
				   ,sc.CashbackRate
				   ...
				   ,COALESCE(s.SuperSegmentID,9999) SuperSegmentID
				   ,COALESCE(SuperSegmentDescription,'''') SuperSegment_Description
			 FROM Sandbox.Hayden.SSControlGroups x
			 CROSS JOIN '+@DBName+' c     
			 INNER JOIN #SelectIronCodes sc ON sc.IronOfferID=x.IronOfferID
			 JOIN Warehouse.Relational.CINList cl on cl.CINID = c.CINID
			 JOIN SLC_Report.dbo.Fan b on b.SourceUID = cl.CIN
			 LEFT JOIN #SoW_FanID_Control sow ON sow.FanID=b.ID AND sc.ClientServicesRef=sow.ClientServicesRef and sc.PartnerID=sow.PartnerID
			 LEFT JOIN Warehouse.MI.SuperSegmentHTMLink l ON l.HTMID=COALESCE(sc.HTMSegment, NULL, sow.HTMID,9999)
			 LEFT JOIN Warehouse.MI.SuperSegmentGroups s ON s.SuperSegmentID=l.SuperSegmentID
			 LEFT JOIN warehouse.Relational.HeadroomTargetingModel_Groups htm_G ON COALESCE(sc.HTMSegment, NULL, sow.HTMID,9999)=htm_G.HTMID 
			 WHERE '+@ColName+'_RecencySegment = x.Recency and '+@ColName+'_CorePrimeSegment = x.CorePrime
	 --  ) x
  --  ) y
   -- WHERE Rank <= '+@Cnt + '
')
