...
-- =============================================
CREATE PROCEDURE [MI].[WarehousePortalCrossCheck_Fetch_v2]
...
BEGIN
	...
	SELECT COALESCE(a.TranDate, b.TranDate, c.TranDate) AS TranDate
		, COALESCE(a.AddedDate, b.AddedDate, c.AddedDate) AS AddedDate
	...
	FROM
	(
		SELECT TransactionDate AS TranDate, AddedDate as AddedDate, COUNT(*) AS TranCount, SUM(CashbackEarned) AS Cashback
		FROM Relational.PartnerTrans
		WHERE TransactionDate > @StartDate
		AND AddedDate <= @CashACAAddedDate
		GROUP BY TransactionDate, AddedDate
	) a
	FULL OUTER JOIN
	(
		SELECT TranDate, AddedDate as AddedDate, COUNT(*) AS TranCount, SUM(CashbackEarned) AS Cashback
		FROM Relational.AdditionalCashbackAward
		WHERE TranDate > @StartDate
		AND AddedDate <= @CashACAAddedDate
		GROUP BY TranDate , AddedDate
	) b ON A.TranDate = B.TranDate
	FULL OUTER JOIN
	(
	SELECT ac.AddedDate AS TranDate, AddedDate as AddedDate, COUNT(*) AS TranCount, SUM(ac.CashbackEarned) AS Cashback
