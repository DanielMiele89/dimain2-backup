...
CREATE PROCEDURE [MI].[CampaignResults_Calculate_Part1_SS] (@ClientServicesRef varchar(25)=NULL, @StartDate DATE=NULL, @DatabaseName NVARCHAR(400)='Sandbox') 
...
IF @Error=0 
    ...
    INNER JOIN #SelectIronCodes sc ON sc.IronOfferID=b.IronOfferID and b.SDate = sc.StartDate and b.EDate = sc.EndDate
    LEFT JOIN Warehouse.Relational.IronOfferMember im ON im.CompositeID=b.Compositeid AND im.IronOfferID=b.IronOfferID and im.StartDate = sc.StartDate and im.EndDate = sc.EndDate
    ...
    BEGIN
	   ...
	   INNER JOIN Warehouse.Relational.IronOfferMember im ON im.CompositeID=b.Compositeid AND im.IronOfferID=sc.IronOfferID and im.StartDate = sc.StartDate and im.EndDate = sc.EndDate
	   LEFT JOIN #SoW_FanID sow ON sow.FanID=b.FanID AND sc.ClientServicesRef=sow.ClientServicesRef and sc.PartnerID=sow.PartnerID
