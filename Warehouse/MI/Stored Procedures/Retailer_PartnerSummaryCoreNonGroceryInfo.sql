...
-- =============================================
CREATE PROCEDURE [MI].[Retailer_PartnerSummaryCoreNonGroceryInfo] 
	(
		@MonthID INT
	...
	SELECT SUTM.MonthDesc
	  ...
	  INNER JOIN (SELECT RM.PartnerID
					...
					WHERE PaymentTypeID = 0 
					  ...
					  AND PartnerID != 4433  --tesco
					  AND PartnerID != 4447  --co-op
	  ...
	  AND PeriodTypeID = 1
	  AND ClientServiceRef = '0' --CORE
	  AND rm.PartnerID != 4433  --tesco
	  AND rm.PartnerID != 4447  --co-op
	  AND DateID = @MonthID
