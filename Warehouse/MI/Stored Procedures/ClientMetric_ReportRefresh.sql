...
***************************************************************************/
CREATE PROCEDURE [MI].[ClientMetric_ReportRefresh]
AS
BEGIN
    ...
    TRUNCATE TABLE MI.ClientMetrics

    INSERT INTO MI.ClientMetrics (Retailer, Date, Value, Club, Forecast)
    ...
    SELECT Tbl_PartnerName, Dates, SUM(Override), Club, Forecast FROM (
	   SELECT 
		  ...
		  , 'MyRewards' as Club
		  , 1 as Forecast
	   from MI.ClientMetric_CampDashboard x
	   JOIN MI.ClientMetric_Exceptions y on y.PartnerName = x.Retailer or y.Tbl_PartnerName = x.Retailer
    ) x
    ...
    select 
	   ...
	   , 'MyRewards'
	   , 0
    from MI.ClientMetric_FinanceXL x
    JOIN MI.ClientMetric_Exceptions y on y.PartnerName = x.Retailer or y.Tbl_PartnerName = x.Retailer
    ...
    SELECT
	   ...
	   , 'Quidco'
	   , 0
    FROM MI.ClientMetric_QuidcoActuals q
    LEFT JOIN MI.ClientMetric_Exceptions y on y.PartnerName = q.Retailer or y.Tbl_PartnerName = q.Retailer
