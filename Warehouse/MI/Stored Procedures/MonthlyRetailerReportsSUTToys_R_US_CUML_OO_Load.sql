...
-- =============================================
CREATE PROCEDURE [MI].[MonthlyRetailerReportsSUTToys_R_US_CUML_OO_Load]
...
SELECT * 
INTO   #ch 
FROM   [Warehouse].[Relational].[campaign_history] 
WHERE  grp = 'Control' 
       AND partnerid = 4494 
...
GROUP  BY --SUTM.id,
case when SUT.isonline = 1 then 105 else 106 end,
          p.partnerid, 
          CH.ironofferid 
--ORDER  BY SUTM.id 
drop table #ch
...
,[ControlTrans]= CS.[TranCount]
 ...
 inner join #ConSplit CS on RM.PartnerID = CS.partnerid 
	...
	and RM.ironofferid  = CS.ironofferid 
	and RM.[LabelID] = lableid
