...
CREATE PROCEDURE [MI].[CampaignReport_Email_ClientServices_versionbefore_2016_05_27]
AS
BEGIN
	SET NOCOUNT ON;
...
set @insight = 'Insight@rewardinsight.com;ed.allison@rewardinsight.com'
set @update = ''
...
SELECT '<td><a href="S:\Data Insight\Team\Work Templates\Campaign\Campaign Measurement\Campaign Reports\'+ x.ClientServicesRef + '_' + LEFT(CONVERT(VARCHAR, x.StartDate, 120), 10) + ' - Final Campaign Report.pptx">'
	...
	+ FORMAT(x.Investment, 'C00', 'en-GB') + '</td><td align="right">' 
     + FORMAT(
	   CASE WHEN (x.IncrementalSales + IncrementalSalesAw + IncrementalSalesLo)/ ISNULL(NULLIF(((x.Sales + SalesAw + SalesLo) - (x.IncrementalSales + IncrementalSalesAw + IncrementalSalesLo)), 0), 0.001) < 0.01 THEN 0.01
	   ELSE (x.IncrementalSales + IncrementalSalesAw + IncrementalSalesLo)/ ISNULL(NULLIF(((x.Sales + SalesAw + SalesLo) - (x.IncrementalSales + IncrementalSalesAw + IncrementalSalesLo)), 0), 0.001) END
...
set @intTable = ISNULL(@intTable, '<td colspan="100">There are no interim reports to be delviered next week <br /> If you believe this is an error, please inform us</td>')
...
set @extTable = ISNULL(@extTable, '<td colspan="100">There are no extended reports to be delviered next week <br />If you believe this is an error, please inform us</td>')
