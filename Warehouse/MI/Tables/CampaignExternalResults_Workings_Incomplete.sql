CREATE TABLE [MI].[CampaignExternalResults_Workings_Incomplete] (
    ...
    CONSTRAINT [UNQ_CampaignResultsOutofProgramme_Workings_Incomplete] UNIQUE NONCLUSTERED ([ControlGroup] ASC, [SalesType] ASC, [CustomerUniverse] ASC, [ClientServicesRef] ASC, [StartDate] ASC, [Level] ASC, [SegmentID] ASC, [Cell] ASC)
);


GO
CREATE NONCLUSTERED INDEX [IND_3]
    ON [MI].[CampaignExternalResults_Workings_Incomplete]([Cell] ASC);


GO
CREATE NONCLUSTERED INDEX [IND_2]
    ON [MI].[CampaignExternalResults_Workings_Incomplete]([SegmentID] ASC);


GO
CREATE NONCLUSTERED INDEX [IND_1]
    ON [MI].[CampaignExternalResults_Workings_Incomplete]([StartDate] ASC);


GO
CREATE CLUSTERED INDEX [IND_C]
    ON [MI].[CampaignExternalResults_Workings_Incomplete]([ClientServicesRef] ASC);
