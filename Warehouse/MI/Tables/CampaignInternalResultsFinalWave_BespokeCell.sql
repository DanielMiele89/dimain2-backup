CREATE TABLE [MI].[CampaignInternalResultsFinalWave_BespokeCell] (
    ...
    CONSTRAINT [UNQ_CampaignResultsFinalWave_BespokeCell] UNIQUE NONCLUSTERED ([ClientServicesRef] ASC, [StartDate] ASC, [Cell] ASC)
);


GO
CREATE NONCLUSTERED INDEX [IND]
    ON [MI].[CampaignInternalResultsFinalWave_BespokeCell]([ClientServicesRef] ASC, [StartDate] ASC);


GO
CREATE NONCLUSTERED INDEX [IND_3]
    ON [MI].[CampaignInternalResultsFinalWave_BespokeCell]([Cell] ASC);
...
CREATE TRIGGER [MI].[TrgAfterInsert_InternalResultsFinalWave_BespokeCell] ON [Warehouse].[MI].[CampaignInternalResultsFinalWave_BespokeCell]
AFTER INSERT
AS
...
CREATE TRIGGER [MI].[TrgAfterInsert_ResultsFinalWave_BespokeCell] ON [Warehouse].[MI].[CampaignInternalResultsFinalWave_BespokeCell]
AFTER INSERT
AS
    UPDATE Warehouse.MI.CampaignInternalResultsFinalWave_BespokeCell
    SET Inserted = GETDATE(), InsertedBy=SYSTEM_USER
