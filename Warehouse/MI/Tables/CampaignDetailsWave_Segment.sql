CREATE TABLE [MI].[CampaignDetailsWave_Segment] (
    ...
    CONSTRAINT [UN_CampaignDetailsWave_Segment] UNIQUE NONCLUSTERED ([ClientServicesRef] ASC, [StartDate] ASC, [SegmentID] ASC)
);


GO
CREATE NONCLUSTERED INDEX [IND1]
    ON [MI].[CampaignDetailsWave_Segment]([ClientServicesRef] ASC);


GO

CREATE TRIGGER [MI].[TrgAfterInsert_Wave_Segment] ON [Warehouse].[MI].[CampaignDetailsWave_Segment]
AFTER INSERT
AS
    UPDATE Warehouse.MI.CampaignDetailsWave_Segment
...
CREATE TRIGGER [MI].[TrgAfterUpdate_Wave_Segment] ON [Warehouse].[MI].[CampaignDetailsWave_Segment]
AFTER UPDATE
AS
    UPDATE Warehouse.MI.CampaignDetailsWave_Segment
