CREATE TABLE [MI].[CampaignResults_Segment_DO_NOT_USE] (
    ...
    CONSTRAINT [pk_Segment2] PRIMARY KEY CLUSTERED ([ClientServicesRef] ASC, [SegmentID] ASC)
);


GO

CREATE TRIGGER [MI].[NewTrgAfterInsert5] ON [Warehouse].[MI].[CampaignResults_Segment]
AFTER INSERT
AS
    UPDATE Warehouse.MI.CampaignResults_Segment
    SET Updated = GETDATE(), UpdatedBy=SYSTEM_USER,Inserted = GETDATE(), InsertedBy=SYSTEM_USER
    WHERE EXISTS (SELECT 1 FROM Inserted i where i.ClientServicesRef=Warehouse.MI.CampaignResults_Segment.ClientServicesRef and i.SegmentID=Warehouse.MI.CampaignResults_Segment.SegmentID)
GO

CREATE TRIGGER [MI].[NewTrgAfterUpdate5] ON [Warehouse].[MI].[CampaignResults_Segment]
AFTER UPDATE
AS
    UPDATE Warehouse.MI.CampaignResults_Segment 
