CREATE TABLE [MI].[CampaignInternalResults_Workings] (
    ...
    CONSTRAINT [UNQ_CampaignResults_Workings] UNIQUE NONCLUSTERED ([ControlGroup] ASC, [SalesType] ASC, [CustomerUniverse] ASC, [ClientServicesRef] ASC, [StartDate] ASC, [Level] ASC, [SegmentID] ASC, [Cell] ASC)
);


GO
CREATE NONCLUSTERED INDEX [IND_3]
    ON [MI].[CampaignInternalResults_Workings]([Cell] ASC);


GO
CREATE NONCLUSTERED INDEX [IND_2]
    ON [MI].[CampaignInternalResults_Workings]([SegmentID] ASC);


GO
CREATE NONCLUSTERED INDEX [IND_1]
    ON [MI].[CampaignInternalResults_Workings]([StartDate] ASC);


GO
CREATE CLUSTERED INDEX [IND_C]
    ON [MI].[CampaignInternalResults_Workings]([ClientServicesRef] ASC);
