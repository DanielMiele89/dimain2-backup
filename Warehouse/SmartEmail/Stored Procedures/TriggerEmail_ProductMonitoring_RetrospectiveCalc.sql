/*
		...
		Purpose:	For those people that have had a MyReward account open 65 or 125 days from a specific date & have not earned enough
*/

CREATE PROCEDURE [SmartEmail].[TriggerEmail_ProductMonitoring_RetrospectiveCalc] (	@Days INT
																				,	@SendDateFrom DATE
																				,	@SendDateTo DATE)
AS
BEGIN
...
*******************************************************************************************************************************************/
	...
	***********************************************************************************************************************/
		...
		AND (iof.EndDate IS NULL OR iof.EndDate > @Today)
...
*******************************************************************************************************************************************/
	...
	/***********************************************************************************************************************
		2.1.  Fetch all all current loyalty customers that opened an account x days ago
	***********************************************************************************************************************/

		--DECLARE @Days INT = 125
		DECLARE @DateFrom DATE = DATEADD(day, - @Days, @SendDateFrom)
			,	@DateTo DATE = DATEADD(day, - @Days, @SendDateTo)
		...
		INNER JOIN [SLC_Report].[dbo].[IronOfferMember] iom
			ON fa.CompositeID = iom.CompositeID
		...
		AND MIN(iom.StartDate) BETWEEN @DateFrom AND @DateTo
	...
	/***********************************************************************************************************************
		2.2.  Fetch all the accounts of the previously fetched customers
	***********************************************************************************************************************/
...
*******************************************************************************************************************************************/
	...
	***********************************************************************************************************************/
		...
		DECLARE @35DaysAgo DATETIME = DATEADD(DAY, -35, GETDATE())
...
*******************************************************************************************************************************************/
	...
	IF @Days = 65
		BEGIN
			INSERT INTO [SmartEmail].[TriggerEmail_ProductMonitoring_Retrospective] (	FanID
																					,	Day60AccountName
																					,	Day120AccountName
																					,	JointAccount)
