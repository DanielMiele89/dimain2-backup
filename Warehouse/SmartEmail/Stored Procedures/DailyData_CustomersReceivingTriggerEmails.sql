...
CREATE PROCEDURE [SmartEmail].[DailyData_CustomersReceivingTriggerEmails]
...
BEGIN
	...
	***********************************************************************************************************************/
		...
		LEFT JOIN ( -- CJM
			SELECT FanID, RetailerPaymentNumber = REPLACE(RetailerPaymentNumber,';',',')  
			FROM [Actito].[MFDDData]
	...
	***********************************************************************************************************************/
		...
		SELECT FanID, CAST('Dummy' AS VARCHAR(100)) AS Email, Brand, Loyalty, WelcomeEmailCode, Homemover, FirstEarnDate, FirstEarnType, Day60AccountName, Day120AccountName, DirectDebitEarn_PartnerID, DirectDebitEarn_TransactionNumber, MobileDormant, LoyaltyAccount, TriggerEmail
		INTO #DailyData_TriggerEmailCustomers
		FROM SmartEmail.DailyData_TriggerEmailCustomers
		WHERE 0 = 1

		--	Credit Card - Adding CC Customers
		INSERT INTO #DailyData_TriggerEmailCustomers 
			(FanID, Email, Brand, Loyalty, WelcomeEmailCode, Homemover, FirstEarnDate, FirstEarnType, Day60AccountName, Day120AccountName, DirectDebitEarn_PartnerID, DirectDebitEarn_TransactionNumber, MobileDormant, LoyaltyAccount, TriggerEmail)
		SELECT FanID
				, Email
				, Brand
				, Loyalty
		...
		FROM #TriggerEmailsTemp dd
		WHERE MobileDormantDate IS NOT NULL

		
		INSERT INTO SmartEmail.DailyData_TriggerEmailCustomers 
			(SendDate, TriggerEmail, Brand, Loyalty, FanID, InCountsTable, WelcomeEmailCode, Homemover, FirstEarnDate, FirstEarnType, Day60AccountName, Day120AccountName, LoyaltyAccount, DirectDebitEarn_PartnerID, DirectDebitEarn_TransactionNumber, MobileDormant)
		SELECT @SendDate AS SendDate
			 ...
			 , [all].MobileDormant
		FROM #DailyData_TriggerEmailCustomers [all]
		WHERE NOT EXISTS (
			SELECT 1 FROM SmartEmail.DailyData_TriggerEmailCustomers tec
