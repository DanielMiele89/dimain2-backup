...
CREATE PROCEDURE [SmartEmail].[TriggerEmail_ProductMonitoringMopUp_20200712]
AS
BEGIN
...
*******************************************************************************************************************************************/
	...
	***********************************************************************************************************************/
		...
		AND NOT EXISTS (SELECT 1
						...
						WHERE fa.ID = tec.FanID
						AND tec.TriggerEmail = 'Product Monitoring 120 Day'
						AND tec.SendDate IN ('2020-07-13', '2020-07-14'))
	...
	***********************************************************************************************************************/
		...
		AND MIN(iom.StartDate) = '2020-02-01 00:00:00.000'
...
*******************************************************************************************************************************************/
	...
	INNER JOIN PriortyBankAccount pba
		...
		AND cte.BankAccountID = pba.BankAccountID
			...
			IF OBJECT_ID('tempdb..#CustomersToEmail_TopHalf') IS NOT NULL DROP TABLE #CustomersToEmail_TopHalf
			SELECT cte.*
			INTO #CustomersToEmail_TopHalf
			FROM #CustomersToEmail cte
			INNER JOIN SmartEmail.DailyData dd
				ON cte.FanID = dd.FanID
			WHERE cte.TypeRank = 1
			AND LoyaltyAccount = 1
			AND Marketable = 1
			AND NOT (dd.ClubID = 132 AND dd.IsLoyalty = 0)
			AND cte.TypeRank = 1
			...
			SELECT DISTINCT
				 ...
				 , cte.TypeDesc
				 , cte.JointAccount
			FROM #CustomersToEmail_TopHalf cte
			WHERE cte.TypeRank = 1
			AND NOT EXISTS (	SELECT 1
								FROM [Staging].[SLC_Report_ProductMonitoring] pm
								WHERE cte.FanID = pm.FanID)
