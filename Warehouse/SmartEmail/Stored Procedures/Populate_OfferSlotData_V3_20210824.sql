...
*/
CREATE PROCEDURE [SmartEmail].[Populate_OfferSlotData_V3_20210824]
AS
BEGIN
	...
	*******************************************************************************************************************************************/
		...
		***********************************************************************************************************************/
			IF OBJECT_ID('tempdb..#CustomerSample') IS NOT NULL
				DROP TABLE #CustomerSample
			;WITH cte AS
			(
					SELECT	sc.FanID
				,	sc.CompositeID
				,	MAX(cu.ClubID) AS ClubID
				,	MAX(CASE
			...
			INNER JOIN [Relational].[Customer] cu
				ON sc.FanID = cu.FanID
			INNER JOIN [Relational].[Customer_RBSGSegments] sg
				ON sc.FanID = sg.FanID
			...
			(
				SELECT	*
					,	ROW_NUMBER() OVER (PARTITION BY clubID,IsLoyalty ORDER BY Fanid DESC) AS rn
				FROM cte
			),cte3 AS
			(
				SELECT	*
					,	ROW_NUMBER() OVER (PARTITION BY clubID,IsLoyalty ORDER BY Fanid DESC) AS rn
				FROM [SmartEmail].[SampleCustomersList]
			)
			SELECT	cte3.ID SampleCustomerID
				,	cte2.FanID RealCustomerFanID
			INTO #CustomerSample
			FROM cte2
			INNER JOIN cte3
			on cte2.ClubID = cte3.ClubID
			AND cte2.IsLoyalty = cte3.IsLoyalty
			AND cte2.rn = cte3.rn
			ORDER BY	cte2.ClubID
					,	cte2.IsLoyalty
					,	cte2.rn


			TRUNCATE TABLE [SmartEmail].[SampleCustomerLinks];
			WITH
			...
			INSERT INTO [SmartEmail].[SampleCustomerLinks] --** Insert new mapping
			SELECT	SampleCustomerID,RealCustomerFanID
			FROM	#CustomerSample
	...
	*******************************************************************************************************************************************/
		...
		***********************************************************************************************************************/
			...
			FROM (	SELECT LionSendID
						 ...
						 , IronOfferID
					FROM #Customers) cu
			PIVOT(AVG(IronOfferID) FOR Slot IN ([1], [2], [3], [4], [5], [6], [7])) AS pt
		...
		***********************************************************************************************************************/
			...
			FROM (	SELECT LionSendID
						 ...
						 , StartDate
					FROM #Customers) cu
			PIVOT(MIN(StartDate) FOR Slot IN ([1], [2], [3], [4], [5], [6], [7])) AS pt
		...
		***********************************************************************************************************************/
			...
			FROM (	SELECT LionSendID
						 ...
						 , EndDate
					FROM #Customers) cu
			PIVOT(MIN(EndDate) FOR Slot IN ([1], [2], [3], [4], [5], [6], [7])) AS pt
	...
	*******************************************************************************************************************************************/
		...
		TRUNCATE TABLE [SmartEmail].[OfferSlotData]
		INSERT INTO [SmartEmail].[OfferSlotData]
		SELECT oi.FanID
			 , oi.LionSendID
			 , oi.Offer1
			 , oi.Offer2
