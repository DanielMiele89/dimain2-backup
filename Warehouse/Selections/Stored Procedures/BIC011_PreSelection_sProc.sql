...
CREATE PROCEDURE [Selections].[BIC011_PreSelection_sProc]
AS
BEGIN
...
AND camg.CAMEO_CODE_GROUP_Category NOT IN ('Business Elite','Content Communities','Flourishing Society','Prosperous Professionals')
...
IF OBJECT_ID('Sandbox.SamH.BicesterABLower7517122021') IS NOT NULL DROP TABLE Sandbox.SamH.BicesterABLower7517122021
SELECT	CINID
INTO	Sandbox.SamH.BicesterABLower7517122021
FROM	#NtileEngaged
WHERE	NTILE_4 IN (2,3,4)
GROUP BY CINID


	IF OBJECT_ID('[Warehouse].[Selections].[BIC011_PreSelection]') IS NOT NULL DROP TABLE [Warehouse].[Selections].[BIC011_PreSelection]
	SELECT FanID
	INTO [Warehouse].[Selections].[BIC011_PreSelection]
	FROM #FB fb
	WHERE EXISTS (	SELECT 1
					FROM Sandbox.SamH.BicesterABLower7517122021  st
