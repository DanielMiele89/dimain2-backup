...
IF OBJECT_ID('[Warehouse].[Selections].[LE029_PreSelection_Top]') IS NOT NULL DROP TABLE [Warehouse].[Selections].[LE029_PreSelection_Top]
SELECT	fb.FanID
INTO [Warehouse].[Selections].[LE029_PreSelection_Top]
FROM #FB fb
WHERE EXISTS (	SELECT 1
				FROM Sandbox.bastienc.landsend_top50pct sb
				WHERE fb.CINID = sb.CINID)

IF OBJECT_ID('[Warehouse].[Selections].[LE029_PreSelection_Bottom]') IS NOT NULL DROP TABLE [Warehouse].[Selections].[LE029_PreSelection_Bottom]
SELECT	fb.FanID
INTO [Warehouse].[Selections].[LE029_PreSelection_Bottom]
FROM #FB fb
WHERE EXISTS (	SELECT 1
				FROM Sandbox.bastienc.landsend_Bottom50pct sb
				WHERE fb.CINID = sb.CINID)
