...
EXEC [ExcelQuery].[ROCEFT_LIVE_SpendStretch_Calculate] 
 ...
 @EndDate = NULL --Optional parameter, supply the end of the time period
	...
	DECLARE @AcquireLength INT = 24,
		...
		@Year int = 2020,
		@BrandName varchar(500) = 'Sunday Times Wine Club'
...
GROUP BY C.CustomerSegmentOnTranDate
 ...
 --IF OBJECT_ID('Sandbox.SamW.STWCAcquiredEngaged02072020') IS NOT NULL DROP TABLE Sandbox.SamW.STWCAcquiredEngaged02072020
 --SELECT DISTINCT CINID
 --INTO Sandbox.SamW.STWCAcquiredEngaged02072020
 --FROM	#FB F
 --WHERE	AwarenessLevel = '1 - Gold'
 --AND CINID NOT IN (SELECT CINID FROM #LapsedandShoppers)

 IF OBJECT_ID('Sandbox.SamW.STWCAcquiredNotEngaged02072020') IS NOT NULL DROP TABLE Sandbox.SamW.STWCAcquiredNotEngaged02072020
 SELECT DISTINCT CINID, FanID
 INTO Sandbox.SamW.STWCAcquiredNotEngaged02072020
 FROM	#FB F
 WHERE	AwarenessLevel <> '1 - Gold'
 AND CINID NOT IN (SELECT CINID FROM #LapsedandShoppers)
...
If Object_ID('Warehouse.Selections.STW009_PreSelection') Is Not Null Drop Table Warehouse.Selections.STW009_PreSelectionSelect FanIDInto Warehouse.Selections.STW009_PreSelectionFROM  SANDBOX.SAMW.STWCACQUIREDNOTENGAGED02072020END