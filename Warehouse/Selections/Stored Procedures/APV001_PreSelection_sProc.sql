-- =============================================-- Author:  <Rory Francis>-- Create date: <2021-06-25>-- Description: < sProc to run preselection code per camapign >-- =============================================CREATE PROCEDURE [Selections].[APV001_PreSelection_sProc]ASBEGIN
...
FROM	#NtileEngaged
WHERE	NTILE_5 IN (1)
GROUP BY CINIDIf Object_ID('Warehouse.Selections.APV001_PreSelection') Is Not Null Drop Table Warehouse.Selections.APV001_PreSelectionSelect FanIDInto Warehouse.Selections.APV001_PreSelectionFROM #FB fbWHERE EXISTS (	SELECT 1				FROM Sandbox.RukanK.AmazonPrimeTOP20pct r				WHERE fb.CINID = r.CINID)END