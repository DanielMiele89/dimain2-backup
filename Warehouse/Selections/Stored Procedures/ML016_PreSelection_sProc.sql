-- =============================================-- Author:  <Rory Francis>-- Create date: <2020-11-13>-- Description: < sProc to run preselection code per camapign >-- =============================================CREATE PROCEDURE Selections.ML016_PreSelection_sProcASBEGIN
...
If Object_ID('Warehouse.Selections.ML016_PreSelection') Is Not Null Drop Table Warehouse.Selections.ML016_PreSelectionSelect FanIDINTO Warehouse.Selections.ML016_PreSelectionFROM #FullBase fbWHERE EXISTS (	SELECT 1				FROM Sandbox.SamW.MatalanCompetitorStealScot161020 st				WHERE fb.CINID = st.CINID)
AND EXISTS (	SELECT 1
				FROM #Roc_Shopper_Segment_Members sg
				WHERE sg.ShopperSegmentTypeID = 7
				AND fb.FanID = sg.FanID)				INSERT INTO Warehouse.Selections.ML016_PreSelectionSelect FanIDFROM #FullBase fbWHERE EXISTS (	SELECT 1				FROM Sandbox.SamW.MatalanCompetitorStealWales161020 st				WHERE fb.CINID = st.CINID)
AND EXISTS (	SELECT 1
				FROM #Roc_Shopper_Segment_Members sg
				WHERE sg.ShopperSegmentTypeID = 7
				AND fb.FanID = sg.FanID)				INSERT INTO Warehouse.Selections.ML016_PreSelectionSelect FanIDFROM #FullBase fbWHERE EXISTS (	SELECT 1				FROM Sandbox.SamW.MatalanCompetitorStealNorhternIreland161020 st				WHERE fb.CINID = st.CINID)
AND EXISTS (	SELECT 1
				FROM #Roc_Shopper_Segment_Members sg
				WHERE sg.ShopperSegmentTypeID = 7
				AND fb.FanID = sg.FanID)				INSERT INTO Warehouse.Selections.ML016_PreSelectionSelect FanIDFROM #FullBase fbWHERE EXISTS (	SELECT 1				FROM Sandbox.SamW.MatalanTopSpendersScot161020 st				WHERE fb.CINID = st.CINID)
AND EXISTS (	SELECT 1
				FROM #Roc_Shopper_Segment_Members sg
				WHERE sg.ShopperSegmentTypeID = 9
				AND fb.FanID = sg.FanID)INSERT INTO Warehouse.Selections.ML016_PreSelectionSelect FanIDFROM #FullBase fbWHERE EXISTS (	SELECT 1				FROM Sandbox.SamW.MatalanTopSpendersWales161020 st				WHERE fb.CINID = st.CINID)
AND EXISTS (	SELECT 1
				FROM #Roc_Shopper_Segment_Members sg
				WHERE sg.ShopperSegmentTypeID = 9
				AND fb.FanID = sg.FanID)INSERT INTO Warehouse.Selections.ML016_PreSelectionSelect FanIDFROM #FullBase fbWHERE EXISTS (	SELECT 1				FROM Sandbox.SamW.MatalanTopSpendersNorthernIreland161020 st				WHERE fb.CINID = st.CINID)
AND EXISTS (	SELECT 1
				FROM #Roc_Shopper_Segment_Members sg
				WHERE sg.ShopperSegmentTypeID = 9
				AND fb.FanID = sg.FanID)
