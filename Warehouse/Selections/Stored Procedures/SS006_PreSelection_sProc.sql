...
WHERE cc.BrandID = 2697 --,201 768,2174,2094,2508,2697)
...
IF OBJECT_ID('Sandbox.bastienc.week_spender_STAYBRIDGE') IS NOT NULL DROP TABLE Sandbox.bastienc.week_spender_STAYBRIDGE
SELECT distinct 
		ct.CINID
INTO Sandbox.bastienc.week_spender_STAYBRIDGE
FROM #CCIDs CCs
...
GROUP BY BrandName,ct.CINIDIf Object_ID('Warehouse.Selections.SS006_PreSelection') Is Not Null Drop Table Warehouse.Selections.SS006_PreSelectionSelect FanIDInto Warehouse.Selections.SS006_PreSelectionFROM #FB fbWHERE EXISTS (	SELECT 1				FROM Sandbox.bastienc.week_spender_STAYBRIDGE cl				WHERE fb.CINID = cl.CINID)END