...
CREATE PROCEDURE [Selections].[__CampaignCode_AutoGeneration_ROC_SS_V1_9_ALS_Loop_Archived]
		(	@PartnerID CHAR(4),
...
9.			Rory Francis	09/08/2018		* Where c.FanID NOT IN (SELECT DISTINCT FanID FROM Warehouse.Selections.CampaignCode_Selections_PartnerDedupe) changed to:
											  Where c.CompositeID NOT IN (SELECT DISTINCT CompositeID FROM Warehouse.Selections.CampaignCode_Selections_PartnerDedupe)
...
SET @UserName = (SELECT CASE	WHEN USER_NAME() = 'Suraj' THEN 'Suraj Chahal'
				...
				WHEN USER_NAME() = 'Rory' THEN 'Rory Francis'
				ELSE SYSTEM_USER
			END)
...
INNER JOIN SLC_Report..IronOfferMember ch
...
SELECT	DISTINCT 
			CompositeID
		into #cha
		FROM SLC_Report..IronOfferMember cha (NOLOCK)
...
Into #Members
From SLC_Report..IronOfferMember as ch
...
Inner join #cha cha
	on cb.CompositeID = cha.CompositeID
'
...
Where Not Exists (Select 1
				  From Warehouse.Selections.CampaignCode_Selections_PartnerDedupe pd
				  Where c.CompositeID = pd.CompositeID)'+
	CASE
...
--INSERT INTO Warehouse.Selections.NominatedOfferMember_TableNames ( PartnerID
--																 , EmailDate
--																 , ClientServicesRef
--																 , TableName
--																 , NewCampaign
--																 , InOfferMemberAdditions)
--SELECT '''+ @PartnerID + ''' as PartnerID
--	 , '''+ @StartDate + ''' as EmailDate
--	 , '''+ @ClientServicesRef + ''' as ClientServicesRef
--	 , '''+ @OutputTableName + ''' as TableName
--	 , 0 as InOfferMemberAdditions
