...
CREATE PROCEDURE [Selections].[MOR141_PreSelection_sProc]
AS
BEGIN
	...
	IF OBJECT_ID('[Warehouse].[Selections].[MOR141_PreSelection]') IS NOT NULL DROP TABLE [Warehouse].[Selections].[MOR141_PreSelection]
	SELECT	cu.FanID
	INTO [Warehouse].[Selections].[MOR141_PreSelection]
	FROM [Relational].[Customer] cu
	INNER JOIN [Relational].[CINList] cl
		ON cu.SourceUID = cl.CIN
	WHERE EXISTS (	SELECT 1
					FROM Sandbox.rukank.Morrisons_High_SoW_ATV_0_20_01032022 sb
