...
CREATE PROCEDURE [Selections].[MV014_PreSelection_sProc]
AS
BEGIN
...
IF OBJECT_ID('Sandbox.GunayS.MVEngagementT75160322') IS NOT NULL DROP TABLE Sandbox.GunayS.MVEngagementT75160322
SELECT	CINID
INTO	Sandbox.GunayS.MVEngagementT75160322
FROM	#NtileEngaged
WHERE	NTILE_4 IN (1,2,3)
GROUP BY CINID

	IF OBJECT_ID('[Warehouse].[Selections].[MV014_PreSelection]') IS NOT NULL DROP TABLE [Warehouse].[Selections].[MV014_PreSelection]
	SELECT FanID
	INTO [Warehouse].[Selections].[MV014_PreSelection]
	FROM #FB fb
	WHERE EXISTS (SELECT 1 FROM Sandbox.GunayS.MVEngagementT75160322 s WHERE fb.CINID = s.CINID)
