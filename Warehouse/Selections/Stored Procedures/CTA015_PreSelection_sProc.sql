-- =============================================-- Author:  <Rory Francis>-- Create date: <2021-05-30>-- Description: < sProc to run preselection code per camapign >-- =============================================CREATE PROCEDURE [Selections].[CTA015_PreSelection_sProc]ASBEGINIF OBJECT_ID('tempdb..#FB') IS NOT NULL DROP TABLE #FB
SELECT	CINID
		,FanID
		,AgeCurrent
INTO #FB
...
GROUP BY F.CINIDIf Object_ID('Warehouse.Selections.CTA015_PreSelection') Is Not Null Drop Table Warehouse.Selections.CTA015_PreSelectionSelect FanIDInto Warehouse.Selections.CTA015_PreSelectionFROM Relational.Customer cuWHERE EXISTS (	SELECT 1				FROM Sandbox.vernon.costa_sow25 st				INNER JOIN Relational.CINList cl					ON st.CINID = cl.CINID				WHERE cu.SourceUID = cl.CIN)UNION ALLSELECT FanIDFROM Relational.Customer cuWHERE EXISTS (	SELECT 1				FROM Sandbox.vernon.costa_AL st				INNER JOIN Relational.CINList cl					ON st.CINID = cl.CINID				WHERE cu.SourceUID = cl.CIN)END