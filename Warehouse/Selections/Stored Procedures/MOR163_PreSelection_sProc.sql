...
CREATE PROCEDURE [Selections].[MOR163_PreSelection_sProc]
AS
BEGIN
...
-- ATV btw 20 - 50	-- SS £50
IF OBJECT_ID('Sandbox.rukank.Morrisons_High_SoW_ATV_20_50_31032022') IS NOT NULL DROP TABLE Sandbox.rukank.Morrisons_High_SoW_ATV_20_50_31032022		-- 41,335	(Feb run 42,093)
SELECT	F.CINID
INTO	Sandbox.rukank.Morrisons_High_SoW_ATV_20_50_31032022
FROM	#Txn F
...
GROUP BY F.CINID
	...
	IF OBJECT_ID('[Warehouse].[Selections].[MOR163_PreSelection]') IS NOT NULL DROP TABLE [Warehouse].[Selections].[MOR163_PreSelection]
	SELECT FanID
	INTO [Warehouse].[Selections].[MOR163_PreSelection]
	FROM #FB fb
	WHERE EXISTS (	SELECT 1
					FROM Sandbox.rukank.Morrisons_High_SoW_ATV_20_50_31032022  st
