...
CREATE PROCEDURE [Selections].[__CampaignSetup_Selection_IndividualCampaign_POS_20190909_Archived] (@PartnerID CHAR(4)
...
*/
	...
	*******************************************************************************************************************************************/
		...
		***********************************************************************************************************************/
			...
			IF OBJECT_ID ('tempdb..#CustomerBase_temp') IS NOT NULL DROP TABLE #CustomerBase_temp
			SELECT Distinct 
				   cb.FanID
				 , cb.CompositeID
				 , cb.Postcode
				 , cb.ShopperSegmentTypeID
			INTO #CustomerBase_temp
			...
			CREATE CLUSTERED INDEX CIX_Comp ON #CustomerBase_temp (CompositeID)
	...
	*******************************************************************************************************************************************/
		...
		IF @ControlGroupPercentage > 0
			BEGIN
				...
				SET @SQLCode = '
				IF OBJECT_ID(''Warehouse.InsightArchive.' + @ClientServicesRef + '_ControlGroup_InProgram'') IS NULL
				CREATE TABLE Warehouse.InsightArchive.' + @ClientServicesRef + '_ControlGroup_InProgram ([PartnerID] [INT] NOT NULL
				...
				INSERT INTO Warehouse.InsightArchive.' + @ClientServicesRef + '_ControlGroup_InProgram
				SELECT DISTINCT
					   PartnerID
					 , ClientServicesRef
					 , IronOfferID
