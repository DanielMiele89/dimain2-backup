...
CREATE PROCEDURE [Selections].[MOR127_PreSelection_sProc]
AS
BEGIN
	...
	-- ATV btw 15 - 35
	IF OBJECT_ID('Sandbox.rukank.Morrisons_Medium_SoW_ATV_15_35_10012022') IS NOT NULL DROP TABLE Sandbox.rukank.Morrisons_Medium_SoW_ATV_15_35_10012022
	SELECT	F.CINID
	INTO	Sandbox.rukank.Morrisons_Medium_SoW_ATV_15_35_10012022
	FROM	#Txn F
	WHERE	ATV BETWEEN 15 AND 35
	GROUP BY F.CINID

	IF OBJECT_ID('[Warehouse].[Selections].[MOR127_PreSelection]') IS NOT NULL DROP TABLE [Warehouse].[Selections].[MOR127_PreSelection]
	SELECT FanID
	INTO [Warehouse].[Selections].[MOR127_PreSelection]
	FROM #FB fb
	WHERE EXISTS (	SELECT 1
					FROM Sandbox.RukanK.Morrisons_Medium_SoW_ATV_15_35_10012022 st
