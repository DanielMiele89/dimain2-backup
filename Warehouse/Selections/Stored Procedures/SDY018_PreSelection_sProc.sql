...
BEGIN
	...
	-- shoppers - SOW - < 33%
	IF OBJECT_ID('Sandbox.samh.superdry_sow33_25012021') IS NOT NULL DROP TABLE Sandbox.samh.superdry_sow33_25012021
	SELECT	F.CINID, FanID
	INTO	Sandbox.samh.superdry_sow33_25012021
	FROM	#shoppper_sow F
	WHERE	SuperdryShopper = 1
			AND SuperdrySoW < 0.3333
	GROUP BY F.CINID, FanID

	IF OBJECT_ID('[Warehouse].[Selections].[SDY018_PreSelection]') IS NOT NULL DROP TABLE [Warehouse].[Selections].[SDY018_PreSelection]
	SELECT FanID
	INTO [Warehouse].[Selections].[SDY018_PreSelection]
	FROM Sandbox.samh.superdry_sow33_25012021
