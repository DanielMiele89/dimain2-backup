...
CREATE PROCEDURE [Selections].[MOR156_PreSelection_sProc]
AS
BEGIN
...
-- ATV > 50	and <= 75	-- SS £75
IF OBJECT_ID('Sandbox.rukank.Morrisons_High_SoW_ATV_50_75_31032022') IS NOT NULL DROP TABLE Sandbox.rukank.Morrisons_High_SoW_ATV_50_75_31032022		-- 15,916	(Feb run 16,266)
SELECT	F.CINID
INTO	Sandbox.rukank.Morrisons_High_SoW_ATV_50_75_31032022
FROM	#Txn F
WHERE	ATV > 50 
		AND ATV <= 75
GROUP BY F.CINID
	...
	IF OBJECT_ID('[Warehouse].[Selections].[MOR156_PreSelection]') IS NOT NULL DROP TABLE [Warehouse].[Selections].[MOR156_PreSelection]
	SELECT FanID
	INTO [Warehouse].[Selections].[MOR156_PreSelection]
	FROM #FB fb
	WHERE EXISTS (	SELECT 1
					FROM Sandbox.rukank.Morrisons_High_SoW_ATV_50_75_31032022  st
