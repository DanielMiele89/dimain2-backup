-- =============================================-- Author:  <Rory Francis>-- Create date: <2021-08-20>-- Description: < sProc to run preselection code per camapign >-- =============================================CREATE PROCEDURE [Selections].[UE007_PreSelection_sProc]ASBEGINIF OBJECT_ID('tempdb..#CC') IS NOT NULL DROP TABLE #CC
SELECT	BrandName 
		,CC.BrandID
...
WHERE transactions >= 3 
	 AND ATV >= 20
GROUP BY F.CINIDIf Object_ID('Warehouse.Selections.UE007_PreSelection') Is Not Null Drop Table Warehouse.Selections.UE007_PreSelectionSelect FanIDInto Warehouse.Selections.UE007_PreSelectionFROM Relational.Customer cuWHERE EXISTS (	SELECT 1				FROM SANDBOX.BASTIENC.UBEREATSCOMP s				INNER JOIN Relational.CINList cl					ON s.CINID = cl.CINID				WHERE cu.SourceUID = cl.CIN)END