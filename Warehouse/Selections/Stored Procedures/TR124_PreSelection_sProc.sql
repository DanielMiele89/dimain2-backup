...
Create Procedure Selections.TR124_PreSelection_sProc
AS
BEGIN
...
,1327,2220,2222,2061,2219,1930) -- extra bus companies added
...
, case 
  ...
  when count(1) >= 2 and count(1) <= 3 then 'Low'
  else 'Other_Freq' end as RailSectorSpend,
 case
...
delete t
from #Selection3 t
inner join #Spenders12to24 a
	on a.CINID = t.CINID
where t.TrainlineSoW = 'Pure Acqusition'

update s
set Segment =	case 
					when s.RailSectorSpend = 'High'  and s.TrainlineSoW = 'Pure Acqusition' and s.GeoLoc = 'Midlands' then '124.H-M'
					when s.RailSectorSpend = 'High'  and s.TrainlineSoW = 'Pure Acqusition' and s.GeoLoc = 'Other'  then '125.H-O'
					when s.RailSectorSpend = 'High'  and s.TrainlineSoW = 'Pure Acqusition' and s.GeoLoc = 'South East' then '126.H-SE'
					when s.RailSectorSpend = 'Medium' and s.TrainlineSoW = 'Pure Acqusition' and s.GeoLoc = 'Midlands' then '127.M-M'
					when s.RailSectorSpend = 'Medium' and s.TrainlineSoW = 'Pure Acqusition' and s.GeoLoc = 'Other'  then '128.M-O'
					when s.RailSectorSpend = 'Medium' and s.TrainlineSoW = 'Pure Acqusition' and s.GeoLoc = 'South East' then '129.M-SE'
					else 'NA' 
				end
from #Selection3 s
...
Create Clustered Index CIX_TRPreSelection_SegmentFan On Warehouse.Selections.TR_PreSelection (Segment, FanID)

If Object_ID('Warehouse.Selections.TR124_PreSelection') Is Not Null Drop Table Warehouse.Selections.TR124_PreSelection
Select FanID
Into Warehouse.Selections.TR124_PreSelection
From Warehouse.Selections.TR_PreSelection s
where s.Segment = '124.H-M'
