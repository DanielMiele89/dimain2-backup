...
IF OBJECT_ID('Sandbox.RukanK.SweatyBetty_CompSteal_20pct_20072021') IS NOT NULL DROP TABLE Sandbox.RukanK.SweatyBetty_CompSteal_20pct_20072021
SELECT	CINID
INTO	Sandbox.RukanK.SweatyBetty_CompSteal_20pct_20072021
FROM	#Trans
WHERE	CINID IN (SELECT CINID FROM #NtileEngaged WHERE NTILE_2 IN (1))If Object_ID('Warehouse.Selections.SWB011_PreSelection') Is Not Null Drop Table Warehouse.Selections.SWB011_PreSelectionSelect FanIDInto Warehouse.Selections.SWB011_PreSelectionFROM Relational.Customer cuWHERE EXISTS (	SELECT 1				FROM [Relational].[CINList] cl				INNER JOIN Sandbox.RukanK.SweatyBetty_CompSteal_20pct_20072021 fo					ON cl.CINID = fo.CINID				WHERE cu.SourceUID = cl.CIN)END