...
EXEC [ExcelQuery].[ROCEFT_LIVE_SpendStretch_Calculate] 
 ...
 @EndDate = NULL --Optional parameter, supply the end of the time period
	...
	DECLARE @AcquireLength INT = 12,
		@LapsedLength INT = 6,
		@PASTMONTHS INT = 24, -- HOW FAR TO LOOK BACK AT TRANSACTIONS
		@BrandID INT = 247,
		@OriginCycleStartDate DATE = '2019-07-18', -- Cycle
		@Year int = 2020,
		@BrandName varchar(500) = 'Lakeland'
...
INNER JOIN #FB F
	ON F.CINID = CT.CINID
LEFT JOIN Sandbox.SamW.LakelandRetention220620 S -- SELECT SANDBOX 
	ON F.CINID = S.CINID
INNER JOIN #Cycles C
...
LEFT JOIN #SegmentonTranDate S ON C.CINID = S.CINID
...
WHERE BrandID = 247
AND TranDate BETWEEN '2020-03-23' AND '2020-06-14'
GROUP BY F.CINID, f.FanID

IF OBJECT_ID('tempdb..#Last12months') IS NOT NULL DROP TABLE #Last12months
...
JOIN Relational.ConsumerCombination CC 
	ON CC.ConsumerCombinationID = CT.ConsumerCombinationID
WHERE BrandID = 247
AND TranDate BETWEEN DATEADD(MONTH,-12,GETDATE()) AND '2020-03-23'
GROUP BY F.CINID, f.FanID

IF OBJECT_ID('Sandbox.SamW.LakelandRetention220620') IS NOT NULL DROP TABLE Sandbox.SamW.LakelandRetention220620
SELECT DISTINCT CINID, FanID
INTO Sandbox.SamW.LakelandRetention220620
FROM #Shoppers
WHERE CINID NOT IN (SELECT CINID FROM #Last12months)
AND Transactions = 1
If Object_ID('Warehouse.Selections.LL007_PreSelection') Is Not Null Drop Table Warehouse.Selections.LL007_PreSelectionSelect FanIDInto Warehouse.Selections.LL007_PreSelectionFROM  SANDBOX.SAMW.LAKELANDRETENTION220620END