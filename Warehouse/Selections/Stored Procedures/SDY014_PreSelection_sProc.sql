...
CREATE PROCEDURE [Selections].[SDY014_PreSelection_sProc]
AS
BEGIN
...
--JOIN	#CC CC ON CT.ConsumerCombinationID = CC.ConsumerCombinationID
--JOIN	#FB FB ON CT.CINID = FB.CINID
--WHERE	TranDate >= DATEADD(MONTH,-12,GETDATE()) AND TranDate <= DATEADD(MONTH,-24,GETDATE())
--		AND Amount > 0
--GROUP BY CT.CINID
...
--IF OBJECT_ID('Sandbox.samh.superdryAcq_GROUP1_04022022') IS NOT NULL DROP TABLE Sandbox.samh.superdryAcq_GROUP1_04022022
--SELECT	CINID
--INTO Sandbox.samh.superdryAcq_GROUP1_04022022
--FROM	#custs
...
--IF OBJECT_ID('Sandbox.samh.superdryAcq_GROUP3_04022022') IS NOT NULL DROP TABLE Sandbox.samh.superdryAcq_GROUP3_04022022
--SELECT	CINID
--INTO Sandbox.samh.superdryAcq_GROUP3_04022022
--FROM	#custs
--WHERE	NTILE_3 IN (3)  -- GROUP 3
--GROUP BY CINID

	IF OBJECT_ID('[Warehouse].[Selections].[SDY014_PreSelection]') IS NOT NULL DROP TABLE [Warehouse].[Selections].[SDY014_PreSelection]
	SELECT FanID
	INTO [Warehouse].[Selections].[SDY014_PreSelection]
	FROM #FB fb
	WHERE EXISTS (	SELECT 1
					FROM Sandbox.samh.superdryAcq_GROUP3_04022022 st
