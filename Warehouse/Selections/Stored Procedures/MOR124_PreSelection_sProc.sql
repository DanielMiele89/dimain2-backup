...
BEGIN
	...
	FROM	#FB1
	WHERE	CINID NOT IN (SELECT CINID FROM Sandbox.RukanK.Morrisons_TOP25pct_Spenders_24022022_CH)
	GROUP BY CINID
	...
	-- shoppers - SOW - < 30%
	IF OBJECT_ID('Sandbox.rukank.Morrisons_LoW_SoW_01032022') IS NOT NULL DROP TABLE Sandbox.rukank.Morrisons_LoW_SoW_01032022		-- 460,109
	SELECT	F.CINID
	INTO	Sandbox.rukank.Morrisons_LoW_SoW_01032022
	FROM	#shoppper_sow F
	WHERE	BrandShopper = 1
			AND SoW < 0.30
			AND Transactions >= 15
			AND CINID NOT IN (SELECT CINID FROM 
												(SELECT CINID  FROM	Sandbox.rukank.Morrisons_Aldi_Lidl_01032022
												UNION
												SELECT CINID  FROM	Sandbox.rukank.Morrisons_Nursery_Acquire_01032022
												) A
							)
	GROUP BY F.CINID
	...
	FROM #FB fb
	WHERE EXISTS (	SELECT 1
					FROM Sandbox.RukanK.Morrisons_LoW_SoW_01032022 st
					WHERE fb.CINID = st.CINID)
	AND EXISTS (	SELECT 1
					FROM [Segmentation].[Roc_Shopper_Segment_Members] sg
					WHERE fb.FanID = sg.FanID
