...
and CONSUMERCOMBINATIONiD IN (SELECT * FROM #CC)
AND CURRENTLYACTIVE = 1	
AND TranDate  >= DATEADD(month,-1,getdate())

IF OBJECT_ID('tempdb..#final_output') IS NOT NULL DROP TABLE #final_output;
select distinct cinid
into #final_output
from #CT_elegible
WHERE CINID NOT IN (SELECT * FROM #CT_PN)
AND CINID NOT IN (SELECT * FROM #CT_GT1)If Object_ID('Warehouse.Selections.AZF007_PreSelection') Is Not Null Drop Table Warehouse.Selections.AZF007_PreSelectionSelect FanIDInto Warehouse.Selections.AZF007_PreSelectionFROM Relational.Customer cuWHERE EXISTS (	SELECT 1				FROM [Relational].[CINList] cl				INNER JOIN #final_output fo					ON cl.CINID = fo.CINID				WHERE cu.SourceUID = cl.CIN)END