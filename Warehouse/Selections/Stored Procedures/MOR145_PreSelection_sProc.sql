...
CREATE PROCEDURE [Selections].[MOR145_PreSelection_sProc]
AS
BEGIN
...
IF OBJECT_ID('Sandbox.RukanK.Morrisons_BAU_AL_EXCL_01032022_CH') IS NOT NULL DROP TABLE Sandbox.RukanK.Morrisons_BAU_AL_EXCL_01032022_CH		-- 3,366,225
SELECT	CINID
INTO	Sandbox.RukanK.Morrisons_BAU_AL_EXCL_01032022_CH
FROM	#FB2
WHERE	CINID NOT IN (SELECT CINID FROM Sandbox.RukanK.Morrisons_TOP25pct_Spenders_31032022_CH)
GROUP BY CINID
...
-- shoppers - SOW - < 30%
IF OBJECT_ID('Sandbox.rukank.Morrisons_LoW_SoW_05042022') IS NOT NULL DROP TABLE Sandbox.rukank.Morrisons_LoW_SoW_05042022		-- 412,308 (05.04.2022 run), 460,109 (01.03.2022 run)
SELECT	F.CINID
INTO	Sandbox.rukank.Morrisons_LoW_SoW_05042022
FROM	#shoppper_sow F
WHERE	BrandShopper = 1
		AND SoW < 0.30
		AND Transactions >= 15
		AND CINID NOT IN (SELECT CINID FROM 
											(SELECT CINID  FROM	Sandbox.rukank.Morrisons_Aldi_Lidl_05042022
											UNION
											SELECT CINID  FROM	Sandbox.rukank.Morrisons_Nursery_Acquire_05042022
											) A
						)
GROUP BY F.CINID


	IF OBJECT_ID('[Warehouse].[Selections].[MOR145_PreSelection]') IS NOT NULL DROP TABLE [Warehouse].[Selections].[MOR145_PreSelection]
	SELECT FanID
	INTO [Warehouse].[Selections].[MOR145_PreSelection]
	FROM #FB2 fb
	WHERE EXISTS (	SELECT 1
					FROM Sandbox.RukanK.Morrisons_BAU_AL_EXCL_01032022_CH  st
	...
	INSERT INTO [Warehouse].[Selections].[MOR145_PreSelection]
	SELECT FanID
	FROM #FB2 fb
	WHERE EXISTS (	SELECT 1
					FROM Sandbox.rukank.Morrisons_LoW_SoW_05042022  st
					WHERE fb.CINID = st.CINID)
	AND EXISTS (	SELECT 1
					FROM Segmentation.Roc_Shopper_Segment_Members sg
					WHERE fb.FanID = sg.FanID
