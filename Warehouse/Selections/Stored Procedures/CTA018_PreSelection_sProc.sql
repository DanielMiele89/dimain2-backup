-- =============================================-- Author:  <Rory Francis>-- Create date: <2021-06-25>-- Description: < sProc to run preselection code per camapign >-- =============================================CREATE PROCEDURE [Selections].[CTA018_PreSelection_sProc]ASBEGIN
...
-- shoppers - SOW - < 30%
IF OBJECT_ID('Sandbox.rukank.costa_sow30') IS NOT NULL DROP TABLE Sandbox.rukank.costa_sow30
SELECT	F.CINID
INTO Sandbox.rukank.costa_sow30
FROM #shoppper_sow F
...
IF OBJECT_ID('tempdb..#SegmentAssignment') IS NOT NULL DROP TABLE #SegmentAssignmentSELECT FanIDINTO #SegmentAssignmentFROM #FB fbWHERE EXISTS (	SELECT 1				FROM Sandbox.rukank.costa_sow30 cs				WHERE fb.CINID = cs.CINID)UNION ALLSELECT	FanIDFROM #AcquireLapsedIf Object_ID('Warehouse.Selections.CTA018_PreSelection') Is Not Null Drop Table Warehouse.Selections.CTA018_PreSelectionSelect FanIDInto Warehouse.Selections.CTA018_PreSelectionFROM #FB fbWHERE EXISTS (	SELECT 1				FROM Sandbox.rukank.costa_sow30 cs				WHERE fb.CINID = cs.CINID)UNION ALLSELECT	FanIDFROM #AcquireLapsedEND