...
WHERE cc.BrandID = 2094 --,201 768,2174,2094,2508,2697)
...
IF OBJECT_ID('Sandbox.bastienc.week_spender_INTERCONTINENTAL') IS NOT NULL DROP TABLE Sandbox.bastienc.week_spender_INTERCONTINENTAL
SELECT distinct 
		ct.CINID
INTO Sandbox.bastienc.week_spender_INTERCONTINENTAL
FROM #CCIDs CCs
...
GROUP BY BrandName,ct.CINIDIf Object_ID('Warehouse.Selections.ICH006_PreSelection') Is Not Null Drop Table Warehouse.Selections.ICH006_PreSelectionSelect FanIDInto Warehouse.Selections.ICH006_PreSelectionFROM #FB fbWHERE EXISTS (	SELECT 1				FROM Sandbox.bastienc.week_spender_INTERCONTINENTAL cl				WHERE fb.CINID = cl.CINID)END