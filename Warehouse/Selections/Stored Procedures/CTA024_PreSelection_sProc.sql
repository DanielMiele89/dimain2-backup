...
IF OBJECT_ID('tempdb..#SegmentAssignment') IS NOT NULL DROP TABLE #SegmentAssignmentSELECT FanIDINTO #SegmentAssignmentFROM #FB fbWHERE EXISTS (	SELECT 1				FROM Sandbox.rukank.costa_sow30 cs				WHERE fb.CINID = cs.CINID)UNION ALLSELECT	FanIDFROM #AcquireLapsedIf Object_ID('Warehouse.Selections.CTA024_PreSelection') Is Not Null Drop Table Warehouse.Selections.CTA024_PreSelectionSelect FanIDInto Warehouse.Selections.CTA024_PreSelectionFROM #FB fbWHERE EXISTS (	SELECT 1				FROM Sandbox.rukank.costa_sow30 cs				WHERE fb.CINID = cs.CINID)UNION ALLSELECT	FanIDFROM #AcquireLapsedEND