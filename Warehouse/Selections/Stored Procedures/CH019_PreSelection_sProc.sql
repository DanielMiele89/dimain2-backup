-- =============================================-- Author:  <Rory Francis>-- Create date: <2019-06-14>-- Description: < sProc to run preselection code per camapign >-- =============================================Create Procedure Selections.CH019_PreSelection_sProcASBEGIN	declare @CB int = 1950
	...
	on	cl.CINID = b.CINID
			...
			-- General ALS
			if OBJECT_ID('Sandbox.Conal.ChefAndBrewer_General') is not null drop table Sandbox.Conal.ChefAndBrewer_General
			select	 CINID
					, FanID
			into Sandbox.Conal.ChefAndBrewer_General
			from #PubRestaurant_Spend
			where (CB_Comp_List = 0 and Chef_Brewer_shopper = 0 and Tesco_spender_6m = 0)
				or (CB_Comp_List = 1 and Chef_Brewer_shopper = 1 and Tesco_spender_6m = 0)
				or (CB_Comp_List = 0 and Chef_Brewer_shopper = 0 and Tesco_spender_6m = 1)
				or (CB_Comp_List = 0 and Chef_Brewer_shopper = 1 and Tesco_spender_6m = 0)
				or CB_Comp_List is nullIf Object_ID('Warehouse.Selections.CH019_PreSelection') Is Not Null Drop Table Warehouse.Selections.CH019_PreSelectionSelect FanIDInto Warehouse.Selections.CH019_PreSelectionFrom Sandbox.Conal.ChefAndBrewer_GeneralEND