...
CREATE PROCEDURE [Selections].[MOR150_PreSelection_sProc]
AS
BEGIN


--SELECT * FROM Relational.Brand WHERE BrandName LIKE '%waitrose%'
...
IF OBJECT_ID('tempdb..#Aldi_Lidl') IS NOT NULL DROP TABLE #Aldi_Lidl																-- 47,336 (05.04.2022 run), 47,068 (01.03.2022 run)
...
IF OBJECT_ID('Sandbox.rukank.Morrisons_Aldi_Lidl_03052022') IS NOT NULL DROP TABLE Sandbox.rukank.Morrisons_Aldi_Lidl_03052022		-- 16,381 (03.05.2022 run), 16,374 (05.04.2022 run), 15,824 (01.03.2022 run)
SELECT	F.CINID
...
IF OBJECT_ID('[Warehouse].[Selections].[MOR150_PreSelection]') IS NOT NULL DROP TABLE [Warehouse].[Selections].[MOR150_PreSelection]
	SELECT FanID
	INTO [Warehouse].[Selections].[MOR150_PreSelection]
	FROM #FB fb
	WHERE EXISTS (	SELECT 1
					FROM Sandbox.rukank.Morrisons_Aldi_Lidl_05042022  st
