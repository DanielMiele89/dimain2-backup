...
DECLARE @MainBrand smallint = 485	 -- Main Brand	
	...
	FROM 
		...
		SELECT	 CINID
				, FanID
				, CASE	WHEN	Prem_flag = 'Optimised' and (flag = '06.30-40%' OR flag = '07.40-50%')
						THEN '01.Ocado 6+'
						WHEN	Prem_flag = 'Everyone Else' and (flag = '06.30-40%' OR flag = '07.40-50%')
						THEN '02.Optimised 5+'
						WHEN	Prem_flag = 'Optimised' and (flag = '08.50-60%')
						THEN '03.Optimised 4+'
		...
		FROM	#segmentAssignment
		WHERE	Prem_flag = 'Optimised' AND flag = '08.50-60%'		If Object_ID('Warehouse.Selections.WA205_PreSelection') Is Not Null Drop Table Warehouse.Selections.WA205_PreSelection		Select FanID		Into Warehouse.Selections.WA205_PreSelection
		FROM Sandbox.Conal.Waitrose_Acquire_Lapsed_0_10_ShopperEND