...
CREATE PROCEDURE [Selections].[PEX001_PreSelection_sProc]
AS
BEGIN
...
DECLARE @DATE DATE = DATEADD(MONTH,-18,'2019-07-01')
...
JOIN	#CC C 	ON C.ConsumerCombinationID = CT.ConsumerCombinationID
WHERE	Amount > 0
AND		TranDate BETWEEN @DATE AND '2019-06-30'
GROUP BY F.CINID
...
IF OBJECT_ID('[Warehouse].[Selections].[PEX001_PreSelection]') IS NOT NULL DROP TABLE [Warehouse].[Selections].[PEX001_PreSelection]
SELECT	FanID
INTO [Warehouse].[Selections].[PEX001_PreSelection]
FROM #FB fb
