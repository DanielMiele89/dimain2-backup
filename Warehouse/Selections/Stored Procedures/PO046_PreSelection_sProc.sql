...
IF OBJECT_ID('tempdb..#FB_rbs_age') IS NOT NULL DROP TABLE #FB_rbs_age
SELECT	CINID, FanID
INTO	#FB_rbs_age
FROM	Relational.Customer C
JOIN	Relational.CINList CL ON CL.CIN = C.SourceUID
...
IF OBJECT_ID('Sandbox.RukanK.PO_Ferries_AgeTarget_17122021') IS NOT NULL DROP TABLE Sandbox.RukanK.PO_Ferries_AgeTarget_17122021
SELECT	CINID
INTO	Sandbox.RukanK.PO_Ferries_AgeTarget_17122021
FROM	#FB_rbs_age
WHERE	CINID NOT IN (SELECT CINID FROM #Trans_rbs)

	IF OBJECT_ID('[Warehouse].[Selections].[PO046_PreSelection]') IS NOT NULL DROP TABLE [Warehouse].[Selections].[PO046_PreSelection]
	SELECT	fb.FanID
	INTO [Warehouse].[Selections].[PO046_PreSelection]
	FROM #FB_rbs_age fb
	WHERE EXISTS (	SELECT 1
					FROM Sandbox.RukanK.PO_Ferries_AgeTarget_17122021 st
