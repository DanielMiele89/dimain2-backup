...
IF OBJECT_ID('sandbox.bastienc.amz_fresh_eligible') IS NOT NULL DROP TABLE sandbox.bastienc.amz_fresh_eligible;
select distinct cinid
into sandbox.bastienc.amz_fresh_eligible
from #CT
...
and A.CINID in (select * from sandbox.bastienc.amz_fresh_eligible)
--and a.CINID not in (select * from Sandbox.BastienC.AMAZON_FRESH_1)
--group by A.CINID,month(trandate),year(trandate);

IF OBJECT_ID('Sandbox.BastienC.AMAZON_FRESH_2') IS NOT NULL DROP TABLE Sandbox.BastienC.AMAZON_FRESH_2;
select distinct cinid
into Sandbox.BastienC.AMAZON_FRESH_2
from #CT2
...
IF OBJECT_ID('tempdb..#CT3') IS NOT NULL DROP TABLE #CT3;
SELECT A.CINID --,month(trandate) as tran_month,year(trandate) as tran_year,sum(amount) as spend,count(*) as transactions
INTO #CT3
FROM warehouse.Relational.ConsumerTransaction_MyRewards AS A
join Warehouse.Relational.CINList cin on cin.CINID = a.CINID
...
IF OBJECT_ID('Sandbox.BastienC.AMAZON_FRESH_3') IS NOT NULL DROP TABLE Sandbox.BastienC.AMAZON_FRESH_3;
select distinct cinid
into Sandbox.BastienC.AMAZON_FRESH_3
from #CT3

If Object_ID('Warehouse.Selections.AZF004_PreSelection') Is Not Null Drop Table Warehouse.Selections.AZF004_PreSelectionSelect FanIDInto Warehouse.Selections.AZF004_PreSelectionFROM Relational.Customer cuWHERE EXISTS (	SELECT 1				FROM Relational.CINList cl				INNER JOIN SANDBOX.BASTIENC.AMAZON_FRESH_2 s					ON cl.CINID = s.CINID				WHERE cu.SourceUID = cl.CIN)OR EXISTS (	SELECT 1				FROM Relational.CINList cl				INNER JOIN SANDBOX.BASTIENC.AMAZON_FRESH_3 s					ON cl.CINID = s.CINID				WHERE cu.SourceUID = cl.CIN)END