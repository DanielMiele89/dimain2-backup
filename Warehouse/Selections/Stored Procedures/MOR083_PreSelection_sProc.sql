...
IF OBJECT_ID('Sandbox.SamW.NonTeachers170121') IS NOT NULL DROP TABLE Sandbox.SamW.NonTeachers170121
SELECT F.FanID
INTO Sandbox.SamW.NonTeachers170121
FROM #FB F
JOIN Relational.ConsumerTransaction_DD_MyRewards CT ON F.FanID = CT.FanID
JOIN #CC C ON C.ConsumerCombinationID_DD = CT.ConsumerCombinationID_DD
----LEFT JOIN Sandbox.SamW.MorrisonsHealthWorksers080720 S ON S.FanID = f.FanID
WHERE Narrative_RBS NOT LIKE '%Teacher%'
AND		F.FanID NOT IN (SELECT FANID FROM Sandbox.SamW.MorrisonsHealthWorksers080720)
GROUP BY F.FanID

--IF OBJECT_ID('Sandbox.SamW.Teachers170121') IS NOT NULL DROP TABLE Sandbox.SamW.Teachers170121
--SELECT F.FanID
--INTO Sandbox.SamW.Teachers170121
--FROM #FB F
--JOIN Relational.ConsumerTransaction_DD_MyRewards CT ON F.FanID = CT.FanID
--JOIN #CC C ON C.ConsumerCombinationID_DD = CT.ConsumerCombinationID_DD
------LEFT JOIN Sandbox.SamW.MorrisonsHealthWorksers080720 S ON S.FanID = f.FanID
--WHERE Narrative_RBS LIKE '%Teacher%'
--AND		F.FanID NOT IN (SELECT FANID FROM Sandbox.SamW.MorrisonsHealthWorksers080720)
--GROUP BY F.FanID

If Object_ID('Warehouse.Selections.MOR083_PreSelection') Is Not Null Drop Table Warehouse.Selections.MOR083_PreSelectionSelect FanIDInto Warehouse.Selections.MOR083_PreSelectionFROM  Sandbox.SamW.NonTeachers170121END