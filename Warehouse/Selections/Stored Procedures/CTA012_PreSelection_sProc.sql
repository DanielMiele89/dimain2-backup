-- =============================================-- Author:  <Rory Francis>-- Create date: <2021-03-06>-- Description: < sProc to run preselection code per camapign >-- =============================================CREATE PROCEDURE [Selections].[CTA012_PreSelection_sProc]ASBEGINIF OBJECT_ID('tempdb..#FB') IS NOT NULL DROP TABLE #FB
SELECT	CINID
		,FanID
		,AgeCurrent
INTO #FB
...
GROUP BY F.CINIDIf Object_ID('Warehouse.Selections.CTA012_PreSelection') Is Not Null Drop Table Warehouse.Selections.CTA012_PreSelectionSelect FanIDInto Warehouse.Selections.CTA012_PreSelectionFROM  #FB fbWHERE EXISTS (	SELECT 1				FROM Sandbox.vernon.costa55_sow25 sb				WHERE fb.CINID = sb.CINID)OR EXISTS (	SELECT 1				FROM Sandbox.vernon.costa55_AL sb				WHERE fb.CINID = sb.CINID)END