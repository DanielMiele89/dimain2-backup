-- =============================================-- Author:  <Rory Francis>-- Create date: <2020-11-13>-- Description: < sProc to run preselection code per camapign >-- =============================================CREATE PROCEDURE Selections.ML017_PreSelection_sProcASBEGIN
...
If Object_ID('Warehouse.Selections.ML017_PreSelection') Is Not Null Drop Table Warehouse.Selections.ML017_PreSelectionSelect FanIDINTO Warehouse.Selections.ML017_PreSelectionFROM #FullBase fbWHERE EXISTS (	SELECT 1				FROM Sandbox.SamW.MatalanCompetitorStealEng161020 st				WHERE fb.CINID = st.CINID)
AND EXISTS (	SELECT 1
				FROM #Roc_Shopper_Segment_Members sg
				WHERE sg.ShopperSegmentTypeID = 7
				AND fb.FanID = sg.FanID)INSERT INTO Warehouse.Selections.ML017_PreSelectionSelect FanIDFROM #FullBase fbWHERE EXISTS (	SELECT 1				FROM Sandbox.SamW.MatalanTopSpendersEng161020 st				WHERE fb.CINID = st.CINID)
AND EXISTS (	SELECT 1
				FROM #Roc_Shopper_Segment_Members sg
