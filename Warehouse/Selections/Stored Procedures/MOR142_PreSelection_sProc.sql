...
CREATE PROCEDURE [Selections].[MOR142_PreSelection_sProc]
AS
BEGIN
	...
	IF OBJECT_ID('[Warehouse].[Selections].[MOR142_PreSelection]') IS NOT NULL DROP TABLE [Warehouse].[Selections].[MOR142_PreSelection]
	SELECT	cu.FanID
	INTO [Warehouse].[Selections].[MOR142_PreSelection]
	FROM [Relational].[Customer] cu
	INNER JOIN [Relational].[CINList] cl
		ON cu.SourceUID = cl.CIN
	WHERE EXISTS (	SELECT 1
					FROM Sandbox.rukank.Morrisons_High_SoW_ATV_20_50_01032022 sb
