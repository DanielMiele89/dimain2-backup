...
CREATE PROCEDURE [Selections].[__CampaignSetup_Selection_IndividualCampaign_POS_20200312_Archived] (@PartnerID CHAR(4)
...
*/
	...
	*******************************************************************************************************************************************/
		...
		***********************************************************************************************************************/
			...
			FROM InsightArchive.MyRewards_ControlGroup_InProgram cg
		...
		***********************************************************************************************************************/
			...
			SELECT DISTINCT 
				 ...
				 , cb.DOB
				 , CASE
						WHEN cb.ActivatedDate > @ActivatedDate AND EXISTS (SELECT 1 FROM #OfferIDs WHERE ShopperSegmentTypeID = 10) THEN 10
			...
			WHERE cb.PartnerID = @PartnerID
			AND cb.Gender IN (SELECT Gender FROM #Gender)
			AND cb.MarketableByEmail IN (SELECT MarketableByEmail FROM #MarketableByEmail)
			AND cb.AgeCurrent BETWEEN @AgeRange_Min AND @AgeRange_Max

			CREATE CLUSTERED INDEX CIX_CompositeID ON #CustomerBaseTemp (CompositeID)
	
			EXEC Staging.oo_TimerMessage 'Collecting #CustomerBaseTemp', @Time OUTPUT
		...
		***********************************************************************************************************************/
			...
			EXEC Staging.oo_TimerMessage 'Filter on Social Class', @Time OUTPUT
	...
	*******************************************************************************************************************************************/
		...
		IF @ControlGroupPercentage > 0
			BEGIN
				...
				IF EXISTS (SELECT 1 FROM [InsightArchive].[MyRewards_ControlGroup_InProgram] WHERE [ClientServicesRef] = @ClientServicesRef)
					BEGIN
						DELETE ipcg
						FROM [InsightArchive].[MyRewards_ControlGroup_InProgram] ipcg
						WHERE EXISTS (SELECT 1
									  FROM #ControlGroup cg
				...
				INSERT INTO [InsightArchive].[MyRewards_ControlGroup_InProgram] ([PartnerID]
																			   , [ClientServicesRef]
																			   , [IronOfferID]
																			   , [ShopperSegmentTypeID]
																			   , [ShopperSegment]
				...
				WHERE NOT EXISTS (	SELECT 1
									FROM [InsightArchive].[MyRewards_ControlGroup_InProgram] cne
	...
	/*******************************************************************************************************************************************
		14. Force senior staff into top offers
	*******************************************************************************************************************************************/
			
		/***********************************************************************************************************************
			14.1. Store offer details if campaign contains top offer
		***********************************************************************************************************************/
			

				-- 2019-11-04 **** This needs to be fixed RF

			IF OBJECT_ID('tempdb..#TopPartnerOffer') IS NOT NULL DROP TABLE #TopPartnerOffer
			SELECT o.IronOfferID
				 , MAX(o.ShopperSegmentTypeID) AS ShopperSegmentTypeID
			INTO #TopPartnerOffer
			FROM Selections.CampaignSetup_TopPartnerOffer tpo
			...
			IF (SELECT COUNT(1) FROM #TopPartnerOffer) > 0
				...
				/***********************************************************************************************************************
					14.1.1 Where campaign includes the top offer per partner then store senior staff where not already on partner offer
				***********************************************************************************************************************/
				...
				/***********************************************************************************************************************
					14.1.2 Where campaign includes the top offer per partner then force senior staff into offer
				***********************************************************************************************************************/
		...
		EXEC Staging.oo_TimerMessage 'Force senior staff into top offers', @Time OUTPUT
	...
	*******************************************************************************************************************************************/
	...
	/*******************************************************************************************************************************************
		16. Insert to NominatedOfferMember_TableNames
	*******************************************************************************************************************************************/
	
		INSERT INTO Selections.NominatedOfferMember_TableNames (TableName, ClientServicesRef)
	...
	*******************************************************************************************************************************************/
		...
		EXEC Staging.oo_TimerMessage 'Insert to NominatedOfferMember_TableNames, CBP_CampaignNames, IronOffer_Campaign_Type, IronOffer_ROCOffers', @Time OUTPUT
