...
CREATE PROCEDURE [Selections].[CampaignSetup_Selection_IndividualCampaign_POS_V4_Draft] (@PartnerID CHAR(4)
...
*/
	...
	*******************************************************************************************************************************************/
		...
		CREATE TABLE #Selection (	FanID BIGINT
								...
								,	StartDate DATETIME
								,	EndDate DATETIME
								,	ControlGroupCustomer BIT
								,	WithinThrottling BIT)
		...
		IF @IsThrottleApplied = 0
			BEGIN
				...
				SELECT	FanID = cb.FanID
					,	CompositeID = cb.CompositeID
					,	ShopperSegmentTypeID = cb.ShopperSegmentTypeID
					,	IronOfferID = iof.IronOfferID
					,	StartDate = @StartDate
					,	EndDate = @EndDateTime
					,	ControlGroupCustomer = cb.ControlGroupCustomer
					,	WithinThrottling = 1
				FROM ##CustomerBase cb
		...
		IF @IsThrottleApplied = 1 AND @RandomThrottle = 0
			BEGIN
				...
				SELECT	FanID = cb.FanID
					,	CompositeID = cb.CompositeID
					,	ShopperSegmentTypeID = cb.ShopperSegmentTypeID
					,	IronOfferID = cb.IronOfferID
					,	StartDate = @StartDate
					,	EndDate = @EndDateTime
					,	ControlGroupCustomer = cb.ControlGroupCustomer
					,	WithinThrottling =	CASE
												WHEN RowNum <= Throttling THEN 1
												ELSE 0
											END
				FROM (	SELECT	FanID = cb.FanID
							,	CompositeID = cb.CompositeID
							,	ShopperSegmentTypeID = cb.ShopperSegmentTypeID
							,	IronOfferID = iof.IronOfferID
							,	Throttling = iof.Throttling
							,	RowNum = ROW_NUMBER() OVER (PARTITION BY iof.IronOfferID ORDER BY cr.Engagement_Rank ASC)
							,	ControlGroupCustomer = cb.ControlGroupCustomer
						FROM ##CustomerBase cb
						INNER JOIN #OfferIDs iof
							ON cb.ShopperSegmentTypeID = iof.ShopperSegmentTypeID
						LEFT JOIN [Warehouse].[InsightArchive].[EngagementScore] cr
							ON cb.FanID = cr.FanID) cb
			END
		...
		IF @IsThrottleApplied = 1 AND @RandomThrottle = 1
			BEGIN
				...
				FROM (	SELECT	FanID = cb.FanID
							...
							,	IronOfferID = iof.IronOfferID
							,	Throttling = iof.Throttling
							,	RowNum = ROW_NUMBER() OVER (PARTITION BY iof.IronOfferID ORDER BY ABS(CHECKSUM(NewID())) ASC)
							,	ControlGroupCustomer = cb.ControlGroupCustomer
						FROM ##CustomerBase cb
						INNER JOIN #OfferIDs iof
	...
	*******************************************************************************************************************************************/
		...
		IF @ControlGroupPercentage > 0
			BEGIN
				...
				DECLARE @AcquireCount	INT =	((SELECT COUNT(*) FROM #Selection WHERE ShopperSegmentTypeID = 7 AND WithinThrottling = 1) * 1.0 / 100) * @ControlGroupPercentage
					  , @LaspsedCount	INT =	((SELECT COUNT(*) FROM #Selection WHERE ShopperSegmentTypeID = 8 AND WithinThrottling = 1) * 1.0 / 100) * @ControlGroupPercentage
					  , @ShopperCount	INT =	((SELECT COUNT(*) FROM #Selection WHERE ShopperSegmentTypeID = 9 AND WithinThrottling = 1) * 1.0 / 100) * @ControlGroupPercentage
					  , @WelcomeCount	INT =	((SELECT COUNT(*) FROM #Selection WHERE ShopperSegmentTypeID = 10 AND WithinThrottling = 1) * 1.0 / 100) * @ControlGroupPercentage
					  , @HomemoverCount INT =	((SELECT COUNT(*) FROM #Selection WHERE ShopperSegmentTypeID = 11 AND WithinThrottling = 1) * 1.0 / 100) * @ControlGroupPercentage
					  , @BirthdayCount	INT =	((SELECT COUNT(*) FROM #Selection WHERE ShopperSegmentTypeID = 12 AND WithinThrottling = 1) * 1.0 / 100) * @ControlGroupPercentage
				...
				SELECT	FanID = s.FanID
					,	ShopperSegmentTypeID = s.ShopperSegmentTypeID
					,	IronOfferID = s.IronOfferID
					,	ControlGroupCount =	CASE
					...
					,	ControlGroupOrder = 1
				FROM #Selection s
				WHERE s.ControlGroupCustomer = 1
				...
				SELECT	FanID = s.FanID
					...
					,	ControlGroupOrder = 2
				FROM #Selection s
				WHERE s.ControlGroupCustomer = 0
				AND s.WithinThrottling = 0
				...
				SELECT	PartnerID = @PartnerID
					...
					,	ShopperSegmentTypeID = ShopperSegmentTypeID
					,	StartDate = @StartDate
					,	EndDate = @ControlGroupEndDate
					,	FanID = FanID
				INTO #ControlGroup
				FROM (	SELECT	FanID = cg.FanID
							,	ShopperSegmentTypeID = cg.ShopperSegmentTypeID
							,	IronOfferID = cg.IronOfferID
							,	ControlGroupCount = cg.ControlGroupCount
							,	RowNum = ROW_NUMBER() OVER (PARTITION BY ShopperSegmentTypeID ORDER BY ControlGroupOrder, ABS(CHECKSUM(NEWID())) ASC)
					  FROM #ControlGroup_Setup cg) cg
				WHERE cg.RowNum <= cg.ControlGroupCount

				DELETE s
				FROM #Selection s
				WHERE s.WithinThrottling = 0
