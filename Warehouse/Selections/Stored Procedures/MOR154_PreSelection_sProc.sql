...
CREATE PROCEDURE [Selections].[MOR154_PreSelection_sProc]
AS
BEGIN
...
-- ATV btw 0 - 20	-- SS £25
IF OBJECT_ID('Sandbox.rukank.Morrisons_High_SoW_ATV_0_20_31032022') IS NOT NULL DROP TABLE Sandbox.rukank.Morrisons_High_SoW_ATV_0_20_31032022			-- 29,789	(Feb run 29,534)
SELECT	F.CINID
INTO	Sandbox.rukank.Morrisons_High_SoW_ATV_0_20_31032022
FROM	#Txn F
WHERE	ATV < 20
GROUP BY F.CINID

	IF OBJECT_ID('[Warehouse].[Selections].[MOR154_PreSelection]') IS NOT NULL DROP TABLE [Warehouse].[Selections].[MOR154_PreSelection]
	SELECT FanID
	INTO [Warehouse].[Selections].[MOR154_PreSelection]
	FROM #FB fb
	WHERE EXISTS (	SELECT 1
					FROM Sandbox.rukank.Morrisons_High_SoW_ATV_0_20_31032022  st
