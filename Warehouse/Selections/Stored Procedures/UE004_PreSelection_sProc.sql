-- =============================================-- Author:  <Rory Francis>-- Create date: <2021-05-30>-- Description: < sProc to run preselection code per camapign >-- =============================================CREATE PROCEDURE [Selections].[UE004_PreSelection_sProc]ASBEGIN
...
--SELECT Classification, COUNT(T.CINID) AS CustVol	FROM #Trans T	GROUP BY Classification
...
IF OBJECT_ID('Sandbox.RukanK.UberEatsEngagementTOP30pct') IS NOT NULL DROP TABLE Sandbox.RukanK.UberEatsEngagementTOP30pct
SELECT	F.FanID
INTO Sandbox.RukanK.UberEatsEngagementTOP30pct
FROM	#FB F
JOIN	#Trans T 	ON F.CINID = T.CINID
WHERE	F.CINID IN (SELECT CINID FROM #NtileEngaged WHERE NTILE_10 IN (1,2,3))
GROUP BY F.FanID


IF OBJECT_ID('Warehouse.Selections.UE004_PreSelection') IS NOT NULL DROP TABLE Warehouse.Selections.UE004_PreSelection
SELECT	FanID
INTO Warehouse.Selections.UE004_PreSelection
FROM	Sandbox.RukanK.UberEatsEngagementTOP30pctEND