...
CREATE PROCEDURE [Lion].[OPE_CustomerRelevance_20210428]
AS
BEGIN
	...
	*******************************************************************************************************************************************/
		...
		select 
		c.FanID, c.CompositeID, c.HeatmapID, c.BrandID, c.PartnerID,
		case
			when fc.fanid is not null then 'Shopper'
			else 'Acquire' end as Type,
		case 
			when c.PremiumRetailer * c.PremiumCustomer = 1 then 1 else 0 end as PremiumFlag,
		...
		LEFT JOIN #Spenders_FaniD2 fc on fc.FanID = c.FanID 
			and fc.BrandID = c.BrandID
		left join #Propensity_MFDD p_MFDD on p_MFDD.HeatmapID = c.HeatmapID
											 and p_MFDD.BrandID = c.BrandID
		left join #Propensity p on p.HeatmapID = c.HeatmapID
	...
	*******************************************************************************************************************************************/
		...
		CREATE NONCLUSTERED COLUMNSTORE INDEX CSI_FanIDPropensity ON #CustomerBrand_Frame (FanID)
...
GO
GRANT EXECUTE
    ON OBJECT::[Lion].[OPE_CustomerRelevance_20210428] TO [SamW]
    AS [dbo];
