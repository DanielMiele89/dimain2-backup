...
CREATE PROCEDURE [Lion].[BOPE_AssignMembers_V5] (@JokerSlotPartners_CommaSeperated VARCHAR(100)
											,	@FullRefresh BIT = 1)
...
/***********************************************************************************************************************
		1. Truncate [Lion].[NominatedLionSendComponent_RedemptionOffers] if we're doing a full refresh
***********************************************************************************************************************/
...
***********************************************************************************************************************/
	...
	--DECLARE @JokerSlotPartners_CommaSeperated VARCHAR(100) = ''
		...
		***************************************************************************************************/
					...
					into #BrandComp
					from Lion.BOPE_BrandCompetitor
				...
				*******************************************************************************/
					...
					Select DISTINCT
						   p.PartnerID
                         , CASE
								WHEN p.PartnerID = 1000008 THEN 'John Lewis Partnership'
								ELSE p.PartnerName
						   END AS PartnerName
						 , CASE
								WHEN p.BrandID = 2317 THEN 66
								WHEN p.PartnerID = 1000008 THEN @BrandID_WRJL
								ELSE p.BrandID 
						   END AS BrandID
						 , ri.RedeemID
				...
				*******************************************************************************/
						...
						*******************************************************************************/
							...
							FROM [dbo].[il_SplitDelimitedStringArray] (@JokerSlotPartners_CommaSeperated, ',')
		...
		***************************************************************************************************/
			...
			DECLARE @CashbackDate DATE

			SELECT @CashbackDate = MAX(Date)
			FROM [Staging].[Customer_CashbackBalances]

			If Object_ID('tempdb..#Customer_CashbackBalances') Is Not Null Drop Table #Customer_CashbackBalances
			SELECT	FanID
				,	ClubCashAvailable
			INTO #Customer_CashbackBalances
			FROM [Staging].[Customer_CashbackBalances]
			WHERE Date = @CashbackDate

			CREATE CLUSTERED INDEX CIX_FanCash ON #Customer_CashbackBalances (FanID, ClubCashAvailable)
			...
			Inner join #Customer_CashbackBalances cb	--	SLC_Report..Fan fa
				on cu.FanID = cb.FanID
			Left join Relational.CINList cl
				on cu.SourceUID = cl.CIN
			WHERE NOT EXISTS (	SELECT 1
								FROM [Lion].[NominatedLionSendComponent_RedemptionOffers] ro
								WHERE nlsc.CompositeID = ro.CompositeID)
		...
		***************************************************************************************************/
				...
				****************************************************************************************************/
							...
							************************************************************************************************/
							...
							OR EXISTS (		SELECT 1
											FROM [Lion].[BOPEForceIN_ControlGroup_20201008] cg
											WHERE t1.FanID = cg.FanID)
				...
				****************************************************************************************************/
							...
							***********************************************************************************/
							...
							--		JOIN	Relational.ConsumerCombination c ON c.ConsumerCombinationID = p.ConsumerCombinationID
							--		WHERE	BrandID = 85
							--	),	postcodes_less30mins_cineworld AS (
							--		SELECT	DISTINCT REPLACE(ToSector, ' ', '') AS CustomerPostSector
							--		FROM	Relational.DriveTimeMatrix d with (nolock)
...
***********************************************************************************************************************/
		...
		***************************************************************************************************/
			...
			Select	LionSendID = LionSendID
				,	CompositeID = rvp.CompositeID
				,	TypeID = 3
				,	ItemRank =	Case
								...
								END
				,	ItemID = Coalesce(rvp.RedeemID, dt.RedeemID)
			...
			CREATE CLusTERED INDEX CIX_ItemRank ON #NominatedLionSendComponent_RedemptionOffers (ItemRank)

			IF @JokerSlotPartners_CommaSeperated IN ('1000011', '2365')
				BEGIN
					...
					SELECT	LionSendID
						...
						,	TypeID
						,	ItemRank =	CASE
											WHEN ItemRank = 4 THEN 5
											WHEN ItemRank = 5 THEN 1
											ELSE ItemRank + 1
										END
						,	ItemID
			...
			ALTER INDEX [IUX_LSIDOfferTypeCompRank] ON [Lion].[NominatedLionSendComponent_RedemptionOffers] REBUILD WITH (DATA_COMPRESSION = ROW, FILLFACTOR = 90, SORT_IN_TEMPDB = ON)
			UPDATE STATISTICS [Lion].[NominatedLionSendComponent_RedemptionOffers]
