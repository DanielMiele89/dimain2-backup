CREATE Procedure [Lion].[LionSendTracking] @LionSendID Int
as
Begin
...
BEGIN TRY
	...
	If Not Exists (Select 1 From Lion.LionSend_Customers Where LionSendID = @LionSendID) And @EmailSendDate Is Not Null And DateDiff(day, @EmailSendDate, Convert(Date, GetDate())) = 1
		Begin
			...
			--Alter Index IX_LionSendCustomers_LionCampaignClubLoyaltyFan On [Lion].[LionSend_Customers] Rebuild
			Alter Index IX_LionSendCustomers_LionCampaignClubLoyaltyFan On [Lion].[LionSend_Customers] Rebuild WITH (DATA_COMPRESSION = PAGE)
