...
CREATE PROCEDURE [Lion].[OPE_CustomerRelevance_20211129]
AS
BEGIN
	...
	*******************************************************************************************************************************************/
		...
		SELECT @EmailDate = MIN(op.EmailDate)
		FROM [Selections].[CampaignSetup_POS] op
		WHERE op.EmailDate > GETDATE()

		IF OBJECT_ID('tempdb..#Brands') IS NOT NULL DROP TABLE #Brands
		select distinct
			   PartnerID = CONVERT(SMALLINT, p.PartnerID)
			 , p.BrandID
		...
		AND (EXISTS (SELECT 1
					...
					WHERE p.PartnerID = cs.PartnerID
					AND cs.EmailDate = @EmailDate))
	...
	*******************************************************************************************************************************************/
		...
		***********************************************************************************************************************/
		...
		SET @Msg = '		' + '6. Get base rates and heatmap specific rates per brandid - DD'
		EXEC [Staging].[oo_TimerMessage_V2] @Msg, @Time Output
	...
	*******************************************************************************************************************************************/
		...
		SET @Msg = '		' + '8. Create propensity table	-	Insert'
		EXEC [Staging].[oo_TimerMessage_V2] @Msg, @Time Output

		CREATE CLUSTERED INDEX CIX_FanIDPropensity ON #CustomerBrand_Frame (FanID, CompositeID, Propensity, PartnerID, Type)
					
		SET @Msg = '		' + '8. Create propensity table	-	Index'
		EXEC [Staging].[oo_TimerMessage_V2] @Msg, @Time Output
	...
	*******************************************************************************************************************************************/
		...
		INTO #BudgetRetailers
		FROM	Relational.Brand B
		JOIN	Relational.Partner P ON P.BrandID = B.BrandID
		--JOIN	Relational.ConsumerCombination CC ON B.BrandID = CC.BrandID
		WHERE	B.BrandID IN (6,19,23,1001,1664,1050,504,1168,1878,1950,108,108,2425,1904,2016,1729,160,1951,3049,166,524,844,193,1495,1757,201,768,212,475,1083,283,266,277,283,1610,296,2523,303,318,409)
		...
		SET @Msg = '		' + '9. Bespoke module	-	Pets At Home Acquire'
		EXEC [Staging].[oo_TimerMessage_V2] @Msg, @Time Output
		...
		SET @Msg = '		' + '9. Bespoke module	-	Morrisons Northern Ireland'
		EXEC [Staging].[oo_TimerMessage_V2] @Msg, @Time Output
		...
		SET @Msg = '		' + '9. Bespoke module	-	Bicester Village'
		EXEC [Staging].[oo_TimerMessage_V2] @Msg, @Time Output

		update #CustomerBrand_Frame
		set Propensity = 0
		from #CustomerBrand_Frame C
		LEFT JOIN #Private P ON C.FanID = P.FanID
		WHERE P.FanID IS NOT NULL 
		AND		 PartnerID IN (SELECT PartnerID FROM #BudgetRetailers)

	/*******************************************************************************************************************************************
		10. Insert to final table
	*******************************************************************************************************************************************/

		IF OBJECT_ID('tempdb..#OPE_CustomerRanking_V2') IS NOT NULL DROP TABLE #OPE_CustomerRanking_V2
		SELECT	cb.PartnerID
			,	cb.FanID
			,	cb.CompositeID
			,	cb.Type
			,	CONVERT(SMALLINT, ROW_NUMBER() OVER (PARTITION BY cb.FanID ORDER BY cb.Propensity DESC)) AS CustomerRanking
		INTO #OPE_CustomerRanking_V2
		FROM #CustomerBrand_Frame cb
					
		SET @Msg = '		' + '10. Insert to final table	-	#OPE_CustomerRanking_V2 insert'
		EXEC [Staging].[oo_TimerMessage_V2] @Msg, @Time Output

		CREATE CLUSTERED INDEX CIX_FanIDPropensity ON #OPE_CustomerRanking_V2 (FanID, CompositeID, Type, CustomerRanking, PartnerID)
					
		SET @Msg = '		' + '10. Insert to final table	-	#OPE_CustomerRanking_V2 index'
		EXEC [Staging].[oo_TimerMessage_V2] @Msg, @Time Output
		...
		SET @Query = '	
						...
						DECLARE @MaxFanID INT
							,	@FanID_Start INT = 1
							,	@FanID_End INT = 60000
							,	@ExecutionCurrent INT = 1
							,	@ExecutionTotal INT
							,	@ExecutionInserted VARCHAR(10)

						SELECT  @MaxFanID = MAX(FanID) FROM #CustomerBrand_Frame

						SET @ExecutionTotal = @MaxFanID / @FanID_End

						WHILE @FanID_Start <= @MaxFanID
							BEGIN
								...
								SELECT cb.PartnerID
									 ...
									 , cb.Type AS Segment
									 , cb.CustomerRanking
								FROM #OPE_CustomerRanking_V2 cb
								WHERE cb.FanID BETWEEN @FanID_Start AND @FanID_End

								SET @ExecutionInserted = @@ROWCOUNT

								SET @Msg_2 = ''		10. Insert to final table - Loop '' + CONVERT(VARCHAR(10), @ExecutionCurrent) + '' / '' + CONVERT(VARCHAR(10), @ExecutionTotal) + '' - '' + CONVERT(VARCHAR(10), @ExecutionInserted) + '' rows''
								EXEC [Staging].[oo_TimerMessage_V2] @Msg_2, @Time_2 Output
								
								SET @FanID_Start = @FanID_Start + 60000
								SET @FanID_End = @FanID_End + 60000
								SET @ExecutionCurrent = @ExecutionCurrent + 1
						...
						SET @Msg_2 = ''		10. Insert to final table - CREATE UNIQUE CLUSTERED INDEX [UCX_PartnerFan]''
						EXEC [Staging].[oo_TimerMessage_V2] @Msg_2, @Time_2 Output
						...
						SET @Msg_2 = ''		10. Insert to final table - ALTER INDEX [IX_PartnerComp_IncSegRank''
						EXEC [Staging].[oo_TimerMessage_V2] @Msg_2, @Time_2 Output
						'
		EXEC(@Query)
