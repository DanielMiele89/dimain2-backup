...
CREATE PROCEDURE [Lion].[OPE_CustomerRelevance_DIMAIN]
AS
BEGIN
	...
	IF	(SELECT GETDATE()) BETWEEN '2022-03-06 12:50:00' AND '2022-03-07' RETURN
	...
	*******************************************************************************************************************************************/
		...
		WHERE EXISTS (	SELECT 1
						FROM #Private P
						WHERE C.FanID = P.FanID)
		AND EXISTS (	SELECT 1
						FROM #BudgetRetailers br
						WHERE c.PartnerID = br.PartnerID)
					
		SET @Msg = '		' + '9. Bespoke module	-	Budget Retailers'
		EXEC [Staging].[oo_TimerMessage_V2] @Msg, @Time Output
