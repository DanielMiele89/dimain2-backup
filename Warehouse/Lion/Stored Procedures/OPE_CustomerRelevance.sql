...
CREATE PROCEDURE [Lion].[OPE_CustomerRelevance]
AS
BEGIN
	...
	*******************************************************************************************************************************************/
		...
		SET @Msg = '		10. Insert to final table - Started'
		EXEC [Staging].[oo_TimerMessage_V2] @Msg, @Time Output
		...
		WHILE @FanID_Start <= @MaxFanID
			BEGIN
				...
				SET @Msg = '		10. Insert to final table - Loop ' + CONVERT(VARCHAR(10), @ExecutionCurrent) + ' / ' + CONVERT(VARCHAR(10), @ExecutionTotal) + ' - ' + CONVERT(VARCHAR(10), @ExecutionInserted) + ' rows'
				EXEC [Staging].[oo_TimerMessage_V2] @Msg, @Time Output
