...
******************************************************************************/
CREATE PROCEDURE [Staging].[ControlSetup_VisaBarclaycardNonAAM_Load_Partners_To_Segment]
...
BEGIN
	...
	CREATE TABLE [Warehouse].[Staging].[ControlSetup_PartnersToSeg_VisaBarclaycard] (ControlGroupID INT
																		,	PartnerID INT
																		,	StartDate DATE
																		,	EndDate DATE
																		,	Segment VARCHAR(50)
																		,	CONSTRAINT PK_ControlSetup_PartnersToSeg_VisaBarclaycard PRIMARY KEY CLUSTERED (ControlGroupID, PartnerID, StartDate, EndDate))
	...
	******************************************************************************/
	...
	-- Load PartnerIDs
		...
		TRUNCATE TABLE [Warehouse].[Staging].[ControlSetup_PartnersToSeg_VisaBarclaycard];

		INSERT INTO [Warehouse].[Staging].[ControlSetup_PartnersToSeg_VisaBarclaycard]
		SELECT	DISTINCT
				pcg.ControlGroupID
			,	b.PartnerID
			,	pcg.StartDate
		...
		INNER JOIN [WH_Visa].[Derived].[Partner] p
	...
	-- Load more PartnerIDs
		...
		SELECT	DISTINCT 
			...
			,	pcg.Segment
		FROM [WH_Visa].[Report].[PartnerControlGroupIDs] pcg
		LEFT JOIN [Warehouse].[Staging].[ControlSetup_PartnersToSeg_VisaBarclaycard] pts
	...
	CREATE TABLE [Warehouse].[Staging].[ControlSetup_Validation_VisaBarclaycardNonAAM_Partners_To_Segment] (	ID INT IDENTITY (1,1)
																		...
																		,	StartDate DATE
																		,	EndDate DATE
																		,	CONSTRAINT PK_ControlSetup_Validation_VisaBarclaycardNonAAM_Partners_To_Segment PRIMARY KEY CLUSTERED (ID))

	******************************************************************************/

	-- Load errors

		TRUNCATE TABLE [Warehouse].[Staging].[ControlSetup_Validation_VisaBarclaycardNonAAM_Partners_To_Segment];

		INSERT INTO [Warehouse].[Staging].[ControlSetup_Validation_VisaBarclaycardNonAAM_Partners_To_Segment] (	PublisherType
																									,	PartnerID
																									,	Segment
																									,	ControlGroupID
																									,	StartDate
