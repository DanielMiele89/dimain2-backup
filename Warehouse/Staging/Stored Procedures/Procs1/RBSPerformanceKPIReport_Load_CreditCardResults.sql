...
Created: 11/04/2019
Purpose: 
	- Load metrics for RBS Performance KPI Report, credit card only results
	
------------------------------------------------------------------------------
Modification History

******************************************************************************/
CREATE PROCEDURE Staging.RBSPerformanceKPIReport_Load_CreditCardResults
...
BEGIN
	...
	******************************************************************************/
	...
	/******************************************************************************
	Load active RBS credit card customer counts
	******************************************************************************/
	...
	WHERE EXISTS (
		SELECT NULL FROM Warehouse.Relational.CustomerPaymentMethodsAvailable pm
		WHERE c.FanID = pm.FanID
		AND pm.StartDate <= cal.EndDate
		AND	(pm.EndDate > cal.EndDate OR pm.EndDate IS NULL)
		AND pm.PaymentMethodsAvailableID IN (1,2) -- Credit Only or Debit and Credit customers
	)
	GROUP BY
		cal.PeriodType
		, cal.StartDate
	...
	/******************************************************************************
	Load active RBS credit card customer spend
	******************************************************************************/
	...
	-- Create calendar table to iterate over for load of BPD spend and transactions
	...
	-- Load credit card customers

	IF OBJECT_ID('tempdb..#CCFanIDs') IS NOT NULL DROP TABLE #CCFanIDs; 
	...
	SELECT DISTINCT
		...
		, pm.FanID
	INTO #CCFanIDs
	FROM #CalendarIter cal
	INNER JOIN Warehouse.Relational.CustomerPaymentMethodsAvailable pm
		ON pm.StartDate <= cal.EndDate
		AND (pm.EndDate > cal.EndDate OR pm.EndDate IS NULL)
		AND pm.PaymentMethodsAvailableID IN (1,2) -- Credit Only or Debit and Credit
		
	CREATE UNIQUE CLUSTERED INDEX CIX_CCFanIDs ON #CCFanIDs (StartDate, EndDate, FanID);
	...
	BEGIN
		...
		INNER JOIN #CCFanIDs cc -- Credit Only or Debit and Credit
			ON cl.FanID = cc.FanID
	...
	/******************************************************************************
	Load active RBS credit card customer earnings
	******************************************************************************/
	...
	/******************************************************************************
	Load active RBS credit card customer redemptions
	******************************************************************************/
	...
	/******************************************************************************
	Load RBS credit card first time redeemers
	******************************************************************************/
	...
	/******************************************************************************
	Load RBS credit card customer web logins
	******************************************************************************/
	...
	/******************************************************************************
	Load RBS credit card customer registrations
	******************************************************************************/
	...
	/******************************************************************************
	Load RBS credit card marketable customers
	******************************************************************************/
