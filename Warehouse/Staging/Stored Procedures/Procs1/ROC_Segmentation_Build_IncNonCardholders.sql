...
CREATE procedure [Staging].[ROC_Segmentation_Build_IncNonCardholders] (@PartnerID_E int)
As
...
--------------------------------------------------------------------------
	...
	WHERE			f.clubID = 12 -- Quidco
					/*and p.removaldate is null */ ----*********** Used for Debenhams to make sure all customers get offer
	...
	--select top 10 * from #finalsegmentation
...
Begin
	Update staging.ROC_Segmentation_Members
	Set EndDate = DateAdd(day,-1,Getdate())
	From staging.ROC_Segmentation_Members
	Where ID in 
	(
	...
	inner join staging.ROC_Segmentation_Members as a
		on	fs.fanid = a.FanID and
			d.SegmentID <> a.SegmentID and
			a.PartnerID = @PartnerID and
			a.EndDate is null
	Where	RowNo Between @RowNo and @RowNo+(@ChunkSize-1)
	)
	
	Insert into staging.ROC_Segmentation_Members 
	select		fs.FanID,
				d.SegmentID,
				@PartnerID,
	...
	inner join Staging.ROC_Segmentation_Descriptions as d
		on fs.final_segments = d.SegmentDescription
	Left Outer join staging.ROC_Segmentation_Members as a
		on	fs.fanid = a.FanID and
			d.SegmentID = a.SegmentID and
			a.PartnerID = @PartnerID and
			a.EndDate is null
	Where	RowNo Between @RowNo and @RowNo+(@ChunkSize-1) and
			a.FanID is null
	Set @RowNo = @RowNo +@ChunkSize
End	

--select Count(*)--top 10 *(1241256)
--from #finalsegmentation as fs
--inner join Staging.ROC_Segmentation_Descriptions as d
--	on fs.final_segments = d.SegmentDescription
...
--Select * from staging.ROC_Segmentation_Members 
--Where FanID in (20396945,20396946)

--Update staging.ROC_Segmentation_Members 
--Set SegmentID = 2
--Where ID in (1,2)

--Truncate Table staging.ROC_Segmentation_Members 

--Select * from Staging.ROC_Segmentation_PartnerSettings 