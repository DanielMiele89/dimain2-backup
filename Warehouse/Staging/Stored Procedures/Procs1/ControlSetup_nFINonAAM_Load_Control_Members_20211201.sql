...
******************************************************************************/
CREATE PROCEDURE [Staging].[ControlSetup_nFINonAAM_Load_Control_Members_20211201]
AS
BEGIN
	...
	******************************************************************************/
	...
	From (
		...
		From Warehouse.Staging.ControlSetup_PartnersToSeg_nFI a -- Or #Missing if running manually for nFI control groups missing members
		) as a
		...
		Select distinct
			Case 
				...
				When i.Segment = 'A' then 
					...
					+'.'
					+i.TableName
					+' Where SegmentID in (7) '+	'Order by NewID()'
				...
				Else 
					...
					+System_User+'.'
					+i.TableName
					+' Where SegmentID in (9) ' +'Order by NewID()'
				End
			AS ImportCode
