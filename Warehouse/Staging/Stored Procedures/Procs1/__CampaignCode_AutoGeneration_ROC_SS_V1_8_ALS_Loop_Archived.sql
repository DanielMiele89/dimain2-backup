...
CREATE PROCEDURE [Staging].[__CampaignCode_AutoGeneration_ROC_SS_V1_8_ALS_Loop_Archived]
		(	@PartnerID CHAR(4),
			@StartDate DATE, 
			@EndDate DATE,
...
Case 
	...
	IF Object_id('+CHAR(39)+'tempdb..#SOW'+CHAR(39)+') IS NOT NULL  DROP TABLE #SOW  
		...
		End +
		'
		Where (DATEADD(YEAR,-(DATEPART(YEAR,DOB)-1900),c.DOB) between '''+ cast(@birthdayStart as varchar(20)) +''' and '''+ cast(@BirthdayEnd as varchar(20)) +''' 
					or c.Homemover = 1) '+
		Case 
			when @Offer4 <> 0 then '
