...
Created: 12/03/2018
Purpose:
	- Load new nFI Control Group member counts INTO [WH_Visa].[Report].[ControlGroupMember_Counts] table
...
******************************************************************************/
CREATE PROCEDURE [Staging].[ControlSetup_VisaBarclaycardNonAAM_Load_Control_Counts]
AS
BEGIN
	...
	******************************************************************************/
		...
		SET @UN_ID = (SELECT UniversalControlGroupID FROM [Warehouse].[Staging].[ControlSetup_UniversalOffer_VisaBarclaycard]);
		SET @SDate = (SELECT StartDate FROM [Warehouse].[Staging].[ControlSetup_Cycle_Dates]);

	/******************************************************************************
	Load new Control Group member counts INTO [WH_Visa].[Report].[ControlGroupMember_Counts] table
	******************************************************************************/
	...
	CountsByDate AS (	SELECT	pcg.PartnerID
						...
						INNER JOIN [WH_Visa].[Report].[ControlGroupMembers] AS cgm
	...
	INSERT INTO [WH_Visa].[Report].[ControlGroupMember_Counts] (PartnerID, SuperSegmentID, ControlGroupID, StartDate, NumberofFanIDs)
	...
	WHERE NOT EXISTS (	SELECT 1
						FROM [WH_Visa].[Report].[ControlGroupMember_Counts] cgmc
	...
	WHERE NOT EXISTS (	SELECT 1
						FROM [WH_Visa].[Report].[ControlGroupMember_Counts] d
	...
	CREATE TABLE [Warehouse].[Staging].[ControlSetup_Validation_VisaBarclaycardNonAAM_Control_Counts] (	PublisherType VARCHAR(50)
																							,	PartnerID INT
																							...
																							,	StartDate DATE
																							,	NumberofFanIDs INT
																							,	CONSTRAINT PK_ControlSetup_Validation_VisaBarclaycardNonAAM_Control_Counts PRIMARY KEY CLUSTERED (PartnerID, ControlGroupID, StartDate))

	******************************************************************************/

	TRUNCATE TABLE [Warehouse].[Staging].[ControlSetup_Validation_VisaBarclaycardNonAAM_Control_Counts];

	INSERT INTO [Warehouse].[Staging].[ControlSetup_Validation_VisaBarclaycardNonAAM_Control_Counts] (	PublisherType
																							,	PartnerID
																							,	SuperSegmentID
																							,	SegmentName
																							,	ControlGroupID
	...
	SELECT	'VisaBarclaycard' AS PublisherType
		...
		,	mc.StartDate
		,	mc.NumberofFanIDs
	FROM [WH_Visa].[Report].[ControlGroupMember_Counts] mc
...
SELECT *
FROM Warehouse.Staging.ControlSetup_Validation_VisaBarclaycardNonAAM_Control_Counts c
