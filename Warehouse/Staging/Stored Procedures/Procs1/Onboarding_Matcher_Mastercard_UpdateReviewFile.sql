/*
	...
	Purpose:	To produce a list of MerchantIDs to be provided to Mastercoard for onboarding

*/


CREATE PROCEDURE [Staging].[Onboarding_Matcher_Mastercard_UpdateReviewFile]
AS
BEGIN
	...
	*******************************************************************************************************************************************/
		...
		FROM [Staging].[Onboarding_Matcher_Mastercard_FileImport] fi
	...
	*******************************************************************************************************************************************/
		...
		WHILE @PartnerNumber <= @MaxPartnerNumber
			...
			*******************************************************************************************************************************************/
				...
				IF OBJECT_ID('tempdb..#Mastercard_FileImport') IS NOT NULL DROP TABLE #Mastercard_FileImport
				SELECT ActionCode AS InitialActionCode 
					 , *
				INTO #Mastercard_FileImport
				FROM [Staging].[Onboarding_Matcher_Mastercard_FileImport] fi
				WHERE EXISTS (	SELECT 1
			...
			*******************************************************************************************************************************************/
				...
				SET fin.PassThru1 = ro.PartnerID
				  , fin.PassThru2 = ro.ID
				FROM #Mastercard_FileImport fin
				...
				SET fi.ActionCode = fit.ActionCode
				  ...
				  , fi.IsFileReviewed = 1
				FROM [Staging].[Onboarding_Matcher_Mastercard_FileImport] fi
				INNER JOIN #Mastercard_FileImport fit
