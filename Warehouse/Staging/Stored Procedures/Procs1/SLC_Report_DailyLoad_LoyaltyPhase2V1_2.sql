...
CREATE Procedure [Staging].[SLC_Report_DailyLoad_LoyaltyPhase2V1_2]
...
Declare @msg VARCHAR(2048),@time DATETIME, @Date date
		...
		select Distinct 
				f.ID as FanID,
				CAST(SourceUID as NVARCHAR(20)) as SourceUID,
				Case
		...
		--**Select * from #LoyaltyAccounts  OK
		-------------------------------------------------------------------------------------
		...
		Exec Staging.SLC_Report_DailyLoad_FirstEarnDD
...
/*	
		...
		Where	t.ClubCash*tt.Multiplier > 0 and
				cast(t.ProcessDate as date) = DATEADD(dd, -1, DATEDIFF(dd, 0, @Date))
...
*/
		...
		inner join SLC_Report.[dbo].[IssuerBankAccount] as iba
			on	ic.ID = iba.IssuerCustomerID and
				(IBA.CustomerStatus = 1 or IBA.CustomerStatus is null)
		inner join SLC_Report.[dbo].BankAccount as BA 
			ON	IBA.BankAccountID = BA.ID AND (BA.[Status] = 1 or BA.[Status] is null)
		INNER JOIN SLC_Report.[dbo].BankAccountTypeHistory AS BAH 
		...
		inner join staging.DirectDebit_EligibleAccounts  as e
			on	bah.Type = e.AccountType and
				((la.IssuerID =  2 and e.ClubID = 132) or (la.IssuerID = 1 and e.ClubID = 138))
		...
		Where	ChangeDate is Not null and
				AccountName is not null and
				AccountNo is not null and
