...
Created: 09/03/2018
Purpose: 
	- Add exposed members to [WH_Virgin].[Report].[CampaignHistory] for new IronOfferCyclesIDs in nFI.Relational.ironoffercycles
...
******************************************************************************/
CREATE PROCEDURE [Staging].[ControlSetup_VirginNonAAM_Load_Exposed_Members_20220407]
...
BEGIN
	...
	/******************************************************************************
	Add exposed members to [WH_Virgin].[Report].[CampaignHistory] for new IronOfferCyclesIDs added to nFI.Relational.ironoffercycles
	******************************************************************************/

	-- Declare iteration variables

	Declare @IOCID int = (Select COALESCE(Max(IronOfferCyclesID), 0) From [WH_Virgin].[Report].[CampaignHistory])+1;
	Declare @IOCID_Max int = (Select Max(IronOfferCyclesID) From [WH_Virgin].[Report].[IronOfferCycles]);
	...
	Begin
		...
		If not exists (Select null from [WH_Virgin].[Report].[CampaignHistory] where IronOfferCyclesID = @IOCID)
		...
		Begin
			...
			Inner join [WH_Virgin].[Report].[OfferCycles] as oc
			...
			Insert into [WH_Virgin].[Report].[CampaignHistory]
			Select
				s.ironoffercyclesid
				, s.FanID
