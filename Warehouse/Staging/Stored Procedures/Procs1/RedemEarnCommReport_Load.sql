...
Jason Shipp 25/02/2019
	...
	-- Changed CJM 20201106 see DBA-207
******************************************************************************/
CREATE PROCEDURE [Staging].[RedemEarnCommReport_Load]
...
*/
		...
		DECLARE @MINMONTHSTART DATE, @MAXMONTHEND DATE       
		SELECT @MINMONTHSTART = MIN(MonthStart), @MAXMONTHEND = MAX(MonthEnd) FROM #MonthDates
       ...
       left JOIN #MonthDates d
              ON d.MonthStart <= aca.TranDate
              AND aca.TranDate <= d.MonthEnd
       WHERE @MINMONTHSTART <= aca.TranDate
              AND aca.TranDate <= @MAXMONTHEND
       GROUP BY
              aca.AdditionalCashbackAwardTypeID
              , aca.FanID
              , d.ID
	...
	******************************************************************************/
	...
	CREATE NONCLUSTERED INDEX nix_Combo ON #SummaryWebLogins1M (ID) INCLUDE (FanID);
