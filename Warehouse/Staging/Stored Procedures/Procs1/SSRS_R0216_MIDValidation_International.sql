...
CREATE PROCEDURE [Staging].[SSRS_R0216_MIDValidation_International] @BrandID INT
AS
	BEGIN
	...
	WHERE BrandName LIKE '%bath%'
	*/

	--	DECLARE @BrandID INT = 3503
...
*******************************************************************************************************************************************/
	...
	CREATE NONCLUSTERED INDEX IX_MID ON #ConsumerCombination_Removed0 (MID, ConsumerCombinationID)
	...
	CREATE NONCLUSTERED INDEX IX_MID ON #ConsumerCombination (MID, ConsumerCombinationID)
	...
	INNER JOIN #CombinedMIDs cm
		ON cc.MID LIKE '%0' + cm.MerchantID
	...
	FROM #ConsumerCombination_Removed0 cc
	INNER JOIN #CombinedMIDs cm
		ON cm.MerchantID LIKE '%0' + cc.MID
	--	AND cc.MID NOT IN ('', '%', '0')
	WHERE NOT EXISTS (	SELECT 1
						FROM #ConsumerCombination c
						WHERE cc.ConsumerCombinationID = c.ConsumerCombinationID)
...
*******************************************************************************************************************************************/
	...
	IF OBJECT_ID('tempdb..#ConsumerCombination_Warba') IS NOT NULL DROP TABLE #ConsumerCombination_Warba;
	SELECT	cm.MerchantID
		,	cc.MID
		,	cc.Narrative
		,	cc.MCC
		,	BrandID = COALESCE(br.BrandID, 944)
		,	Warba_brandingID = cc.ID
		,	cm.InClientList
		,	cm.OnGAS
		,	cm.OnGASOnTheCorrectPartnerRecord
	INTO #ConsumerCombination_Warba
	FROM [Warehouse].[InsightArchive].[Warba_branding] cc
	INNER JOIN #CombinedMIDs cm
		ON cc.MID = cm.MerchantID
	LEFT JOIN [Warehouse].[Relational].[Brand] br
		ON cc.BrandName = br.BrandName
		
	CREATE NONCLUSTERED INDEX IX_MID ON #ConsumerCombination_Warba (MID, Warba_brandingID)
					
	INSERT INTO #ConsumerCombination_Warba
	SELECT	cm.MerchantID
		,	cc.MID
		,	cc.Narrative
		,	cc.MCC
	...
	AND NOT EXISTS (SELECT 1
					FROM #ConsumerCombination_Warba c
					WHERE cc.ID = c.Warba_brandingID)
				
	IF OBJECT_ID('tempdb..#CT_Warba') IS NOT NULL DROP TABLE #CT_Warba;
	SELECT	cc.MerchantID
		,	cc.MID
		,	cc.Narrative
		,	mcc.MCCDesc
		,	LocationCountry = c.[Country Code]
		,	cc.BrandID
		,	cc.InClientList
		,	cc.OnGAS
		,	cc.OnGASOnTheCorrectPartnerRecord
		,	SUM(TRY_CONVERT(FLOAT, REPLACE(c.[Source Amount], ' ', ''))) AS SpendInLastYear
		,	SUM(0) AS OnlineSpendInLastYear
		,	MIN(TRY_CONVERT(DATE, REPLACE(c.[Transaction Date], ' ', ''))) AS FirstTran
		,	MAX(TRY_CONVERT(DATE, REPLACE(c.[Transaction Date], ' ', ''))) AS LastTran
	INTO #CT_Warba
	FROM #ConsumerCombination_Warba cc
	INNER JOIN [WH_AllPublishers].[Staging].[WRBA_Reward_Insight] c
		ON cc.MID = c.[Merchant ID]
		AND cc.Narrative = c.[Transaction Narrative]
		AND cc.MCC = c.[Merchant Category Code]
	INNER JOIN [Relational].[MCCList] mcc
		ON cc.MCC = mcc.MCC
	WHERE cc.MID != ''
	AND cc.MID NOT LIKE 'VCR%'
	AND cc.Narrative NOT LIKE 'PayPal%'
	AND cc.Narrative NOT LIKE 'PP%'
	AND @LastYear < TRY_CONVERT(DATE, REPLACE(c.[Transaction Date], ' ', ''))
	GROUP BY	cc.MerchantID
			,	cc.MID
			,	cc.Narrative
			,	mcc.MCCDesc
	...
	CREATE CLUSTERED INDEX CIX_CID ON #CT_Warba (MID, Narrative)

	UPDATE cc
	SET cc.LocationCountry = c.Alpha2Code
	FROM #CT_Warba cc
	INNER JOIN [Relational].[CountryCodes_ISO_2] c
		ON cc.LocationCountry = c.Alpha3Code


	
	IF OBJECT_ID('tempdb..#CT_All') IS NOT NULL DROP TABLE #CT_All
	SELECT	ct.MerchantID
		,	ct.MID
		,	ct.Narrative
		,	ct.MCCDesc
		,	ct.LocationCountry
		...
		,	MAX(CT.LastTran) AS LastTran
	INTO #CT_All
	FROM (SELECT * FROM #CT UNION ALL SELECT * FROM #CTC UNION ALL SELECT * FROM #CT_Warba) ct
...
*******************************************************************************************************************************************/
	...
	SELECT	COALESCE(ct.BrandID, cm.BrandID) AS BrandID
		...
		,	cm.PartnerID
		,	cm.PartnerName
		,	COALESCE(ct.MID, cm.MerchantID, ct.MerchantID) AS MerchantID
		,	cm.Address1
		,	cm.Address2
		,	cm.City
		,	cm.Postcode
	...
	FULL OUTER JOIN #CT_All ct
		ON cm.MerchantID = ct.MerchantID
	...
	ORDER BY	CASE
				...
				END
			,	cm.PartnerName
			,	COALESCE(cm.MerchantID, ct.MerchantID)
			,	ct.LastTran
...
*******************************************************************************************************************************************/
	...
	INNER JOIN #CreditDataMax cdm
		ON cm.MerchantID = cdm.MerchantID
