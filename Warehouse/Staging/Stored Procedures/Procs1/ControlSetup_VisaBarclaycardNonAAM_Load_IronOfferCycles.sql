...
Purpose: 
	...
	- Add entries to [WH_Visa].[Report].[IronOfferCycles] table
	- Load validatiON of entries added to [WH_Visa].[Report].[IronOfferCycles]
...
******************************************************************************/
CREATE PROCEDURE [Staging].[ControlSetup_VisaBarclaycardNonAAM_Load_IronOfferCycles]
...
BEGIN
	...
	******************************************************************************/
	...
	CREATE TABLE [Warehouse].[Staging].[ControlSetup_UniversalOffer_VisaBarclaycard] (	UniversalControlGroupID INT
																			,	CONSTRAINT PK_ControlSetup_UniversalOffer_VisaBarclaycard PRIMARY KEY CLUSTERED (UniversalControlGroupID))

	******************************************************************************/

	-- Clear Staging.ControlSetup_UniversalOffer_nFI table

		TRUNCATE TABLE [Warehouse].[Staging].[ControlSetup_UniversalOffer_VisaBarclaycard];
	...
	******************************************************************************/
	...
	-- ControlGroupID assignment
		...
		FROM (	SELECT MAX(ControlGroupID) AS ControlGroupID
				FROM [WH_Visa].[Report].[IronOfferCycles]
				UNION
				SELECT MAX(ControlGroupID) AS ControlGroupID
				FROM [WH_Visa].[Report].[ControlGroupMembers]) cgi;
		...
		IF @UniversalControlGroupID IS NULL AND @UniversalOfferCount > 0
			BEGIN
				...
				INSERT INTO [Warehouse].[Staging].[ControlSetup_UniversalOffer_VisaBarclaycard] (UniversalControlGroupID)
				SELECT @UniversalControlGroupID; -- Store Universal ControlGroupID for later Use
			END
			
		TRUNCATE TABLE [WH_Visa].[Report].[PartnerControlGroupIDs]; -- VisaBarclaycard table

		INSERT INTO	[WH_Visa].[Report].[PartnerControlGroupIDs]
		SELECT	PartnerID
			,	Segment
		...
		FROM (	SELECT	DISTINCT
				...
				--	,	s.Segment
					,	a.StartDate
					,	a.EndDate
			FROM [Warehouse].[Staging].[ControlSetup_OffersSegment_VisaBarclaycard] a
			LEFT JOIN #PartnerAlternate pa 
				ON a.PartnerID = pa.PartnerID
			CROSS JOIN (SELECT	DISTINCT
								Segment
						FROM [Warehouse].[Staging].[ControlSetup_OffersSegment_VisaBarclaycard]) s) a;
	...
	-- UPDATE ControlGroupID for different PartnerIDs that can be associated to the same RetailerID ON Control Group table
		...
		FROM [WH_Visa].[Report].[PartnerControlGroupIDs] pcg
		INNER JOIN [Warehouse].[iron].[PrimaryRetailerIdentification] pri
			ON pcg.PartnerID = pri.PartnerID
		INNER JOIN [WH_Visa].[Report].[PartnerControlGroupIDs] pcg2
	...
	-- UPDATE ControlGroupID for cases WHERE a ControlGroupID already exists for that retailer segment in that cycle 
		...
		FROM (	SELECT	DISTINCT
				...
				FROM [Warehouse].[Staging].[ControlSetup_OffersSegment_VisaBarclaycard] s
				LEFT JOIN #PartnerAlternate pa 
					ON s.PartnerID = pa.PartnerID
				INNER JOIN [WH_Visa].[Report].[IronOffer_References] ior
					ON s.IronOfferID = ior.IronOfferID
				INNER JOIN [WH_Visa].[Report].[IronOfferCycles] ioc
					ON ior.IronOfferCyclesID = ioc.IronOfferCyclesID
				INNER JOIN [WH_Visa].[Report].[OfferCycles] cyc
					ON ioc.OfferCyclesID = cyc.OfferCyclesID
					AND CONVERT(DATE, s.StartDate) BETWEEN CONVERT(DATE, cyc.StartDate) AND CONVERT(DATE, cyc.EndDate)) ecg
		WHERE ecg.ControlGroupIDRank = 1;

		INSERT INTO #ExistingControlGroups
		SELECT	ecg.PartnerID
			,	ecg.Segment	
			,	ecg.ControlGroupID 
		FROM (	SELECT	DISTINCT
		...
		AND NOT EXISTS (SELECT 1
						FROM #ExistingControlGroups e
						WHERE ecg.PartnerID = e.PartnerID
						AND ecg.Segment = e.Segment);

		UPDATE cls
		SET ControlGroupID = ecg.ControlGroupID
		FROM [WH_Visa].[Report].[PartnerControlGroupIDs] cls
		INNER JOIN #ExistingControlGroups ecg
			ON cls.PartnerID = ecg.PartnerID
			AND cls.Segment = ecg.Segment;

	/******************************************************************************
	Add new entries to [WH_Visa].[Report].[IronOfferCycles]
	******************************************************************************/

		INSERT INTO [WH_Visa].[Report].[IronOfferCycles]
		SELECT	a.IronOfferID
			,	oc.OfferCyclesID
			,	CASE
					WHEN a.Segment = 'B' THEN (SELECT UniversalControlGroupID FROM [Warehouse].[Staging].[ControlSetup_UniversalOffer_VisaBarclaycard])
		...
		LEFT JOIN #PartnerAlternate pa 
			ON a.PartnerID = pa.PartnerID
		LEFT JOIN [WH_Visa].[Report].[PartnerControlGroupIDs] PG
		...
		WHERE NOT EXISTS (	SELECT 1
							FROM [WH_Visa].[Report].[IronOfferCycles] d
	...
	CREATE TABLE [Warehouse].[Staging].[ControlSetup_Validation_VisaBarclaycardNonAAM_IronOfferCycles] (	ID INT IDENTITY (1,1)
																							...
																							,	ControlGroupID INT
																							,	Error VARCHAR(200)
																							,	CONSTRAINT PK_ControlSetup_Validation_VisaBarclaycardNonAAM_IronOfferCycles PRIMARY KEY CLUSTERED (ID))
	...
	******************************************************************************/
	...
	-- Check row count matches rows in [Warehouse].[Staging].[ControlSetup_OffersSegment_VisaBarclaycard] table

	-- Load new ironoffercycles data

		IF OBJECT_ID ('tempdb..#IOCCheckData') IS NOT NULL DROP TABLE #IOCCheckData;
		SELECT	'VisaBarclaycard' AS PublisherType
		...
		LEFT JOIN [WH_Visa].[Report].[PartnerControlGroupIDs] pcg
	...
	-- Load retailer segments associated with more than 1 control group
		...
		LEFT JOIN [Warehouse].[Staging].[ControlSetup_OffersSegment_VisaBarclaycard] seg
		...
		CROSS JOIN (SELECT MAX(UniversalControlGroupID) AS UniversalControlGroupID
					FROM [Warehouse].[Staging].[ControlSetup_UniversalOffer_VisaBarclaycard]) u
		WHERE seg.Segment = 'B'
	...
	-- Load errors
		...
		TRUNCATE TABLE [Warehouse].[Staging].[ControlSetup_Validation_VisaBarclaycardNonAAM_IronOfferCycles];

		INSERT INTO [Warehouse].[Staging].[ControlSetup_Validation_VisaBarclaycardNonAAM_IronOfferCycles] (	PublisherType
																								,	PartnerID
																								,	Segment
																								,	IronOfferID
																								,	IronOfferName
		...
		LEFT JOIN [WH_Visa].[Report].[IronOfferCycles] ioc -- Check for multiple control groups associated with the same Iron Offer in IronOfferReferences table
		...
		FROM [Warehouse].[Staging].[ControlSetup_OffersSegment_VisaBarclaycard] seg
		WHERE NOT EXISTS (	SELECT 1
							FROM #IOCCheckData ioc
