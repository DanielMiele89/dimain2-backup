...
******************************************************************************/
CREATE PROCEDURE [Staging].[ControlSetup_AMEX_Load_Control_Members_20211201]
...
BEGIN
	...
	******************************************************************************/
	...
	Select distinct
		...
		, m.RetailerID
		, 'Sandbox.'+System_User+'.Control_'+Cast(m.RetailerID as varchar(6)) + '_' +CONVERT(varchar, COALESCE(p.StartDate, @SDate), 112) as TableName
		, TargetAudience
	...
	From (	
		Select distinct
				...
				+ Case
					...
					When s.SegmentCode = 'S' then 'in (9)'
					When s.SegmentCode = 'L' then 'in (8)'
					When s.SegmentCode = 'A' then 'in (7)'
					Else ' > 0'
			End AS EXCcode
