CREATE Procedure [Staging].[ShareOfWallet_SegmentAssignment_Prior35_02_V1_3]
...
-------------------------------------------------------------------------------------------------
Set @TableName = 'Staging.ShareOfWallet_Prior35_'+ @PartnerName+convert(varchar,Cast(GETDATE()as DATE),112)
Set @Qry =
...
Select	c.ID as FanID,
		a.Segment as HTMID,
		P.PartnerID,
		Dateadd(day,-35,Cast(getdate() as date)) as StartDate, 
...
inner join [Relational].[ShareOfWallet_Members_Prior35] as b
...
--------------------------------------------------------------------------------------------------------------
--CB+ Customers
Update  [Relational].[ShareOfWallet_Members_Prior35]
Set		EndDate = Dateadd(day,-36,Cast(getdate() as date))
...
--------------------------------------------------------------------------------------------------------------		
Insert into [Relational].[ShareOfWallet_Members_Prior35]
...
Insert into Relational.ShareofWallet_RunLog_Prior35
...
Set @EndDate = (Select Dateadd(day,-35,EndDate) From Staging.ShareofWallet_EndDate)
Set @StartDate = DateAdd(Day,1,DateAdd(month,-Cast(@Mth as int),@EndDate))

Insert into Relational.ShareOfWallet_Dates_Prior35
Select	(Select Max(ID) as ShareOfWalletID from Relational.ShareofWallet_RunLog_Prior35) as ShareOfWalletID,
		@StartDate,
		@EndDate
...
----------------------------------------Log Segment Counts ---------------------------------------------------
--------------------------------------------------------------------------------------------------------------	
Insert into Relational.ShareOfWallet_SegmentCounts_Prior35
Select	(Select Max(ID) as ShareOfWalletID from Relational.ShareofWallet_RunLog_Prior35) as ShareOfWalletID,
		Segment as htmID,
		Count(*) as Members,
