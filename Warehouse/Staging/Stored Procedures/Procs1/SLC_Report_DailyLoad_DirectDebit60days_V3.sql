...
CREATE PROCEDURE [Staging].[SLC_Report_DailyLoad_DirectDebit60days_V3] 
AS
BEGIN
...
----------------------------------------------------------------------------------------
	...
	SET @Date = DATEADD(day, -60, GETDATE())
...
----------------------------------------------------------------------------------------
	...
	FinalCustomers AS (SELECT fc.FanID
							, REPLACE(dud.AccountName1,' account','') AS Day60AccountName
					   ...
					   UNION ALL
					   SELECT fc.FanID
							, REPLACE(dud.AccountName2,' account','') AS Day60AccountName
					   FROM #FinalCustomers fc
					   INNER JOIN [SLC_Report].[dbo].[FanSFDDailyUploadData_DirectDebit] dud
				   			ON fc.FanID = dud.FanID
					   ...
					   UNION ALL
					   SELECT fc.FanID
				   			 , REPLACE(dud.AccountName3,' account','') AS Day60AccountName
					   FROM #FinalCustomers fc
					   INNER JOIN [SLC_Report].[dbo].[FanSFDDailyUploadData_DirectDebit] dud
				   			ON fc.FanID = dud.FanID
	...
	FROM (SELECT FanID
			   ...
			   , ROW_NUMBER() OVER(PARTITION BY FanID ORDER BY CASE
															WHEN Day60AccountName LIKE '%Black%' THEN 1
															WHEN Day60AccountName LIKE '%Plat%' THEN 2
															WHEN Day60AccountName LIKE '%Silve%' THEN 3
															ELSE 4
														 END ASC) AS RowNo
		  FROM FinalCustomers) fc
	WHERE RowNo = 1
