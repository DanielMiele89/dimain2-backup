...
CREATE PROCEDURE [Staging].[SLC_Report_DailyLoad_CBP_ProcessDirectDebitStats_SFD_RETIRED20180302]
...
BEGIN
	...
	Where	i.StartDate <= CAST(getdate() as DATE) and
	...
	inner join [SLC_Report].[dbo].IronOfferClub as ioc
		on i.ID = ioc.IronOfferID
	Where	i.StartDate <= CAST(getdate() as DATE) and
			(i.EndDate >= getdate() or i.EndDate is null) and
			ba.DirectDebitEligible = 1
	SET TRANSACTION ISOLATION LEVEL READ COMMITTED
	...
	From (
		...
		inner join [SLC_Report].[dbo].IronOfferMember as iom
			on	f.CompositeID = iom.CompositeID and
				(	iom.StartDate <= CAST(getdate() as DATE) or iom.StartDate is null	) and
				(	iom.EndDate >= getdate() or iom.EndDate is null		)
	...
	--Create Clustered index IX_Customers_IssuerIDRowNo on #Customers (IssuerCustomerID,RowNo)
	CREATE UNIQUE CLUSTERED INDEX ucx_Stuff ON #Customers (RowNo) --CJM/NB
	...
	---------------************ Create table to hold First DD tran Date****************----------------							
	if object_id('tempdb..#FirstTrans') is not null
										drop table #FirstTrans
	...
	if object_id('tempdb..#FirstEligible') is not null
										drop table #FirstEligible
	...
	if object_id('tempdb..#EligibleAccounts') is not null 
										drop table #EligibleAccounts
	...
	Where	ChangeSourceType  = 3 --3	= RBS Feed 
			and 
			StartDate = Dateadd(day,DATEDIFF(dd, 0, GETDATE())-1,0) --or EndDate >= Dateadd(day,DATEDIFF(dd, 0, GETDATE())-2,0))
	) as a
	...
	Begin
		...
		truncate table #FirstTrans;
		truncate table #FirstEligible;
		truncate table #EligibleAccounts;
		...
		from #Accounts as a
		inner join [SLC_Report].[dbo].[IssuerBankAccount] as iba
				on	a.BankAccountID = iba.BankAccountID AND 
					COALESCE(IBA.CustomerStatus, 1) = 1	and 
					a.IssuerCustomerID <> iba.IssuerCustomerID
		inner join [SLC_Report].[dbo].IssuerCustomerAttribute as ica
				on	iba.IssuerCustomerID = ica.IssuerCustomerID and
					ica.EndDate is null
		inner join #OffersAccounts as oa
			on	oa.BankAccountType = a.[Type] and
				oa.ClubID = a.ClubID
		Where	AlreadyValid = 0 and
				(oa.CustomerSegment is null or oa.CustomerSegment = ica.Value) and
				RowNo between @RowNo and @RowNo + (@ChunkSize-1)
		SET TRANSACTION ISOLATION LEVEL READ COMMITTED
		...
		(
			...
			inner join Warehouse.staging.DirectDebit_EligibleAccounts as e
				on	a.Type = e.AccountType and
					a.clubid = e.ClubID 				
			Where a.RowNo between @RowNo and @RowNo + (@ChunkSize-1)
		...
		LEFT OUTER Join #RBSNomChange AS nc
			ON c.FanID = nc.FanID
		Where c.RowNo between @RowNo and @RowNo + (@ChunkSize-1)
		AND not exists (select 1 FROM [SLC_Report].[dbo].[FanSFDDailyUploadData_DirectDebit]
