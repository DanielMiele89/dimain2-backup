...
******************************************************************************/
CREATE PROCEDURE [Staging].[OfferReport_Insert_CustomerErrors_20210310] 
...
BEGIN
    ...
    FROM (
	   SELECT * 
		  , STUFF((
			 SELECT ',' + Error
				FROM (
				SELECT DISTINCT a.*, 'Control' Error 
				FROM #Control c
				JOIN Staging.OfferReport_AllOffers a
					ON a.ControlGroupID = c.ControlGroupID
					AND a.isWarehouse = c.isWarehouse
				WHERE NOT EXISTS (
				    SELECT 1 FROM Warehouse.Relational.controlgroupmembers cm
				    WHERE cm.controlgroupid = c.ControlGroupID
					   AND c.isWarehouse = 1
    
				    UNION ALL

				    SELECT 1 FROM nFI.Relational.controlgroupmembers cm
				    WHERE cm.controlgroupid = c.ControlGroupID
					   AND c.isWarehouse = 0

				    UNION ALL

				    SELECT 1 FROM nFI.Relational.AmexControlGroupMembers cm
				...
				SELECT DISTINCT a.*, 'Exposed' Error
				FROM #Exposed e
				JOIN Staging.OfferReport_AllOffers a 
					ON a.IronOfferCyclesID = e.IronOfferCyclesID
					AND a.isWarehouse = e.isWarehouse
				WHERE NOT EXISTS (
				    SELECT 1 FROM Warehouse.Relational.CampaignHistory c
				    WHERE c.ironoffercyclesid = e.IronOfferCyclesID
					   AND e.isWarehouse = 1
    
				    UNION ALL

				    SELECT 1 FROM nFI.Relational.CampaignHistory c
				    WHERE c.IronOffercyclesID = e.IronOfferCyclesID
					   AND e.isWarehouse = 0
				)
				...
				SELECT DISTINCT a.*, 'OutlierExclusion' Error 
				FROM Staging.OfferReport_AllOffers a
				WHERE NOT EXISTS (
				    SELECT 1 FROM Staging.OfferReport_OutlierExclusion ox
				    WHERE ox.PartnerID = a.PartnerID
				    AND a.offerStartDate	between ox.StartDate and ISNULL(ox.EndDate, GETDATE())
				)
			 ) x
			 WHERE x.ID = a.ID
			 FOR XML PATH('')
    ...
    DELETE a FROM Staging.OfferReport_AllOffers a
    JOIN Staging.OfferReport_AllOffers_Errors e
	   ON e.IronOfferID = a.IronOfferID
	   AND (
		  e.IronOfferCyclesID = a.IronOfferCyclesID
