...
-- Description: Fetch the most recent Direct Debit transactions for Octopus 
--				from Archive_Light.dbo.CBP_DirectDebit_TransactionHistory
--				and insert them into the Octopus Direct Debit table
-- ***************************************************************************
CREATE PROCEDURE [Staging].[DirectDebit_FetchOctopusDDs]
...
SELECT	StoredProcedureName = OBJECT_NAME(@@PROCID),
	...
	TableName = 'CBP_DirectDebit_TransactionHistory_Octopus_DD',
	StartDate = GETDATE(),
	EndDate = NULL,
	TableRowCount  = NULL,
	AppendReload = 'A'
...
*******************************************************************************************************************************************/
	...
	INSERT INTO Staging.CBP_DirectDebit_TransactionHistory_Octopus_DD_FileIDsAlreadyProcessed (FileID
																						 , DateProcessed
																						 , InsertCompleted)
	SELECT ID as FileID
	...
	AND NOT EXISTS (SELECT 1
					FROM Staging.CBP_DirectDebit_TransactionHistory_Octopus_DD_FileIDsAlreadyProcessed fap
...
/*******************************************************************************************************************************************
	2. Find all Octopus Service User Numbers (oins / OINs)
*******************************************************************************************************************************************/

	IF OBJECT_ID ('tempdb..#OINs') IS NOT NULL DROP TABLE #OINs
	SELECT 505645 AS OIN
	INTO #OINs
...
*******************************************************************************************************************************************/
	...
	TRUNCATE TABLE Staging.CBP_DirectDebit_TransactionHistory_Octopus_DD_Holding
	
	WHILE EXISTS (SELECT 1 FROM Staging.CBP_DirectDebit_TransactionHistory_Octopus_DD_FileIDsAlreadyProcessed WHERE InsertCompleted = 0)
	BEGIN
	
		SET @FileID = (SELECT MAX(FileID) FROM Staging.CBP_DirectDebit_TransactionHistory_Octopus_DD_FileIDsAlreadyProcessed WHERE InsertCompleted = 0)

		INSERT INTO Staging.CBP_DirectDebit_TransactionHistory_Octopus_DD_Holding (FileID
																			 , RowNum
																			 , OIN
																			 , Narrative
																			 , TranDate
		...
		UPDATE Staging.CBP_DirectDebit_TransactionHistory_Octopus_DD_FileIDsAlreadyProcessed
...
*******************************************************************************************************************************************/
	...
	***********************************************************************************************************************/
		...
		SELECT DISTINCT
			   BankAccountID
		INTO #BankAccount
		FROM Staging.CBP_DirectDebit_TransactionHistory_Octopus_DD_Holding
	...
	***********************************************************************************************************************/
		...
		FROM Staging.CBP_DirectDebit_TransactionHistory_Octopus_DD_Holding dd
...
*******************************************************************************************************************************************/
	...
	ALTER INDEX [IX_FanTranDateIncl_OINAmount_Octopus] ON [Staging].[CBP_DirectDebit_TransactionHistory_Octopus_DD] DISABLE

	INSERT INTO Staging.CBP_DirectDebit_TransactionHistory_Octopus_DD
	...
	ALTER INDEX [IX_FanTranDateIncl_OINAmount_Octopus] ON [Staging].[CBP_DirectDebit_TransactionHistory_Octopus_DD] REBUILD
