...
CREATE Procedure [Staging].[__ROC_CampaignCode_ParameterInsert_ALS_Archived] (
			@EmailDate date,
			@PartnerID CHAR(4),
			@StartDate DATE, 
			@EndDate DATE,
...
;WITH ssDates
    ...
    (
	   SELECT	--1 as CycleNo,
				...
				cast(case 
					when @LaunchOfferID <> '' then @LaunchOfferID +','+@LaunchOfferID +','+@LaunchOfferID +',00000,00000,00000'
					Else @OfferID
	   ...
	   SELECT	--CycleNo + 1 as CycleNo,
				...
				Case when @PriorityFlag = 1 then 1 else 0 End as PriorityFlag
