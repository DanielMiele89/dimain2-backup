...
******************************************************************************/
CREATE PROCEDURE [Staging].[OfferReport_Fetch_CTMetrics_Threshold_V2_20220329] (@ID INT)
...
BEGIN
	...
	SET NOCOUNT ON
		...
		CREATE TABLE #Results (	[IronOfferID] [int] NOT NULL
	...
	--	DECLARE @ID INT = 1
		...
		SELECT	*
			,	IsCardScheme =	CASE
									WHEN o.IsWarehouse IS NULL AND o.IsVirgin IS NULL AND o.IsVisaBarclaycard IS NULL THEN 1
									ELSE 0
								END
		...
		IF @IsCardScheme = 1
			BEGIN
				...
				SELECT	GroupID = cu.GroupID
					,	FanID = cu.FanID
					,	CINID = COALESCE(cu.CINID_Warehouse, cu.CINID_Virgin)
					,	Exposed = cu.Exposed
					,	IsWarehouse = cu.IsWarehouse
					,	IsVirgin = cu.IsVirgin
					,	IsVisaBarclaycard = cu.IsVisaBarclaycard
