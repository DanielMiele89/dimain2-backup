...
******************************************************************************/
CREATE PROCEDURE [Staging].[OfferReport_Fetch_CTMetrics_Threshold_V2_20220327] (@ID INT)
...
BEGIN
	...
	--	DECLARE @ID INT = 1
		...
		IF OBJECT_ID('tempdb..#OfferReport_CTCustomers') IS NOT NULL DROP TABLE #OfferReport_CTCustomers
		SELECT	*
			,	COALESCE(cu.CINID_Warehouse, cu.CINID_Virgin, cu.CINID_VisaBarclaycard) AS CINID
		...
		CREATE NONCLUSTERED INDEX IX_CINDateAmount_IncCCID ON #OfferReport_CTCustomers (Exposed) INCLUDE (GroupID, IsWarehouse, IsVirgin, IsVisaBarclaycard, CINID)
		CREATE NONCLUSTERED COLUMNSTORE INDEX CSI_All ON #OfferReport_CTCustomers (GroupID, Exposed, IsWarehouse, IsVirgin, IsVisaBarclaycard, CINID)
		...
		CREATE NONCLUSTERED COLUMNSTORE INDEX CSI_All ON #OfferReport_ConsumerCombinations (PartnerID, IsWarehouse, IsVirgin, IsVisaBarclaycard, ConsumerCombinationID)
		...
		WHERE EXISTS (	SELECT 1
						...
						AND COALESCE(ao.IsWarehouse, ao.IsVirgin, ao.IsVisaBarclaycard) IS NOT NULL
		...
		CREATE NONCLUSTERED COLUMNSTORE INDEX CSI_All ON #OfferReport_ConsumerTransaction (TranDate, IsWarehouse, IsVirgin, IsVisaBarclaycard, ConsumerCombinationID, CINID, IsOnline)
