...
******************************************************************************/
CREATE PROCEDURE [Staging].[ControlSetup_RBS_Load_Control_Counts]
AS
BEGIN
	...
	******************************************************************************/
	...
	WHERE EXISTS (	SELECT 1
					FROM [Warehouse].[Relational].[ControlGroupMember_Counts] cgm
	...
	******************************************************************************/
	...
	Declare @UN_ID int = (select UniversalControlGroupID from Warehouse.Staging.ControlSetup_UniversalOffer_Warehouse);
	...
	Where 
		...
		AND (
			...
			OR ( -- Check Lapsed and Shopper member counts
				...
				AND NOT (mc.SuperSegmentID in (8, 9) and mc.NumberofFanIDs BETWEEN 0 AND 100 AND mc.PartnerID IN (4752, 4751, 4906, 4916, 4753, 4764, 4812))
			)
		);
