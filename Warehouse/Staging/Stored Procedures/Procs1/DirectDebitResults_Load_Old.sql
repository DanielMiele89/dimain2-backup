...
Jason Shipp 12/02/2019
	- Updated exposed DD transaction metrics load to point to SLC_Report.dbo.Match instead of Archive_Light.dbo.CBP_DirectDebit_TransactionHistory
...
Jason Shipp 24/07/2019
	...
	- Changed source of business logic from hard coded figures to SLC_Report.dbo.DirectDebitOffers and SLC_Report.dbo.DirectDebitOfferRules tables
...
Jason Shipp 30/08/2019
	- Repointed Warehouse.Relational.DirectDebit_MFDD_IncentivisedOINs to SLC_Report.dbo.DirectDebitOfferOINs, and removed dependency on OIN start and end dates
	- Repointed Warehouse.Relational.DirectDebitOriginator to SLC_Report.dbo.DirectDebitOriginator

Jason Shipp 09/03/2020
	- Updated link between OINs and Iron Offers to use PartnerCommissionRuleID when joining to the SLC_Report.dbo.Match table
...
******************************************************************************/
CREATE PROCEDURE Staging.[DirectDebitResults_Load_Old] (@RetailerID int, @DateFirst int, @MinStartDate date, @SplitIronOfferTargeting bit)
...
BEGIN
	...
	******************************************************************************/
	...
	CREATE UNIQUE CLUSTERED INDEX CIX_DDSuppliers ON #DDSuppliers (DirectDebitOriginatorID, StartDate, EndDate);
	CREATE UNIQUE NONCLUSTERED INDEX IX_DDSuppliers ON #DDSuppliers (OIN);
	...
	******************************************************************************/
	...
	CREATE UNIQUE CLUSTERED INDEX CIX_Rules ON #Rules (DirectDebitOriginatorID, PartnerCommissionRuleID);
	...
	******************************************************************************/
	...
	DECLARE @FileID1 int = 19909 -- FileIDs >= 19910 contain transaction data >= 2019-04-11, so contain DD data we are interested in

	WHILE 1 = 1 BEGIN
		SELECT @FileID1 = MIN(t.FileID) FROM Archive_Light.dbo.CBP_DirectDebit_TransactionHistory t WHERE t.FileID > @FileID1
		IF @@ROWCOUNT = 0 OR @FileID1 IS NULL BREAK
		INSERT INTO #FileIDs (FileID) VALUES (@FileID1);
	END
	...
	CREATE NONCLUSTERED INDEX IX_TransactionHistoryData ON #TransactionHistoryData (FanID, [Date]) INCLUDE (Amount);
	CREATE NONCLUSTERED INDEX IX2_TransactionHistoryData ON #TransactionHistoryData (OIN);
	...
	******************************************************************************/
	...
	BEGIN
		...
		INNER JOIN SLC_Report.dbo.IssuerBankAccount iba2 -- Expand bank accounts to include all FanIDs associated with the same bank accounts 
			ON iba.BankAccountID = iba2.BankAccountID
		INNER JOIN SLC_Report.dbo.IssuerCustomer ic2
			ON iba2.IssuerCustomerID = ic2.ID
		INNER JOIN SLC_Report.dbo.Fan f2
		...
		WITH GroupedByFirstSecondDD AS (
			...
			, m.Amount
			, m.AffiliateCommissionAmount
			, m.RewardStatus AS DDRankByDateGroup
			...
			INNER JOIN #Rules r -- Duplication due to OINs being linked to multiple Iron Offers resolved in WHERE clause
				ON m.DirectDebitOriginatorID = r.DirectDebitOriginatorID
				AND (m.PartnerCommissionRuleID = r.PartnerCommissionRuleID OR (m.PartnerCommissionRuleID IS NULL AND r.PartnerCommissionRuleID IS NULL)) -- Join on PartnerCommissionRuleID where available, to handle cases where an OIN is linked to multiple Iron Offers
			INNER JOIN SLC_Report.dbo.IssuerBankAccount iba 
				ON m.IssuerBankAccountID = iba.ID
			INNER JOIN SLC_Report.dbo.IssuerCustomer ic 
				ON iba.IssuerCustomerID = ic.ID
			INNER JOIN SLC_Report.dbo.Fan f
				ON ic.SourceUID = f.SourceUID
			INNER JOIN #DDSuppliers dds
				ON m.DirectDebitOriginatorID = dds.DirectDebitOriginatorID
			...
			WHERE
				m.[Status] = 1 -- Valid transactions
				AND m.[RewardStatus] IN (1,15) -- 15 = insufficient prior DD transactions for incentivisation, 1 = incentivised DD transaction
				AND m.VectorID = 40 -- RBS DDs
				AND (r.IronOfferID = @IronOfferID OR @IronOfferID = 'Overall')
		)
		...
		WITH GroupedByFirstSecondDD AS (
			...
			INNER JOIN #Rules r -- Duplication due to OINs being linked to multiple Iron Offers resolved in WHERE clause
				ON dd.OIN = r.OIN
			INNER JOIN #ControlGroupMembers c
				ON dd.FanID = c.FanID
			WHERE 
				dd.[Date] >= r.PassiveWindowEndDate -- Exclude passive spend
