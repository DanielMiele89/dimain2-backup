...
Purpose: 
	...
	- Add entries to [WH_VirginPCA].[Report].[IronOfferCycles] table
	- Load validatiON of entries added to [WH_VirginPCA].[Report].[IronOfferCycles]
...
******************************************************************************/
CREATE PROCEDURE [Staging].[ControlSetup_VirginPCANonAAM_Load_IronOfferCycles]
...
BEGIN
	...
	******************************************************************************/
	...
	CREATE TABLE [Warehouse].[Staging].[ControlSetup_UniversalOffer_VirginPCA] (	UniversalControlGroupID INT
																			,	CONSTRAINT PK_ControlSetup_UniversalOffer_VirginPCA PRIMARY KEY CLUSTERED (UniversalControlGroupID))

	******************************************************************************/

	-- Clear Staging.ControlSetup_UniversalOffer_nFI table

		TRUNCATE TABLE [Warehouse].[Staging].[ControlSetup_UniversalOffer_VirginPCA];
	...
	******************************************************************************/
	...
	-- ControlGroupID assignment
		...
		FROM (	SELECT MAX(ControlGroupID) AS ControlGroupID
				FROM [WH_VirginPCA].[Report].[IronOfferCycles]
				UNION
				SELECT MAX(ControlGroupID) AS ControlGroupID
				FROM [WH_VirginPCA].[Report].[ControlGroupMembers]) cgi;
		...
		IF @UniversalControlGroupID IS NULL AND @UniversalOfferCount > 0
			BEGIN
				...
				INSERT INTO [Warehouse].[Staging].[ControlSetup_UniversalOffer_VirginPCA] (UniversalControlGroupID)
				SELECT @UniversalControlGroupID; -- Store Universal ControlGroupID for later Use
			END
			
		TRUNCATE TABLE [WH_VirginPCA].[Report].[PartnerControlGroupIDs]; -- VirginPCA table

		INSERT INTO	[WH_VirginPCA].[Report].[PartnerControlGroupIDs]
		SELECT	PartnerID
			,	Segment
		...
		FROM (	SELECT	DISTINCT
				...
				--	,	s.Segment
					,	a.StartDate
					,	a.EndDate
			FROM [Warehouse].[Staging].[ControlSetup_OffersSegment_VirginPCA] a
			LEFT JOIN #PartnerAlternate pa 
				ON a.PartnerID = pa.PartnerID
			CROSS JOIN (SELECT	DISTINCT
								Segment
						FROM [Warehouse].[Staging].[ControlSetup_OffersSegment_VirginPCA]) s) a;
	...
	-- UPDATE ControlGroupID for different PartnerIDs that can be associated to the same RetailerID ON Control Group table
		...
		INNER JOIN [WH_VirginPCA].[Report].[PartnerControlGroupIDs] pcg2
	...
	-- UPDATE ControlGroupID for cases WHERE a ControlGroupID already exists for that retailer segment in that cycle 
		...
		FROM (	SELECT	DISTINCT
				...
				FROM [Warehouse].[Staging].[ControlSetup_OffersSegment_VirginPCA] s
				LEFT JOIN #PartnerAlternate pa 
					ON s.PartnerID = pa.PartnerID
				INNER JOIN [WH_VirginPCA].[Report].[IronOffer_References] ior
					ON s.IronOfferID = ior.IronOfferID
				INNER JOIN [WH_VirginPCA].[Report].[IronOfferCycles] ioc
					ON ior.IronOfferCyclesID = ioc.IronOfferCyclesID
				INNER JOIN [WH_VirginPCA].[Report].[OfferCycles] cyc
		...
		FROM [WH_VirginPCA].[Report].[PartnerControlGroupIDs] cls
		INNER JOIN #ExistingControlGroups ecg
			ON cls.PartnerID = ecg.PartnerID
			AND cls.Segment = ecg.Segment;

	/******************************************************************************
	Add new entries to [WH_VirginPCA].[Report].[IronOfferCycles]
	******************************************************************************/

		INSERT INTO [WH_VirginPCA].[Report].[IronOfferCycles]
		SELECT	a.IronOfferID
			,	oc.OfferCyclesID
			,	CASE
					WHEN a.Segment = 'B' THEN (SELECT UniversalControlGroupID FROM [Warehouse].[Staging].[ControlSetup_UniversalOffer_VirginPCA])
		...
		LEFT JOIN #PartnerAlternate pa 
			ON a.PartnerID = pa.PartnerID
		LEFT JOIN [WH_VirginPCA].[Report].[PartnerControlGroupIDs] PG
		...
		WHERE NOT EXISTS (	SELECT 1
							FROM [WH_VirginPCA].[Report].[IronOfferCycles] d
	...
	CREATE TABLE [Warehouse].[Staging].[ControlSetup_Validation_VirginPCANonAAM_IronOfferCycles] (	ID INT IDENTITY (1,1)
																							...
																							,	ControlGroupID INT
																							,	Error VARCHAR(200)
																							,	CONSTRAINT PK_ControlSetup_Validation_VirginPCANonAAM_IronOfferCycles PRIMARY KEY CLUSTERED (ID))
	...
	******************************************************************************/
	...
	-- Check row count matches rows in [Warehouse].[Staging].[ControlSetup_OffersSegment_VirginPCA] table
	...
	-- Load new ironoffercycles data
		...
		LEFT JOIN [WH_VirginPCA].[Report].[PartnerControlGroupIDs] pcg
	...
	-- Load retailer segments associated with more than 1 control group
		...
		LEFT JOIN [Warehouse].[Staging].[ControlSetup_OffersSegment_VirginPCA] seg
		...
		CROSS JOIN (SELECT MAX(UniversalControlGroupID) AS UniversalControlGroupID
					FROM [Warehouse].[Staging].[ControlSetup_UniversalOffer_VirginPCA]) u
		WHERE seg.Segment = 'B'
	...
	-- Load errors
		...
		TRUNCATE TABLE [Warehouse].[Staging].[ControlSetup_Validation_VirginPCANonAAM_IronOfferCycles];

		INSERT INTO [Warehouse].[Staging].[ControlSetup_Validation_VirginPCANonAAM_IronOfferCycles] (	PublisherType
																								,	PartnerID
																								,	Segment
																								,	IronOfferID
																								,	IronOfferName
		...
		LEFT JOIN [WH_VirginPCA].[Report].[IronOfferCycles] ioc -- Check for multiple control groups associated with the same Iron Offer in IronOfferReferences table
		...
		FROM [Warehouse].[Staging].[ControlSetup_OffersSegment_VirginPCA] seg
		WHERE NOT EXISTS (	SELECT 1
							FROM #IOCCheckData ioc
