...
******************************************************************************/
CREATE PROCEDURE [Staging].[Draft_WeeklySummaryV2_LoadCardholderCounts_New3]
AS
BEGIN
	...
	******************************************************************************/
		...
		-- Consolidate overlapping date ranges
			...
			CREATE NONCLUSTERED INDEX IX_FanDates ON #CardHolder_Warehouse (FanID) INCLUDE (CardCustStartDate, CardCustEndDate);
			CREATE NONCLUSTERED COLUMNSTORE INDEX CSX_All ON #CardHolder_Warehouse (CompositeID, FanID, CardCustStartDate, CardCustEndDate);

			IF OBJECT_ID ('tempdb..#CardHolder_Virgin') IS NOT NULL DROP TABLE #CardHolder_Virgin;
			SELECT	FanID
				,	CompositeID
				,	CardCustStartDate = MIN(ts)
				,	CardCustEndDate = MAX(ts)
			INTO #CardHolder_Virgin
			FROM (	SELECT	FanID	-- f
						,	e.CompositeID
						,	e.ts
						,	RowPair = (1 + ROW_NUMBER() OVER(PARTITION BY CompositeID ORDER BY rn))/2 
			...
			CREATE CLUSTERED INDEX CIX_CompDates ON #CardHolder_Virgin (CompositeID, CardCustStartDate, CardCustEndDate);
			CREATE NONCLUSTERED INDEX IX_FanDates ON #CardHolder_Virgin (FanID) INCLUDE (CardCustStartDate, CardCustEndDate);
			CREATE NONCLUSTERED COLUMNSTORE INDEX CSX_All ON #CardHolder_Virgin (CompositeID, FanID, CardCustStartDate, CardCustEndDate);
					...
					FROM (	SELECT	FanID	 -- e
							...
							FROM (	SELECT	FanID	-- d
									...
									FROM (	SELECT	m.FanID	-- c
											...
											CROSS APPLY (VALUES	(CardCustStartDate, 1)
															,	(CardCustEndDate, -1)) x (ts, [Type])
											WHERE m.PublisherID NOT IN (132, 138, 166)) c 
									) d
							) e 
					WHERE e.RangeEnd = 0
					OR e.RangeStart = 0) f
			...
			CREATE CLUSTERED INDEX CIX_CompDates ON #CardHolder_nFI (CompositeID, CardCustStartDate, CardCustEndDate);
			CREATE NONCLUSTERED INDEX IX_FanDates ON #CardHolder_nFI (FanID) INCLUDE (CardCustStartDate, CardCustEndDate);
			CREATE NONCLUSTERED COLUMNSTORE INDEX CSX_All ON #CardHolder_nFI (CompositeID, FanID, CardCustStartDate, CardCustEndDate);
		...
		BEGIN
			...
			*****************************/
				...
				CROSS APPLY (	SELECT	iom.CompositeID
								...
								AND @MinStartDate <= iom.StartDate
