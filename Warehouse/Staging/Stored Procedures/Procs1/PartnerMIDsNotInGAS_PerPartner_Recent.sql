CREATE Procedure [Staging].[PartnerMIDsNotInGAS_PerPartner_Recent] 
					@PartnerID int
As
Declare  @BrandID int
--Set @PartnerID = 3996
...
from Relational.Outlet as o
Where partnerid = @PartnerID and left(MerchantID,1) like '[1-9]'

Insert into #MIDs
select OutletID,right(MerchantID,len(MerchantID)-1) as MerchantID
from Relational.Outlet as o
Where partnerid = @PartnerID and left(MerchantID,1) like '[0]'
...
where TranDate >= '01 Jul 2015' and TranDate > ac.ActivatedDate
----------------------------------------------------------------------------
