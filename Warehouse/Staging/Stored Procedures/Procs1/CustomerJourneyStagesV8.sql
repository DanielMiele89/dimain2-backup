...
Create Procedure [Staging].[CustomerJourneyStagesV8]
					--@EndDate date,
...
select	--top 1000
		...
		Sum(Case
			...
			End) 
			Over (Partition By f.FanID Order By Dateadd(day,t.ActivationDays,t.[Date])) as RT,  --Running total based on cashback available (not pending)
		ROW_NUMBER() OVER(PARTITION BY f.FanID ORDER BY Dateadd(day,t.ActivationDays,t.[Date])) AS RowNo
...
into	#Redemptions        
from  slc_report.dbo.Fan as c with (nolock)
inner join slc_report.dbo.Trans as t with (nolock)
	on t.FanID = c.ID
...
Select FanID,
		Case 
			...
			when Datediff(month,AgreedTCsDate,@Today) >= 12 then CJ_Status + ' - Low Value'
			When Datediff(month,AgreedTCsDate,@Today) <= 11 and AnnualCashback >= Datediff(month,AgreedTCsDate,@Today)*1.25 then CJ_Status + ' - High Value'
...
(Select	cj.FanID,
		...
		Sum(Case
				When DateAvailable  between dateadd(day,1,dateadd(year,-1,@Today)) and @Today and
					DateAvailable >= f.AgreedTCsDate then Cashback
				Else 0
			End) AnnualCashback
from #cj3 as cj
inner join #Trans as T
...
if object_id('Warehouse.Staging.CustomerJourneyStaging') is not null drop table Warehouse.Staging.CustomerJourneyStaging
Select	FanID,
		CJ_Status as CustomerJourneyStatus,
		@Today as [Date]
Into Warehouse.Staging.CustomerJourneyStaging
From #cj3
