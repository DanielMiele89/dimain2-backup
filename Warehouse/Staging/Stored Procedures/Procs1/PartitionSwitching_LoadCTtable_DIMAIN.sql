...
*/
CREATE PROCEDURE [Staging].[PartitionSwitching_LoadCTtable_DIMAIN]
...
BEGIN
	...
	-- load the partitions one at a time from the holding table, biggest partition first
	...
	WHILE 1 = 1 BEGIN
		...
		EXEC Staging.PartitionSwitching_LoadPartitionSwitch @CurrentPartitionID, @CurrentPartitionStart, @Rows	--	PartitionSwitching_LoadPartitionSwitch_CJM

		SET @msg = 'Loaded partition ' + CAST(@CurrentPartitionID AS VARCHAR(2)) + ' [' + CAST(@Rows AS VARCHAR(10)) + ']'
		EXEC MI.ProcessLog_Insert 'PartitionSwitching_LoadCTtable', @msg
	...
	EXEC Staging.RefreshCustomerMinTrandate
