...
******************************************************************************/
CREATE PROCEDURE [Staging].[OfferReport_Fetch_IronOfferReferences] 
AS
BEGIN
    ...
    FROM (
	   ...
	   SELECT DISTINCT StartDate, EndDate
	   FROM [WH_Virgin].[Report].[IronOffer_References] ior
	   JOIN [WH_Virgin].[Report].[IronOfferCycles] ioc on ioc.IronOfferCyclesID = ior.IronOfferCyclesID
	   JOIN [WH_Virgin].[Report].[OfferCycles] oc on oc.OfferCyclesID = ioc.OfferCyclesID

	   UNION ALL

	   SELECT DISTINCT StartDate, EndDate
	   FROM [WH_VirginPCA].[Report].[IronOffer_References] ior
	   JOIN [WH_VirginPCA].[Report].[IronOfferCycles] ioc on ioc.IronOfferCyclesID = ior.IronOfferCyclesID
	   JOIN [WH_VirginPCA].[Report].[OfferCycles] oc on oc.OfferCyclesID = ioc.OfferCyclesID

	   UNION ALL

	   SELECT DISTINCT StartDate, EndDate
	   FROM [WH_Visa].[Report].[IronOffer_References] ior
	   JOIN [WH_Visa].[Report].[IronOfferCycles] ioc on ioc.IronOfferCyclesID = ior.IronOfferCyclesID
	   JOIN [WH_Visa].[Report].[OfferCycles] oc on oc.OfferCyclesID = ioc.OfferCyclesID
    ...
    SELECT DISTINCT
	   ...
	   , s.OfferTypeForReports
    FROM [WH_Virgin].[Report].[IronOffer_References] ior
    JOIN [WH_Virgin].[Report].[IronOfferCycles] ioc 
	   ON ioc.IronOfferCyclesID = ior.IronOfferCyclesID
    JOIN nFI.Relational.ControlGroupType cgt
	   ON cgt.ControlGroupTypeID = 0
    JOIN [WH_Virgin].[Report].[OfferCycles] oc 
	   ON oc.OfferCyclesID = ioc.OfferCyclesID
    JOIN Warehouse.Staging.OfferReport_OfferCycles ox
	   ON ox.StartDate = cast(oc.StartDate as date)
	   AND ox.EndDate = cast(oc.EndDate as date)
    JOIN [WH_Virgin].[Derived].[IronOffer] [io] 
	   ON [io].IronOfferID = ior.IronOfferID
    --LEFT JOIN nFI.Segmentation.ROC_Shopper_Segment_Types rss 
	   --ON rss.ID = ior.ShopperSegmentTypeID
    --LEFT JOIN nfI.Segmentation.ROC_Shopper_Segment_Super_Types rst 
	...
	LEFT JOIN Warehouse.Relational.IronOfferSegment s
		ON ior.IronOfferID = s.IronOfferID
	LEFT JOIN (SELECT IronOfferID, MAX(ClientServicesRef) AS ClientServicesRef FROM WH_Virgin.Derived.IronOffer_Campaign_HTM GROUP BY IronOfferID) htm
		ON ior.IronOfferID = htm.IronOfferID
    UNION ALL
    ---------------------------------------------------------------------------
    -- Virgin Other
    ...
    SELECT DISTINCT
	   ...
	   , s.OfferTypeForReports
    FROM [WH_Visa].[Report].[IronOffer_References] ior
    JOIN [WH_Visa].[Report].[IronOfferCycles] ioc 
	   ON ioc.IronOfferCyclesID = ior.IronOfferCyclesID
    JOIN nFI.Relational.ControlGroupType cgt
	   ON cgt.ControlGroupTypeID = 0
    JOIN [WH_Visa].[Report].[OfferCycles] oc 
	   ON oc.OfferCyclesID = ioc.OfferCyclesID
    JOIN Warehouse.Staging.OfferReport_OfferCycles ox
	   ON ox.StartDate = cast(oc.StartDate as date)
	   AND ox.EndDate = cast(oc.EndDate as date)
    JOIN [WH_Visa].[Derived].[IronOffer] [io] 
	   ON [io].IronOfferID = ior.IronOfferID
    --LEFT JOIN nFI.Segmentation.ROC_Shopper_Segment_Types rss 
	   --ON rss.ID = ior.ShopperSegmentTypeID
    --LEFT JOIN nfI.Segmentation.ROC_Shopper_Segment_Super_Types rst 
	...
	LEFT JOIN Warehouse.Relational.IronOfferSegment s
		ON ior.IronOfferID = s.IronOfferID
	LEFT JOIN (SELECT IronOfferID, MAX(ClientServicesRef) AS ClientServicesRef FROM WH_Visa.Derived.IronOffer_Campaign_HTM GROUP BY IronOfferID) htm
		ON ior.IronOfferID = htm.IronOfferID
    UNION ALL
    ---------------------------------------------------------------------------
    -- Visa Barclaycard Other
    ---------------------------------------------------------------------------
    SELECT DISTINCT
	   ior.IronOfferID
	   , ior.ClubID
    ...
    SELECT DISTINCT
	   ...
	   , s.OfferTypeForReports
    FROM [WH_VirginPCA].[Report].[IronOffer_References] ior
    JOIN [WH_VirginPCA].[Report].[IronOfferCycles] ioc 
	   ON ioc.IronOfferCyclesID = ior.IronOfferCyclesID
    JOIN nFI.Relational.ControlGroupType cgt
	   ON cgt.ControlGroupTypeID = 0
    JOIN [WH_VirginPCA].[Report].[OfferCycles] oc 
	   ON oc.OfferCyclesID = ioc.OfferCyclesID
    JOIN Warehouse.Staging.OfferReport_OfferCycles ox
	   ON ox.StartDate = cast(oc.StartDate as date)
	   AND ox.EndDate = cast(oc.EndDate as date)
    JOIN [WH_VirginPCA].[Derived].[IronOffer] [io] 
	   ON [io].IronOfferID = ior.IronOfferID
    --LEFT JOIN nFI.Segmentation.ROC_Shopper_Segment_Types rss 
	   --ON rss.ID = ior.ShopperSegmentTypeID
    --LEFT JOIN nfI.Segmentation.ROC_Shopper_Segment_Super_Types rst 
	...
	LEFT JOIN Warehouse.Relational.IronOfferSegment s
		ON ior.IronOfferID = s.IronOfferID
	LEFT JOIN (SELECT IronOfferID, MAX(ClientServicesRef) AS ClientServicesRef FROM [WH_VirginPCA].Derived.IronOffer_Campaign_HTM GROUP BY IronOfferID) htm
		ON ior.IronOfferID = htm.IronOfferID
    UNION ALL
    ---------------------------------------------------------------------------
    -- Virgin PCA Other
