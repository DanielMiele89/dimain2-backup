CREATE Procedure [Staging].[SSRS_R0145_PartnerMIDsNotInGAS_ForMID] 
					(@MID varchar(25),@PartnerID int)
As
--Declare  @BrandID int
--Set @PartnerID = 3996
...
Select Count(*) as ActivatedCustomers From #AC
----------------------------------------------------------------------------
...
where TranDate >= Dateadd(Month,-6,getdate()) and TranDate > ac.ActivatedDate

Select Count(*) as TransactionsforActivatedCustomers From #ActivatedTrans
----------------------------------------------------------------------------
...
Where LastTran >= dateadd(week,-6,Getdate()) and a.Narrative not Like 'PayPal%' and replace(a.LocationCountry,' ','') = 'GB'
