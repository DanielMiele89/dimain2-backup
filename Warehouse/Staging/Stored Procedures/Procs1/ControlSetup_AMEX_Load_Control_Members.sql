...
******************************************************************************/
CREATE PROCEDURE [Staging].[ControlSetup_AMEX_Load_Control_Members]
...
BEGIN
	...
	******************************************************************************/
	...
	SELECT	DISTINCT
		...
		,	m.RetailerID
		,	'Sandbox.'+System_User+'.Control_' + COALESCE(CONVERT(VARCHAR(5), p.PartnerID), 'Universal') + '_' + CONVERT(VARCHAR(10), COALESCE(p.StartDate, @SDate), 112) as TableName
		,	TargetAudience
		,	CONVERT(VARCHAR(10), COALESCE(p.StartDate, @SDate), 112) as StartDate
	...
	LEFT JOIN (	SELECT DISTINCT PartnerID, StartDate FROM [Warehouse].[Staging].[ControlSetup_PartnersToSeg_nFI]
				UNION 
				SELECT DISTINCT PartnerID, StartDate FROM [Warehouse].[Staging].[ControlSetup_PartnersToSeg_Warehouse]) p
		ON m.RetailerID = p.PartnerID
	WHERE pa.Matcher NOT IN (52)
	AND NOT EXISTS (SELECT 1
					FROM [nFI].[Relational].[AmexControlGroupMembers] c
					WHERE a.AmexControlGroupID = c.AmexControlgroupID)
	AND EXISTS (	SELECT 1
					FROM [Staging].[Partners_IncFuture] pif
					WHERE m.RetailerID = pif.PartnerID)
	...
	******************************************************************************/
	...
	BEGIN
		...
		BEGIN
			...
			SET @Qry = (
				'EXEC '	+	CASE
								WHEN @RetailerID IN (SELECT PartnerID FROM #MFDDPartners) THEN @MFDDSegmentStoredProcedure
								ELSE @CLOSegmentStoredProcedure
							END
						+	' '
						+	COALESCE(CONVERT(VARCHAR(5), @RetailerID), 'NULL')
						+	','''
	...
	******************************************************************************/
	...
	From (	
		Select distinct
				...
				+ ' Where SegmentID '
				+ Case
					When s.SegmentCode = 'SR' then 'in (6, 9)'
					When s.SegmentCode = 'SG' then 'in (5, 9)'
					When s.SegmentCode = 'S' then 'in (5, 6, 9)'
					When s.SegmentCode = 'L' then 'in (3, 4, 8)'
					When s.SegmentCode = 'A' then 'in (1, 2, 7)'
					Else ' > 0'
			End AS EXCcode
