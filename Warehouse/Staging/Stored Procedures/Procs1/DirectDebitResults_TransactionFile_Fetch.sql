...
CREATE PROCEDURE [Staging].[DirectDebitResults_TransactionFile_Fetch] (@RetailerID INT, @PeriodType VARCHAR(10))
...
BEGIN
	...
	******************************************************************************/
	...
	---- For testing 
	--DECLARE @RetailerID int = 4846; -- Hearst primary PartnerID
	--DECLARE @PeriodType VARCHAR(10) = 'Monthly'; -- Hearst primary PartnerID
	DECLARE @DateFirst int = 1; -- For setting Friday as the first day of the week
	DECLARE @MinStartDate date = '2020-11-19'; -- Hearst go live date
	...
	******************************************************************************/
	...
	DECLARE @PastMonthsToCalculate int = 200; -- Weeks over which to analyse. Weeks before this should not change, so can be taken from previous calculations
	...
	FROM (
		SELECT	DISTINCT
				CASE 
					WHEN DATEFROMPARTS(YEAR(CalDate), MONTH(CalDate), 1) < @MinStartDate THEN @MinStartDate -- Don't let StartDate go before analysis start date
					ELSE DATEFROMPARTS(YEAR(CalDate), MONTH(CalDate), 1)
				END	AS StartDate -- For each calendar date in #Dates, minus days since the most recent Monday  
			,	CASE
					WHEN @MaxEndDate < EOMONTH(CalDate) THEN @MaxEndDate
					ELSE EOMONTH(CalDate)
				END AS EndDate -- For each calendar date in #Dates, minus days since the most recent Sunday
			,	'Monthly' AS PeriodType
		FROM TallyDates
		WHERE CalDate >= (DATEADD(month, - @PastMonthsToCalculate, @Today))
		AND CalDate < DATEFROMPARTS(YEAR(GETDATE()), MONTH(GETDATE()), 1)
		 -- Only include the x most recent complete weeks, where x is the number of weeks in the Incentivised DD tracking window. Within this window, customers can change CustomerGroup
		--	AND DATEADD(dd, -(DATEPART(dw, CalDate)-1)+6, CalDate) <= CalDate -- Only include complete weeks
	) x

	DELETE ca
	FROM #Calendar ca
	WHERE PeriodType NOT IN (@PeriodType, 'Cumulative')
	...
	******************************************************************************/
		...
		SET @EmailRecipients = (SELECT	CASE
											WHEN @RetailerID = 4846 THEN 'DIProcessCheckers@RewardInsight.com;Matthew.OConnell@RewardInsight.com;Emma.Young@RewardInsight.com;leah.warren@rewardinsight.com'
