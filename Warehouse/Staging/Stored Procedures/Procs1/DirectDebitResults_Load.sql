...
Rory Francis 13/05/2021
	- Adapted joins to #Rules temp table to cope with OINs being linked to multiple Iron Offers 
	
******************************************************************************/

CREATE PROCEDURE [Staging].[DirectDebitResults_Load] (@RetailerID int, @PeriodType VARCHAR(10), @DateFirst int, @MinStartDate date, @SplitIronOfferTargeting bit)
...
BEGIN
	...
	******************************************************************************/
	...
	---- For testing 
	--DECLARE @RetailerID int = 4913; -- Sky primary PartnerID
	--DECLARE @PeriodType VARCHAR(10) = 'Weekly'; -- Weekly / Monthly
	--DECLARE @DateFirst int = 5; -- For setting Friday as the first day of the week
	--DECLARE @MinStartDate date = '2021-08-12'; -- Sky go live date
	...
	******************************************************************************/
	...
	IF @PastWeeksToCalculate = 0
		BEGIN
			SET @PastWeeksToCalculate = 4
		END
	
	DECLARE @PastMonthsToCalculate int = 2; -- Weeks over which to analyse. Weeks before this should not change, so can be taken from previous calculations
