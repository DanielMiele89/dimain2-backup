...
Created: 15/03/2018
Purpose:
	- Load new entries INTO [WH_Visa].[Report].[IronOffer_References] table
...
******************************************************************************/
CREATE PROCEDURE [Staging].[ControlSetup_VisaBarclaycardNonAAM_Load_IronOffer_References]
...
BEGIN
	...
	******************************************************************************/
	...
	LEFT JOIN [WH_Visa].[Report].[OfferCycles] oc
	...
	******************************************************************************/
	...
	If  object_id ('tempdb..#CashbackRate') IS NOT NULL DROP TABLE #CashbackRate;
	...
	/******************************************************************************
	Generate entries to be added to [WH_Visa].[Report].[IronOffer_References] table
	******************************************************************************/

	IF OBJECT_ID ('tempdb..#IronOffer_Ref_VisaBarclaycard') IS NOT NULL DROP TABLE #IronOffer_Ref_VisaBarclaycard;
	...
	INTO #IronOffer_Ref_VisaBarclaycard
	FROM [WH_Visa].[Report].[IronOfferCycles] ioc
	INNER JOIN [WH_Visa].[Derived].[IronOffer] i
		on ioc.ironofferid = i.IronOfferID
	INNER JOIN #CashbackRate cb
	...
	******************************************************************************/
	...
	INSERT INTO [WH_Visa].[Report].[IronOffer_References]
	Select
		i.*
	FROM #IronOffer_Ref_VisaBarclaycard i
	LEFT JOIN [WH_Visa].[Report].[IronOffer_References] a
		on i.IronOfferCyclesID = a.IronOfferCyclesID
	WHERE
		a.IronOfferCyclesID IS NULL;

	/******************************************************************************
	CHECK POINT: Validate entries added to [WH_Visa].[Report].[IronOffer_References] table

	CREATE TABLE for storing validation results

	CREATE TABLE [Warehouse].[Staging].[ControlSetup_Validation_VisaBarclaycardNonAAM_IronOffer_References]
		...
		, CONSTRAINT PK_ControlSetup_Validation_VisaBarclaycardNonAAM_IronOffer_References PRIMARY KEY CLUSTERED (IronOfferID, StartDate, EndDate)  
		)
	******************************************************************************/
	...
	Select
		...
		, o.IronOfferName
	INTO #IORCheckData
	FROM #IronOffer_Ref_VisaBarclaycard AS i
	LEFT JOIN [WH_Visa].[Report].[IronOffer_References] AS a
		on i.IronOfferCyclesID = a.IronOfferCyclesID
	LEFT JOIN [WH_Visa].[Report].[IronOfferCycles] AS ioc
		on i.IronOfferCyclesID = ioc.IronOfferCyclesID
	LEFT JOIN [WH_Visa].[Report].[OfferCycles] oc
		on ioc.OfferCyclesID = oc.OfferCyclesID
	INNER JOIN [WH_Visa].[Derived].[IronOffer] AS o
	...
	TRUNCATE TABLE [Warehouse].[Staging].[ControlSetup_Validation_VisaBarclaycardNonAAM_IronOffer_References];

	INSERT INTO [Warehouse].[Staging].[ControlSetup_Validation_VisaBarclaycardNonAAM_IronOffer_References]
		(PublisherType
		, IronOfferID
		, StartDate
		, EndDate
	...
	WHERE
		...
		or (d.SegmentName LIKE ('%Lapsed%') AND d.TypeDescription LIKE ('%ShopperSegment%') AND d.IronOfferName not LIKE ('%Lapsed%') AND d.IronOfferName not LIKE ('%Lasped%') AND d.IronOfferName not LIKE ('%Winback%'))
		or (d.SegmentName LIKE ('%Retain%') AND d.TypeDescription LIKE ('%ShopperSegment%') AND d.IronOfferName not LIKE ('%Retain%'))
		or (d.SegmentName LIKE ('%Shopper%') AND d.TypeDescription LIKE ('%ShopperSegment%') AND d.IronOfferName not LIKE ('%Shopper%') AND d.IronOfferName not LIKE ('%Nursery%') AND d.IronOfferName not LIKE ('%Lapsing%') AND d.IronOfferName not LIKE ('%Retain&Grow%'))
