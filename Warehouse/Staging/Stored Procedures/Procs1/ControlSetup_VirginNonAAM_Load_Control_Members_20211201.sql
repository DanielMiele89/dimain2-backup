...
Created: 09/03/2018
Purpose:
	- Load control group members INTO [WH_Virgin].[Report].[ControlGroupMembers]
...
******************************************************************************/
CREATE PROCEDURE [Staging].[ControlSetup_VirginNonAAM_Load_Control_Members_20211201]
AS
BEGIN
	...
	******************************************************************************/
		...
		FROM (	SELECT	pts.ControlGroupID
				...
				FROM [Warehouse].[Staging].[ControlSetup_PartnersToSeg_Virgin] pts) im -- OR #Missing if running manually for nFI control groups missing members
...
INSERT INTO [WH_Virgin].[Report].[ControlGroupMembers]
SELECT	TOP (950000)
		'
		...
		FROM (	SELECT	DISTINCT
				...
				WHERE NOT EXISTS (	SELECT 1
									FROM [WH_Virgin].[Report].[ControlGroupMembers] cgm
									WHERE ControlGroupID_Var = cgm.ControlGroupID)) ic
		WHERE ic.ControlRowNum = 1;

	/******************************************************************************
	Load nFI control group members INTO [WH_Virgin].[Report].[ControlGroupMembers]
	******************************************************************************/
		...
		BEGIN
			...
			IF EXISTS (SELECT * FROM [WH_Virgin].[Report].[ControlGroupMembers] WHERE ControlGroupID = @ControlGroupID)
	...
	/******************************************************************************
	Load Universal control group members INTO [WH_Virgin].[Report].[ControlGroupMembers]
	******************************************************************************/
		...
		DECLARE @ControlGroupID_Universal NVARCHAR(MAX) = (SELECT UniversalControlGroupID FROM [Warehouse].[Staging].[ControlSetup_UniversalOffer_Virgin]);
...
SELECT	TOP (950000)
	...
	CREATE TABLE [Warehouse].[Staging].[ControlSetup_Validation_VirginNonAAM_Control_Members] (	PublisherType VARCHAR(50)
																							,	PartnerID INT
																							,	ControlGroupID INT
																							...
																							,	CONSTRAINT PK_ControlSetup_Validation_VirginNonAAM_Control_Members PRIMARY KEY CLUSTERED (IronOfferCyclesID))
	...
	******************************************************************************/
	...
	SELECT	DISTINCT 
		...
		,	d.EndDate
	INTO #OCs
	FROM [Warehouse].[Staging].[ControlSetup_OffersSegment_Virgin] AS d
	...
	TRUNCATE TABLE [Warehouse].[Staging].[ControlSetup_Validation_VirginNonAAM_Control_Members];

	INSERT INTO [Warehouse].[Staging].[ControlSetup_Validation_VirginNonAAM_Control_Members] (	PublisherType
																							,	PartnerID
																							,	ControlGroupID
																							,	PartnerName
																							,	IronOfferName
	...
	INNER JOIN [WH_Virgin].[Derived].[Partner] pa
