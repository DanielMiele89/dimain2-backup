...
Purpose: 
	...
	- Load offer segments INTO [Warehouse].[Staging].[ControlSetup_OffersSegment_VisaBarclaycard] table
...
******************************************************************************/
CREATE PROCEDURE [Staging].[ControlSetup_VisaBarclaycardNonAAM_Load_Segments] (	@OnlyRunForFlashReportRetailers BIT
...
BEGIN
	...
	******************************************************************************/
		...
		IF OBJECT_ID('tempdb..#OffersWithMembersVisaBarclaycard') IS NOT NULL DROP TABLE #OffersWithMembersVisaBarclaycard
		SELECT	o.IronOfferID
			...
			,	o.ClubID
			,	o.PartnerName
		INTO #OffersWithMembersVisaBarclaycard
		FROM #Offers o
	...
	******************************************************************************/
		...
		INSERT INTO [WH_Visa].[Report].[OfferCycles]
		SELECT	DISTINCT
				owm.StartDate
			,	owm.Enddate 
		FROM #OffersWithMembersVisaBarclaycard owm
		WHERE NOT EXISTS (	SELECT 1
							FROM [WH_Visa].[Report].[OfferCycles] oc
	...
	- Load results INTO [Warehouse].[Staging].[ControlSetup_OffersSegment_VisaBarclaycard] table

	CREATE TABLE for storing results:

	CREATE TABLE [Warehouse].[Staging].[ControlSetup_OffersSegment_VisaBarclaycard] (IronOfferID INT
																		...
																		,	Segment VARCHAR(50)
																		,	CONSTRAINT PK_ControlSetup_OffersSegment_VisaBarclaycard PRIMARY KEY CLUSTERED (IronOfferID, StartDate, EndDate))

	******************************************************************************/
	 
		TRUNCATE TABLE [Warehouse].[Staging].[ControlSetup_OffersSegment_VisaBarclaycard];
		INSERT INTO [Warehouse].[Staging].[ControlSetup_OffersSegment_VisaBarclaycard] (IronOfferID
																					,	IronOfferName
																					,	StartDate
																					,	EndDate
																					,	PartnerID
		...
		FROM #OffersWithMembersVisaBarclaycard o
	...
	--	to prevent a second control group being sest up for the same retailer / segment
		...
		FROM [Warehouse].[Staging].[ControlSetup_OffersSegment_VisaBarclaycard] os
		...
		INNER JOIN [WH_Visa].[Report].[OfferCycles] oc2
	...
	CREATE TABLE [Warehouse].[Staging].[ControlSetup_Validation_VisaBarclaycardNonAAM_Segments] (	PublisherType VARCHAR(50)
																			,	IronOfferID INT
																			,	IronOfferName NVARCHAR(200)
																			,	StartDate DATE
																			,	EndDate DATE
																			...
																			,	Segment VARCHAR(10)
																			,	CONSTRAINT PK_ControlSetup_Validation_VisaBarclaycardNonAAM_Segments PRIMARY KEY CLUSTERED (IronOfferID, StartDate, EndDate))

	******************************************************************************/

	-- Load errors

	TRUNCATE TABLE [Warehouse].[Staging].[ControlSetup_Validation_VisaBarclaycardNonAAM_Segments];
	INSERT INTO [Warehouse].[Staging].[ControlSetup_Validation_VisaBarclaycardNonAAM_Segments] (	PublisherType  
																					,	IronOfferID
																					,	IronOfferName
																					,	StartDate
																					,	EndDate
	...
	SELECT	'Visa B' AS PublisherType  
		...
		,	d.PartnerName
		,	d.Segment
	FROM [Warehouse].[Staging].[ControlSetup_OffersSegment_VisaBarclaycard] d
	WHERE Segment = ''
	OR Segment IS NULL;
