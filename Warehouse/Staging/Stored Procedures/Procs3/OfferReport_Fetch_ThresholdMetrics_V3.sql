...
******************************************************************************/
CREATE PROCEDURE [Staging].[OfferReport_Fetch_ThresholdMetrics_V3] (
    @ID INT = NULL
)
AS
...
BEGIN
	...
	--	AND cc.IsWarehouse = ct.IsWarehouse
		...
		AND cc.IsVirginPCA = ct.IsVirginPCA
		AND cc.IsVisaBarclaycard = ct.IsVisaBarclaycard
	...
	SELECT	ao.PartnerID
		...
		,	ao.PublisherID
		,	cu.Exposed
		,	COALESCE(cu.CINID_Warehouse, cu.CINID_Virgin, cu.CINID_VisaBarclaycard, cu.IsVirginPCA) AS CINID
	INTO #OfferReport_CTCustomers
	FROM #OfferReport_AllOffers ao
	INNER JOIN [Staging].[OfferReport_CTCustomers] cu
        ON cu.PublisherID = ao.PublisherID
        AND cu.isWarehouse = ao.isWarehouse
		AND cu.IsVirgin = ao.IsVirgin
		AND cu.IsVirginPCA = ao.IsVirginPCA
		AND cu.IsVisaBarclaycard = ao.IsVisaBarclaycard
		AND cu.GroupID = ao.ControlGroupID
		AND Exposed = 0
	WHERE EXISTS (	SELECT 1
					FROM #OfferReport_ConsumerTransaction ct
					WHERE COALESCE(cu.CINID_Warehouse, cu.CINID_Virgin, cu.CINID_VisaBarclaycard, cu.IsVirginPCA) = ct.CINID
    ...
    CROSS APPLY (	SELECT	SUM(ct.Amount) Sales
				...
				--	AND cu.isWarehouse = ct.IsWarehouse
					...
					AND cu.IsVirginPCA = ct.IsVirginPCA
					AND cu.IsVisaBarclaycard = ct.IsVisaBarclaycard
