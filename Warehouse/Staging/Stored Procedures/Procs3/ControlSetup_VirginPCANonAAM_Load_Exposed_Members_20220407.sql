...
Created: 09/03/2018
Purpose: 
	- Add exposed members to [WH_VirginPCA].[Report].[CampaignHistory] for new IronOfferCyclesIDs in nFI.Relational.ironoffercycles
...
******************************************************************************/
CREATE PROCEDURE [Staging].[ControlSetup_VirginPCANonAAM_Load_Exposed_Members_20220407]
...
BEGIN
	...
	/******************************************************************************
	Add exposed members to [WH_VirginPCA].[Report].[CampaignHistory] for new IronOfferCyclesIDs added to nFI.Relational.ironoffercycles
	******************************************************************************/

	-- Declare iteration variables

	Declare @IOCID int = (Select COALESCE(Max(IronOfferCyclesID), 0) From [WH_VirginPCA].[Report].[CampaignHistory])+1;
	Declare @IOCID_Max int = (Select Max(IronOfferCyclesID) From [WH_VirginPCA].[Report].[IronOfferCycles]);
	...
	Begin
		...
		If not exists (Select null from [WH_VirginPCA].[Report].[CampaignHistory] where IronOfferCyclesID = @IOCID)
		...
		Begin
			...
			Insert into [WH_VirginPCA].[Report].[CampaignHistory]
			Select
				s.ironoffercyclesid
				, s.FanID
