...
Create Procedure Staging.SmartFocusPostUploadAssessment
...
Where Left(cj.ShortCode,2) <> Left(d.CustomerJourneyStatus,2) and cj.EndDate is null
) as a
Where RowNo <= 3
...
(select rbs.[Customer ID],
		Max(Case
			When Cast(rbs.Offer1 as int) = lsc.IronOfferID and OfferSlot = 1 then ''Yes''
			Else ''No''
		End) as Offer1,
		Max(Case
			When Cast(rbs.Offer2 as int) = lsc.IronOfferID and OfferSlot = 2 then ''Yes''
			Else ''No''
		End) as Offer2,
		Max(Case
			When Cast(rbs.Offer3 as int) = lsc.IronOfferID and OfferSlot = 3 then ''Yes''
			Else ''No''
		End) as Offer3,
		Max(Case
			When Cast(rbs.Offer4 as int) = lsc.IronOfferID and OfferSlot = 4 then ''Yes''
			Else ''No''
		End) as Offer4,
		Max(Case
			When Cast(rbs.Offer5 as int) = lsc.IronOfferID and OfferSlot = 5 then ''Yes''
			Else ''No''
		End) as Offer5,
		Max(Case
			When Cast(rbs.Offer6 as int) = lsc.IronOfferID and OfferSlot = 6 then ''Yes''
			Else ''No''
		End) as Offer6,
		Max(Case
			When Cast(rbs.Offer7 as int) = lsc.IronOfferID and OfferSlot = 7 then ''Yes''
			Else ''No''
		End) as Offer7
from ' + @TableName + ' as rbs
inner join warehouse.relational.lionsendcomponent as lsc
	on	rbs.[Customer id] = lsc.fanid and
