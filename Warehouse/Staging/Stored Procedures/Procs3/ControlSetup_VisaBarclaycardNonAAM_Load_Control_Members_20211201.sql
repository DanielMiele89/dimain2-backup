...
Created: 09/03/2018
Purpose:
	- Load control group members INTO [WH_Visa].[Report].[ControlGroupMembers]
...
******************************************************************************/
CREATE PROCEDURE [Staging].[ControlSetup_VisaBarclaycardNonAAM_Load_Control_Members_20211201]
AS
BEGIN
	...
	******************************************************************************/
		...
		FROM (	SELECT	pts.ControlGroupID
					,	pts.PartnerID
					,	pts.Segment
					,	'Control_' + CONVERT(VARCHAR(5), pts.PartnerID) + '_' + CONVERT(VARCHAR(10), pts.StartDate, 112) AS TableName
				FROM [Warehouse].[Staging].[ControlSetup_PartnersToSeg_VisaBarclaycard] pts) im -- OR #Missing if running manually for nFI control groups missing members
...
INSERT INTO [WH_Visa].[Report].[ControlGroupMembers]
SELECT	TOP (950000)
		'
		...
		FROM (	SELECT	DISTINCT
						CASE 
							...
							WHEN i.Segment = 'A' THEN @InsertQry + ControlGroupID_Var + ' AS ControlGroupID' + CHAR(10) + '		,	FanID' + CHAR(10) + 'FROM Sandbox.' + SYSTEM_USER + '.' + i.TableName + CHAR(10) + 'WHERE SegmentID IN (7)' + CHAR(10) + 'ORDER BY ABS(CHECKSUM(NEWID()))'
							WHEN i.Segment = 'L' THEN @InsertQry + ControlGroupID_Var + ' AS ControlGroupID' + CHAR(10) + '		,	FanID' + CHAR(10) + 'FROM Sandbox.' + SYSTEM_USER + '.' + i.TableName + CHAR(10) + 'WHERE SegmentID IN (8)' + CHAR(10) + 'ORDER BY ABS(CHECKSUM(NEWID()))'
							ELSE @InsertQry + ControlGroupID_Var + ' AS ControlGroupID' + CHAR(10) + '		,	FanID' + CHAR(10) + 'FROM Sandbox.' + SYSTEM_USER + '.' + i.TableName + CHAR(10) + 'WHERE SegmentID IN (9)' + CHAR(10) + 'ORDER BY ABS(CHECKSUM(NEWID()))'
						END	AS ImportCode
					,	ROW_NUMBER() OVER (PARTITION BY i.ControlGroupID ORDER BY i.TableName Desc) AS ControlRowNum
					,	i.ControlGroupID
				FROM #Imports i
				WHERE NOT EXISTS (	SELECT 1
									FROM [WH_Visa].[Report].[ControlGroupMembers] cgm
									WHERE ControlGroupID_Var = cgm.ControlGroupID)) ic
		WHERE ic.ControlRowNum = 1;

	/******************************************************************************
	Load nFI control group members INTO [WH_Visa].[Report].[ControlGroupMembers]
	******************************************************************************/
		...
		BEGIN
			...
			IF EXISTS (SELECT * FROM [WH_Visa].[Report].[ControlGroupMembers] WHERE ControlGroupID = @ControlGroupID)
	...
	/******************************************************************************
	Load Universal control group members INTO [WH_Visa].[Report].[ControlGroupMembers]
	******************************************************************************/
		...
		FROM (	SELECT	pts.ControlGroupID
					,	pts.PartnerID
					,	pts.Segment
					,	'Control' + CONVERT(VARCHAR(5), pts.PartnerID) + '_' + CONVERT(VARCHAR(10), pts.StartDate, 112) AS TableName
				FROM [Warehouse].[Staging].[ControlSetup_PartnersToSeg_VisaBarclaycard] pts) im -- OR #Missing if running manually for nFI control groups missing members
		INNER JOIN [Sandbox].[sys].[tables] t
			ON t.name = im.TableName
		INNER JOIN [Sandbox].[sys].[schemas] s
		...
		DECLARE @ControlGroupID_Universal NVARCHAR(MAX) = (SELECT UniversalControlGroupID FROM [Warehouse].[Staging].[ControlSetup_UniversalOffer_VisaBarclaycard]);
...
SELECT	TOP (950000)
		...
		IF NOT EXISTS (	SELECT * 
						FROM [WH_Visa].[Report].[ControlGroupMembers]
	...
	CREATE TABLE [Warehouse].[Staging].[ControlSetup_Validation_VisaBarclaycardNonAAM_Control_Members] (	PublisherType VARCHAR(50)
																							,	PartnerID INT
																							,	ControlGroupID INT
																							...
																							,	CONSTRAINT PK_ControlSetup_Validation_VisaBarclaycardNonAAM_Control_Members PRIMARY KEY CLUSTERED (IronOfferCyclesID))
	...
	******************************************************************************/
	...
	SELECT	DISTINCT 
		...
		,	d.EndDate
	INTO #OCs
	FROM [Warehouse].[Staging].[ControlSetup_OffersSegment_VisaBarclaycard] AS d
	INNER JOIN [WH_Visa].[Report].[OfferCycles] oc
		ON CONVERT(DATE, d.StartDate) = CONVERT(DATE, oc.StartDate)
		AND CONVERT(DATE, d.EndDate) = CONVERT(DATE, oc.EndDate);

	-- Load errors

	TRUNCATE TABLE [Warehouse].[Staging].[ControlSetup_Validation_VisaBarclaycardNonAAM_Control_Members];

	INSERT INTO [Warehouse].[Staging].[ControlSetup_Validation_VisaBarclaycardNonAAM_Control_Members] (	PublisherType
																							,	PartnerID
																							,	ControlGroupID
																							,	PartnerName
																							,	IronOfferName
																							...
																							,	IronOfferCyclesID)
	SELECT	DISTINCT
			'VisaBarclaycard' AS PublisherType
		,	iof.PartnerID
		,	ioc.ControlGroupID
		,	pa.PartnerName
		,	iof.IronOfferName
	...
	INNER JOIN [WH_Visa].[Derived].[Partner] pa
