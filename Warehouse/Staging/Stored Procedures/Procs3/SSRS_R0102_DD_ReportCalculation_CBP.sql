...
-- ***************************************************************************
CREATE PROCEDURE [Staging].[SSRS_R0102_DD_ReportCalculation_CBP]
...
SELECT	StoredProcedureName = 'SSRS_R0102_DD_ReportCalculation_CBP',
	TableSchemaName = 'Staging',
	TableName = 'R_0102_DD_DataTable_CBP',
	StartDate = GETDATE(),
	EndDate = NULL,
	TableRowCount  = NULL,
	AppendReload = 'R'
...
BEGIN
	...
	SELECT	dd.FileID,
		...
		dd.Date as TranDate,
		o.SupplierID
	FROM Archive_Light.dbo.CBP_DirectDebit_TransactionHistory dd (NOLOCK)
	INNER JOIN Warehouse.Relational.Customer c (NOLOCK) --Only CBP Customers
		ON dd.SourceUID = c.SourceUID
...
INNER JOIN Warehouse.Relational.DirectDebit_OINs ddo
	ON dd.OIN = ddo.OIN
	AND dd.SupplierID = ddo.SupplierID
...
**********************************************************/
IF OBJECT_ID ('Warehouse.Staging.R_0102_DD_DataTable_CBP') IS NOT NULL DROP TABLE Warehouse.Staging.R_0102_DD_DataTable_CBP
SELECT	StartOfMonth,
	...
	Transactions,
	Customers
INTO Warehouse.Staging.R_0102_DD_DataTable_CBP
FROM #Summary s
...
SET EndDate = GETDATE()
WHERE	StoredProcedureName = 'SSRS_R0102_DD_ReportCalculation_CBP' 
	AND TableSchemaName = 'Staging'
	AND TableName = 'R_0102_DD_DataTable_CBP' 
	AND EndDate IS NULL
...
UPDATE Staging.JobLog_Temp
SET TableRowCount = (SELECT COUNT(1) FROM Warehouse.Staging.R_0102_DD_DataTable_CBP)
WHERE	StoredProcedureName = 'SSRS_R0102_DD_ReportCalculation_CBP'
	AND TableSchemaName = 'Staging'
	AND TableName = 'R_0102_DD_DataTable_CBP' 
