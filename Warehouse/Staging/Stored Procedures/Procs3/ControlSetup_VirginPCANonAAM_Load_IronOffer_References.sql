...
Created: 15/03/2018
Purpose:
	- Load new entries INTO [WH_VirginPCA].[Report].[IronOffer_References] table
...
******************************************************************************/
CREATE PROCEDURE [Staging].[ControlSetup_VirginPCANonAAM_Load_IronOffer_References]
...
BEGIN
	...
	******************************************************************************/
	...
	LEFT JOIN [WH_VirginPCA].[Report].[OfferCycles] oc
	...
	******************************************************************************/
	...
	/******************************************************************************
	Generate entries to be added to [WH_VirginPCA].[Report].[IronOffer_References] table
	******************************************************************************/

	IF OBJECT_ID ('tempdb..#IronOffer_Ref_VirginPCA') IS NOT NULL DROP TABLE #IronOffer_Ref_VirginPCA;
	...
	INTO #IronOffer_Ref_VirginPCA
	FROM [WH_VirginPCA].[Report].[IronOfferCycles] ioc
	INNER JOIN [WH_VirginPCA].[Derived].[IronOffer] i
	...
	******************************************************************************/
	...
	INSERT INTO [WH_VirginPCA].[Report].[IronOffer_References]
	Select
		i.*
	FROM #IronOffer_Ref_VirginPCA i
	LEFT JOIN [WH_VirginPCA].[Report].[IronOffer_References] a
		on i.IronOfferCyclesID = a.IronOfferCyclesID
	WHERE
		a.IronOfferCyclesID IS NULL;

	/******************************************************************************
	CHECK POINT: Validate entries added to [WH_VirginPCA].[Report].[IronOffer_References] table

	CREATE TABLE for storing validation results

	CREATE TABLE [Warehouse].[Staging].[ControlSetup_Validation_VirginPCANonAAM_IronOffer_References]
		...
		, CONSTRAINT PK_ControlSetup_Validation_VirginPCANonAAM_IronOffer_References PRIMARY KEY CLUSTERED (IronOfferID, StartDate, EndDate)  
		)
	******************************************************************************/
	...
	Select
		...
		, o.IronOfferName
	INTO #IORCheckData
	FROM #IronOffer_Ref_VirginPCA AS i
	LEFT JOIN [WH_VirginPCA].[Report].[IronOffer_References] AS a
		on i.IronOfferCyclesID = a.IronOfferCyclesID
	LEFT JOIN [WH_VirginPCA].[Report].[IronOfferCycles] AS ioc
		on i.IronOfferCyclesID = ioc.IronOfferCyclesID
	LEFT JOIN [WH_VirginPCA].[Report].[OfferCycles] oc
		on ioc.OfferCyclesID = oc.OfferCyclesID
	INNER JOIN [WH_VirginPCA].[Derived].[IronOffer] AS o
	...
	TRUNCATE TABLE [Warehouse].[Staging].[ControlSetup_Validation_VirginPCANonAAM_IronOffer_References];

	INSERT INTO [Warehouse].[Staging].[ControlSetup_Validation_VirginPCANonAAM_IronOffer_References]
		(PublisherType
		, IronOfferID
		, StartDate
		, EndDate
