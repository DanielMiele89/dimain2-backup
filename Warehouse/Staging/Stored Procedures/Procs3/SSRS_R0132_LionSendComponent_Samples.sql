...
CREATE Procedure [Staging].[SSRS_R0132_LionSendComponent_Samples] (@LionSendID int,@ClubID int)
As
------------------------------------------------------------------------------------
...
Set @EmailDate = (	Select Max([EmailSendDate])
					from Warehouse.[Relational].[Customers_ReducedFrequency_Weeks]
					Where [EmailSendDate] < GetDate()
				 )
...
Where ee.emaileventcodeID = 1301 and
		ee.eventdate >= @LastEmailDate
------------------------------------------------------------------------------------
----------------------Find those who are active-----------------------
------------------------------------------------------------------------------------
if object_id('tempdb..#Customers') is not null drop table #Customers
Select t.FanID,c.CLubID,c.CompositeID
into #Customers
From #Opens as t
inner join warehouse.relational.customer as c
	on t.fanid = c.fanid
Where c.MarketableByEmail = 1 and
		c.clubid = @ClubID
...
Create Table #Cust (CompositeID bigint,slots int,Primary Key(CompositeID))

IF OBJECT_ID('tempdb..#t1') IS NOT NULL DROP TABLE #t1
Create Table #t1 (CompositeID bigint,OfferID int,Primary Key(OfferID))


Declare @More bit
Set @More = 1

While (Select IsNull(Count(*),0) from #Offers where [Sample] = 0) > 0 and @More > 0
Begin
		Truncate Table #Cust
		------------------------------------------------------------------------
		...
		------------------------------------------------------------------------
		Insert Into #Cust
		Select Top 1 os.CompositeID,Count(*) as slots
		From #Offers as o
		...
		Set @More = (Select IsNULL(Count(*),0) from #Cust)
		...
		--truncate table #t1
		Insert into #t1
		Select c.CompositeID,o.ItemID as OfferID  from #OfferVsSlots as os
...
------------------------------------------------------------------------
Insert into #t1
Select CompositeID,ItemID 
From 
(
Select n.CompositeID,o.ItemID,ROW_NUMBER() OVER(PARTITION BY o.ItemID ORDER BY n.CompositeID ASC) AS RowNo
From #Offers as o
inner join Lion.NominatedLionSendComponent as n
	on o.ItemID = n.ItemID
inner join warehouse.relational.customer as c
	on n.CompositeId = c.CompositeID
Where LionSendID = @LionSendID and
		[Sample] = 0
) as a
Where RowNo <= 1
...
Select *, ROW_NUMBER() OVER(ORDER BY [Rows] DESC) AS RowNo
into #Order 
From (
Select CompositeID,COunt(*) as [rows]
From #t1
