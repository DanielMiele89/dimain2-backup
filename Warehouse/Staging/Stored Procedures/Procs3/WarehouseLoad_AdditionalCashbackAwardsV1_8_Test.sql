...
*/
CREATE Procedure [Staging].[WarehouseLoad_AdditionalCashbackAwardsV1_8_Test]
As
/*--------------------------------------------------------------------------------------------------
...
Select	StoredProcedureName = 'WarehouseLoad_AdditionalCashbackAwardsV1_8_Test',
...
-----------------------------------Set Parameters---------------------------------------
----------------------------------------------------------------------------------------
Declare @DayDate date, @MaxDay date,@RowNo int, @MaxRowNo int, @ChunkSize int
...
Truncate table InsightArchive.AdditionalCashbackAward -- --**********************NEEDS CHANGING
...
Begin
	...
	Insert Into InsightArchive.AdditionalCashbackAward
	...
	from SLC_Report.DBO.Trans as t with (nolock)
		 ...
		 inner join #Customer as c
				on t.FanID = c.fanid
	Where	
			t.VectorMajorID is not null and
...
----------------------------------------------------------------------------------------------------*/
Update InsightArchive.AdditionalCashbackAward
--select AdditionalCashbackAwardID,MatchID
Set MatchID = m.ID
from InsightArchive.AdditionalCashbackAward as aca
...
----/*--------------------------------------------------------------------------------------------------
----------------------------Delete those records not in customer table----------------------------------
--------------------------------------------------------------------------------------------------------*/
...
Set		EndDate = GETDATE()
where	StoredProcedureName = 'WarehouseLoad_AdditionalCashbackAwardsV1_8_Test' and
