...
CREATE PROCEDURE [Staging].[SSRS_R0149_nFICustomersvsCustomersonOffers_V2] (@Date DATE
																		   , @Type INT)
AS
BEGIN
...
-- ***********************************************************************************************
	...
	WHERE (iof.EndDate > @IDate OR iof.EndDate IS NULL)
	AND iof.StartDate <= @IDate
...
-- ***********************************************************************************************
	...
	FROM [DIMAIN_TR].[SLC_REPL].[dbo].[Club] cl
	LEFT JOIN Fan fa
		ON cl.ID = fa.ClubID
	WHERE EXISTS (SELECT 1
...
-- ***********************************************************************************************
	...
	FROM #CurrentIronOffers cio
	INNER JOIN [DIMAIN_TR].[SLC_REPL].[dbo].[IronOfferMember] iom
		ON (iom.EndDate > @IDate OR iom.EndDate IS NULL)
		AND iom.StartDate <= @IDate
...
-- ***********************************************************************************************
	...
	LEFT JOIN [DIMAIN_TR].[SLC_REPL].[dbo].[Partner] pa
	...
	GROUP BY sc.ClubName
		   ...
		   , cpo.OffersLive
	HAVING @Itype = 1
	OR (SUM(co.NoOfCustomers) - sc.NoOfCustomers) < 0
	ORDER BY pa.Name
		   , sc.ClubName
