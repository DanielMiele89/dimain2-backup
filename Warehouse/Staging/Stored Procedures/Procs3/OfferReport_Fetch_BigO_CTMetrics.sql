...
******************************************************************************/
CREATE PROCEDURE [Staging].[OfferReport_Fetch_BigO_CTMetrics] 
...
BEGIN
    ...
    JOIN Staging.OfferReport_BigO_CustOffers c
	   ON CHARINDEX('1', c.BinaryOffers, o.ID) = o.ID
    ...
    JOIN Staging.OfferReport_OutlierExclusion oe -- Get outlier exclusion values based on when the cycle started to maintain consistency between partial and complete campaigns
	   ...
	   AND o.offerStartDate between oe.StartDate and oe.EndDate
