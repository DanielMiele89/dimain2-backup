...
CREATE PROCEDURE [Staging].[SLC_Report_DailyLoad_CBP_ProcessDirectDebitStats_SFD]
...
*/
	...
	IF OBJECT_ID('tempdb..#NewFirstTrans') IS NOT NULL DROP TABLE #NewFirstTrans
	SELECT c.FanID, FirstTrans = CAST(MIN(t.FirstTrans) AS DATE)
	INTO #NewFirstTrans
	FROM #Customers as c
	CROSS JOIN #TranTypes as tt
	CROSS APPLY (
		SELECT MIN([ProcessDate]) as FirstTrans
		FROM  SLC_REPL.[dbo].Trans as t  
		WHERE T.FanID = c.FanID
			AND	t.TypeID = tt.TransactionTypeID 
			AND t.ItemID = tt.ItemID
	) t
	GROUP BY c.FanID

	INSERT INTO #FirstTrans (FanID, FirstTran)
	SELECT FanID, FirstTrans 
	FROM #NewFirstTrans WHERE FirstTrans IS NOT NULL
	-- (2,114,062 rows affected) / 00:00:22
