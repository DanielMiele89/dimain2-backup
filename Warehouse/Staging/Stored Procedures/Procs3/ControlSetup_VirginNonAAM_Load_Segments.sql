...
Purpose: 
	...
	- Add entries to [WH_Virgin].[Report].[OfferCycles] table
	- Load offer segments INTO [Warehouse].[Staging].[ControlSetup_OffersSegment_Virgin] table
...
******************************************************************************/
CREATE PROCEDURE [Staging].[ControlSetup_VirginNonAAM_Load_Segments] (	@OnlyRunForFlashReportRetailers BIT
...
BEGIN
	...
	******************************************************************************/
	...
	/******************************************************************************
	Load IrON Offers active during period that are NOT already in [WH_Virgin].[Report].[IronOfferCycles]
	******************************************************************************/
		...
		FROM (	SELECT	iof.ClubID
				...
				AND iof.PartnerID NOT IN (4920, 4921) -- Exclude Bank Funded Amazon & Grocer Offers
	...
	******************************************************************************/
		...
		IF OBJECT_ID('tempdb..#OffersWithMembersVirgin') IS NOT NULL DROP TABLE #OffersWithMembersVirgin
	...
	******************************************************************************/
		...
		INSERT INTO [WH_Virgin].[Report].[OfferCycles]
		SELECT	DISTINCT
				owm.StartDate
			,	owm.Enddate 
		FROM #OffersWithMembersVirgin owm
		WHERE NOT EXISTS (	SELECT 1
							FROM [WH_Virgin].[Report].[OfferCycles] oc
	...
	- Load results INTO [Warehouse].[Staging].[ControlSetup_OffersSegment_Virgin] table

	CREATE TABLE for storing results:

	CREATE TABLE [Warehouse].[Staging].[ControlSetup_OffersSegment_Virgin] (IronOfferID INT
																		...
																		,	Segment VARCHAR(50)
																		,	CONSTRAINT PK_ControlSetup_OffersSegment_Virgin PRIMARY KEY CLUSTERED (IronOfferID, StartDate, EndDate))

	******************************************************************************/
	 
		TRUNCATE TABLE [Warehouse].[Staging].[ControlSetup_OffersSegment_Virgin];
		INSERT INTO [Warehouse].[Staging].[ControlSetup_OffersSegment_Virgin] (	IronOfferID
																			,	IronOfferName
																			,	StartDate
																			,	EndDate
																			,	PartnerID
		...
		FROM #OffersWithMembersVirgin o
		LEFT JOIN [Warehouse].[Relational].[IronOfferSegment] s
			ON o.IronOfferID = s.IronOfferID;

	--	In the case that a IronOFfer has had a change in it's start date OR end date since the last crontrol group run, UPDATE the OfferCyclesID in [WH_Virgin].[Report].[IronOfferCycles]
	--	to prevent a second control group being sest up for the same retailer / segment

		UPDATE ioc
		SET ioc.OfferCyclesID = oc2.OfferCyclesID
		FROM [Warehouse].[Staging].[ControlSetup_OffersSegment_Virgin] os
		...
		INNER JOIN [WH_Virgin].[Report].[OfferCycles] oc2
	...
	CREATE TABLE [Warehouse].[Staging].[ControlSetup_Validation_VirginNonAAM_Segments] (	PublisherType VARCHAR(50)
																			,	IronOfferID INT
																			,	IronOfferName NVARCHAR(200)
																			,	StartDate DATE
																			,	EndDate DATE
																			...
																			,	Segment VARCHAR(10)
																			,	CONSTRAINT PK_ControlSetup_Validation_VirginNonAAM_Segments PRIMARY KEY CLUSTERED (IronOfferID, StartDate, EndDate))

	******************************************************************************/

	-- Load errors

	TRUNCATE TABLE [Warehouse].[Staging].[ControlSetup_Validation_VirginNonAAM_Segments];
	INSERT INTO [Warehouse].[Staging].[ControlSetup_Validation_VirginNonAAM_Segments] (	PublisherType  
																					,	IronOfferID
																					,	IronOfferName
																					,	StartDate
																					,	EndDate
	...
	SELECT	'Virgin' AS PublisherType  
		...
		,	d.PartnerName
		,	d.Segment
	FROM [Warehouse].[Staging].[ControlSetup_OffersSegment_Virgin] d
	WHERE Segment = ''
	OR Segment IS NULL;
