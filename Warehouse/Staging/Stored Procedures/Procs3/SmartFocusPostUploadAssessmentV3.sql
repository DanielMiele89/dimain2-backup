...
CREATE Procedure [Staging].[SmartFocusPostUploadAssessmentV3]
...
Group by Left(CJS,2),
		...
		ClubID
Order By Case
			When left(CJS,2) = ''M1'' then 1
			When left(CJS,2) = ''M2'' then 2
			When left(CJS,2) = ''M3'' then 3
			When left(CJS,2) = ''SL'' then 4
			When left(CJS,2) = ''SH'' then 5
			When left(CJS,2) = ''RL'' then 6
			When left(CJS,2) = ''RH'' then 7
		  End
'
--select @Qry
Exec sp_sqlexec @Qry
...
Select *,
		Case
			When Shortcode in ('M1','M1O') and (Available <> '£0 or less' or Pending <> '£0 or less') then 'Yes'
			When ShortCode in ('M2','M2O') and Available = '£5+' then 'Yes'
...
Select	'''+ @TableName+''' as TableName,
		Case
			When cj.CustomerJourneyStatus = ''MOT1'' then ''M1''
			When cj.CustomerJourneyStatus = ''MOT2'' then ''M2''
			When cj.CustomerJourneyStatus = ''MOT3'' then ''M3''
			Else Left(cj.CustomerJourneyStatus,2)
		End as Shortcode_Yesterday,
		Left(d.CJS,2) as Shortcode_Today,
		Count(*) as CustomerCount
from Warehouse.relational.CustomerJourneyV2 as cj
inner join ' + @TableName + ' as d
	on cj.FanID = d.[Customer ID] and cj.EndDate =dateadd(day,-1,Cast(getdate() as date))
Group by Case
			When cj.CustomerJourneyStatus = ''MOT1'' then ''M1''
			When cj.CustomerJourneyStatus = ''MOT2'' then ''M2''
			When cj.CustomerJourneyStatus = ''MOT3'' then ''M3''
		 ...
		 End,Left(d.CJS,2)
--Order By Case
--			When cj.CustomerJourneyStatus = ''MOT1'' then 1
--			When cj.CustomerJourneyStatus = ''MOT2'' then 2
--			When cj.CustomerJourneyStatus = ''MOT3'' then 3
--			When Left(cj.CustomerJourneyStatus,2) = ''Sa'' then 4
--			Else 5
--		 End
--			,
--		  Case
--		  When left(d.CJS,2) = ''M1'' then 1
--			When left(d.CJS,2) = ''M2'' then 2
--			When left(d.CJS,2) = ''M3'' then 3
--			When left(d.CJS,2) = ''Sa'' then 4
--			Else 5
--		  End
'
