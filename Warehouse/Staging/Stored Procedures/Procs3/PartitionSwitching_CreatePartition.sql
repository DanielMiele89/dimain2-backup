CREATE PROCEDURE [Staging].[PartitionSwitching_CreatePartition] 
...
/*****************************************************************************************
This script, run on the first of the month, performs regular (monthly) partition maintenance
for the [Relational].[ConsumerTransaction] table.
...
-- This program runs on the first day of the month and sets up the following month
-- So if today is the first of October then next month will be November.
...
BEGIN 
	...
	EXEC('CREATE NONCLUSTERED INDEX [IX_Relational_ConsumerTransaction_CINIDTranDateIncl_ForMissingTranQuery] ON [Relational].[ConsumerTransaction_p' + @NewPartition + '_Stage]
		([CINID] ASC,[TranDate] ASC) INCLUDE ([FileID],[RowNum],[ConsumerCombinationID],[LocationID],[Amount],[IsOnline])')
