...
-- *******************************************************************************
CREATE PROCEDURE [Staging].[SSRS_R0057_WeeklyEmailPerformanceReport_Load_V2]
...
SELECT	StoredProcedureName = 'SSRS_R0057_WeeklyEmailPerformanceReport_Load_V2',
...
GROUP BY CAST(Warehouse.Staging.fnGetStartOfWeek(c.SendDate)+1 AS DATE),
	CAST(c.SendDate AS DATE),a.ClubID,
	c.CampaignName,
	CASE	
		WHEN a.CJS LIKE 'M1%' THEN 'MOT1'
		WHEN a.CJS LIKE 'M2%' THEN 'MOT2'
...
SET EndDate = GETDATE()
WHERE	StoredProcedureName = 'SSRS_R0057_WeeklyEmailPerformanceReport_Load_V2' 
