...
*/
CREATE Procedure [Staging].[SLC_Report_DailyLoad_FirstSpend_RetroRun]
...
Select	StoredProcedureName = 'Staging.SLC_Report_DailyLoad_FirstSpend_RetroRun',
		TableSchemaName = 'Staging',
		TableName = 'Customers_Passed0GBP_RetroRun',
		StartDate = GETDATE(),
		EndDate = null,
		TableRowCount  = null,
		AppendReload = 'A'
...
Set @RowNo = (Select Count(*) from Staging.Customers_Passed0GBP_RetroRun)
...
inner join #ACATranTypes as a with (nolock)
	on	t.TypeID = a.TypeID and
		t.ItemID = a.ItemID
Where t.ProcessDate >= DATEADD(dd, -3, DATEDIFF(dd, 0, @Date)) and
...
Where	m.[status] = 1 and rewardstatus in (0,1) 
		and	t.ProcessDate >= DATEADD(dd, -3, DATEDIFF(dd, 0, @Date)) 
...
-------------------------------------------------------------------------------------------------
SELECT @msg = 'Start - Update Warehouse.Staging.Customers_Passed0GBP_RetroRun'
EXEC SLC_Report.dbo.oo_TimerMessage @msg, @time OUTPUT
Truncate Table Staging.Customers_Passed0GBP_RetroRun

Insert Into Staging.Customers_Passed0GBP_RetroRun
Select	FanID,
		Cast(Dateadd(day,-1,getdate()) as date) as FirstEarnDate,
		FirstEarnValue,
		'direct debit frontbook' as FirstEarnType,
		MyRewardAccount
from Staging.SLC_Report_FirstSpendDD_Retro
...
Select	a.FanID,
		...
		f.FirstEarnValue,
		f.FirstEarnType,
		Null as MyRewardAccount
from #Customer as a
inner join #FirstSpend as f
	on a.FanID = f.FanID
Left Outer join Staging.Customers_Passed0GBP_RetroRun as b
	on a.fanid = b.fanid
Where b.FaniD is null

SELECT @msg = 'End - Update Warehouse.Staging.Customers_Passed0GBP_RetroRun - '+ Cast((Select Count(*) from #FirstSpend) as Varchar)
EXEC SLC_Report.dbo.oo_TimerMessage @msg, @time OUTPUT
...
where	StoredProcedureName = 'Staging.SLC_Report_DailyLoad_FirstSpend' and
		...
		TableName = 'Customers_Passed0GBP_RetroRun' and
		EndDate is null
/*--------------------------------------------------------------------------------------------------
...
Update  staging.JobLog_Temp
Set		TableRowCount = ((Select COUNT(1) from Warehouse.Staging.Customers_Passed0GBP_RetroRun)-@RowNo)
where	StoredProcedureName = 'Staging.SLC_Report_DailyLoad_FirstSpend' and
		TableSchemaName = 'Staging' and
		TableName = 'Customers_Passed0GBP_RetroRun' and
