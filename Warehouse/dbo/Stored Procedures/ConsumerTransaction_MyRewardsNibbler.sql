/*
Removes rows from Relational.ConsumerTransaction which have a trandate 
older than the beginning of the current month, five years ago. This takes about 10 seconds 
per partition.
*/
CREATE PROCEDURE [dbo].[ConsumerTransaction_MyRewardsNibbler] AS

SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED;
SET NOCOUNT ON;

DECLARE @msg varchar(8000), @time1 datetime = GETDATE(), @SSMS BIT = 1;
EXEC master.dbo.oo_TimerMessageV2 'Started ConsumerTransaction_MyRewards nibbler', @time1 OUTPUT, @SSMS OUTPUT
...
;WITH PartitionData AS (
	...
	WHERE pstats.object_id = OBJECT_ID('relational.consumertransaction_MyRewards')
)
...
WHILE 1 = 1 BEGIN
	...
	EXEC [Staging].[PartitionSwitching_CreateShadowTable_MyRewards] @strPartitionID, @ThisPartitionStartDate, @NextPartitionStartDate, @PartitionFilegroupName, 'PAGE'
	...
	IF @@SERVERNAME <> 'DIMAIN2' BEGIN
		EXEC('DROP INDEX csx_Stuff ON Relational.ConsumerTransaction_MyRewards_p' + @strPartitionID + '_Stage');
	END
	...
	IF @@SERVERNAME <> 'DIMAIN2' BEGIN
		EXEC('CREATE NONCLUSTERED COLUMNSTORE INDEX [csx_Stuff] ON Relational.ConsumerTransaction_MyRewards_p' + @strPartitionID + '_Stage
			([TranDate], [CINID], [ConsumerCombinationID], [Amount], [IsOnline], [PaymentTypeID])'); 
	END
	...
	EXEC('ALTER TABLE Relational.ConsumerTransaction_MyRewards_p' + @strPartitionID + '_Stage SWITCH TO Relational.ConsumerTransaction_MyRewards PARTITION ' + @strPartitionID);

	-- drop the shadow table, we're finished with it 
	EXEC('DROP TABLE Relational.ConsumerTransaction_MyRewards_p' + @strPartitionID + '_Stage');
