...
-- CJM 20180209 Slowest queries marked
-- CJM 20181128 Slowest queries marked again
-- =============================================
CREATE PROCEDURE [gas].[CustomerAttributes_Refresh]
...
BEGIN
	...
	ALTER INDEX IX_CustomerAttribute_RecencyYearRetail ON Relational.CustomerAttribute DISABLE -- was missed CJM
	...
	UPDATE Relational.CustomerAttribute SET -- ## 6 slowest 
		ParentOfYoung = 1
	FROM Relational.CustomerAttribute c
	INNER JOIN 
	(
		SELECT c.CINID, COUNT(1) AS Frequency	
		FROM Relational.ConsumerTransaction c WITH (NOLOCK)
		INNER JOIN #YoungParentCombos y WITH (NOLOCK) on c.ConsumerCombinationID = y.ConsumerCombinationID
		WHERE C.TranDate BETWEEN @SixMonthsEarlier AND @MaxFileDate	
		GROUP BY c.CINID	
		HAVING COUNT(1) >= 2
	) o on c.CINID = o.CINID	
	...
	UPDATE Relational.CustomerAttribute SET -- ## 12 slowest query
		OnlineGroceriesTentative = 1
	FROM Relational.CustomerAttribute c
	INNER JOIN 
	(
		SELECT C.CINID, COUNT(1) AS Frequency	
	...
	*/
	...
	, ISNULL(SUM(T.Spend),0) AS TotalSpend
	, ISNULL(SUM(O.Spend),0) AS OnlineSpend
	FROM #CustTotal T
	FULL OUTER JOIN #CustOnline O ON T.CINID = O.CINID
	GROUP BY T.CINID, O.CINID
	HAVING SUM(t.Spend) > 0 OR SUM(o.Spend) > 0
	...
	*/
	...
	--------------------------------------------------------------------------------------------------------------------
	-- beginning of section to modify
	--------------------------------------------------------------------------------------------------------------------
	...
	ALTER TABLE #RecencyCombosCoalition ADD PRIMARY KEY(ConsumerCombinationID)
	
	ALTER TABLE #RecencyCombosNonCoalition ADD PRIMARY KEY(ConsumerCombinationID)


	UPDATE Relational.CustomerAttribute SET -- ## 5 slowest query
	...
	UPDATE Relational.CustomerAttribute SET -- ## 2 slowest query
	...
	UPDATE Relational.CustomerAttribute SET -- ## 1 slowest query
	...
	UPDATE Relational.CustomerAttribute SET -- ## 7 slowest query
	...
	UPDATE Relational.CustomerAttribute SET -- ## 4 slowest query 
	...
	*/
	...
	UPDATE relational.CustomerAttribute SET -- ## 8 slowest query
		FrequencyYearRetail = t.Frequency, ValueYearRetail = t.Value, RecencyYearRetail = t.MostRecent
	...
	UPDATE Relational.CustomerAttribute SET -- ## 3 slowest query
		BankID = b.BankID
	FROM Relational.CustomerAttribute A
	INNER JOIN
	...
	ALTER INDEX IX_CustomerAttribute_RecencyYearRetail ON Relational.CustomerAttribute REBUILD -- was missed CJM
