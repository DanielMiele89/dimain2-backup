...
*/
CREATE PROCEDURE [gas].[MIDI_ConditionalPart]
...
EXEC master.dbo.oo_TimerMessagev2 'Start gas.MIDI_ConditionalPart', @Time OUTPUT, @SSMS OUTPUT
...
IF @DayName = 'SATURDAY' BEGIN
	...
	EXEC master.dbo.oo_TimerMessagev2 'Load ConsumerTransactionForFile data flow task', @Time OUTPUT, @SSMS OUTPUT

	-- Partition switching
	EXEC [Staging].[PartitionSwitching_LoadCTtable];  EXEC master.dbo.oo_TimerMessagev2 'Partition switching', @Time OUTPUT, @SSMS OUTPUT

	-- CT Partition Completion Email
	SET @Subject = '[' + @@SERVERNAME + '] Success: ConsumerTransaction Partition Switching COMPLETE'
	...
	EXEC master.dbo.oo_TimerMessagev2 'Partition switching CC', @Time OUTPUT, @SSMS OUTPUT

	-- CT MyRewards Partition Completion Email
	SET @Subject = '[' + @@SERVERNAME + '] Success: ConsumerTransaction_MyRewards Partition Switching COMPLETE'
	EXEC msdb.dbo.sp_send_dbmail 
        @profile_name = 'Administrator', 
		@recipients='DIProcessCheckers@rewardinsight.com;DevDB@rewardinsight.com',
...
IF @DayName = 'SUNDAY' BEGIN
	...
	EXEC gas.CTLoad_BrandSuggestions_Clear;  EXEC master.dbo.oo_TimerMessagev2 'CTLoad_BrandSuggestions_Clear', @Time OUTPUT, @SSMS OUTPUT
	...
	EXEC master.dbo.oo_TimerMessagev2 'Load CTLoad_MIDINewCombo data flow task', @Time OUTPUT, @SSMS OUTPUT
	...
	EXEC MIDI.ManualModule;  EXEC dbo.oo_TimerMessagev2 'MIDI.ManualModule', @Time OUTPUT, @SSMS OUTPUT

END


-- Enable ConsumerTransactionHolding Indexes
EXEC gas.CTLoad_ConsumerTransactionHolding_EnableIndexes;  EXEC master.dbo.oo_TimerMessagev2 'CTLoad_ConsumerTransactionHolding_EnableIndexes', @Time OUTPUT, @SSMS OUTPUT

-- Log Package Complete
EXEC MI.ProcessLog_Insert 'ConsumerTransactionHoldingLoad', 'Complete'

SET @Subject = '[' + @@SERVERNAME + '] Success: MID Identification process COMPLETE'
