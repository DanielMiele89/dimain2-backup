-- =============================================
-- Author:		JEA
-- Create date: 08/11/2012
-- Description:	Used by Merchant Processing Module.
-- Retrieves the transactions requiring processing
-- =============================================
CREATE PROCEDURE [gas].[TransactionsToProcess_Rainbow_Fetch]
	
AS
BEGIN

	SET NOCOUNT ON;

    SELECT FileID, RowNum, BankID, MerchantID AS varMID, LocationName AS varNarrative, LocationAddress AS varLocationAddress
    , LocationCountry AS varLocationCountry, MCC, CardholderPresentData, TranDate, PaymentCardID, Amount
	FROM Archive.dbo.NobleRainbowTransactionPreliminary WITH (NOLOCK)
	WHERE FileID = 1072
	
END