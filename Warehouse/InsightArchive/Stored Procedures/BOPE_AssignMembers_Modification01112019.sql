...
CREATE Procedure [InsightArchive].[BOPE_AssignMembers_Modification01112019] @JokerSlotPartners_CommaSeperated VarChar(100)
										  , @LionSendID Int
		...
		***************************************************************************************************/
			...
			Declare @Today Date = GetDate()
				...
				*******************************************************************************/
					...
					Select case when cc.BrandID in (485, 234) then @BrandID_WRJL else cc.BrandID end as BrandID
...
***********************************************************************************************************************/
		...
		***************************************************************************************************/
			...
			If Object_ID('sandbox.gabor.BOPE_JLW_Test') Is Not Null Drop Table sandbox.gabor.BOPE_JLW_Test
			Select @LionSendID as LionSendID
				 , rvp.CompositeID
				 , 3 as TypeID
				 , Case
				   ...
				   End as ItemRank
				 , Coalesce(rvp.RedeemID, dt.RedeemID) as ItemID
			Into sandbox.gabor.BOPE_JLW_Test
			From #Allocated_RetailerVouchers_Pre rvp
			Inner join (Select BrandID
							 , RedeemID
						From #DenominationsTable
