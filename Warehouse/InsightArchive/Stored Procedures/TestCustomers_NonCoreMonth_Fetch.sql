...
-- =============================================
CREATE PROCEDURE [InsightArchive].[TestCustomers_NonCoreMonth_Fetch] 
	(
		@DateID INT
	)
AS
BEGIN
	...
	FROM Stratification.BaseOfferMembers_NonCore_Compressed c
	INNER JOIN (SELECT PartnerID, ClientServicesref FROM MI.WorkingCumlDates WHERE Cumlitivetype = 2 AND ClientServicesref != '0') wcd ON c.PartnerID = wcd.Partnerid AND C.ClientServicesRef = wcd.ClientServicesref
