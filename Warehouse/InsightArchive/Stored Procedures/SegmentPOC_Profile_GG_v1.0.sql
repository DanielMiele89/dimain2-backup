...
-- =============================================
CREATE PROCEDURE [InsightArchive].[SegmentPOC_Profile_GG_v1.0]
	@Segment VARCHAR(20),
	@CycleStart DATE,
	@CycleEnd DATE,
	@BrandID INT,
...
BEGIN
	...
	CREATE TABLE #Engagement
		(
			...
			CINID INT,
			EngagementScore INT,
			EngagementNTILE VARCHAR(30)
		)

	IF @Engagement = 1
		BEGIN
			IF @TargetPopulation = 1 -- OOP CONTROL
				BEGIN
				...
				BEGIN
					...
					INSERT INTO #Engagement
						SELECT	c.FanID,
								c.CINID,
								NULL AS EngagementScore,
								eng.EngagementCat_HighFreq AS EngagementNTILE
						FROM	#Customers c
						LEFT JOIN Warehouse.InsightArchive.SegmentPOC_Customers eng on eng.fanid = c.fanid
						where	CycleStart = @CycleStart
