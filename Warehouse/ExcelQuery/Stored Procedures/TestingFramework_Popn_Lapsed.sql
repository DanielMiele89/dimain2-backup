CREATE PROCEDURE [ExcelQuery].[TestingFramework_Popn_Lapsed]		
...
SET NOCOUNT ON	
	...
	While @i <= @j 
		Begin
			...
			Where	trandate between DATEADD(MM,@Acquire,@enddate) and DATEADD(DD,-1,DATEADD(MM,@Lapsed,@enddate))
			...
			CREATE CLUSTERED INDEX c_cinid ON #existing(cinid)
			...
			SET @sql_1 =	'
							...
							Left Outer Join #existing e on c.CINID = e.CINID
							Where			e.CINID is null
							Order By	newid()
							'
			INSERT INTO #Lapsed execute (@sql_1)
