...
-- =============================================
CREATE PROCEDURE [ExcelQuery].[ROCEFT_NaturalSales_Calculate](@BrandID INT)
...
BEGIN
	...
	--		Set default date to the last complete cycle (giving one week grace for data to flow in)
	---------------------------------------------------------------------------------
	SET DATEFIRST 1

	SET @CurrentCycle = (SELECT ID
							FROM Warehouse.ExcelQuery.ROCEFT_ROC_Cycle_Calendar_Extended
							WHERE	DATEADD(week,-1,cast(GETDATE() as Date)) between CycleStart AND CycleEnd )					-- Select the current cycle, defined as the cycle that was active one week ago, to allow for data.  Eg if ou are within the first week of a new cycle, data will be generated for the second to last full cycle
	...
	INTO		#DateRanges
	FROM		Warehouse.ExcelQuery.ROCEFT_ROC_Cycle_Calendar_Extended
	WHERE		ID = @CycleIDRef																					-- SELECT * from #Dateranges
	---------------------------------------------------------------------------------
	-- Calendar Complete
	---------------------------------------------------------------------------------
	...
	BEGIN
				...
				CREATE NONCLUSTERED INDEX ix_ccID on #cc(ConsumerCombinationID)						-- SELECT * from #cc
				...
				Select		x.CINID
							...
							,case when isnull(DATEDIFF(MONTH,LastDate,(@endhistoricperiod)),9999) <= @Lapsed then 'Shopper'                                                              
								when isnull(DATEDIFF(MONTH,LastDate,(@endhistoricperiod)),9999) <= @Acquire then 'Lapsed' 
								else 'Acquire' 
