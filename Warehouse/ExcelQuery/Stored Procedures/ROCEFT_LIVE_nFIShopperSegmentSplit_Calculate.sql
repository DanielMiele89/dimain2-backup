...
-- Jason Shipp 20/11/2019
	-- Added Extra insert for publishers in the Warehouse.ExcelQuery.ROCEFT_Publishers table but with no members in the SLC_Report.dbo.Fan table
-- =============================================
CREATE PROCEDURE [ExcelQuery].[ROCEFT_LIVE_nFIShopperSegmentSplit_Calculate]
...
BEGIN
	...
	EXEC Prototype.oo_TimerMessage 'ROCEFT - nFI_ShopperSegmentSplits -- Start', @time OUTPUT
	...
	SELECT	ClubName,
			...
			CASE 	WHEN LastTranDate IS NULL THEN 'Acquire' 
					WHEN LastTranDate < DATEADD(m,CASE WHEN @Bespoke = 1 THEN -@AcquireL ELSE -AcquireL END, @HistoricPeriod_End) THEN 'Acquire'
					WHEN LastTranDate < DATEADD(m,CASE WHEN @Bespoke = 1 THEN -@LapserL ELSE -LapserL END, @HistoricPeriod_End) THEN 'Lapsed'
					WHEN LastTranDate >= DATEADD(m,CASE WHEN @Bespoke = 1 THEN -@LapserL ELSE -LapserL END, @HistoricPeriod_End) THEN 'Shopper'
					ELSE 'Error' 
...
*/
	...
	IF @BrandList IS NULL
		BEGIN
			TRUNCATE TABLE	Warehouse.ExcelQuery.ROCEFT_nFI_ShopperSegmentSplits
		END
	ELSE
		BEGIN
			DELETE FROM		Warehouse.ExcelQuery.ROCEFT_nFI_ShopperSegmentSplits
	...
	-- Extra insert for publishers in the Warehouse.ExcelQuery.ROCEFT_Publishers table but with no members in the SLC_Report.dbo.Fan table
	-- Uses MyRewards partner splits, based on the Warehouse.ExcelQuery.ROCEFT_ShopperSegmentSplit table
	INSERT INTO Warehouse.ExcelQuery.ROCEFT_nFI_ShopperSegmentSplits (BrandID, Publisher, ShopperSegment, PercentageSplit)
	SELECT 	
		b.BrandID
		, p.PublisherName AS Publisher
		, seg.Segment AS ShopperSegment
		, CASE 
			WHEN seg.Segment = 'Universal' THEN 1
			WHEN seg.Segment = 'Acquire' THEN 
				1-(MAX(CASE WHEN seg.Segment = 'Lapsed' THEN sp.SegmentPercentage ELSE 0 END) OVER (PARTITION BY b.BrandID, p.PublisherName))
				- (MAX(CASE WHEN seg.Segment = 'Shopper' THEN sp.SegmentPercentage ELSE 0 END) OVER (PARTITION BY b.BrandID, p.PublisherName))
			ELSE sp.SegmentPercentage
		END AS PercentageSplit
	FROM (SELECT DISTINCT BrandID FROM Warehouse.ExcelQuery.ROCEFT_ShopperSegmentSplit) b
	CROSS JOIN (
		SELECT DISTINCT PublisherName FROM Warehouse.ExcelQuery.ROCEFT_Publishers p 
		WHERE PublisherName <> 'RBS' AND NOT EXISTS (
			SELECT NULL FROM Warehouse.ExcelQuery.ROCEFT_nFI_ShopperSegmentSplits x WHERE p.PublisherName = x.Publisher
		)
	) p
	CROSS JOIN (SELECT 'Acquire' AS Segment UNION ALL SELECT 'Lapsed' UNION ALL SELECT 'Shopper' UNION ALL SELECT 'Universal') seg
	LEFT JOIN (
			SELECT 
			BrandID
			, Shopper_Segment
			, MAX(SegmentPercentage) AS SegmentPercentage
			FROM Warehouse.ExcelQuery.ROCEFT_ShopperSegmentSplit
			WHERE Segment = 'MyRewards'
			GROUP BY
			BrandID
			, Shopper_Segment
		) sp
		ON b.BrandID = sp.BrandID
		AND seg.Segment = sp.Shopper_Segment
	WHERE
		p.PublisherName IS NOT NULL;

	EXEC Prototype.oo_TimerMessage 'ROCEFT - nFI_ShopperSegmentSplits -- End', @time OUTPUT
