...
-- =============================================
CREATE PROCEDURE [ExcelQuery].[ROCEFT_CumulGainsRBS_v1]
AS
BEGIN
	SET NOCOUNT ON;
	-------------------------------------------------------------------------------------
	...
	WHILE @rowno  <= (select max(rowno) from Warehouse.ExcelQuery.ROCEFT_RefreshBrand)        
		BEGIN
			...
			INTO	#Partner
			FROM	#Brand br
			JOIN	(	SELECT  DISTINCT BrandID, PartnerID
						FROM	Warehouse.Relational.Partner
						WHERE BrandID IS NOT NULL
						UNION
						SELECT  DISTINCT br.BrandID, p.PartnerID
					...
					) p ON  br.BrandID = p.BrandID
			WHERE	p.PartnerID < 1000000 -- Eject weird looking PartnerID's	

			DECLARE @PartnerNo INT
			SET @PartnerNo = (SELECT COUNT(*) FROM #Partner)
