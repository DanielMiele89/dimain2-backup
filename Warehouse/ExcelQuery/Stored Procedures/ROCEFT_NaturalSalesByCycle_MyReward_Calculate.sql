/****** Object:  StoredProcedure [ExcelQuery].[ROCEFT_NaturalSalesByCycle_MyReward_Calculate]    Script Date: 01/04/2018 13:07:03 ******/
--SET ANSI_NULLS ON
--GO
...
--	 v2.0 - 01/05/2018 - Run on ConsumerTrans_MyRewards
--*/
---- =============================================
CREATE PROCEDURE [ExcelQuery].[ROCEFT_NaturalSalesByCycle_MyReward_Calculate]
	(
...
BEGIN
	...
	-- DECLARE @BrandList VARCHAR(500) = '12'
	...
	ELSE
		BEGIN
			...
			DELETE FROM Warehouse.ExcelQuery.ROCEFT_NaturalSpendCycles_MyReward WHERE CHARINDEX(',' + CAST(BrandID AS VARCHAR) + ',', ',' + @BrandList + ',') > 0
		END
	...
	*/
	...
	WHILE @RowNo <= (SELECT MAX(RowNo) FROM #Brand)
		BEGIN
			...
			LEFT JOIN	Warehouse.Segmentation.ROC_Shopper_Segment_Partner_Settings part on p.PartnerID = part.PartnerID
			...
			SELECT	d.ID
					...
					,COUNT(1) AS Trans
					,SUM(CASE WHEN ct.IsOnline = 1 THEN 1 ELSE 0 END) AS OnlineTrans
			INTO	#FutureSpend
			FROM	#CINTrans ct WITH (NOLOCK)
			...
			FROM	(							
						SELECT		s.ID
									...
									,SUM(OnlineTrans) AS OnlineTransactions
									,COUNT(DISTINCT fs.CINID) AS Spenders
									,COUNT(DISTINCT CASE WHEN OnlineTrans > 0 THEN fs.CINID ELSE NULL END) AS OnlineSpenders
						FROM		#Shoppers s
						JOIN		#FutureSpend fs
								ON  s.ID = fs.ID
			...
			INSERT INTO Warehouse.ExcelQuery.ROCEFT_NaturalSpendCycles_MyReward
				SELECT		@BrandID AS BrandID
							...
							,ISNULL(sp.OnlineTransactions,0) AS OnlineTranactions
							,ISNULL(sp.Spenders,0) AS Spenders
							,ISNULL(sp.OnlineSpenders,0) AS OnlineSpenders
	...
	--IF OBJECT_ID('Warehouse.ExcelQuery.ROCEFT_NaturalSpendCycles_MyReward') IS NOT NULL DROP TABLE Warehouse.ExcelQuery.ROCEFT_NaturalSpendCycles_MyReward
	--CREATE TABLE Warehouse.ExcelQuery.ROCEFT_NaturalSpendCycles_MyReward
	--	(
