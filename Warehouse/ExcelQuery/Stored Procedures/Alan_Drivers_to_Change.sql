...
CREATE PROCEDURE [ExcelQuery].[Alan_Drivers_to_Change]		(@Edate Date		--end date
															,@Brand int			--brand
...
--Declare		@Edate			date
--Set			@Edate			= '2016-08-31'  -- SELECT THE LAST DAY IN THE LAST MONTH REQUIRED

--Declare		@Brand			int
--Set			@Brand			= 12			-- SELECT THE DESIRED MAIN BRAND
--									-- SELECT * FROM relational.brand where brandname like '%american%'
...
if @FBRequired = 1		begin
						...
						declare @fixedbaseenddate date 
							set @fixedbaseenddate = (select top 1 Dates from #Dates Order By dates desc)
...
Begin
			...
			from		Relational.ConsumerTransaction ct with (nolock)
						...
						join #Dates d on @StartDate <= ct.TranDate and @EndDate >= ct.TranDate
...
While @RowNoC <= @MaxRowNoC
		Begin
			...
			where		ISRefund = 0 and
						c.RowNo Between @RowNoC and @RowNoC+(@ChunkSizeC-1)
...
if object_id('tempdb..#TotalSpend') is not null drop table #TotalSpend
	select	MONTH
			...
			,1.0*count(distinct CINID) as Customers
...
into #AggSpend
from #TotalSpend t
join #ExistingSpend e on t.month = e.month
join #AcquireCSSpend cs on t.month = cs.month
join #AcquireNewSpend n on t.month = n.month
...
select	top 12 Month
		,1.0*Sales/LEAD(Sales, 12) OVER (ORDER BY Month desc) as Sales_Change
		,1.0*Customers/LEAD(Customers, 12) OVER (ORDER BY Month desc) as Customer_Change
		,1.0*ATV/LEAD(ATV, 12) OVER (ORDER BY Month desc) as ATV_Change
		,1.0*ATF/LEAD(ATF, 12) OVER (ORDER BY Month desc) as ATF_Change
...
----------  Output Tables
----------------------------------------------------------------------------------------
if object_id('sandbox.Alan.DoC_TotalSpend') is not null drop table sandbox.Alan.DoC_TotalSpend
if object_id('sandbox.Alan.DoC_ExistingSpend') is not null drop table sandbox.Alan.DoC_ExistingSpend
if object_id('sandbox.Alan.DoC_AcquireCSSpend') is not null drop table sandbox.Alan.DoC_AcquireCSSpend
if object_id('sandbox.Alan.DoC_AcquireNewSpend') is not null drop table sandbox.Alan.DoC_AcquireNewSpend
if object_id('sandbox.Alan.DoC_AggSpend') is not null drop table sandbox.Alan.DoC_AggSpend
if object_id('sandbox.Alan.DoC_TotalSpendChange') is not null drop table sandbox.Alan.DoC_TotalSpendChange
if object_id('sandbox.Alan.DoC_ExistingSpendChange') is not null drop table sandbox.Alan.DoC_ExistingSpendChange
if object_id('sandbox.Alan.DoC_AcquireCSSpendChange') is not null drop table sandbox.Alan.DoC_AcquireCSSpendChange
if object_id('sandbox.Alan.DoC_AcquireNewSpendChange') is not null drop table sandbox.Alan.DoC_AcquireNewSpendChange
if object_id('sandbox.Alan.DoC_AcquireNewSpend') is not null drop table sandbox.Alan.DoC_AcquireNewSpend
...
select	* 
into	sandbox.alan.DoC_TotalSpend
from	#TotalSpend order by Month

select 'Copy table and replace Existing Spend'
select	*
into	sandbox.alan.DoC_ExistingSpend
from	#ExistingSpend order by Month

select 'Copy table and replace Acquire: CS Spend'
select	*
into	sandbox.alan.DoC_AcquireCSSpend
from	#AcquireCSSpend order by Month

select 'Copy table and replace Acquire: New Spend'
select	*
into	sandbox.alan.DoC_AcquireNewSpend
from	#AcquireNewSpend order by Month

select 'Copy table and replace Agg Spend'
select	*
into	sandbox.alan.DoC_AggSpend
from	#AggSpend order by Month

select 'Copy table and replace Total Spend Change'
select	*
into	sandbox.alan.DoC_TotalSpendChange
from #TotalSpendChange order by Month

select 'Copy table and replace Existing Spend Change'
select	*
into	sandbox.alan.DoC_ExistingSpendChange
from #ExistingSpendChange order by Month

select 'Copy table and replace Acquire: CS Spend Change'
select	*
into	sandbox.alan.DoC_AcquireCSSpendChange
from #AcquireCSSpendChange order by Month

select 'Copy table and replace Acquire: New Spend Change'
select	*
into	sandbox.alan.DoC_AcquireNewSpendChange
from #AcquireNewSpendChange order by Month
