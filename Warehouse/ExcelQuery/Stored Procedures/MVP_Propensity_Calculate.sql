...
-- =============================================
*/
CREATE PROCEDURE [ExcelQuery].[MVP_Propensity_Calculate]
	@BrandList VARCHAR(500)
AS
BEGIN

	--DECLARE @BrandList VARCHAR(500) = '292'
	...
	IF @BrandList IS NULL	-- Fortnightly Update
		BEGIN
			TRUNCATE TABLE Warehouse.ExcelQuery.MVP_SpendStretchPropensityRank
			TRUNCATE TABLE Warehouse.ExcelQuery.MVP_SpendStretchTotal
			TRUNCATE TABLE Warehouse.ExcelQuery.MVP_NaturalSalesByCycle
			...
			INSERT INTO #BrandList
				...
				FROM	Warehouse.ExcelQuery.MVP_BrandList
				WHERE	EndDate IS NULL
		END
	ELSE					-- Brand(s) Addition/Refresh
		BEGIN
		
			DELETE FROM Warehouse.ExcelQuery.MVP_SpendStretchPropensityRank WHERE CHARINDEX(',' + CAST(BrandID AS VARCHAR) + ',', ',' + @BrandList + ',') > 0
			DELETE FROM Warehouse.ExcelQuery.MVP_SpendStretchTotal WHERE CHARINDEX(',' + CAST(BrandID AS VARCHAR) + ',', ',' + @BrandList + ',') > 0
			DELETE FROM Warehouse.ExcelQuery.MVP_NaturalSalesByCycle WHERE CHARINDEX(',' + CAST(BrandID AS VARCHAR) + ',', ',' + @BrandList + ',') > 0
	...
	CROSS APPLY (
		   ...
		   FROM Warehouse.ExcelQuery.MVP_ControlAdjustedSCD ca
	...
	WHILE @RowNo <= (SELECT MAX(RowNo) FROM #BrandList)
		BEGIN
			...
			JOIN	#WorkingDates db
				ON	c.ID = db.ID
			LEFT JOIN	Warehouse.ExcelQuery.MVP_Engagement e
			...
			*/
			...
			-- Collate Outputs
			INSERT INTO	Warehouse.ExcelQuery.MVP_NaturalSalesByCycle
				...
				JOIN	Warehouse.ExcelQuery.MVP_DateTable b
					ON	a.ID = b.ID
			...
			-- Total SS
			INSERT INTO	Warehouse.ExcelQuery.MVP_SpendStretchTotal
				SELECT	GETDATE() AS RunDate,
			...
			-- Propensity SS
			INSERT INTO	Warehouse.ExcelQuery.MVP_SpendStretchPropensityRank
				SELECT	GETDATE() AS RunDate,
						'Control' AS GroupType,
						BrandID,
						a.ID,
		...
		--IF OBJECT_ID('Warehouse.ExcelQuery.MVP_NaturalSalesByCycle') IS NOT NULL DROP TABLE Warehouse.ExcelQuery.MVP_NaturalSalesByCycle
		--CREATE TABLE Warehouse.ExcelQuery.MVP_NaturalSalesByCycle
		--	(
		...
		--IF OBJECT_ID('Warehouse.ExcelQuery.MVP_SpendStretchTotal') IS NOT NULL DROP TABLE Warehouse.ExcelQuery.MVP_SpendStretchTotal
		--CREATE TABLE Warehouse.ExcelQuery.MVP_SpendStretchTotal
		--	(
		--		PKID INT IDENTITY(1,1) PRIMARY KEY,
		--		RunDate DATE,
		--		GroupName VARCHAR(50),
		...
		--IF OBJECT_ID('Warehouse.ExcelQuery.MVP_SpendStretchPropensityRank') IS NOT NULL DROP TABLE Warehouse.ExcelQuery.MVP_SpendStretchPropensityRank
		--CREATE TABLE Warehouse.ExcelQuery.MVP_SpendStretchPropensityRank
		--	(
		--		PKID INT IDENTITY(1,1) PRIMARY KEY,
		--		RunDate DATE,
		--		GroupName VARCHAR(50),
