...
-- =============================================
CREATE PROCEDURE [ExcelQuery].[ROCEFT_Test_NaturalSalesByCycle_Calculate_Ocado_LowFreq]
...
BEGIN
	...
	FROM	(SELECT	*
			 FROM	#Dates 
			 WHERE	CycleStart <= DATEADD(DAY,-364,@Segmentation_Date)
				AND DATEADD(DAY,-364,@Segmentation_Date) <= CycleEnd) a
	JOIN	#Dates b
		ON  b.ID >= a.ID
	  AND b.ID < a.ID + 14
