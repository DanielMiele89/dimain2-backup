-- =============================================
-- Author:		<Shaun Hide>
-- Create date: <21st February 2019>
...
-- =============================================
CREATE PROCEDURE [ExcelQuery].[ROCEFT_BESPOKE_LIVE_NaturalSalesByCycle_Calculate]
	@BrandID INT,
...
BEGIN
	...
	DECLARE @PopSize INT = (SELECT COUNT(1) FROM #Population)
	...
	SELECT	br.BrandName
			,br.BrandID
			,COALESCE(@AcquireL,part.Acquire,blk.acquireL,lk.AcquireL,12) as AcquireL
			,COALESCE(@LapserL,part.Lapsed,blk.LapserL,lk.LapserL,6) as LapserL
			,br.SectorID
	INTO	#Settings
...
---------------------------------------------------------------------------------------------
	...
	-- Save The New Run Output's
		SELECT	a.BrandID,
				...
				a.Segment,
				CASE
				  WHEN a.Segment = 7 THEN @PopSize - e.LapsedShopper
				  ELSE b.Population
				END AS SegmentSize,
