...
-- =============================================
CREATE PROCEDURE [ExcelQuery].[ROCEFT_Heatmap_v1]
AS
BEGIN
	SET NOCOUNT ON;
	-------------------------------------------------------------------------------------
		...
		*/
		...
		IF @RunHeatmap = 1
			BEGIN
				...
				DECLARE @HistoricStartDate DATE = DATEADD(MONTH,-12,DATEADD(DAY,-13,GETDATE()))
				DECLARE @HistoricEndDate DATE = DATEADD(DAY,-14,GETDATE())
				...
				INNER JOIN	(
					...
					ON	cin.CINID = ct.CINID
				WHERE	IsRefund = 0
					AND TranDate BETWEEN @HistoricStartDate AND @HistoricEndDate
				GROUP BY cin.CINID
