CREATE PROCEDURE [ExcelQuery].[TestingFramework_Popn_Acquire]		
...
SET NOCOUNT ON	
	...
	While @i <= @j 
		Begin
			...
			Where	trandate between DATEADD(MM,@Acquire,@enddate) and @enddate
			...
			SET @sql_1 =	'
							...
							Where not exists	(
													Select	1
													From	#spenders s
													Where	s.CINID=c.CINID
												)
							Order By newid()
							'
			INSERT INTO #Acquire execute (@sql_1)
