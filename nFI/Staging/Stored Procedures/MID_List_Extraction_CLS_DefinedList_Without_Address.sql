...
Create Procedure [Staging].[MID_List_Extraction_CLS_DefinedList_Without_Address] (@PartnerID Int,@TableName varchar(350))
...
---------------------------------------------------------------------------------------------------
Declare @PID int,@Rows smallint,@Qry nvarchar(max)

Set @PID = @PartnerID

Truncate Table nFI.Staging.MIDListExtraction_Interim

Set @Qry = 'Insert into nFI.Staging.MIDListExtraction_Interim
			Select OutletID
			From '+@TableName
...
(
		...
		inner join SLC_Report..partner as p
			on o.PartnerID = p.ID
		inner join nFI.Staging.MIDListExtraction_Interim as a
			on o.ID = a.OutletID
	Where p.ID = @PID
) as a
