...
SELECT DISTINCT CompositeID,c.FanID
INTO #AlreadyPresent
FROM Relational.Ironoffermember iom with (nolock) -- ix_Stuff2 / IX_IronOfferMember_CompositeIDEndDateStartDate_InclIronOfferID
inner join Relational.Customer as c
	on  iom.FanID = c.FanID
...
INSERT INTO Relational.IronOfferMember (IronOfferID, FanID, StartDate, EndDate, ImportDate)
...
SELECT iom.IronOfferID, c.FanID, iom.StartDate, iom.EndDate, iom.ImportDate
...
CREATE UNIQUE CLUSTERED INDEX ucx_Stuff ON #IronOfferMember (IronOfferID, FanID, StartDate)
...
WHERE EXISTS (
SELECT 1 FROM relational.IronOfferMember i with (nolock)
	WHERE iom.IronOfferID = i.IronOfferID 
	AND iom.FanID = i.FanID 
