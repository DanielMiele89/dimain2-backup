...
CREATE Procedure [Staging].[OfferSelection_V3_0_byClub_Dev] (@OfferDate date,@PartnerID int,@RunType char(1),@ClubID int, @ShopperThrottle int, @LapsedThrottle int, @AcquireThrottle int)
as

Declare @AlreadyOnOfferCount int
...
Begin
		...
		Select	Distinct
				...
				Max(Case
						...
						Else 1
					End) as ActiveCard,
				Row_Number() over (partition by m.ShopperSegmentTypeID order by cr.Ranking) [RankingOrder]
		Into	#Customers
		...
		Left join nfi.segmentation.Roc_Shopper_Segment_CustomerRanking cr
				on cr.FanID = c.FanID
		...
		-- ********************** Amendment Start (ZT '2017-03-23') ********************** 
		-- ***** Reason for amendment: If throttling is required and it is a new campaign
		Set @AlreadyOnOfferCount = (select count(*) from #AlreadyOnOffer)

		If @AlreadyOnOfferCount = 0
		BEGIN
			If @ShopperThrottle is not NULL
			Begin
				Delete from #Customers 
				Where ShopperSegmentTypeID = 7 and RankingOrder > @ShopperThrottle			
			End
			If @LapsedThrottle is not NULL
			Begin
				Delete from #Customers 
				Where ShopperSegmentTypeID = 8 and RankingOrder > @LapsedThrottle			
			End
			If @AcquireThrottle is not NULL
			Begin
				Delete from #Customers 
				Where ShopperSegmentTypeID = 9 and RankingOrder > @AcquireThrottle			
			End
		End
		-- *********************** Amendment End  (ZT '2017-03-23') ********************** 
...
Begin	
		...
		Select 'OfferProcessLog' as [Type]
				,* 
		...
		Select 'OfferMemberClosure' as [Type],
				EndDate,Count(*) as Customer 
		From Warehouse.[iron].[OfferMemberClosure]
		Where IronOfferID in 
							(Select IronOfferID From #Offers)
		Group by EndDate

		Select 'OfferMemberAddition' as [Type],
				StartDate,
				EndDate,
				Count(*) as Customer 
