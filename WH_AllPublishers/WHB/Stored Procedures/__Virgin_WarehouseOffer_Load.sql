...
CREATE PROC [WHB].[__Virgin_WarehouseOffer_Load]
AS

 SET XACT_ABORT ON;

 DECLARE @MergeCounts TABLE(MergeAction VARCHAR(20))
 DECLARE @SourceType INT = 8

 MERGE INTO [WH_AllPublishers].[dbo].[Offer] AS TGT
 USING [WH_AllPublishers].[Inbound].[Virgin_IronOffer] AS SRC
...
WHEN NOT MATCHED THEN
	...
	VALUES ([IronOfferName],  [ClubID], [ClubID], [StartDate], [EndDate], [PartnerID], [PartnerID], [CampaignType], [SegmentName], [HydraOfferID], @SourceType, GETDATE(), GETDATE())
