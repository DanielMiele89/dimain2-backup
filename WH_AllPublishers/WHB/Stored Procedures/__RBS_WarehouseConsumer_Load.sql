...
CREATE PROC [WHB].[__RBS_WarehouseConsumer_Load]
AS

 SET XACT_ABORT ON;

 DECLARE @MergeCounts TABLE(MergeAction VARCHAR(20))
 DECLARE @SourceType INT = 1

 MERGE INTO [WH_AllPublishers].[dbo].[Consumer] AS TGT
 USING [WH_AllPublishers].[Inbound].[RBS_Customer] AS SRC
 ...
 WHEN MATCHED THEN 
	...
	,TGT.[MarketableByPush]	= [SRC].[MarketableByDirectMail]
  ...
  WHEN NOT MATCHED THEN
	...
	VALUES ([FanID], [ClubID], [ClubID],  [AccountType], [Title], [City], [County], [Region], [PostalSector], [PostCodeDistrict], [PostArea], [Gender], [AgeCurrent], [AgeCurrentBandText], [MarketableByEmail], [MarketableByDirectMail], [CurrentlyActive], [RegistrationDate], [DeactivatedDate], [SourceUID], @SourceType, GETDATE(), GETDATE())
