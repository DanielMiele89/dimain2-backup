...
CREATE PROC [WHB].[__nFI_WarehouseOffer_Load]
AS
 ...
 USING [WH_AllPublishers].[Inbound].[nFI_IronOffer] AS SRC
	ON TGT.SourceTypeID = 5
	AND TGT.SourceID = CAST(SRC.[ID] AS VARCHAR(36))
WHEN MATCHED THEN 
	UPDATE SET
...
WHEN NOT MATCHED THEN
	INSERT ([OfferName], [ClubID], [PublisherID], [StartDateTime], [EndDateTime], [PartnerID], [RetailerID],[SourceID], [SourceTypeID], [CreatedDateTime], [UpdatedDateTime])
	VALUES ([IronOfferName],  [ClubID], [ClubID], [StartDate], [EndDate], [PartnerID], [PartnerID], [ID], 5, GETDATE(), GETDATE())
