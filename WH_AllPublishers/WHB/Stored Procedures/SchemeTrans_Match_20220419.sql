...
******************************************************************************/
CREATE PROCEDURE [WHB].[SchemeTrans_Match_20220419]
...
BEGIN
	...
	*/
	...
	CREATE NONCLUSTERED INDEX IX_PanID ON #Match (PanID)
	...
	INTO #Trans
	FROM [DIMAIN_TR].[SLC_REPL].[dbo].[Trans] tr
	LEFT JOIN [DIMAIN_TR].[SLC_REPL].[dbo].[TransactionType] tt
		ON tr.TypeID = tt.ID
	WHERE EXISTS (	SELECT 1
					FROM #Match ma
