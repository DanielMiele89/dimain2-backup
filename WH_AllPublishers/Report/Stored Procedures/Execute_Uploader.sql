CREATE PROCEDURE [Report].[Execute_Uploader] (	@SourceFolder VARCHAR(MAX)
...
BEGIN
	...
	SET @sql = 'aws s3 cp "' + @SourceFolder + '" "' + @S3Location + '" --recursive --acl bucket-owner-full-control --profile trusted-rwd-engineer-etl-role-048'
	...
	--SELECT	@RunID = MAX(LatestRunID)
	--FROM [Report].vw_PackageLog_LatestRunID
	...
	INSERT INTO [Report].[ExecLog] (Msg, isError, CreatedDateTime, RunID, ExecCommand)
	SELECT	o.line
		,	x.isError
		,	GETDATE()
		,	@RunID
		,	@sql
	FROM #output o
	CROSS APPLY (	SELECT CAST(COALESCE(MAX(1), 0) AS BIT)
					FROM #output
					WHERE line like @ErrorString
					OR line like @ErrorString2
					OR line like @ErrorString3) x(isError)

	--IF (SELECT TOP 1 1 FROM [Report].[ExecLog] WHERE RunID = @RunID AND isError = 1) IS NOT NULL
	--BEGIN
