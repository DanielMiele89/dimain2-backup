...
******************************************************************************/
CREATE PROCEDURE [SamW].[ForecastTool_ForecastDetails_Merge_V2]
--WITH EXECUTE AS 'Rory'
AS
	BEGIN
declare @publisherid2 VARCHAR(500) = 'PublisherID2';
			...
			THEN UPDATE SET	 
			 ...
			 ALTER TABLE SamW.ForecastingBudgetTracking
				ADD [PublisherID2] int NULL;

			 update SamW.ForecastingBudgetTracking 
			 set @publisherid2 = PublisherID;

			  ALTER TABLE SamW.ForecastingBudgetTracking
				drop column  [PublisherID];
	  		 exec sp_rename 'SamW.ForecastingBudgetTracking.PublisherID2', 'PublisherID', 'COLUMN';
