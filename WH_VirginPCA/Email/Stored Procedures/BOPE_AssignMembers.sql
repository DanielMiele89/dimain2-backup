...
CREATE PROCEDURE [Email].[BOPE_AssignMembers] (@JokerSlotPartners_CommaSeperated VARCHAR(100))
...
--	DECLARE @JokerSlotPartners_CommaSeperated VARCHAR(100) = '0E7C4CAE-9176-4C75-B734-FDAFE164464B'
		...
		***************************************************************************************************/
				...
				*******************************************************************************/
					...
					Select DISTINCT
						   ro.RedemptionPartnerGUID AS PartnerID
                         , rp.PartnerName AS PartnerName
						 , pa.BrandID 
						 , ro.ID AS RedeemID
						 , ro.TradeUp_CashbackRequired as MinCBreq
						 , Dense_Rank() Over (Order by ro.TradeUp_CashbackRequired Asc) as MinCBreq_Rank
					Into #VoucherSetUp_All
					FROM [WH_VirginPCA].[Derived].[RedemptionOffers] ro
					INNER JOIN [WH_VirginPCA].[Derived].[RedemptionPartners] rp
						ON ro.RedemptionPartnerGUID = rp.RedemptionPartnerGUID
					LEFT JOIN #Partner pa
						ON rp.PartnerName = pa.PartnerName
					Where ro.Status = 'Live'
					AND ro.TradeUp_CashbackRequired IS NOT NULL
				...
				*******************************************************************************/
						...
						*******************************************************************************/
							...
							Create Table #JokerPartners (PartnerGUID VARCHAR(64))
							INSERT INTO #JokerPartners
							SELECT Item AS PartnerGUID
						...
						*******************************************************************************/
							...
							Select PartnerID
								 ...
								 , Case
										When CONVERT(VARCHAR(64), r.PartnerID) in (Select CONVERT(VARCHAR(64), PartnerGUID) From #JokerPartners) Then 'JokerSlot'
										Else 'DefaultSlot'
								   End as SlotType
							Into #VoucherSetUp
		...
		***************************************************************************************************/
				...
				****************************************************************************************************/
							...
							***********************************************************************************/
							...
							--		JOIN	Trans.ConsumerCombination c ON c.ConsumerCombinationID = p.ConsumerCombinationID
							--		WHERE	BrandID = 85
							...
							--SELECT		DISTINCT FanID, 85, 1, '2. Forced'
