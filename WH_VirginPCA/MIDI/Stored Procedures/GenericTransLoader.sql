...
IF @MaxPartitionDate < DATEADD(DAY,1,EOMONTH(GETDATE())) BEGIN
	...
	ALTER PARTITION SCHEME PartitionByMonthScheme NEXT USED [PRIMARY];
...
WHILE 1 = 1 BEGIN 
	...
	--------------------------------------------------------------------------------------------------------------------------
	EXEC('
		INSERT INTO [Trans].[ConsumerTransaction_shadow] WITH (TABLOCKX) (	[TransactionID]
																		,	[FileID]
																		,	[RowNum]
		...
		WHERE NOT EXISTS (	SELECT 1
							...
							WHERE cth.[TransactionID] = ct.[TransactionID])
...
WHERE NOT EXISTS (	SELECT 1
					FROM [MIDI].[ConsumerTransaction_ExportToAWS] cte
					WHERE ct.[TransactionID] = cte.[TransactionID])
