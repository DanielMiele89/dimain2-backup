...
*******************************************************************************************************************************************/
	...
	***********************************************************************************************************************/
		...
		WHERE EXISTS (	SELECT 1
						FROM [MIDI].[CTLoad_MIDIHolding] mh
						WHERE mh.CardID = ca.CardGUID)
	
		UPDATE mh
		SET mh.CIN = ls.SourceUID
		,	mh.BankID = ls.BankID
		,	mh.PaymentTypeID = 2
		FROM [MIDI].[CTLoad_MIDIHolding] mh
		CROSS APPLY (	SELECT	TOP(1)	--	non-deterministic UPDATE
...
*******************************************************************************************************************************************/
	...
	***********************************************************************************************************************/
		...
		FROM [MIDI].[CTLoad_MIDIHolding] mh WITH (TABLOCK)
		CROSS APPLY (	SELECT	TOP 1	--	non-deterministic update
						...
						AND mh.MerchantName = cc.Narrative
	...
	***********************************************************************************************************************/
		...
		CROSS APPLY (	SELECT	TOP 1	--	non-deterministic update
						...
						AND mh.MCCID = cc.MCCID
						AND (mh.MerchantAcquirerBin = cc.OriginatorID OR mh.MerchantAcquirerBin IS NULL)
						AND mh.MerchantName LIKE cc.Narrative
		...
		--	AND (mh.MerchantAcquirerBin = cc.OriginatorID OR mh.MerchantAcquirerBin IS NULL)
		--	AND cc.PaymentGatewayStatusID != 1	--	not default Paypal
		--	AND (SUBSTRING(mh.MerchantName, 5, 999) LIKE cc.Narrative AND cc.IsHighVariance = 1)
		--WHERE mh.ConsumerCombinationID IS NULL
		...
		--	AND (mh.MerchantAcquirerBin = cc.OriginatorID OR mh.MerchantAcquirerBin IS NULL)
		--	AND cc.PaymentGatewayStatusID != 1	--	not default Paypal
		--	AND (SUBSTRING(mh.MerchantName, 4, 999) LIKE cc.Narrative AND cc.IsHighVariance = 1)
		--WHERE mh.ConsumerCombinationID IS NULL
...
*******************************************************************************************************************************************/
	...
	AND NOT EXISTS (SELECT 1
					...
					WHERE mh.TransactionID = cth.TransactionID)
