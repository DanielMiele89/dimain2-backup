...
*******************************************************************************************************************************************/
	...
	***********************************************************************************************************************/	
		...
		UPDATE cis
		SET cis.MCCID = mcc.MCCID
		FROM [MIDI].[CTLoad_InitialStage] cis
		CROSS APPLY (	SELECT	TOP(1)	--	non-deterministic UPDATE
								mcc.MCCID
						FROM [Warehouse].[Relational].[MCCList] mcc
						WHERE cis.MCC = mcc.MCC
						ORDER BY mcc.MCCID DESC) mcc;
	...
	***********************************************************************************************************************/
		...
		CREATE TABLE #LiveStuff ([CardGUID] uniqueidentifier, [SourceUID] VARCHAR(64), [BankID] VARCHAR(64), [RegistrationDate] DATE)

		CREATE CLUSTERED INDEX cx_Stuff ON #LiveStuff ([CardGUID], [RegistrationDate])

		INSERT INTO #LiveStuff WITH (TABLOCKX) ([CardGUID], [SourceUID], [BankID], [RegistrationDate]) 
		SELECT	ca.[CardGUID]
			,	cu.[SourceUID]
			,	ca.[BankID]
			,	cu.[RegistrationDate]
		...
		WHERE EXISTS (	SELECT 1
						FROM [MIDI].[CTLoad_InitialStage] cis
						WHERE cis.CardID = ca.CardGUID)
	
		UPDATE cis
		SET cis.CIN = ls.SourceUID
		,	cis.BankID = ls.BankID
		,	cis.PaymentTypeID = 2
		FROM [MIDI].[CTLoad_InitialStage] cis
		CROSS APPLY (	SELECT	TOP(1)	--	non-deterministic UPDATE
								ls.SourceUID
							,	ls.BankID
						FROM #LiveStuff ls
						WHERE cis.CardID = ls.CardGUID
						ORDER BY ls.[RegistrationDate]) ls;
...
*******************************************************************************************************************************************/
	...
	***********************************************************************************************************************/
		...
		FROM [MIDI].[CTLoad_InitialStage] cis WITH (TABLOCK)
		CROSS APPLY (	SELECT	TOP 1	--	non-deterministic update
								cc.ConsumerCombinationID
						FROM [Trans].[ConsumerCombination] cc
						WHERE cc.PaymentGatewayStatusID != 1	--	not default Paypal
						AND cis.MID = cc.MID
						AND cis.MerchantCountry = cc.LocationCountry
						AND cis.MCCID = cc.MCCID
						AND (cis.MerchantAcquirerBin = cc.OriginatorID OR cis.MerchantAcquirerBin IS NULL)
						AND cis.MerchantName = cc.Narrative
						AND cc.IsHighVariance = 0
						ORDER BY ConsumerCombinationID DESC) cc
		...
		--	AND (SUBSTRING(cis.MerchantName, 5, 999) = cc.Narrative AND cc.IsHighVariance = 0)
		...
		--	AND (cis.MerchantAcquirerBin = cc.OriginatorID OR cis.MerchantAcquirerBin IS NULL)
		--	AND cc.PaymentGatewayStatusID != 1	--	not default Paypal
		--	AND (SUBSTRING(cis.MerchantName, 4, 999) = cc.Narrative AND cc.IsHighVariance = 0)
	...
	***********************************************************************************************************************/
		...
		CROSS APPLY (	SELECT	TOP 1	--	non-deterministic update
						...
						AND cis.MCCID = cc.MCCID
						AND (cis.MerchantAcquirerBin = cc.OriginatorID OR cis.MerchantAcquirerBin IS NULL)
						AND cis.MerchantName LIKE cc.Narrative
						AND cc.IsHighVariance = 1
						ORDER BY ConsumerCombinationID DESC) cc
		WHERE cis.ConsumerCombinationID IS NULL
		...
		--	AND (cis.MerchantAcquirerBin = cc.OriginatorID OR cis.MerchantAcquirerBin IS NULL)
		--	AND cc.PaymentGatewayStatusID != 1	--	not default Paypal
		--	AND (SUBSTRING(cis.MerchantName, 5, 999) LIKE cc.Narrative AND cc.IsHighVariance = 1)
		--WHERE cis.ConsumerCombinationID IS NULL
		...
		--	AND (cis.MerchantAcquirerBin = cc.OriginatorID OR cis.MerchantAcquirerBin IS NULL)
		--	AND cc.PaymentGatewayStatusID != 1	--	not default Paypal
		--	AND (SUBSTRING(cis.MerchantName, 4, 999) LIKE cc.Narrative AND cc.IsHighVariance = 1)
		--WHERE cis.ConsumerCombinationID IS NULL
...
*******************************************************************************************************************************************/
	...
	INSERT INTO [MIDI].[ConsumerTransactionHolding] (	[TransactionID]
													,	[FileID]
													,	[RowNum]
													,	[ConsumerCombinationID]
	...
	AND NOT EXISTS (SELECT 1
					...
					WHERE cis.[TransactionID] = cth.[TransactionID])
...
*******************************************************************************************************************************************/
	...
	AND NOT EXISTS (SELECT 1
					FROM [MIDI].[CTLoad_MIDIHolding] cth
					WHERE cis.[TransactionID] = cth.[TransactionID])
...
*******************************************************************************************************************************************/
	...
	***********************************************************************************************************************/
	...
	/***********************************************************************************************************************
		10.2.	Store Transaction IDs of File processed
	***********************************************************************************************************************/

		INSERT INTO [MIDI].[ProcessedTransactionIDs] (	[TransactionID]
													,	[LoadDate]
													,	[FileName])
		SELECT	DISTINCT
				[TransactionID]
	...
	***********************************************************************************************************************/
	...
	/***********************************************************************************************************************
		10.4.	Mark the file as processsed
	***********************************************************************************************************************/
