...
*******************************************************************************************************************************************/
	...
	INSERT INTO [MIDI].[CTLoad_MIDINewCombo] (	MID
											...
											,	MatchCount)
	SELECT	MID
		,	OriginalNarrative =	MerchantName
		,	Narrative_Cleaned =	MerchantName
		,	MerchantCountry
		,	MCCID
		,	MerchantAcquirerBin
		,	COUNT(*)
...
*******************************************************************************************************************************************/
	...
	OUTER APPLY dbo.iTVF_NarrativeCleaner(-1,mnc.Narrative_Cleaned) q1
	...
	ALTER INDEX ALL ON [MIDI].[CTLoad_MIDINewCombo] REBUILD WITH (SORT_IN_TEMPDB = ON)

	SET @RowsAffected = @@ROWCOUNT; SET @Activity = ISNULL(OBJECT_NAME(@@PROCID),'SSMS') + ' - Indexes rebuilt on [MIDI].[CTLoad_MIDINewCombo] [' + CAST(@RowsAffected AS VARCHAR(10)) + ']'; EXEC Monitor.ProcessLogger 'MIDI', @Activity, @time OUTPUT, @SSMS OUTPUT
...
*******************************************************************************************************************************************/
		...
		FROM [WH_VirginPCA].[MIDI].[BrandMatch]
		WHERE BrandID != 944
		UNION
		SELECT	BrandID
			,	Narrative
		...
		CREATE NONCLUSTERED INDEX IX_NarrativeBrand ON #NarrativeBrandLookup (Narrative, BrandID)
...
*******************************************************************************************************************************************/
	...
	***********************************************************************************************************************/
		...
		CROSS APPLY (	SELECT	TOP 1
							...
							,	cc.BrandID
							,	COUNT(*) AS Matches
						FROM [WH_AllPublishers].[Staging].[MIDI_ConsumerCombination] cc
	...
	***********************************************************************************************************************/
		...
		CurveCard AS (	SELECT	MID
							,	MCCID
						FROM [WH_AllPublishers].[Staging].[MIDI_ConsumerCombination] c
	...
	***********************************************************************************************************************/
	...
	/*
		...
		INTO #FuzzyMatch
		FROM [MIDI].[CTLoad_MIDINewCombo] mnc
		CROSS JOIN [WH_AllPublishers].[Staging].[MIDI_ConsumerCombination] cc
...
*******************************************************************************************************************************************/
	...
	MCCIDs AS (	SELECT	BrandID
				...
				FROM (	SELECT	BrandID
						...
						FROM [WH_AllPublishers].[Staging].[MIDI_ConsumerCombination]
						GROUP BY	BrandID
								,	MCCID) c),
