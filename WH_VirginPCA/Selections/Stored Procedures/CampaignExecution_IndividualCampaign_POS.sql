...
DECLARE @ProcessName VARCHAR(50), @Activity VARCHAR(200), @time DATETIME = GETDATE(), @SSMS BIT, @RowsAffected INT
	...
	*******************************************************************************************************************************************/
		...
		***********************************************************************************************************************/
			...
			SET @CustomerBaseQuery =	'IF OBJECT_ID (''tempdb..##CustomerBase_VisaBarclaycard'') IS NOT NULL DROP TABLE ##CustomerBase_VisaBarclaycard
										SELECT DISTINCT 
											   cb.FanID
											 , cb.CompositeID
										...
										INTO ##CustomerBase_VisaBarclaycard
			...
			CREATE CLUSTERED INDEX CIX_CompositeID ON ##CustomerBase_VisaBarclaycard (CompositeID, ShopperSegmentTypeID, ControlGroupCustomer) WITH (DATA_COMPRESSION = PAGE, FILLFACTOR = 80, SORT_IN_TEMPDB = ON)
			CREATE NONCLUSTERED INDEX IX_FanID ON ##CustomerBase_VisaBarclaycard (FanID) WITH (DATA_COMPRESSION = PAGE, FILLFACTOR = 80, SORT_IN_TEMPDB = ON)
			CREATE NONCLUSTERED INDEX IX_CompSeg ON ##CustomerBase_VisaBarclaycard (CompositeID, ShopperSegmentTypeID) WITH (DATA_COMPRESSION = PAGE, FILLFACTOR = 80, SORT_IN_TEMPDB = ON)

			SET @Activity = ISNULL(OBJECT_NAME(@@PROCID),'SSMS') + ' - Collecting ##CustomerBase_VisaBarclaycard [' + CAST(@RowsAffected AS VARCHAR(10)) + ']'; EXEC [Monitor].[ProcessLogger] 'Selections', @Activity, @time OUTPUT, @SSMS OUTPUT
		...
		***********************************************************************************************************************/
			...
			***********************************************************************************************************************/
				...
				--		FROM ##CustomerBase_VisaBarclaycard cb
		...
		/***********************************************************************************************************************
			4.5. Indexing ##CustomerBase_VisaBarclaycard
		***********************************************************************************************************************/

			ALTER INDEX CIX_CompositeID ON ##CustomerBase_VisaBarclaycard REBUILD WITH (DATA_COMPRESSION = PAGE, FILLFACTOR = 80, SORT_IN_TEMPDB = ON)
			ALTER INDEX IX_FanID ON ##CustomerBase_VisaBarclaycard REBUILD WITH (DATA_COMPRESSION = PAGE, FILLFACTOR = 80, SORT_IN_TEMPDB = ON)
			ALTER INDEX IX_CompSeg ON ##CustomerBase_VisaBarclaycard REBUILD WITH (DATA_COMPRESSION = PAGE, FILLFACTOR = 80, SORT_IN_TEMPDB = ON)
			
			SET @Activity = ISNULL(OBJECT_NAME(@@PROCID),'SSMS') + ' - Indexing ##CustomerBase_VisaBarclaycard'; EXEC [Monitor].[ProcessLogger] 'Selections', @Activity, @time OUTPUT, @SSMS OUTPUT
	...
	*******************************************************************************************************************************************/
		...
		LEFT JOIN [Segmentation].[OfferMemberAddition] oma
								ON	s.CompositeID = oma.CompositeID
								AND s.IronOfferID = oma.IronOfferID
								AND s.StartDate = oma.StartDate
								AND s.EndDate = oma.EndDate
	...
	*******************************************************************************************************************************************/
		...
		IF OBJECT_ID('tempdb..#LiveOfferPerPartner') IS NOT NULL DROP TABLE #LiveOfferPerPartner
		IF OBJECT_ID('tempdb..#CustomersOnOfferAlready') IS NOT NULL DROP TABLE #CustomersOnOfferAlready
		IF OBJECT_ID('tempdb..##CustomerBase_VisaBarclaycard') IS NOT NULL DROP TABLE ##CustomerBase_VisaBarclaycard
		IF OBJECT_ID('tempdb..#SelectedInAnotherCampaign') IS NOT NULL DROP TABLE #SelectedInAnotherCampaign
		IF OBJECT_ID('tempdb..#SelectedInAnotherCampaignCustomers') IS NOT NULL DROP TABLE #SelectedInAnotherCampaignCustomers
		IF OBJECT_ID('tempdb..#ExistingUniverseOffers') IS NOT NULL DROP TABLE #ExistingUniverseOffers
		IF OBJECT_ID('tempdb..#ExistingUniverse') IS NOT NULL DROP TABLE #ExistingUniverse
