	...
	BEGIN TRY
		...
		*******************************************************************************************************************************************/
			...
			SELECT	ROW_NUMBER() OVER (ORDER BY t.name) AS RowID
				,	t.name AS TableName
				,	'WH_VirginPCA.' + s.name + '.' + t.name AS SourceTable
			INTO #InboundTables
			FROM [sys].[tables] t
			INNER JOIN [sys].[schemas] s
				ON t.schema_id = s.schema_id
			WHERE s.name = 'Inbound'
			AND t.name IN ('Balances', 'BankAccountCustomerLinks', 'BankAccountNominees', 'BankAccounts', 'Cards', 'CharityOffers', 'CharityRedemptions', 'CustomerExternalIds', 'Customers', 'Goodwill', 'Login', 'Offer', 'OfferDetail', 'RedemptionItems', 'RedemptionOffers', 'RedemptionPartners', 'Redemptions')
			...
			INSERT INTO [WHB].[Inbound_Files]
			SELECT	DISTINCT	
					TableName = 'MatchedTransactions'
				,	LoadDate = CONVERT(DATE, st.LoadDate)
				,	FileName
				,	FileProcessed = 0
			FROM [Inbound].[MatchedTransactions] st
			WHERE NOT EXISTS (	SELECT 1
								FROM [WHB].[Inbound_Files] f
								WHERE st.FileName = f.FileName
								AND CONVERT(DATE, st.LoadDate) = CONVERT(DATE, f.LoadDate)
								AND f.TableName = 'MatchedTransactions')
			ORDER BY	FileName
					,	LoadDate
			...
			WHERE NOT EXISTS (	SELECT 1
								...
								WHERE st.FileName = f.FileName
								AND CONVERT(DATE, st.LoadDate) = CONVERT(DATE, f.LoadDate)
								AND f.TableName = 'Transactions_DD')
			ORDER BY	FileName
					,	LoadDate
