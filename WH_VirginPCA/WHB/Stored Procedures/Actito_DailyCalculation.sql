...
BEGIN
	...
	*******************************************************************************************************************************************/
		...
		UPDATE #Customer
		SET Email = 'VirginClosedAccount@RewardInsight.com'
		,	Title = ''
		,	FirstName = ''
		,	LastName = ''
		,	DOB = '1900-01-01'
	...
	*******************************************************************************************************************************************/
		...
		***************************************************************************************************************************************/
			...
			DECLARE @Saturday_FirstEarn DATE = DATEADD(DAY, -2, GETDATE())
		...
		***************************************************************************************************************************************/
			...
			AND EXISTS (SELECT 1
						FROM #Customer cu
						WHERE fet.FanID = cu.FanID
						AND cu.CashbackPending > 0)
		...
		***************************************************************************************************************************************/
			...
			DECLARE @Yesterday_FirstEarn DATE = DATEADD(DAY, -1, GETDATE())

			IF OBJECT_ID('tempdb..#FirstEarnPOSEmailCustomers') IS NOT NULL DROP TABLE #FirstEarnPOSEmailCustomers
			SELECT	fed.FanID
				,	fed.PartnerName
				,	@Yesterday_FirstEarn AS TranDate	--	fed.TranDate
				,	fed.CashbackAmount
			...
			--IF DATENAME (dw , @Today_FirstEarn) = 'Monday'
			--	BEGIN
				
			--		--SET @Saturday_FirstEarn = '2021-06-14'

			--		INSERT INTO #FirstEarnPOSEmailCustomers
			--		SELECT	fed.FanID
			--			,	fed.PartnerName
			--			,	@Yesterday_FirstEarn AS TranDate	--	fed.TranDate
			--			,	fed.CashbackAmount
			--			,	'POS' AS FirstEarn_Type
			--		FROM [Derived].[Customer_FirstEarnDate] fed
			--		WHERE CONVERT(DATE, fed.LoadDate) >= @Saturday_FirstEarn
			--		AND CONVERT(DATE, fed.LoadDate) < @Today_FirstEarn
			--		AND EXISTS (SELECT 1
			--					FROM [Derived].[Customer] cu
	...
	*******************************************************************************************************************************************/
		...
		***************************************************************************************************************************************/
			...
			SELECT	cb.FanID
				,	CASE
						WHEN DATENAME(WEEKDAY, cb.Date) = 'Monday' THEN COALESCE(cb.PreviousCashbackAvailable_3Day, cb.PreviousCashbackAvailable_2Day, cb.PreviousCashbackAvailable_1Day, 0)
						ELSE COALESCE(cb.PreviousCashbackAvailable_1Day, 0)
					END AS PreviousCashbackAvailable
			INTO #Customer_CashbackBalances
			FROM (	SELECT	cb.ID
						,	cb.FanID
						,	cb.Date
						,	LAG(cb.CashbackAvailable) OVER (PARTITION BY cb.FanID ORDER BY cb.Date) AS PreviousCashbackAvailable_1Day
						,	LAG(cb.CashbackAvailable, 2) OVER (PARTITION BY cb.FanID ORDER BY cb.Date) AS PreviousCashbackAvailable_2Day
						,	LAG(cb.CashbackAvailable, 3) OVER (PARTITION BY cb.FanID ORDER BY cb.Date) AS PreviousCashbackAvailable_3Day
						,	cb.CashbackAvailable
					FROM [Derived].[Customer_CashbackBalances] cb) cb
			WHERE DATENAME(WEEKDAY, cb.Date) NOT IN ('Saturday', 'Sunday')
			AND cb.Date = @Today_RedemptionReminderValue

			CREATE CLUSTERED INDEX CIX_CashbackAvailable ON #Customer_CashbackBalances (FanID)
			...
			FROM [Derived].[Customer] cu
			INNER JOIN #Customer_CashbackBalances cb
	...
	*******************************************************************************************************************************************/
		...
		***************************************************************************************************************************************/
			...
			SELECT	FanID = cu.FanID
				,	Email = CASE
								WHEN cu.EmailStructureValid = 0 THEN 'VisaInvalidEmail@RewardInsight.com'
								ELSE cu.Email
							END
