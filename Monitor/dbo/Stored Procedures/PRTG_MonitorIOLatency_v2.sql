CREATE PROCEDURE [dbo].[PRTG_MonitorIOLatency_v2]
	(@DBName VARCHAR(50))
AS
...
FROM (
	SELECT 
		[IntervalSec],
		[type_desc],
		[io_stall_read_ms] = [io_stall_read_ms]/([IntervalSec] * 1.0), 
		[num_of_reads] = [num_of_reads]/([IntervalSec] * 1.0),  
		[io_stall_write_ms] = [io_stall_write_ms]/([IntervalSec] * 1.0),  
		[num_of_writes] = [num_of_writes]/([IntervalSec] * 1.0), 
		--[io_stall],   
		[num_of_bytes_read] = [num_of_bytes_read]/([IntervalSec] * 1.0),   
		[num_of_bytes_written] = [num_of_bytes_written]/([IntervalSec] * 1.0)
	FROM @Results
) d 
