CREATE PROCEDURE [Reporting].[MonthlyStats_CashbackOverview_Fetch_VirginPCA]
(
	@MonthDate DATE = NULL
)
AS
BEGIN
	...
	SELECT 
		...
		, md.StartDate
		, md.EndDate
	FROM WH_VirginPCA.derived.Customer_CashbackBalances cb
	JOIN WH_VirginPCA.derived.Customer c
		ON c.FanID = cb.FanID
	JOIN MonthDates md
