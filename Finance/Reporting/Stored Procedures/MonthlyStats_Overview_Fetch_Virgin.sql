...
CREATE PROCEDURE [Reporting].[MonthlyStats_Overview_Fetch_Virgin]
(
	@MonthDate DATE = NULL
)
AS
BEGIN
	...
	SELECT 
		...
		, FanID
		, c.RegistrationDate
		, ISNULL(c.DeactivatedDate, '9999-12-31') DeactivatedDate
	INTO #Fans
	...
	FROM (
		SELECT  
			...
			, md.startdate
			, md.enddate
		FROM WH_Virgin.Trans.ConsumerTransaction ct -- Get all transactions
		JOIN WH_Virgin.Trans.ConsumerCombination cc -- with the associated merchant details
	...
	FROM (
		...
		WHERE NOT EXISTS ( -- and they are not a nonqualifying mcc
				SELECT 1
				FROM WH_Virgin.Trans.NonQualifyingMCCS nm -- this list was provided by the client, in consumercombination, MCCs are converted to MCCIDs
	...
	FROM (
		SELECT
			...
			, md.StartDate
			, md.EndDate
		FROM WH_Virgin.Derived.PartnerTrans pt -- Matched transactions i.e. transactions from ConsumerTransaction that was incentivised and therefore a customer earned cashback
		JOIN #Fans f
			ON pt.FanID = f.FanID
		JOIN #MonthDates md
