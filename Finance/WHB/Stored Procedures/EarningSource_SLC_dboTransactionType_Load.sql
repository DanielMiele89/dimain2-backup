...
CREATE PROC [WHB].[EarningSource_SLC_dboTransactionType_Load]
		@RunID INT = NULL
AS
BEGIN
	 SET NOCOUNT ON;
	...
	(
		SELECT 
			...
			, -1 AS PartnerID
			, 0 AS isBankFunded
			, tt.description AS SourceName
			, NULL AS AdditionalInfo1
			, NULL AS AdditionalInfo2
			, tt.description AS DisplayName
			, 'Other' AS FundingType
		FROM DIMAIN_TR.SLC_REPL.dbo.TransactionType tt 
		WHERE ID NOT IN (1, 2, 5, 9, 10, 13, 14, 17, 23, 24)
	) x
