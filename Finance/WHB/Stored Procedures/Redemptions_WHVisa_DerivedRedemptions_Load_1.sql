CREATE PROC [WHB].[Redemptions_WHVisa_DerivedRedemptions_Load]
		@RunID INT = NULL
AS
BEGIN
	 SET NOCOUNT ON;
	...
	(
		SELECT
			...
			, r.CashbackUsed AS RedemptionWorth
			, c.SourceUID AS SourceCustomerID
			, r.RedeemedDate AS RedemptionDate
			, r.RedeemedDate AS RedemptionDateTime
			, r.RedemptionOfferGUID AS SourceRedemptionItemID
			, 0 as isCancelled
		FROM WH_Visa.Derived.Redemptions r
		JOIN WH_Visa.Derived.Customer c
			ON r.CustomerGUID = c.CustomerGuid
		WHERE NOT EXISTS (
			SELECT 1
