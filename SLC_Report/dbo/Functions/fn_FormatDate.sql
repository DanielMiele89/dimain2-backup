...
CREATE FUNCTION [dbo].[fn_FormatDate] (@Date datetime, @Format varchar(256))
...
/*
	...
	HHHH 	The two-digit hour in 24-hour format. Single-digit values are preceded by a zero. 
	...
	y 	The year is displayed as the last two digits, but with no leading zero for any year that is less than 10. 
...
*/
	...
	DECLARE @s varchar(2)		-- seconds
	DECLARE @ss char(2)

	DECLARE @w varchar(2)		-- weeks
	DECLARE @ww char(2)
	...
	SET @s = CAST(DATEPART(s, @Date) AS VARCHAR(2))
	SET @ss = RIGHT('0' + @s, 2)

	SET @w = CAST(DATEPART(wk, @Date) AS VARCHAR(2))
	SET @ww = RIGHT('0' + @w, 2)
	...
	SET @Format = REPLACE(@Format, 'ss', '\\\\20\\\\')
	SET @Format = REPLACE(@Format, 's', '\\\\21\\\\')
	SET @Format = REPLACE(@Format, 'ww', '\\\\22\\\\')
	SET @Format = REPLACE(@Format, 'w', '\\\\23\\\\')
	...
	SET @Format = REPLACE(@Format, '\\\\20\\\\', @ss)
	SET @Format = REPLACE(@Format, '\\\\21\\\\', @s)
	SET @Format = REPLACE(@Format, '\\\\22\\\\', @ww)
	SET @Format = REPLACE(@Format, '\\\\23\\\\', @w)
