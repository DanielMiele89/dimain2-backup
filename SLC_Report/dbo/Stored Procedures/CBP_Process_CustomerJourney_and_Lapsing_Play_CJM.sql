...
*/
--=====================================================================
CREATE PROCEDURE [dbo].[CBP_Process_CustomerJourney_and_Lapsing_Play_CJM]
...
BEGIN
	...
	--BEGIN
		...
		------------------------------------------------------------------------------------------------------------------		
		TRUNCATE TABLE dbo.CustomerJourneyStaging; EXEC dbo.oo_TimerMessageV2 'Cleaning data : CustomerJourneyStaging ', @time OUTPUT, @SSMS OUTPUT		
		TRUNCATE TABLE dbo.CustomerLapse; EXEC dbo.oo_TimerMessageV2 'Cleaning data : CustomerLapse ', @time OUTPUT, @SSMS OUTPUT
		...
		inner join #CustomerJourney as cjs
			on	cj.FanID = cjs.FanID 
			and cj.[StartDate] = cjs.[Date]
		...
		inner join #CustomerJourney as a
			on	cj.FanID = a.FanID 
			and cj.ShortCode <> a.ShortCode 
			and cj.EndDate is null
		Left Outer join dbo.[CustomerJourney] as cj2
			on  cj2.fanid = a.FanID 
			and Left(a.ShortCode,1) = 'M' 
			and Left(cj2.ShortCode,1) not in  ('M','D')
		...
		INNER JOIN #CustomerJourney AS CJ 
			ON CJ.FanID = F.FanID
