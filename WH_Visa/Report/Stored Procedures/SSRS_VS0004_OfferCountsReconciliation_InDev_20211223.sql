...
CREATE PROCEDURE [Report].[SSRS_VS0004_OfferCountsReconciliation_InDev_20211223] @EmailDate DATE

AS 
BEGIN
		
	--	DECLARE @EmailDate DATE = '2021-12-30'
	...
	*******************************************************************************************************************************************/
		...
		SELECT EmailDate
			 , ClientServicesRef
			 , TRY_CONVERT(INT, iof.Item) AS OfferID
			 , sg.Item AS Segment
		...
		CROSS APPLY [Warehouse].[dbo].[il_SplitDelimitedStringArray] (REPLACE(OfferID, '00000', 'x'), ',') iof
	...
	*******************************************************************************************************************************************/
		...
		CampaignSetup_POS AS (	SELECT DISTINCT 
								...
								AND iof.ItemNumber = sg.ItemNumber
	...
	*******************************************************************************************************************************************/
		...
		SELECT EmailDate
			 ...
			 , CASE
					WHEN UpcomingCount_ClientServicesRef BETWEEN PreviousCount_ClientServicesRef * 0.80 AND PreviousCount_ClientServicesRef * 1.20 THEN 0
					ELSE 1
			   END AS OutsideThreshold_ClientServicesRef
			 , CASE
					WHEN UpcomingCount_PartnerID BETWEEN PreviousCount_PartnerID * 0.80 AND PreviousCount_PartnerID * 1.20 THEN 0
					ELSE 1
	...
	*******************************************************************************************************************************************/
		...
		SELECT DISTINCT
			 ...
			 , IronOfferName
			 , OfferCampaign = COALESCE(ion.OfferCampaign, cd.CampaignName)
			 , OfferSegment = COALESCE(ion.OfferSegment, cd.Segment)
