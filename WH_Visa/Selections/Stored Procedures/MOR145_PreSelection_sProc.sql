...
JOIN	WH_Visa.Derived.CINList CL 
...
IF OBJECT_ID('Sandbox.rukank.Barclays_Morrisons_LoW_SoW_01042022') IS NOT NULL DROP TABLE Sandbox.rukank.Barclays_Morrisons_LoW_SoW_01042022		
SELECT	CINID
INTO	Sandbox.rukank.Barclays_Morrisons_LoW_SoW_01042022
FROM	#Trans_bc
WHERE	BrandShopper = 1
	...
	IF OBJECT_ID('[WH_Visa].[Selections].[MOR145_PreSelection]') IS NOT NULL DROP TABLE [WH_Visa].[Selections].[MOR145_PreSelection]
	SELECT FanID
	INTO [WH_Visa].[Selections].[MOR145_PreSelection]
	FROM #FB_bc fb
	...
	INSERT INTO [WH_Visa].[Selections].[MOR145_PreSelection]
	SELECT FanID
	FROM #FB_bc fb
	WHERE EXISTS (	SELECT 1
					FROM Sandbox.rukank.Barclays_Morrisons_LoW_SoW_01042022  st
