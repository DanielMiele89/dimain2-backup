...
CREATE PROCEDURE [Selections].[PM019_PreSelection_sProc]
AS
BEGIN
...
SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED
	...
	*******************************************************************************************************************************************/
		...
		***********************************************************************************************************************/
			...
			INNER JOIN Warehouse.Relational.CINList cl WITH (NOLOCK)
			...
			SELECT cu.FanID
				 , cl.CINID
				 , ROW_NUMBER() OVER (ORDER BY cu.FanID Desc) AS RowNo
			FROM WH_Visa.derived.Customer cu WITH (NOLOCK)
			INNER JOIN WH_Visa.derived.CINList cl WITH (NOLOCK)
				ON cu.SourceUID = cl.CIN
			WHERE cu.CurrentlyActive = 1
		...
		***********************************************************************************************************************/
			...
			SELECT	ConsumerCombinationID
				,	0 AS MyRewards
				,	1 AS Virgin
			FROM WH_Visa.Trans.ConsumerCombination cc WITH (NOLOCK)
	...
	*******************************************************************************************************************************************/
			...
			SELECT	FanID
			FROM WH_Visa.derived.[PartnerTrans]
			WHERE PartnerID = @PartnerID
	...
	*******************************************************************************************************************************************/
		...
		IF OBJECT_ID('Sandbox.Rory.PoochMuttLapsing_20210617_VBC') IS NOT NULL DROP TABLE Sandbox.Rory.PoochMuttLapsing_20210617_VBC
		SELECT *
		INTO Sandbox.Rory.PoochMuttLapsing_20210617_VBC
		FROM #AllCustomers
		WHERE Segment = 'Lapsing'



IF OBJECT_ID('[WH_Visa].[Selections].[PM019_PreSelection]') IS NOT NULL DROP TABLE [WH_Visa].[Selections].[PM019_PreSelection]
SELECT	FanID
INTO [WH_Visa].[Selections].[PM019_PreSelection]
FROM Sandbox.Rory.PoochMuttLapsing_20210617_VBC
