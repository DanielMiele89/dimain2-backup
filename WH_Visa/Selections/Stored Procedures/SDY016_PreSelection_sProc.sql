...
BEGIN
	...
	-- shoppers - SOW - >= 66%
	IF OBJECT_ID('Sandbox.samh.superdry_sow66_BC_25012021') IS NOT NULL DROP TABLE Sandbox.samh.superdry_sow66_BC_25012021
	SELECT	F.CINID, FanID
	INTO	Sandbox.samh.superdry_sow66_BC_25012021
	FROM	#shoppper_sow F
	WHERE	SuperdryShopper = 1
			AND SuperdrySoW >= 0.6666
	GROUP BY F.CINID, FanID

	IF OBJECT_ID('[WH_Visa].[Selections].[SDY016_PreSelection]') IS NOT NULL DROP TABLE [WH_Visa].[Selections].[SDY016_PreSelection]
	SELECT FanID
	INTO [WH_Visa].[Selections].[SDY016_PreSelection]
	FROM Sandbox.samh.superdry_sow66_BC_25012021
