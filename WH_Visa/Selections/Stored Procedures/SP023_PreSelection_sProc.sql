...
IF OBJECT_ID('tempdb..#Trans_lapsed_bc') IS NOT NULL DROP TABLE #Trans_lapsed_bc
SELECT	F.CINID
...
IF OBJECT_ID('Sandbox.RukanK.BC_SpaceNK_Lapsed_Customers') IS NOT NULL DROP TABLE Sandbox.RukanK.BC_SpaceNK_Lapsed_Customers
SELECT	CINID
INTO	Sandbox.RukanK.BC_SpaceNK_Lapsed_Customers
FROM	#Trans_lapsed_bc
WHERE	CINID NOT IN (SELECT CINID FROM #Trans_bc)


	IF OBJECT_ID('[WH_Visa].[Selections].[SP023_PreSelection]') IS NOT NULL DROP TABLE [WH_Visa].[Selections].[SP023_PreSelection]
	SELECT FanID
	INTO [WH_Visa].[Selections].[SP023_PreSelection]
	FROM #FB_bc fb
	WHERE EXISTS (	SELECT 1
					FROM Sandbox.RukanK.BC_SpaceNK_Lapsed_Customers  st
