...
BEGIN
	...
	WHILE (SELECT COUNT(*) FROM [WHB].[Inbound_Files] f WHERE TableName = 'Customers' AND FileProcessed = 0) > 0
		...
		*******************************************************************************************************************************************/
			...
			WHEN MATCHED THEN
				UPDATE SET	target.CustomerID			= source.CustomerID	-- If matched, update to new value
						...
						,	target.MarketableByEmail	= COALESCE(source.MarketableByEmail, 0)
						,	target.EmailTracking		= COALESCE(source.EmailTracking, 0)
						...
						,	target.OptedOutDate			= source.OptOutDate
			...
			WHEN NOT MATCHED THEN											-- If not matched, add new rows
				...
				VALUES (source.CustomerGUID
					...
					,	source.EmailAddress
					,	source.BankID
					,	COALESCE(source.MarketableByEmail, 0)
					,	COALESCE(source.EmailTracking, 0)
