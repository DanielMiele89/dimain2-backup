	...
	BEGIN TRY
		...
		*******************************************************************************************************************************************/
			...
			SELECT	ROW_NUMBER() OVER (ORDER BY t.name) AS RowID
				,	t.name AS TableName
				,	'WH_Visa.' + s.name + '.' + t.name AS SourceTable
			INTO #InboundTables
			FROM [sys].[tables] t
			INNER JOIN [sys].[schemas] s
				ON t.schema_id = s.schema_id
			WHERE s.name = 'Inbound'
			AND t.name IN ('Accounts', 'Balances', 'Cards', 'Customers', 'Goodwill', 'Login', 'Offer', 'OfferDetail', 'Redemptions', 'CharityOffers', 'CharityRedemptions', 'RedemptionItems', 'RedemptionOffers', 'RedemptionPartners')
