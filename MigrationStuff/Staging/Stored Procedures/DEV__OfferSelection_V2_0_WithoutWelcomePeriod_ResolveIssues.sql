...
CREATE Procedure [Staging].[DEV__OfferSelection_V2_0_WithoutWelcomePeriod_ResolveIssues] (@OfferDate date,@PartnerID int,@RunType char(1),@ClubID int)
as
...
Begin
		...
		--------------------------------------------------------------------------------------
		IF OBJECT_ID('Sandbox.stuart.AlreadyonOffer') IS NOT NULL 
													DROP TABLE Sandbox.stuart.AlreadyonOffer
		Select	--IOM.ID as IronofferMemberID,
				...
				Case
					...
					Else 0
				End as [New]
		Into Sandbox.stuart.AlreadyonOffer
		From	#Offers as o
		...
		--Left Outer join Sandbox.stuart.AlreadyonOffer as a
		--	on c.fanid = a.FanID
		Where	--a.FanID is null and
				c.CompositeID = 10022001893
		Order by RegistrationDate

End

Select * from #IronOfferAdditions
--iF @RunType = 'B'
--Begin
