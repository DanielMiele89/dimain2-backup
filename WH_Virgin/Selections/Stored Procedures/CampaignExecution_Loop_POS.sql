			...
			***********************************************************************************************************************/
				...
				IF @PreviouslyRanPartnerID != @PartnerID
					BEGIN
						...
						IF OBJECT_ID('tempdb..##CurrentCustomerSegment_Virgin') IS NOT NULL DROP TABLE ##CurrentCustomerSegment_Virgin
						SELECT	ccs.PartnerID
							,	ccs.FanID
							,	ccs.ShopperSegmentTypeID
						INTO ##CurrentCustomerSegment_Virgin
						FROM [Segmentation].[CurrentCustomerSegment] ccs
						WHERE ccs.PartnerID = @PartnerID

						CREATE CLUSTERED INDEX CIX_FanID ON ##CurrentCustomerSegment_Virgin (FanID) WITH (DATA_COMPRESSION = ROW, FILLFACTOR = 80, SORT_IN_TEMPDB = ON)
						...
						EXEC('	TRUNCATE TABLE [Selections].[CampaignExecution_CustomerBase]
								...
								INNER JOIN ##CurrentCustomerSegment_Virgin ccs
						...
						DROP TABLE ##CurrentCustomerSegment_Virgin
			...
			***********************************************************************************************************************/
				...
				IF @PartnerID IN (4805, 4825)	--	Ideal World, KitBag
					BEGIN 
						...
						INNER JOIN [DIMAIN_TR].[SLC_REPL].[dbo].[IronOffer] iofp
							ON pps.OfferID = iofp.ID
						INNER JOIN [DIMAIN_TR].[SLC_REPL].[dbo].[IronOfferClub] iofcp
						...
						INNER JOIN [DIMAIN_TR].[SLC_REPL].[dbo].[Partner] p
							ON pa.PartnerID = p.ID
						INNER JOIN [DIMAIN_TR].[SLC_REPL].[dbo].[IronOffer] iofs
						...
						INNER JOIN [DIMAIN_TR].[SLC_REPL].[dbo].[IronOfferClub] iofcs
