...
SELECT   F.CINID
FROM #FB F
JOIN [WH_Visa].[Derived].[PartnerTrans] PT on #FB.[Pt].FanID = F.FanID
WHERE #FB.[PT].PartnerID = 4263
AND #FB.[TransactionDate] BETWEEN '2021-08-19' AND '2021-09-22'
AND #FB.[TransactionAmount] > 0
AND #FB.[PT].IronOfferID IN (-1081,-1082,-1080)
...
FROM #Responders F
GROUP BY F.CINID
If Object_ID('[WH_Virgin].Selections.MOR112_PreSelection') Is Not Null Drop Table [WH_Virgin].Selections.MOR112_PreSelectionSelect [fb].[FanID]Into [WH_Virgin].Selections.MOR112_PreSelectionFROM  #FB fbWHERE EXISTS (	SELECT 1				FROM Sandbox.rukank.Morrisons_Nursery_16092021 st				WHERE fb.CINID = #FB.[st].CINID)END
