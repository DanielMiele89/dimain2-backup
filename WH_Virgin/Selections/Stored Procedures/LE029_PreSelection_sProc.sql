CREATE PROCEDURE [Selections].[LE029_PreSelection_sProc]
AS
BEGIN
...
IF OBJECT_ID('[WH_Virgin].[Selections].[LE029_PreSelection_Top]') IS NOT NULL DROP TABLE [WH_Virgin].[Selections].[LE029_PreSelection_Top]
SELECT	fb.FanID
INTO [WH_Virgin].[Selections].[LE029_PreSelection_Top]
FROM #FB fb
WHERE EXISTS (	SELECT 1
				FROM Sandbox.bastienc.VM_landsend_top50pct sb
				WHERE fb.CINID = sb.CINID)

IF OBJECT_ID('[WH_Virgin].[Selections].[LE029_PreSelection_Bottom]') IS NOT NULL DROP TABLE [WH_Virgin].[Selections].[LE029_PreSelection_Bottom]
SELECT	fb.FanID
INTO [WH_Virgin].[Selections].[LE029_PreSelection_Bottom]
FROM #FB fb
WHERE EXISTS (	SELECT 1
				FROM Sandbox.bastienc.VM_landsend_bottom50pct sb
				WHERE fb.CINID = sb.CINID)
