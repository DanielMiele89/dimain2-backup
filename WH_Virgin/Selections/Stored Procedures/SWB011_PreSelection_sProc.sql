-- =============================================-- Author:  <Rory Francis>-- Create date: <2021-08-05>-- Description: < sProc to run preselection code per camapign >-- =============================================CREATE PROCEDURE [Selections].[SWB011_PreSelection_sProc]ASBEGIN
...
WHERE	[WH_Virgin].[trans].[ConsumerCombination].[BrandID] IN (2434,2592,2777,3165,574,568)	
...
SELECT	 #Trans.[CINID]
		, NTILE(2) OVER (ORDER BY #Trans.[Classification_Score] ASC, #Trans.[Engagement_Score] DESC) AS NTILE_2
INTO	#NtileEngaged
FROM	#Trans


IF OBJECT_ID('Sandbox.RukanK.VM_SweatyBetty_CompSteal_20pct_20072021') IS NOT NULL DROP TABLE Sandbox.RukanK.VM_SweatyBetty_CompSteal_20pct_20072021
SELECT	#Trans.[CINID]
INTO Sandbox.RukanK.VM_SweatyBetty_CompSteal_20pct_20072021
FROM #Trans
WHERE	#Trans.[CINID] IN (SELECT #NtileEngaged.[CINID] FROM #NtileEngaged WHERE #NtileEngaged.[NTILE_2] IN (1))If Object_ID('WH_Virgin.Selections.SWB011_PreSelection') Is Not Null Drop Table WH_Virgin.Selections.SWB011_PreSelectionSelect [WH_Virgin].[derived].[Customer].[FanID]Into WH_Virgin.Selections.SWB011_PreSelectionFROM WH_Virgin.derived.Customer cuWHERE EXISTS (	SELECT 1				FROM WH_Virgin.derived.[CINList] cl				INNER JOIN Sandbox.RukanK.VM_SweatyBetty_CompSteal_20pct_20072021 fo					ON cl.CINID = fo.CINID				WHERE cu.SourceUID = cl.CIN)END