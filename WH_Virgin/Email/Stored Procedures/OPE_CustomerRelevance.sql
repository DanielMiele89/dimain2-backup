...
CREATE PROCEDURE [Email].[OPE_CustomerRelevance]
AS
BEGIN
...
--		WHERE op.EmailDate > GETDATE()
	...
	*******************************************************************************************************************************************/
		...
		SELECT	DISTINCT
			...
			,	cl.CINID
			,	cu.ClubID
			,	PremiumCustomer =	CASE
										WHEN cu.AccountType LIKE '%VAA%' THEN 1
										ELSE 0
									END
	...
	*******************************************************************************************************************************************/
		...
		JOIN	Warehouse.Relational.Partner P ON P.BrandID = B.BrandID
		--JOIN	Warehouse.Relational.ConsumerCombination CC ON B.BrandID = CC.BrandID
