	...
	*******************************************************************************************************************************************/
		...
		***************************************************************************************************************************************/
			...
			WHERE EXISTS (	SELECT 1
							...
							WHERE #RedemptionReminder_DaysEmailCustomers.[re].CustomerID = rrdc.FanID)
	...
	*******************************************************************************************************************************************/
		...
		***************************************************************************************************************************************/
			...
			SELECT DISTINCT
				   cu.FanID
				 , Email = CASE
								WHEN cu.CurrentlyActive = 0 THEN 'VirginClosedAccount@RewardInsight.com'
								WHEN cu.EmailStructureValid = 0 THEN 'VirginInvalidEmail@RewardInsight.com'
								ELSE cu.Email
							END
				 , cu.ClubID AS PublisherID
				 , CustomerSegment = COALESCE(cs.CustomerSegment, '')
				 , Title = CASE WHEN cu.CurrentlyActive = 0 THEN '' ELSE COALESCE(cu.Title, '') END
				 , FirstName = CASE WHEN cu.CurrentlyActive = 0 THEN '' ELSE cu.FirstName END
				 , LastName = CASE WHEN cu.CurrentlyActive = 0 THEN '' ELSE cu.LastName END
				 , DOB = CASE WHEN cu.CurrentlyActive = 0 THEN '1900-01-01' ELSE cu.DOB END
				 , cu.CashbackAvailable AS CashbackAvailable
				 , cu.CashbackPending AS CashbackPending
				 , cu.CashbackLTV AS CashbackLTV
				 , PartialPostCode = CASE WHEN cu.CurrentlyActive = 0 THEN '' ELSE cu.PostCodeDistrict END
				 , Marketable = CASE WHEN cu.CurrentlyActive = 0 THEN 0 ELSE cu.MarketableByEmail END
