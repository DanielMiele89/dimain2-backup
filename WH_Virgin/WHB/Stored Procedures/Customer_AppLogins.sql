...
BEGIN
	...
	BEGIN TRY
		...
		*******************************************************************************************************************************************/
			...
			LEFT JOIN [Derived].[LoginInfo] li -- left join in the event that the table isn't updated otherwise rows can be skipped from processing
		...
		*******************************************************************************************************************************************/
			...
			IF OBJECT_ID('tempdb..#LoginInfoMissing') IS NOT NULL DROP TABLE #LoginInfoMissing
			SELECT	dl.ID
				,	il.LoginInformation
			INTO #LoginInfoMissing
			FROM [Derived].[AppLogins] dl
			INNER JOIN [Inbound].[Login] il
				ON dl.FanID = il.CustomerID
			...
			FROM [Derived].[AppLogins] dl
			INNER JOIN #LoginInfoMissing lim
				ON dl.ID = lim.ID
			INNER JOIN [Derived].[LoginInfo] li
				ON lim.LoginInformation = li.UserAgent
