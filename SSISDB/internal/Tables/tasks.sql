...
CREATE NONCLUSTERED INDEX [index_tasks_ReadyForDispatchTime]
    ...
    INCLUDE([CreatedTime], [CreateWorkerAgentId], [ExecutedCount], [ExpiredTime], [InputData], [IsCritical], [JobId], [LastUpdatedTime], [MaxExecutedCount], [Priority], [Status], [TaskType]) WITH (FILLFACTOR = 90);


GO
CREATE NONCLUSTERED INDEX [index_tasks_Status]
    ON [internal].[tasks]([Status] ASC)
    INCLUDE([ExecutedCount], [ExpiredTime], [IsCancelled], [MaxExecutedCount], [WorkerAgentId]) WITH (FILLFACTOR = 90);
